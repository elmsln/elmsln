...
*/let AppDatepicker=Polymer({_template:html`
    <style>
      :host {
        display: block;
        position: relative;
  ...
  `,is:"app-datepicker",behaviors:[NeonAnimationRunnerBehavior],properties:{/**
...
let effectiveChildren=FlattenedNodesObserver.getFlattenedNodes(this).filter(n=>n.nodeType===Node.ELEMENT_NODE);if(effectiveChildren&&0<effectiveChildren.length){for(var i=0;i<effectiveChildren.length;i++){// addEventListener to paper-button with dialog-confirm.
...
async.microTask.run(()=>{this.$.showSelectedYear.focus()})}},_updateListScroller:function(_list){var _sl=dom(_list.root).querySelector("#items"),_slh=_sl.getBoundingClientRect().height||12863.994140625,_sli=Math.floor(_slh/(2100-1900+1)*(this._activeYear-1900-2))+1;// Adjust center position when datepickers has no buttons and in landscape mode.
...
_updateThemeColor:function(_theme){var _themes=["dark-theme","light-theme","goog-theme"],_themeIdx=_themes.indexOf(_theme),_distributedButtons=dom(this).querySelectorAll("paper-button"),_distributedButtonsLen=_distributedButtons.length,_colorCode=["#bcbcbc","#737373","#616161"][_themeIdx];if(0<=_themeIdx){_themes.splice(_themeIdx,1);this.toggleClass(_themes[0],!1,this);this.toggleClass(_themes[1],!1,this);this.toggleClass(_theme,!0,this)}else{this.toggleClass("dark-theme",!1,this);this.toggleClass("light-theme",!1,this);this.toggleClass("goog-theme",!1,this)}// workaround to update custom property of distributed buttons.
