import{html,PolymerElement}from"../../../@polymer/polymer/polymer-element.js";import{dom}from"../../../@polymer/polymer/lib/legacy/polymer.dom.js";import"./map-styles.js";/* styles scoped to inside a custom element must be in a style module */class MapLayer extends PolymerElement{static get template(){return html`
...
for(var i=0,position=1,nodes=dom(this).parentNode.children;i<nodes.length;i++){if("LAYER-"===dom(this).parentNode.children[i].nodeName){if(dom(this).parentNode.children[i]===this){break}position++}}L.setOptions(this._layer,{zIndex:position,opacity:window.getComputedStyle(this).opacity});// make sure the Leaflet layer has a reference to the map
...
if(dom(this).parentNode.controls&&!this.hidden){this._layerControl=dom(this).parentNode._layerControl;this._layerControl.addOrUpdateOverlay(this._layer,this.label)}// toggle the this.disabled attribute depending on whether the layer
...
// See issue #26
this._layer._map.dispatchEvent(new CustomEvent("moveend",{bubbles:!0,cancelable:!0,composed:!0,detail:!0}))}connectedCallback(){super.connectedCallback();if("MAP"!==dom(this).parentNode.nodeName){console.log("ERROR: "+this.localName+"#"+this.id+" must be a child of a map element");return}// if the map has been attached, set this layer up wrt Leaflet map
if(dom(this).parentNode._map){this._attachedToMap()}}_removeEvents(){if(this._layer){this._layer.off("loadstart",!1,this)}}_setUpEvents(){this._layer.on("loadstart",()=>{this.dispatchEvent(new CustomEvent("loadstart",{bubbles:!0,cancelable:!0,composed:!0,detail:{target:this}}))},this)}}window.customElements.define(MapLayer.tag,MapLayer);export{MapLayer};