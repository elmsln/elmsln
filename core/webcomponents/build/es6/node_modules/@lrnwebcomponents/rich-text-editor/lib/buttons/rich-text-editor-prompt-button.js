...
class RichTextEditorPromptButton extends RichTextEditorButton {
  ...
  updatePrompt() {
    this.fields.forEach(field => {
      if (field.property && field.property !== "") {
        this.value[field.property] = this.__selectionContents.getAttribute(field.property);
      } else if (field.property && field.property !== "") {
        this.value[field.slot] = this.__selectionContents.querySelector(field.slot);
      } else {
  ...
  updateSelection() {
    ...
    this.fields.forEach(field => {
      if (field.property && field.property !== "") {
        if (this.value[field.property] !== null && this.value[field.property].trim() !== "") hasTag = true;

        this.__selectionContents.setAttribute(field.property, this.value[field.property].trim());
      } else if (field.slot && field.slot !== "" && this.value[field.slot] !== null && this.value[field.slot].trim() !== "") {
        hasTag = true;
        this.__selectionContents.innerHTML += `${field.slot}${this.value[field.slot].trim()}${field.slot}`;
      } else {
  ...
  cancel() {
    ...
    while (this.__revertContents.firstChild) this.__selection.appendChild(this.__revertContents.firstChild);
  ...
  open() {
    ...
    if (!this.__selectionContents.getAttribute("id")) this.__selectionContents.setAttribute("id", "prompt" + Date.now());
