...
*/class EcoJsonSchemaObject extends mixinBehaviors([AppLocalizeBehavior],PolymerElement){static get tag(){return"eco-json-schema-object"}static get template(){return html`
      <custom-style>
...
for(var prop in property.component.properties){el[prop]=property.component.properties[prop]}this.listen(el,property.component.valueProperty.replace(/([A-Z])/g,"-$1").toLowerCase()+"-changed","_schemaPropertyChanged");if(typeof this.$!==typeof void 0){dom(this).appendChild(el)}// support for slot injection too!
if(""!=property.component.slot){let temp=document.createElement("div");temp.innerHTML=property.component.slot;let cloneDiv=temp.cloneNode(!0);while(null!==dom(cloneDiv).firstChild){dom(el).appendChild(dom(cloneDiv).firstChild)}}this.dispatchEvent(new CustomEvent("form-changed",{bubbles:!0,cancelable:!0,composed:!0,detail:this}))})}_removePropertyEl(el){if(typeof el.schemaProperty!==typeof void 0){this.unlisten(el,el.schemaProperty.component.valueProperty.replace(/([A-Z])/g,"-$1").toLowerCase()+"-changed","_schemaPropertyChanged")}el.schemaProperty=null;dom(this).removeChild(el)}_clearForm(){if(typeof this.$!==typeof void 0){var formEl=dom(this);while(formEl.firstChild){this._removePropertyEl(formEl.firstChild)}}}_schemaChanged(newValue,oldValue){if(newValue){this._clearForm();this._buildSchemaProperties();this._buildForm();this._setValue()}}_errorChanged(){dom(this).childNodes.forEach(el=>{var name=el.getAttribute("name");if(this.error&&this.error[name]){el.error=this.error[name]}else{el.error=null}})}_deepClone(o){return JSON.parse(JSON.stringify(o))}_isSchemaValue(type){return this._isSchemaBoolean(type)||this._isSchemaNumber(type)||this._isSchemaString(type)||this._isSchemaFile(type)}_isSchemaFile(type){if(Array.isArray(type)){return-1!==type.indexOf("file")}else{return"file"===type}}_isSchemaBoolean(type){if(Array.isArray(type)){return-1!==type.indexOf("boolean")}else{return"boolean"===type}}_isSchemaEnum(schema){return!!schema.enum}_isSchemaNumber(type){if(Array.isArray(type)){return-1!==type.indexOf("number")||-1!==type.indexOf("integer")}else{return"number"===type||"integer"===type}}_isSchemaString(type){if(Array.isArray(type)){return-1!==type.indexOf("string")}else{return"string"===type}}_isSchemaObject(type){return"object"===type}_isSchemaArray(type){return"array"===type}focus(){console.log(this)}}window.customElements.define(EcoJsonSchemaObject.tag,EcoJsonSchemaObject);export{EcoJsonSchemaObject};