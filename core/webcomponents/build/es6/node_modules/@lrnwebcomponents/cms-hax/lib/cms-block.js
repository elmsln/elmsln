 ...
 */class CMSBlock extends PolymerElement{static get template(){return html`
      <style>
        :host {
          display: block;
          min-width: 112px;
    ...
    `}static get tag(){return"cms-block"}static get properties(){return{/**
       * Loading state
   ...
   * Generate body data.
   */_generateBodyData(blockModule,blockDelta){if(null!==blockModule&&""!==blockModule&&null!==blockDelta&&""!==blockDelta){return{module:`${blockModule}`,delta:`${blockDelta}`}}}/**
   * Handle the response from the block processing endpoint
   */_handleblockResponse(newValue,oldValue){if(null!==newValue&&typeof newValue.content!==typeof void 0){// store the text and url callbacks
...
if(typeof this.blockEndPoint===typeof void 0&&typeof window.cmsblockEndPoint!==typeof void 0){this.blockEndPoint=window.cmsblockEndPoint}if(this.blockEndPoint){this.loading=!0;microTask.run(()=>{this.$.blockrequest.generateRequest()})}}}/**
   * Attached to the DOM, now fire.
   */connectedCallback(){super.connectedCallback();if(typeof this.blockModule!==typeof void 0&&null!==this.blockModule&&""!==this.blockModule){let slot=dom(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
