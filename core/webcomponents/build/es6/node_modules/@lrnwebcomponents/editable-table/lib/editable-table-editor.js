import{html,PolymerElement}from"../../../@polymer/polymer/polymer-element.js";import{afterNextRender}from"../../../@polymer/polymer/lib/utils/render-status.js";import"../../../@polymer/iron-autogrow-textarea/iron-autogrow-textarea.js";import"../../../@polymer/iron-icons/iron-icons.js";import"../../../@polymer/paper-tooltip/paper-tooltip.js";import"../../dropdown-select/dropdown-select.js";import"../../simple-colors/simple-colors.js";import{ResponsiveUtilityBehaviors}from"../../responsive-utility/lib/responsive-utility-behaviors.js";import"../../a11y-collapse/a11y-collapse.js";import{displayBehaviors,editBehaviors}from"./editable-table-behaviors.js";import"./editable-table-editor-rowcol.js";import"./editable-table-editor-toggle.js";import"./editable-table-editor-cell.js";import"./editable-table-iconset.js";import"./editable-table-styles.js";/**
...
*/class EditableTableEditor extends editBehaviors(displayBehaviors(ResponsiveUtilityBehaviors(PolymerElement))){static get template(){return html`
    ...
    `}static get tag(){return"editable-table-editor"}static get observers(){return["_setMinimumData(data)"]}static get properties(){return{/**
       ...
       */hideTableSort:{type:Boolean,computed:"_tableSortHidden(hideSort,hideFilter)"}}}/**
   * click detection
   */ready(){super.ready();this.onclick=function(e){let el=null!==e.srcElement&&null!==e.srcElement.tagName&&"td"===e.srcElement.tagName.toLowerCase()?e.srcElement:!1;if(el&&null!==el.getElementsByTagName("editable-table-cell")){console.log(el.children[0]);el.children[0].focus()}}}connectedCallback(){super.connectedCallback();afterNextRender(this,function(){this.addEventListener("cell-move",this._onCellMove.bind(this));this.addEventListener("cell-value-changed",this._onCellValueChange.bind(this));this.addEventListener("insert-row",this._handleInsertRow.bind(this));this.addEventListener("insert-column",this._handleInsertColumn.bind(this));this.addEventListener("delete-row",this._handleDeleteRow.bind(this));this.addEventListener("delete-column",this._handleDeleteColumn.bind(this));this.addEventListener("editable-table-setting-changed",this._onTableSettingChange.bind(this));this.addEventListener("dropdown-select-changed",this._onAccentChange.bind(this))})}disconnectedCallback(){this.removeEventListener("cell-move",this._onCellMove.bind(this));this.removeEventListener("cell-value-changed",this._onCellValueChange.bind(this));this.removeEventListener("insert-row",this._handleInsertRow.bind(this));this.removeEventListener("insert-column",this._handleInsertColumn.bind(this));this.removeEventListener("delete-row",this._handleDeleteRow.bind(this));this.removeEventListener("delete-column",this._handleDeleteColumn.bind(this));this.removeEventListener("editable-table-setting-changed",this._onTableSettingChange.bind(this));this.removeEventListener("dropdown-select-changed",this._onAccentChange.bind(this));super.disconnectedCallback()}/**
   * Get the current accent color
   */_getAccentSelected(accentColor){return null!==accentColor?accentColor:"none"}/**
   * Gets the row data for a given row index
   */_getCurrentRow(index,data){let row=null;if(data!==void 0&&null!==data&&data[index]!==void 0&&null!==data[index]){row=data[index]}return row}/**
   * Handles delete column event
   */_handleDeleteColumn(e){this.deleteColumn(e.detail)}/**
   * Handles delete row event
   */_handleDeleteRow(e){this.deleteRow(e.detail)}/**
   * Handles insert column event
   */_handleInsertColumn(e){this.insertColumn(e.detail)}/**
   * Handles insert row event
   */_handleInsertRow(e){this.insertRow(e.detail)}/**
   * Tests for first row of data. Workaround to restamp column headers.
   */_isFirstRow(index){return 0===index}/**
   * Handle accent dropdown-select change
   */_onAccentChange(e){this.accentColor="none"!==e.detail.value?e.detail.value:null}/**
   * Move the focus/cursor to the correct cell when navigation keys are pressed
   */_onCellMove(e){let dir=e.detail.direction,cell=e.detail.cell,row=cell.parentNode,body=this.$.tbody,x=Array.prototype.indexOf.call(row.children,cell),y=Array.prototype.indexOf.call(body.children,row);if("down"===dir){if(y+1<body.children.length-1){body.children[y+1].children[x].children[0].setFocus()}else{this.insertRow(y)}}else if("up"===dir){if(0<y){body.children[y-1].children[x].children[0].setFocus()}}else if("right"===dir){if(x+1<row.children.length-1){row.children[x+1].children[0].setFocus()}else if(y+1<body.children.length-1){body.children[y+1].children[1].children[0].setFocus()}}else if("left"===dir){if(1<x){row.children[x-1].children[0].setFocus()}else if(0<y){body.children[y-2].children[body.children[y-2].children.length-2].children[0].setFocus()}}}/**
   * Updates data when cell value changes
   */_onCellValueChange(e){this.set("data."+e.detail.row+"."+e.detail.column,e.detail.value)}/**
   * Updates table properties when setting changes
   */_onTableSettingChange(e){this[e.detail.prop]=e.detail.value}/**
   * Makes sure there is always on cell to work from
   */_setMinimumData(data){if(1>data.length||1>data[0].length){this.set("data",[["","",""],["","",""],["","",""]])}}/**
   * Are all of the table style choices hidden?
   */_tableStylesHidden(hideBordered,hideCondensed,hideStriped,hideScroll){return hideBordered&&hideCondensed&&hideStriped&&hideScroll}/**
   * Are all of the theme choices hidden?
   */_tableThemeHidden(hideAccentColor,hideDarkTheme){return hideAccentColor&&hideDarkTheme}/**
   * Are all of the theme choices hidden?
   */_tableSortHidden(hideSort,hideFilter){return hideSort&&hideFilter}/**
   ...
   */deleteRow(index){if(confirm("Delete entire row?")){this.splice("data",index,1)}}/**
   * Insert a column at the given index
   */insertColumn(index){let temp=[],first=null;for(let i=0;i<this.data.length;i++){this.splice("data."+i,index,0,"")}}/**
   * Insert a row at the given index
   */insertRow(index){let temp=[];for(let i=0;i<this.data[0].length;i++){temp.push("")}this.splice("data",index+1,0,temp)}}window.customElements.define(EditableTableEditor.tag,EditableTableEditor);export{EditableTableEditor};