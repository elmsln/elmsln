import{html,PolymerElement}from"../../@polymer/polymer/polymer-element.js";import{IronMeta}from"../../@polymer/iron-meta/iron-meta.js";import"../../@polymer/iron-icon/iron-icon.js";import"../../@polymer/marked-element/marked-element.js";export{IconsetDemo};class IconsetDemo extends PolymerElement{static get template(){return html`
    ...
    `}static get properties(){return{__iconList:{name:"__iconList",type:"Array",value:[]},includeSets:{name:"includeSets",type:"String",value:null},excludeSets:{name:"excludeSets",type:"String",value:null}}}static get tag(){return"iconset-demo"}ready(){super.ready();const iconSets=new IronMeta({type:"iconset"});let temp=[];if(void 0!==iconSets&&iconSets.list&&iconSets.list.length){var index=0;iconSets.list.forEach(function(item){let name=item.name;root._hideIconset(name)||(temp.push({name:name,icons:[]}),item.getIconNames().forEach(icon=>{temp[index].icons.push(icon)}),index++)})}this.__iconList=temp}_hideIconset(name){let isets=null!==this.includeSets?this.includeSets.split(/ /):[],included=0===isets.length||isets.includes(name),esets=null!==this.excludeSets?this.excludeSets.split(/ /):[],excluded=esets.length.length>0&&esets.includes(name);return!included||excluded}}window.customElements.define(IconsetDemo.tag,IconsetDemo);