 ...
 */const addInternalListener=(()=>{const testNode=document.createElement("div"),testRoot=testNode.attachShadow({mode:"open"});if(testRoot.getSelection){// getSelection really exists, why are you using us?
document.addEventListener("selectionchange",ev=>{document.dispatchEvent(new CustomEvent("-shadow-selectionchange"))});return()=>{}}let withinInternals=!1;const handlers=[];document.addEventListener("selectionchange",ev=>{if(withinInternals){return}document.dispatchEvent(new CustomEvent("-shadow-selectionchange"));withinInternals=!0;window.setTimeout(()=>{withinInternals=!1},0);handlers.forEach(fn=>fn(ev))});return fn=>handlers.push(fn)})();let wasCaret=!1,resolveTask=null;addInternalListener(ev=>{const s=window.getSelection();if("Caret"===s.type){wasCaret=!0}else if(wasCaret&&!resolveTask){resolveTask=Promise.resolve(!0).then(()=>{wasCaret=!1;resolveTask=null})}});/**
 ...
 */function containsNextElement(s,node,walkForward){const start=node;while(node=walkFromNode(node,walkForward)){// walking (left) can contain our own parent, which we don't want
...
// elements themselves
return node instanceof Element&&s.containsNode(node,!0)}/**
 * @param {!Selection} s the window selection to use
 ...
 */function walkTextFromNode(node,isLeft,s){for(;node;node=walkFromNode(node,isLeft)){if(node.nodeType!==Node.TEXT_NODE){continue}const t=node.textContent;if(isLeft){if(s.length<t.length){return{node,offset:s.length}}const prefix=s.substr(0,t.length);if(prefix!==t){console.debug("unexpected string prefix",prefix,"expected",t)}s=s.substr(t.length)}else{if(s.length<t.length){return{node,offset:t.length-s.length}}const suffix=s.substr(s.length-t.length);if(suffix!==t){console.debug("unexpected string suffix",suffix,"expected",t)}s=s.substr(0,s.length-t.length)}}return null;// too far
}/**
 * @param {!Node} node
 ...
 */function ignoredTrailingSpace(node){if(node.nodeType!==Node.TEXT_NODE){return 0}const trailingSpaceCount=/\s*$/.exec(node.textContent)[0].length;if(!trailingSpaceCount){return 0}return trailingSpaceCount-1;// always allow single last
}const cachedRange=new Map;export function getRange(root){if(root.getSelection){const s=root.getSelection();return s.rangeCount?s.getRangeAt(0):null}const thisFrame=cachedRange.get(root);if(thisFrame){return thisFrame}const initialText=window.getSelection().toString(),result=internalGetShadowSelection(root),rs=result.range&&result.range.toString()||null;if(null!==rs&&rs!==initialText){// TODO: sometimes triggers on single-char hack etc
...
root.insertBefore(fakeSelectionNode,root.childNodes[0]);const includesBeforeRoot=s.containsNode(fakeSelectionNode);fakeSelectionNode.remove();if(includesBeforeRoot){return{range:null,mode:"outside-before"}}// insert a fake 'after' node to see if it's selected
...
s.extend(leftNode,0);const at=measure();s.collapseToEnd();range.setStart(leftNode,at);range.setEnd(leftNode,at);return{range,mode:"caret"}}else if(isNaturalDirection===void 0){if("Range"!==s.type){throw new TypeError("unexpected type: "+s.type)}// This occurs when we can't move because we can't extend left or right to measure the
...
range.setStart(leftNode,0);range.setEnd(rightNode,rightNode.length);return{range,mode:"all"}}const size=measure();let offsetLeft,offsetRight;// only one newline/space char is cared about
const validRightLength=rightNode.length-ignoredTrailingSpace(rightNode);if(isNaturalDirection){// walk in the opposite direction first
