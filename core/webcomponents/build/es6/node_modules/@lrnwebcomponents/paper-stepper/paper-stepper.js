 ...
 */class PaperStepper extends PolymerElement{static get tag(){return"paper-stepper"}static get properties(){return{selected:{type:Number,notify:!0,value:0},/**
       ...
       */noButtons:{type:Boolean,value:!1}}}// Private methods
_tapPrevious(){this.$.selector.selectPrevious()}_tapNext(){this.$.selector.selectNext()}/**
   * Returns true if there is a step before the current and if
   * _getDisablePrevious is set to false
   */_getDisablePrevious(selected,disablePrevious){return 0<selected&&!disablePrevious}/**
   * Returns true if there is a step after the current and if
   * _getDisableNext is set to false
   */_getDisableNext(selected,nrItems,disableNext){return selected<nrItems-1&&!disableNext}/**
   ...
   */_computeProgressValue(selected,items){return selected+1}_onItemsChanged(e){this._items=this.$.selector.items}}window.customElements.define(PaperStepper.tag,PaperStepper);export{PaperStepper};