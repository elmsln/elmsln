import{storiesOf}from"../../@storybook/polymer/dist/client/index.js";import*as storybookBridge from"../../@storybook/addon-knobs/polymer.js";import"../simple-colors/lib/simple-colors-styles.js";import"../es-global-bridge/es-global-bridge.js";/**
...
template=array_matches[1];replacements.forEach(replacement=>{template=template.replace(new RegExp(replacement.find,"g"),replacement.replace)});template=template.replace(/=\\"\\"/g,"").replace(/\\"/g,"\"");return template}/**
   ...
   */addPattern(story){let template=this.cleanHTML(story.file,story.replacements);story.before=story.before||``;story.after=story.after||``;story.demo=storiesOf(story.of,module);story.demo.add(story.name,()=>{return`${story.before}${template}${story.after}`});return[story.before,template,story.after]}/**
...
storybookBridge.text(key,props[key].value||``)}}}});return binding}/**
   ...
   */addLiveDemo(story,escape=!1){story.alias=story.alias!==void 0?story.alias:story.name;story.before=story.before!==void 0?story.before:``;story.after=story.after!==void 0?story.after:``;story.attr=story.attr!==void 0?story.attr:``;story.slotted=story.slotted!==void 0?story.slotted:``;story.slots=story.slots!==void 0?story.slots:{};story.props=story.props!==void 0?story.props:{};story.demo=storiesOf(story.of,module);story.demo.addDecorator(storybookBridge.withKnobs);story.demo.add(story.alias,()=>{story.attrBindings=``;Object.values(this.getBindings(story.props)).forEach(prop=>{if(!1!==prop.value&&""!==prop.value){story.attrBindings+="string"!==typeof prop.value?` ${prop.id}="${prop.value}"`:` ${prop.id}="${prop.value.replace(/'/g,"&apos;").replace(/"/g,"&quot;")}"`}});story.slotBindings=``;Object.values(this.getBindings(story.slots)).forEach(slot=>{story.slotBindings+="slot"!==slot.id?`<div slot="${slot.id}">${slot.value}</div>`:`${slot.value}`});return`
        <h1>${story.alias}</h1>
        ${story.before}
...
window.StorybookUtilities=window.StorybookUtilities||{};window.StorybookUtilities.requestAvailability=()=>{if(!window.StorybookUtilities.instance){window.StorybookUtilities.instance=new StorybookUtilities}};