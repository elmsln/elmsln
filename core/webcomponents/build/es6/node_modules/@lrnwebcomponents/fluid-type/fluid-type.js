...
this.tag=FluidType.tag;this.template=document.createElement("template");this.attachShadow({mode:"open"});if(!delayRender){this.render()}}/**
   ...
   */connectedCallback(){if(window.ShadyCSS){window.ShadyCSS.styleElement(this)}}render(){this.shadowRoot.innerHTML=null;this.template.innerHTML=this.html;if(window.ShadyCSS){window.ShadyCSS.prepareTemplate(this.template,this.tag)}this.shadowRoot.appendChild(this.template.content.cloneNode(!0))}}window.customElements.define(FluidType.tag,FluidType);export{FluidType};