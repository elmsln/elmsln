...
class HAXCMSSiteBuilder extends PolymerElement {
  ...
  constructor() {
    ...
    import("../../../../@polymer/paper-progress/paper-progress.js"); // attempt to set polymer passive gestures globally
    // this decreases logging and improves performance on scrolling

    setPassiveTouchGestures(true); // hide the outdated fallback
        ...
        if (this.manifest.metadata.dynamicElementLoader) {
          ...
          for (var i in tagsFound) {
            ...
            if (this.manifest.metadata.dynamicElementLoader[tagName] && !window.customElements.get(tagName)) {
              import(`${basePath}../../../../${this.manifest.metadata.dynamicElementLoader[tagName]}`).then(response => {// useful to debug if dynamic references are coming in
  ...
  _activeItemChanged(newValue, oldValue) {
    if (newValue && typeof newValue.id !== typeof undefined) {
      ...
      this.notifyPath("queryParams.nodeId"); // get fresh data if not published
  ...
  _themeChanged(newValue, oldValue) {
    if (newValue && oldValue) {
      if (store.cmsSiteEditor && store.cmsSiteEditor.instance && typeof store.cmsSiteEditor.instance.haxCmsSiteEditorElement !== typeof undefined) {
        store.cmsSiteEditor.instance.appendChild(store.cmsSiteEditor.instance.haxCmsSiteEditorElement);
      }
    }
    ...
    if (newValue) {
      ...
      if (typeof this.__imported[theme.element] !== typeof undefined) {
        ...
        try {
          import(pathFromUrl(decodeURIComponent(import.meta.url)) + "../../../../" + newValue.path).then(e => {
