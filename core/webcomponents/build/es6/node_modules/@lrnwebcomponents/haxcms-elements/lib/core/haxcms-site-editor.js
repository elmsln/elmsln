...
class HAXCMSSiteEditor extends PolymerElement {
  ...
  ready() {
    ...
    afterNextRender(this, function () {
      ...
      window.addEventListener("haxcms-load-site-fields", this.loadSiteFields.bind(this));
  ...
  disconnectedCallback() {
    ...
    window.removeEventListener("haxcms-load-site-fields", this.loadSiteFields.bind(this));
  ...
  loadNodeFields(e) {
    this.__nodeFieldsInvoked = e.detail; // pass along the jwt for user "session" purposes
  ...
  loadSiteFields(e) {
    this.__siteFieldsInvoked = e.detail; // pass along the jwt for user "session" purposes
  ...
  _handleGetSiteFieldsResponse(e) {
    ...
    if (this.manifest.metadata.publishedLocation) {
      ...
      h.innerHTML = '<paper-button raised style="text-transform:none;">Access published version</paper-button>';
    }

    let c = document.createElement("hax-schema-form");
    c.addEventListener("value-changed", this._schemaFormValueChanged.bind(this)); // set a min width of 50 viewable
    ...
    this.__siteFieldsForm = c; // build a save button
    ...
    b1.appendChild(document.createTextNode("Save fields"));
    b1.setAttribute("dialog-confirm", "dialog-confirm");
    b1.addEventListener("click", this._saveSiteFieldsTap.bind(this)); // cancel

    let b2 = document.createElement("paper-button");
    b2.appendChild(document.createTextNode("cancel"));
    b2.setAttribute("dialog-dismiss", "dialog-dismiss"); // publish button
    ...
    b3.style.backgroundColor = getComputedStyle(this).getPropertyValue("--haxcms-color");
    let b = document.createElement("div");
    b.style.position = "absolute";
    b.style.bottom = 0;
    b.style.left = 0;
  ...
  _publishTap(e) {
    this.dispatchEvent(new CustomEvent("haxcms-publish-site", {
      bubbles: true,
      composed: true,
      cancelable: false,
      detail: true
  ...
  _saveSiteFieldsTap(e) {
    let values = this.__siteFieldsForm.value; // fire event with details for saving
  ...
  saveNode(e) {
    ...
    this.notifyPath("updateNodeData.siteName"); // get the body from what's there currently

    let body = window.HaxStore.instance.activeHaxBody.haxToContent();
    this.set("updateNodeData.body", body);
    this.notifyPath("updateNodeData.body"); // convert to schema so we can ship that too if we want to process
    ...
    this.notifyPath("updateNodeData.jwt"); // send the request
