import{html,PolymerElement}from"../../../@polymer/polymer/polymer-element.js";import{afterNextRender}from"../../../@polymer/polymer/lib/utils/render-status.js";import{dom}from"../../../@polymer/polymer/lib/legacy/polymer.dom.js";import{pathFromUrl}from"../../../@polymer/polymer/lib/utils/resolve-url.js";import{setPassiveTouchGestures}from"../../../@polymer/polymer/lib/utils/settings.js";import{getRange}from"./shadows-safari.js";import{encapScript,wipeSlot}from"./haxutils.js";import"../../../@polymer/iron-ajax/iron-ajax.js";import"../../simple-toast/simple-toast.js";import{MediaBehaviorsVideo}from"../../media-behaviors/media-behaviors.js";import{HAXElement}from"../../hax-body-behaviors/hax-body-behaviors.js";import{CodeSample}from"../../code-sample/code-sample.js";class HaxStore extends HAXElement(MediaBehaviorsVideo(PolymerElement)){static get template(){return html`
      <style>
        :host {
          display: none;
        }
   ...
   */static get observers(){return["_loadAppStoreData(__ready, __appStoreData, haxAutoloader)","_storePiecesAllHere(haxAutoloader,activeHaxBody, haxPanel, haxToast, haxExport, haxPreferences, haxManager, haxStaxPicker, haxAppPicker)"]}static get properties(){return Object.assign({/**
         ...
         */validTagList:{type:Array,value:["p","div","ol","ul","li","a","strong","kbd","em","i","b","hr","h1","h2","h3","h4","h5","h6","blockquote","code","figure","img","iframe","video","audio","section","grid-plate","template","webview"]},/**
...
if("function"===typeof test.reload){return!0}return!1}},/**
   ...
   */isTextElement(node){if(null!=node&&this.validTagList.includes(node.tagName.toLowerCase())){if(["p","ol","ul","li","a","h1","h2","h3","h4","h5","h6","span","i","bold","em","strong","blockquote","code","figure"].includes(node.tagName.toLowerCase())){return!0}}return!1}/**
...
this._handleDynamicImports(items,haxAutoloader)}}/**
   ...
   * it came from.
   */async _handleDynamicImports(items,haxAutoloader){const basePath=pathFromUrl(decodeURIComponent(import.meta.url));for(var i in items){await import(`${basePath}../../../${items[i]}`).then(response=>{for(var cVal in response){// get the custom element definition we used to add that file
...
dom(haxAutoloader).appendChild(document.createElement(i))}}}).catch(error=>{/* Error handling */console.log(error)})}}_editModeChanged(newValue){if(newValue&&this.globalPreferences.haxVoiceCommands){this.shadowRoot.querySelector("#hal").auto=!0}else{this.shadowRoot.querySelector("#hal").auto=!1}}_globalPreferencesChanged(newValue,oldValue){// regardless of what it is, reflect it globally but only after setup
...
document.body.removeEventListener("hax-insert-content",this._haxStoreInsertContent.bind(this));document.body.removeEventListener("hax-insert-content-array",this._haxStoreInsertMultiple.bind(this));window.removeEventListener("hax-add-voice-command",this._addVoiceCommand.bind(this));// capture events and intercept them globally
...
window.localStorage.setItem("haxUserData",JSON.stringify(this.storageData))}/**
   * ready life cycle
   */ready(){super.ready();afterNextRender(this,function(){// see if a global was used to prevent this check
...
try{let globalData=window.sessionStorage.getItem("haxUserData")?JSON.parse(window.sessionStorage.getItem("haxUserData")):{};this.set("storageData",globalData)}catch(e){}}else{try{let globalData=window.localStorage.getItem("haxUserData")?JSON.parse(window.localStorage.getItem("haxUserData")):{};this.set("storageData",globalData)}catch(e){}}}})}/**
   * attached.
   */connectedCallback(){super.connectedCallback();afterNextRender(this,function(){// capture events and intercept them globally
window.addEventListener("hax-consent-tap",this._haxConsentTap.bind(this));window.addEventListener("onbeforeunload",this._onBeforeUnload.bind(this));window.addEventListener("paste",this._onPaste.bind(this));// set this global flag so we know it's safe to start trusting data
...
if(window.HaxStore.instance.isTextElement(window.HaxStore.instance.activeNode)&&!window.HaxStore.instance.haxManager.opened){e.preventDefault();let text="";// intercept paste event
if(e.clipboardData||e.originalEvent.clipboardData){text=(e.originalEvent||e).clipboardData.getData("text/plain")}else if(window.clipboardData){text=window.clipboardData.getData("Text")}// find the correct selection object
let sel=window.HaxStore.getSelection();var range;if(range=window.HaxStore.getRange()){range.deleteContents();range.insertNode(document.createTextNode(text))}}}/**
   * Created life-cycle to ensure a single global store.
   */constructor(){super();import("./hax-app.js");import("./hax-stax.js");import("./hax-stax-browser.js");import("./hax-blox.js");import("./hax-blox-browser.js");import("../../hal-9000/hal-9000.js");setPassiveTouchGestures(!0);// claim the instance spot. This way we can easily
...
document.body.addEventListener("hax-register-export",this._haxStoreRegisterExport.bind(this));// notice content insert and help it along to the body
...
this.activePlaceHolder=null}else{this.activeHaxBody.haxInsert(e.detail.tag,e.detail.content,properties)}}}/**
...
if(typeof e.target.parentElement!==typeof void 0&&"HAX-STORE"===e.target.parentElement.tagName){dom(e.target.parentElement).removeChild(e.target)}}}/**
...
if(typeof e.target.parentElement!==typeof void 0&&"HAX-AUTOLOADER"===e.target.parentElement.tagName){dom(this.haxAutoloader).removeChild(e.target)}}}}/**
...
let tag=node.tagName.toLowerCase();if(window.HaxStore.instance._isSandboxed&&"iframe"===tag){tag="webview"}let slotContent=window.HaxStore.getHAXSlot(node);// special edge case for slot binding in primatives
 ...
 */window.HaxStore.haxNodeToContent=node=>{if(window.HaxStore.instance.activeHaxBody.globalPreferences.haxDeveloperMode){console.log(node)}// ensure we have access to all the member functions of the custom element
...
let delProps=["inner-text","tabindex","guestinstance"];for(var delProp in delProps){if(typeof propvals[delProps[delProp]]!==typeof void 0){delete propvals[delProps[delProp]]}}// remove id attribute if it's empty, somehow misses above
...
let slotnodes=dom(node).getEffectiveChildNodes();// ensure there's something inside of this
 ...
 */window.HaxStore.getHAXSlot=node=>{let content="";var slotnodes=dom(node).children;// ensure there's something inside of this
if(0<slotnodes.length){// loop through everything found in the slotted area and put it back in
for(var j=0,len2=slotnodes.length;j<len2;j++){if(typeof slotnodes[j].tagName!==typeof void 0){// if we're a custom element, keep digging, otherwise a simple
// self append is fine.
if(0<slotnodes[j].tagName.indexOf("-")){content+="  "+window.HaxStore.haxNodeToContent(slotnodes[j])+"\n"}else{content+="  "+slotnodes[j].outerHTML+"\n"}}// keep comments with a special case since they need wrapped
else if(8===slotnodes[j].nodeType){content+="<!-- "+slotnodes[j].textContent+" -->"}// keep everything NOT an element at this point, this helps
// preserve whitespace because we're crazy about accuracy
 ...
 */window.HaxStore.wipeSlot=(element,slot="")=>{wipeSlot(element,slot)};/**
 * HTML encapsulation of a string on script and style tags
 */window.HaxStore.encapScript=html=>{return encapScript(html)};/**
...
else if(getRange(window.HaxStore.instance.activeHaxBody.parentNode)){return getRange(window.HaxStore.instance.activeHaxBody.parentNode)}}// missed on both, hope the normal one will work
return window.getSelection()};/**
 * Get a normalized range based on current selection
 */window.HaxStore.getRange=()=>{let sel=window.HaxStore.getSelection();if(sel.getRangeAt&&sel.rangeCount){return sel.getRangeAt(0)}else if(sel){return sel}else!1};window.customElements.define(HaxStore.tag,HaxStore);export{HaxStore};