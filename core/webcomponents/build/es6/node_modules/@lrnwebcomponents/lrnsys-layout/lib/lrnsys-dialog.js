...
*/class LrnsysDialog extends PolymerElement{static get template(){return html`
      ...
      <paper-tooltip for="dialogtrigger" animation-delay="0" hidden$="[[!alt]]"
        >[[alt]]</paper-tooltip
      >
    `}static get tag(){return"lrnsys-dialog"}static get properties(){return{/**
   ...
   */tapEventOn(e){if(typeof this.hoverClass!==typeof void 0){var classes=this.hoverClass.split(" ");classes.forEach((item,index)=>{if(""!=item){this.$.dialogtrigger.classList.add(item)}})}}/**
   * Handle a mouse out event and remove the hoverclasses
   * from the classList array for paper-button.
   */tapEventOff(e){if(typeof this.hoverClass!==typeof void 0){var classes=this.hoverClass.split(" ");classes.forEach((item,index)=>{if(""!=item){this.$.dialogtrigger.classList.remove(item)}})}}toggleDialog(){this.openDialog()}/**
   * Toggle the drawer to open / close.
   */openDialog(){// assemble everything in the slot
let nodes=dom(this).getEffectiveChildNodes(),h=document.createElement("span"),c=document.createElement("span"),node={};for(var i in nodes){if(typeof nodes[i].tagName!==typeof void 0){switch(nodes[i].getAttribute("slot")){case"toolbar-primary":case"toolbar-secondary":case"toolbar":case"header":node=nodes[i].cloneNode(!0);node.removeAttribute("slot");h.appendChild(node);break;case"button":// do nothing
...
classes.forEach((item,index)=>{if(""!=item){if(this.focusState){this.$.dialogtrigger.classList.add(item)}else{this.$.dialogtrigger.classList.remove(item)}}})}this.focusState=!this.focusState}/**
   * Ready lifecycle
   */ready(){super.ready();this.__modal=window.SimpleModal.requestAvailability()}/**
   * Attached lifecycle
