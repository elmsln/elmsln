...
*/import"./app-datepicker-icons.js";import"./animations/datepicker-slide-from-left-animation.js";import"./animations/datepicker-slide-from-right-animation.js";import"../@polymer/iron-selector/iron-selector.js";import"../@polymer/iron-list/iron-list.js";import"../@polymer/paper-icon-button/paper-icon-button.js";import"../@polymer/neon-animation/neon-animated-pages.js";import{NeonAnimationRunnerBehavior}from"../@polymer/neon-animation/neon-animation-runner-behavior.js";import"../@polymer/neon-animation/animations/fade-in-animation.js";import"../@polymer/neon-animation/animations/fade-out-animation.js";import"../@polymer/neon-animation/animations/slide-from-left-animation.js";import"../@polymer/neon-animation/animations/slide-from-right-animation.js";import"../@polymer/neon-animation/neon-animatable.js";import{dom}from"../@polymer/polymer/lib/legacy/polymer.dom.js";import{mixinBehaviors}from"../@polymer/polymer/lib/legacy/class.js";import{PolymerElement,html}from"../@polymer/polymer/polymer-element.js";class AppDatepicker extends mixinBehaviors([NeonAnimationRunnerBehavior],PolymerElement){static get template(){return html`
    <style>
      :host {
        display: block;
        position: relative;
    ...
    <div id="dp" class="datepicker">
       ...
       */locale:{type:String,value:()=>{if(window.Intl){return window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US"}return"en-US"}},/**
       ...
       */disableDays:{type:Array,value:function(){return[6,0]}},/**
       * Array of dates need to be disabled for selection.
       */disableDates:{type:Array,value:function(){return[]}},/**
...
this.$.dp.classList.add("with-buttons")}else{this.$.dp.classList.remove("with-buttons")}this.dispatchEvent(new CustomEvent("app-datepicker-connected",{bubbles:!0,composed:!0}))}//  Month Jan Feb Mar Apr May Jun Jul Aug Sep Oct Nov Dec
...
_computeDaysOfMonth(_activeYear,_activeMonth,_firstDayOfWeek,_locale){// No op for Intl is undefined (for Safari below v10.x.x).
...
_computeShiftedDisableDays(_firstDayOfWeek,_isDisableDays){_firstDayOfWeek=0<_firstDayOfWeek&&7>_firstDayOfWeek?_firstDayOfWeek:0;var _sdd=this.disableDays.map(function(_day){_day=_day-_firstDayOfWeek;return 0>_day?7+_day:_day});this.set("_shiftedDisableDays",_sdd)}// Increment the month via user interaction.
_incrementMonth(ev){this.debounce("_incrementMonth",function(){this.set("_isIncrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0===++_month%12){this._activeYear++}this.set("_activeMonth",_month%12);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("incrementEntry")}this.set("_isIncrementMonth",!1)}.bind(this))},100)}// Decrement the month via user interaction.
_decrementMonth(ev){this.debounce("_decrementMonth",function(){this.set("_isDecrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0>--_month){this._activeYear--;_month=11}this.set("_activeMonth",_month);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("decrementEntry")}this.set("_isDecrementMonth",!1)}.bind(this))},100)}// Re-compute active month year if new date is selected.
_computeActiveMonthYear(_activeYear,_activeMonth,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _amy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",month:"short",year:"numeric"}).format(new Date(Date.UTC(_activeYear,_activeMonth,1)));this.set("_activeMonthYear",_amy)}}// Re-compute short version of full date to show on the picker.
_computeShortSelectedDate(_selectedYear,_selectedMonth,_selectedDate,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssd=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",weekday:"short",month:"short",day:"numeric"}).format(new Date(Date.UTC(_selectedYear,_selectedMonth,_selectedDate)));this.set("_shortSelectedDate",_ssd);// When datepicker has no button for date confirmation,
// by asserting `autoUpdateDate` will trigger auto update on
// date change.
if(this.autoUpdateDate){this.enforceDateChange()}}}_computeShowSelectedYear(_selectedYear,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",year:"numeric"}).format(new Date(Date.UTC(_selectedYear,0,1)));this.set("_showSelectedYear",_ssy)}}// When a day of month is selected, a lot of tasks need to be carried out.
_chooseDaysOfMonth(ev){var _target=ev.target;// daysOfMonth is chooseable when:
...
_isToday(_item,_activeYear,_activeMonth){var _now=new Date,_isToday=_item===_now.getDate()&&_activeYear===_now.getFullYear()&&_activeMonth===_now.getMonth();return _isToday?" is-today":""}// Re-compute class when the item (day of month) is empty date & non-selectable.
_isEmptyDate(_item){return this._isNumber(_item)?"":" is-non-selectable"}// Re-compute class if the item (day of month) is the selected date.
_isChosenDaysOfMonth(_item,_selectedYear,_selectedMonth,_selectedDate){// able to retain selected daysOfMonth even after navigating to other months.
...
_isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isLessThanMinDate=!1,_isMoreThanMaxDate=!1,_isDisableDay=!1,_isDisableDates=!1,_isDisableDays=this._shiftedDisableDays.some(function(_n){return _n===_index%7});// ------ < _minDate ---------------- _maxDate > ------
...
_isListOfYearsSelected(selectedYear,year){return+selectedYear===+year?" is-selected":""}_computeDaysOfWeek(_firstDayOfWeek,_locale){// _daysOfWeek needs to be changed as well with firstDayOfWeek and locale.
...
_convertDateStringToDateObject(_date){var _checkDate=_date instanceof Date||"string"!==typeof _date?_date:new Date(_date),_isValidDate="Invalid Date"!==_checkDate.toDateString();return _isValidDate?_checkDate:null}// Dynamically load items into the iron-list when switching page to year view.
_updateList(_activeView){for(var _newList=[],y=1900,i=y;2100>=i;i++){_newList.push({year:i})}this.set("_listOfYears",_newList)}// On neon-animaition-finish, udpate scroller position of the iron-list.
// It is to fix the iron-list so that the selected year will always be centered by
// hardcoded with fixed length of #items.
_onAnimationFinish(ev){var _target=ev.detail;// TO fix the scrolling of iron-list, hard coded with fixed length of #items.
...
this.async(function(){_target.toPage._focusPhysicalItem(_focusableItem)},1)}else{// When user navigate to calendar view from list view,
// re-focus showSelectedYear.
this.async(function(){this.$.showSelectedYear.focus()},1)}}_updateListScroller(_list){var _sl=dom(_list.root).querySelector("#items"),_slh=_sl.getBoundingClientRect().height||12863.994140625,_sli=Math.floor(_slh/(2100-1900+1)*(this._activeYear-1900-2))+1;// Adjust center position when datepickers has no buttons and in landscape mode.
...
this.async(function(){_list.scroll(0,_sli);// Select initial item for the list.
_list.selectItem(this._activeYear-1900)},17);// wait for at least 17ms to update the scroller position.
...
_onIronSelectorSelectedChanged(ev){if("year"===ev.detail.value){if(!this._isListRendered){this._updateList();this.set("_isListRendered",!0)}else{// When noAnimation is truthy, this will update the list scroller.
if(this.noAnimation){this._updateListScroller(this.$$("#listOfYears"))}}}}_onListRendered(ev){// When noAnimation is truthy, this will update the list scroller.
if(ev.target.if&&this.noAnimation){this.async(function(){this._updateListScroller(this.$$("#listOfYears"))},1)}}_goCalendar(ev){// Listen for enter key from keyboard.
...
this.$$("#listOfYears").selectItem(_selectedYear-1900);// Go back to calendar page.
this.set("_activeView","calendar")}// split capturing group of format into year, month and date.
_computeSeparateFormat(_format){var re=/^(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)$/g,m=re.exec(_format),_temp={},_tempArr=[];if(null!==m){_tempArr.push(m[1]);_tempArr.push(m[2]);_tempArr.push(m[3]);for(var i=0,matched;3>i;i++){matched=_tempArr[i];if(0<=matched.indexOf("y")){_temp.y=matched}else if(0<=matched.indexOf("m")){_temp.m=matched}else if(0<=matched.indexOf("d")){_temp.d=matched}}}// Only set _format if the new format is valid.
...
_bindSelectedFulldate(_selectedYear,_selectedMonth,_selectedDate,_format){// show long date (eg. Fri, May 12 2017 instead.
...
_updateToReflectExternalChange(_inputDate){// Due to limitation of input date received by platform"s Date,
...
var _re1=/^(\d{4})\W+(\d{1,2})\W+(\d{1,2})$/i,_re2=/^(\d{4})[ ](\w+)[ ](\d{1,2})$/i,_validWithRe1=_re1.exec(_id),_validWithRe2=_re2.exec(_id);if(null===_validWithRe1&&null===_validWithRe2){return _res}else{var _resultToDate=null;if(null===_validWithRe1){_resultToDate=new Date(_validWithRe2[1]+" "+_validWithRe2[2]+" "+_validWithRe2[3])}else if(null===_validWithRe2){_resultToDate=new Date(+_validWithRe1[1],+_validWithRe1[2]-1,+_validWithRe1[3])}return{valid:!0,result:_resultToDate}}}var _showLongDate=this.showLongDate,_yy=0,_mm=0,_dd=0,_isValidDate=validateDate(_inputDate,_showLongDate);if(_isValidDate.valid){if(this.alwaysResetSelectedDateOnDialogClose){return}var _vd=new Date(_isValidDate.result),_yy=_vd.getFullYear(),_mm=_vd.getMonth();this._setInvalidDate(!1);this.set("_activeYear",_yy);this.set("_activeMonth",_mm);this.set("_selectedYear",_yy);this.set("_selectedMonth",_mm);this.set("_selectedDate",_vd.getDate())}else{this.set("_selectedDate",this._selectedDate||new Date().getDate());this._computeShowLongDate(_showLongDate,this.locale);this._setInvalidDate(!0)}}/**
   ...
   */enforceDateChange(){this._setInvalidDate(!1);this._computeShowLongDate(this.showLongDate,this.locale)}// reset date to today"s date if alwaysResetSelectedDateOnDialogClose is set.
resetDate(){if(this._isSelectedDateConfirmed){return}var now=new Date,nowFy=now.getFullYear(),nowM=now.getMonth(),nowD=now.getDate();this.set("_activeYear",nowFy);this.set("_activeMonth",nowM);this.set("_selectedYear",nowFy);this.set("_selectedMonth",nowM);this.set("_selectedDate",nowD);this._setInvalidDate(!1)}// Accessibility enhancment.
_shouldTabIndex(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isDisableDays=this._isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item);return _item&&0<=_item&&!_isDisableDays?0:-1}_shouldAriaDisabled(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isDisableDays=this._isDisableDays(_index,_firstDayOfWeek,_minDate,_maxDate,_item);return!(_item&&0<=_item&&!_isDisableDays)}// Lodash"s replacements.
_padStart(_string,_length,_chars){var _len=-_length,_str=(_chars+_string).slice(_len);return _str}_isNumber(_value){// return typeof _value == "number" || (!Number.isNaN(parseFloat(_value)) && Number.isFinite(_value));
...
_updateThemeColor(_theme){var _themes=["dark-theme","light-theme","goog-theme"],_themeIdx=_themes.indexOf(_theme),_distributedButtons=dom(this).querySelectorAll("paper-button"),_distributedButtonsLen=_distributedButtons.length,_colorCode=["#bcbcbc","#737373","#616161"][_themeIdx];if(0<=_themeIdx){_themes.splice(_themeIdx,1);this.toggleClass(_themes[0],!1,this);this.toggleClass(_themes[1],!1,this);this.toggleClass(_theme,!0,this)}else{this.toggleClass("dark-theme",!1,this);this.toggleClass("light-theme",!1,this);this.toggleClass("goog-theme",!1,this)}// workaround to update custom property of distributed buttons.
...
_updateDatepickerView(_view){this.toggleClass("horizontal-view","horizontal"===_view,this);this.toggleClass("vertical-view","vertical"===_view,this)}// workaround to update custom property of distributed children until Polymer supports Native custom properties.
_updateDistributedButtonInkColorCustomProp(_node,_colorCode){_node.updateStyles({"--paper-button-ink-color":_colorCode})}// TODO: To add GIF to README.md.
// TODO: To migrate 2nd datepicker to the same month when minDate is set on the 1st datepicker.
}window.customElements.define("app-datepicker",AppDatepicker);