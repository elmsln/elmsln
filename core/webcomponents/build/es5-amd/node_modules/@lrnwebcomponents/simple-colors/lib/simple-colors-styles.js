define(["../../../@polymer/polymer/polymer-element.js"],function(_polymerElement){"use strict";/**
 ...
 */window.SimpleColorsUtilities=window.SimpleColorsUtilities||{};var utils=window.SimpleColorsUtilities;/**
 * The colors object.
...
*/var contrasts={greyColor:{aaLarge:[{min:7,max:12},{min:7,max:12},{min:7,max:12},{min:7,max:12},{min:8,max:12},{min:10,max:12},{min:1,max:3},{min:1,max:5},{min:1,max:6},{min:1,max:6},{min:1,max:6},{min:1,max:6}],aa:[//if bold text < 14pt, or text < 18pt
 ...
 */utils.getContrastingShades=function(isLarge,colorName,colorShade,contrastName){var hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=contrasts[hasGrey][aa][index];return Array(range.max-range.min+1).fill().map(function(_,idx){return range.min+idx})};/**
 * for large or small text given a color and its shade,
 ...
 */utils.getContrastingColors=function(colorName,colorShade,isLarge){var result={};Object.keys(utils.colors).forEach(function(color){result[color]=utils.getContrastingShades(isLarge,colorName,colorShade,color)});return result.color};/**
 ...
 */utils.isContrastCompliant=function(isLarge,colorName,colorShade,contrastName,contrastShade){var hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=contrasts[hasGrey][aa][index];return contrastShade>=range.min&&ontrastShade>=range.max};/**
 * gets the current shade based on the index
 *
 * @param {string} the index
 * @param {number} the shade
 */utils.indexToShade=function(index){return parseInt(index)+1};/**
 * gets the current shade based on the index
 *
 * @param {string} the shade
 * @param {number} the index
 */utils.shadeToIndex=function(shade){return parseInt(shade)-1};/**
 * gets the current shade
 *
 * @param {string} the shade
 * @param {number} the inverted shade
 */utils.invertShade=function(shade){return utils.colors.grey.length+1-parseInt(shade)};/**
 * inverts the current index
 *
 * @param {string} the index
 * @param {number} the inverted index
 */utils.invertIndex=function(index){return utils.colors.grey.length-1-parseInt(index)};/**
 * returns the maximum contrast to the index
 *
 * @param {string} the index
 * @param {number} the index with maximum contrast
 */utils.maxContrastIndex=function(index){return parseInt(index)<utils.colors.grey.length/2?utils.colors.grey.length-1:0};/**
 * returns the maximum contrast to the shade
 *
 * @param {string} the shade
 * @param {number} the shade with maximum contrast
 */utils.maxContrastShade=function(shade){return parseInt(shade)<utils.colors.grey.length/2+1?utils.colors.grey.length:1};/**
 ...
 */utils.makeVariable=function(){var color=0<arguments.length&&arguments[0]!==void 0?arguments[0]:"grey",shade=1<arguments.length&&arguments[1]!==void 0?arguments[1]:1,theme=2<arguments.length&&arguments[2]!==void 0?arguments[2]:"default";return["--simple-colors",theme,"theme",color,shade].join("-")};/**
 * returns a variable based on color name, shade, and fixed theme
 *
 * @param {string} the color name
 * @param {number} the color shade
 ...
 */utils.makeClass=function(){var color=0<arguments.length&&arguments[0]!==void 0?arguments[0]:"grey",shade=1<arguments.length&&arguments[1]!==void 0?arguments[1]:1,theme=2<arguments.length&&arguments[2]!==void 0?arguments[2]:"default",suffix=3<arguments.length&&arguments[3]!==void 0?arguments[3]:"";return[".simple-colors",theme,"theme",color,shade].join("-")+suffix};/**
 * gets the correct hexCode for a color shade,
 * depending on whether or not the list is dark (inverted)
 */utils.getHex=function(hexcodes,index,dark){if(dark){return hexcodes[utils.invertIndex(utils.colors,index)]}else{return hexcodes[index]}};/**
 * adds all CSS variables for a given theme (default, dark, or light)
 */utils.addThemeVariables=function(theme,dark){var str=[];for(var name in utils.colors){str.push(utils.addColorShades(theme,name,utils.colors[name],dark))}return str.join("")};/**
 * adds CSS variables for all shades of contrast for a given theme+color
 * and assigns a hex code to it
 *
 * @returns {string}
 */utils.addColorShades=function(theme,color,hexcodes,dark){for(var str=[],i=0;i<hexcodes.length;i++){var cssvar=utils.makeVariable(color,i+1,theme),hex=dark?hexcodes[utils.invertIndex(i)]:hexcodes[i];str.push(cssvar+":"+hex+"; ")}return str.join("")};/**
 * adds all CSS variables as styles for :host and :host([dark]) selectors
 *
 * @returns {string}
 */utils.addCssVariables=function(){var str=[],greys=utils.colors.grey;str.push(utils.addStatement(":host",utils.addColorShades("default","accent",greys,!1)+utils.addThemeVariables("default",!1)));str.push(utils.addStatement(":host",utils.addColorShades("fixed","accent",greys,!1)+utils.addThemeVariables("fixed",!1)));/**
 ...
 *
 * @returns {object}
 */utils.addAccentVariables=function(){var str=[];for(var color in utils.colors){str.push(utils.addStatement(":host([accent-color=\""+color+"\"])",[utils.addColorShades("default","accent",utils.colors[color],!1),utils.addColorShades("fixed","accent",utils.colors[color],!1),utils.addColorShades("light","accent",utils.colors[color],!1),utils.addColorShades("dark","accent",utils.colors[color],!0)].join("")));str.push(utils.addStatement(":host([dark][accent-color=\""+color+"\"])",[utils.addColorShades("default","accent",utils.colors[color],!0)].join("")))}return utils.addStyle(str.join(""))};/**
 * adds all CSS color classes for a given theme
 *
 * @returns {object}
 */utils.addClasses=function(){for(var themes=["default","fixed","light","dark"],str=[],i=0;i<themes.length;i++){for(var j=0;j<utils.colors.grey.length;j++){var bg=":host "+utils.makeClass("accent",j+1,themes[i]),cssvar=utils.makeVariable("accent",j+1,themes[i]);str.push([utils.addStatement(bg,"background-color: var("+cssvar+");"),utils.addStatement(bg+"-text","color: var("+cssvar+");"),utils.addStatement(bg+"-border","border: 1px solid var("+cssvar+");")].join(""));for(var color in utils.colors){var _bg=":host "+utils.makeClass(color,i+1,themes[i]),cssVar=utils.makeVariable(color,i+1,themes[i]);str.push([utils.addStatement(_bg,"background-color: var("+cssvar+");"),utils.addStatement(_bg+"-text","color: var("+cssvar+");"),utils.addStatement(_bg+"-border","border: 1px solid var("+cssvar+");")].join(""))}}}return utils.addStyle(str.join(""))};utils.addStatement=function(selector,style){return selector+" {\n"+style+"\n}\n"};utils.addStyle=function(content){return"<style>\n"+content+"\n</style>\n"};utils.testStyle=function(str){var temp=document.createElement("template");temp.innerHTML=utils.addStyle(str);console.log(temp)};/**
 * append and register the shared styles
 */var template=document.createElement("template"),styleElement=document.createElement("dom-module");template.innerHTML=utils.addCssVariables()+utils.addAccentVariables()+utils.addClasses();styleElement.appendChild(template);styleElement.register("simple-colors")});