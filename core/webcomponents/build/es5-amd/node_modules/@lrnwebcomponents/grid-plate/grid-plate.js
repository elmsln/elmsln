...
window.removeEventListener("hax-insert-content",this.haxInsertContent.bind(this));babelHelpers.get(babelHelpers.getPrototypeOf(GridPlate.prototype),"disconnectedCallback",this).call(this)}},{key:"preProcessHaxInsertContent",/**
   * Implements preProcessHaxInsertContent to clean up output on save
   */value:function preProcessHaxInsertContent(detail){// ensure this is wiped to avoid issues in building
delete detail.properties.activeItem;return detail}},{key:"_droppableChanged",value:function _droppableChanged(newValue){if(newValue){this.editMode=!0}}/**
   ...
   */},{key:"cancelActive",value:function cancelActive(e){this.activeItem=null}/**
   ...
   */},{key:"canMoveSlot",value:function canMoveSlot(item,before){var dir=before?-1:1,max=this.shadowRoot.querySelectorAll(".column").length,col=item.getAttribute("slot").split("-"),dest=parseInt(col[1])+dir;return 1<=dest&&dest<=max}},{key:"layoutChanged",value:function layoutChanged(newValue,oldValue){var _this2=this;if(newValue&&babelHelpers.typeof(oldValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){// ensure we apply things correctly
if(this.editMode){this.editMode=!1;setTimeout(function(){_this2.editMode=!0},100)}}}/**
...
setTimeout(function(){if(_this3.activeItem&&"function"===typeof _this3.activeItem.focus){_this3.positionArrows(_this3.activeItem);_this3.activeItem.focus()}},100)}/**
...
if(babelHelpers.typeof(oldValue)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!=oldValue){oldValue.classList.remove("active-item");oldValue.blur()}}/**
   ...
   */},{key:"setActiveElement",value:function setActiveElement(e){// support HAX text operations should take priority
...
var children=(0,_polymerDom.dom)(this).getEffectiveChildNodes();if("object"===babelHelpers.typeof(children)){if(newValue&&!oldValue){// walk the children and apply the draggable state needed
...
for(var i in children){if(babelHelpers.typeof(children[i].tagName)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].removeEventListener("drop",this.dropEvent.bind(this));children[i].removeEventListener("dragstart",this.dragStart.bind(this));children[i].removeEventListener("dragenter",this.dragEnter.bind(this));children[i].removeEventListener("dragleave",this.dragLeave.bind(this));children[i].removeEventListener("dragend",this.dragEnd.bind(this));children[i].removeEventListener("dragover",function(e){e.preventDefault()});children[i].removeAttribute("draggable");children[i].removeAttribute("data-draggable");children[i].removeAttribute("tabindex")}}}}}/**
   ...
   */},{key:"dragEnter",value:function dragEnter(e){if(this.editMode){e.preventDefault();e.target.classList.add("hovered")}}/**
   * Leaving an element while dragging.
   */},{key:"dragLeave",value:function dragLeave(e){if(this.editMode){e.target.classList.remove("hovered")}}/**
   * On double check, fire an event for HAX to insert a paragraph.
   * If they aren't using HAX then it won't do anything
   */},{key:"dblclick",value:function dblclick(e){if(this.editMode&&e.target.id){var detail={properties:{slot:e.target.id.replace("col","col-")}};this.dispatchEvent(new CustomEvent("grid-plate-add-item",{bubbles:!0,cancelable:!0,composed:!0,detail:detail}))}}/**
   * Drop an item onto another
   */},{key:"dropEvent",value:function dropEvent(e){var _this4=this;if(this.editMode){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// if we have a slot on what we dropped into then we need to mirror that item
// and place ourselves below it in the DOM
if(babelHelpers.typeof(this.activeItem)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!==this.activeItem&&babelHelpers.typeof(local)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!=local.getAttribute("slot")&&this.activeItem!==local){this.activeItem.setAttribute("slot",local.getAttribute("slot"));(0,_polymerDom.dom)(this).insertBefore(this.activeItem,local);// ensure that if we caught this event we process it
...
for(var i in children){if(babelHelpers.typeof(children[i].classList)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].classList.remove("mover")}}for(var j=1;j<=this.columns;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.remove("mover")}}// position arrows / set focus in case the DOM got updated above
setTimeout(function(){if(_this4.activeItem&&"function"===typeof _this4.activeItem.focus){_this4.positionArrows(_this4.activeItem);_this4.activeItem.focus()}},100)}}/**
   * Start a drag event, this is an element being dragged
   */},{key:"dragStart",value:function dragStart(e){if(this.editMode){var children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(babelHelpers.typeof(children[i].classList)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].classList.add("mover")}}for(var j=1;j<=this.columns;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.add("mover")}}}}/**
   * When we end dragging ensure we remove the mover class.
   */},{key:"dragEnd",value:function dragEnd(e){if(this.editMode){var children=(0,_polymerDom.dom)(this).children;// walk the children and apply the draggable state needed
for(var i in children){if(babelHelpers.typeof(children[i].classList)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))){children[i].classList.remove("mover","hovered")}}for(var j=1;j<=this.columns;j++){if(this.shadowRoot.querySelector("#col"+j)!==void 0){this.shadowRoot.querySelector("#col"+j).classList.remove("mover","hovered")}}}}/**
   * Insert event noticed by HAX
   */},{key:"haxInsertContent",value:function haxInsertContent(e){var _this5=this;// see if WE are the thing that's active when insert was fired
...
setTimeout(function(){_this5.editMode=!0;if(_this5.activeItem&&"function"===typeof _this5.activeItem.focus){_this5.positionArrows(_this5.activeItem);_this5.activeItem.focus()}},100)}}/**
   * Store updated, sync.
       ...
       * For example:```
  {
    ...
    {...}
      ...
      */layouts:{type:Object,readOnly:!0,value:new GridPlateLayoutOptions().layouts},/**
       ...
       */columnWidths:{type:String,computed:"_getColumnWidths(responsiveSize,layout,layouts,disableResponsive)"}}}}]);return GridPlate}(_polymerElement.PolymerElement);/**
 * `grid-plate`
