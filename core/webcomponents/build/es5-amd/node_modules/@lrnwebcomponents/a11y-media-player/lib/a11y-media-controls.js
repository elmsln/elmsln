 ...
 * @customElement
 * @polymer
 */var A11yMediaControls=/*#__PURE__*/function(_A11yMediaBehaviors){babelHelpers.inherits(A11yMediaControls,_A11yMediaBehaviors);function A11yMediaControls(){babelHelpers.classCallCheck(this,A11yMediaControls);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(A11yMediaControls).apply(this,arguments))}babelHelpers.createClass(A11yMediaControls,[{key:"connectedCallback",/**
   * life cycle, element is afixed to the DOM
   */value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yMediaControls.prototype),"connectedCallback",this).call(this);var root=this;root._addResponsiveUtility({element:root,attribute:"responsive-size",relativeToParent:!0,sm:300,md:600,lg:900,xl:1200})}/**
   * sets target for a11y keys
   */},{key:"ready",value:function ready(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yMediaControls.prototype),"ready",this).call(this);var root=this;// when the mute button is in focus,
...
root.$.mute.$.button.onblur=function(){root.$.volume.classList.remove("focus")}}/**
   ...
   */},{key:"setStatus",value:function setStatus(status){this.$.statbar.innerText=status}/**
   * loads tracks from array
   *
   * @param {object} the tracks of the media
   */},{key:"setTracks",value:function setTracks(tracks){this.set("tracks",[]);this.set("tracks",tracks.slice(0))}/**
   ...
   */},{key:"_getCompactControls",value:function _getCompactControls(responsiveSize){return this._testAttribute(responsiveSize,"xs")||this._testAttribute(responsiveSize,"sm")}/**
   * handles when the tracks dropdown selection changes
   * (when the tracks dropdown-select changes, update track and CC button)
   */},{key:"_handleTrackChange",value:function _handleTrackChange(e){var root=this;if(null!==root.__selectedTrack){if(""!==e.detail.value){root.dispatchEvent(new CustomEvent("select-track",{detail:{control:root,value:e.detail.value}}));root.dispatchEvent(new CustomEvent("toggle-cc",{detail:{control:root,value:!0}}))}else{root.dispatchEvent(new CustomEvent("toggle-cc",{detail:{control:root,value:!1}}))}}}/**
   * prevent settings menu from being closed before action is taken
   */},{key:"_handleSettingsSelect",value:function _handleSettingsSelect(e){}//e.preventDefault();
/**
   * prevent settings menu from being closed before action is taken
   */},{key:"_handleSettingsActivate",value:function _handleSettingsActivate(e){}//if (e.target == this.$.settingslist) e.preventDefault();
/**
   * determine which button was clicked and act accordingly
   */},{key:"_onButtonTap",value:function _onButtonTap(e){this.dispatchEvent(new CustomEvent("controls-change",{detail:e.detail}));this.$.settings.close()}/**
   * determine which button was clicked and act accordingly
   */},{key:"_onSettingsChanged",value:function _onSettingsChanged(e){this.dispatchEvent(new CustomEvent("controls-change",{detail:e.target}));this.$.settings.close()}/**
   ...
   */},{key:"_hideTranscriptButton",value:function _hideTranscriptButton(noTranscriptToggle,compactControls){return noTranscriptToggle||compactControls}/**
   ...
   */},{key:"_noPrinting",value:function _noPrinting(standAlone,fixedHeight){return standAlone||!fixedHeight}/**
   * Determines if the transcript toggle feature is available.
   *
   * @param {boolean} Is the player in stand-alone mode?
   * @param {boolean} Is the player in fixed-height/iframe mode?
   * @returns {boolean} Should transcript toggle be unavailable?
   */},{key:"_noTranscriptToggle",value:function _noTranscriptToggle(standAlone,fixedHeight,hasTranscript){return standAlone||fixedHeight||!hasTranscript}}],[{key:"properties",// properties available to the custom element for data binding
get:function get(){return{/**
   ...
   */},{key:"tag",get:function get(){return"a11y-media-controls"}//get player-specifc properties
},{key:"behaviors",get:function get(){return[_a11yMediaBehaviors.A11yMediaBehaviors]}//render function
},{key:"template",get:function get(){return(0,_polymerElement.html)(_templateObject_3f9efe7096cc11e9bb8ed118ba2d1e5c())}}]);return A11yMediaControls}(_a11yMediaBehaviors.A11yMediaBehaviors);_exports.A11yMediaControls=A11yMediaControls;window.customElements.define(A11yMediaControls.tag,A11yMediaControls)});