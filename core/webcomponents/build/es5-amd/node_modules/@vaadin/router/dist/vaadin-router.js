 ...
 */var Router=/*#__PURE__*/function(_Resolver){babelHelpers.inherits(Router,_Resolver);/**
   ...
   */function Router(outlet,options){var _this2;babelHelpers.classCallCheck(this,Router);var baseElement=document.head.querySelector("base");_this2=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(Router).call(this,[],Object.assign({// Default options
baseUrl:baseElement&&baseElement.getAttribute("href")},options)));_this2.resolveRoute=function(context){return _this2.__resolveRoute(context)};var triggers=Router.NavigationTrigger;Router.setTriggers.apply(Router,Object.keys(triggers).map(function(key){return triggers[key]}));/**
     ...
     */_this2.location;_this2.location=createLocation({resolver:babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this2))});_this2.__lastStartedRenderId=0;_this2.__navigationEventHandler=_this2.__onNavigationEvent.bind(babelHelpers.assertThisInitialized(babelHelpers.assertThisInitialized(_this2)));_this2.setOutlet(outlet);_this2.subscribe();return _this2}babelHelpers.createClass(Router,[{key:"__resolveRoute",value:function __resolveRoute(context){var route=context.route,callbacks=Promise.resolve();if(isFunction(route.children)){callbacks=callbacks.then(function(){return route.children(copyContextWithoutNext(context))}).then(function(children){// The route.children() callback might have re-written the
...
if(babelHelpers.instanceof(result,HTMLElement)||result.redirect||result===notFoundResult){return result}}if(isString(route.redirect)){return commands.redirect(route.redirect)}if(route.bundle){return loadBundle(route.bundle).then(function(){},function(){throw new Error(log("Bundle not found: ".concat(route.bundle,". Check if the file name is correct")))})}}).then(function(result){if(isResultNotEmpty(result)){return result}if(isString(route.component)){return commands.component(route.component)}})}/**
   ...
   */},{key:"setRoutes",value:function setRoutes(routes){this.__urlForName=void 0;babelHelpers.get(babelHelpers.getPrototypeOf(Router.prototype),"setRoutes",this).call(this,routes);this.__onNavigationEvent()}/**
   ...
   * @return {!Promise<!Node>}
   */},{key:"render",value:function render(pathnameOrContext,shouldUpdateHistory){var _this3=this,renderId=++this.__lastStartedRenderId,pathname=pathnameOrContext.pathname||pathnameOrContext;// Find the first route that resolves to a non-empty result
...
for(var i=currentContext.__divergedChainIndex;i<currentContext.chain.length;i++){var currentComponent=currentContext.chain[i].element||{},location=createLocation(currentContext,currentContext.chain[i].route);runCallbackIfPossible(currentComponent.onAfterEnter,[location,{},currentContext.resolver],currentComponent)}}},{key:"__animateIfNeeded",value:function __animateIfNeeded(context){for(var from=(this.__disappearingContent||[])[0],to=(this.__appearingContent||[])[0],promises=[],chain=context.chain,config,i=chain.length;0<i;i--){if(chain[i-1].route.animate){config=chain[i-1].route.animate;break}}if(from&&to&&config){var leave=isObject(config)&&config.leave||"leaving",enter=isObject(config)&&config.enter||"entering";promises.push(animate(from,leave));promises.push(animate(to,enter))}return Promise.all(promises).then(function(){return context})}/**
   ...
   * @return {boolean}
   */}],[{key:"setTriggers",value:function setTriggers(){for(var _len=arguments.length,triggers=Array(_len),_key=0;_key<_len;_key++){triggers[_key]=arguments[_key]}setNavigationTriggers(triggers)}},{key:"go",value:function go(pathname){return fireRouterEvent("go",{pathname:pathname})}}]);return Router}(Resolver);_exports.Router=Router;var DEV_MODE_CODE_REGEXP=/\/\*\*\s+vaadin-dev-mode:start([\s\S]*)vaadin-dev-mode:end\s+\*\*\//i;function isMinified(){function test(){/** vaadin-dev-mode:start
    return false;
...
try {
  ...
  vaadin-dev-mode:end **/}var usageStatistics=function usageStatistics(){if("function"===typeof runIfDevelopmentMode){return runIfDevelopmentMode(maybeGatherAndSendStats)}};window.Vaadin=window.Vaadin||{};window.Vaadin.registrations=window.Vaadin.registrations||[];window.Vaadin.registrations.push({is:"@vaadin/router",version:"1.2.0"});usageStatistics();Router.NavigationTrigger={POPSTATE:POPSTATE,CLICK:CLICK};//# sourceMappingURL=vaadin-router.js.map
});