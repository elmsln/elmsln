...
let sel=window.HaxStore.getSelection();var range;if(range=window.HaxStore.getRange()){range.deleteContents();range.insertNode(document.createTextNode(text))}}}/**
   ...
   */constructor(){super();new Promise((res,rej)=>_require.default(["./hax-app.js"],res,rej));new Promise((res,rej)=>_require.default(["./hax-stax.js"],res,rej));new Promise((res,rej)=>_require.default(["./hax-stax-browser.js"],res,rej));new Promise((res,rej)=>_require.default(["./hax-blox.js"],res,rej));new Promise((res,rej)=>_require.default(["./hax-blox-browser.js"],res,rej));new Promise((res,rej)=>_require.default(["../../hal-9000/hal-9000.js"],res,rej));(0,_settings.setPassiveTouchGestures)(!0);// claim the instance spot. This way we can easily
 ...
 */window.HaxStore.getHAXSlot=node=>{let content="";var slotnodes=(0,_polymerDom.dom)(node).children;// ensure there's something inside of this
if(0<slotnodes.length){// loop through everything found in the slotted area and put it back in
for(var j=0,len2=slotnodes.length;j<len2;j++){if(typeof slotnodes[j].tagName!==typeof void 0){// if we're a custom element, keep digging, otherwise a simple
// self append is fine.
