define(["exports","require","../../../@polymer/polymer/polymer-element.js","../../../@polymer/polymer/lib/utils/render-status.js","./hax-shared-styles.js"],function(_exports,_require,_polymerElement,_renderStatus,_haxSharedStyles){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.HaxBloxBrowserItem=void 0;_require=babelHelpers.interopRequireWildcard(_require);/**
 * `hax-blox-browser-item`
 * `A button on the hax-gizmo-browser app display`
 */class HaxBloxBrowserItem extends _polymerElement.PolymerElement{constructor(){super();new Promise((res,rej)=>_require.default(["../../../@polymer/paper-button/paper-button.js"],res,rej));new Promise((res,rej)=>_require.default(["../../../@polymer/paper-card/paper-card.js"],res,rej));new Promise((res,rej)=>_require.default(["../../../@polymer/iron-icon/iron-icon.js"],res,rej))}static get template(){return _polymerElement.html`
   ...
   */tapEventOff(e){this.elevation=1}/**
   * Fire an event that includes the eventName of what was just pressed.
   */_fireEvent(e){let content="";for(var i=0;i<this.blox.length;i++){let node=window.HaxStore.haxElementToNode(this.blox[i].tag,this.blox[i].content,this.blox[i].properties);content+=window.HaxStore.haxNodeToContent(node)}// generate a hax element
let blox={tag:"grid-plate",properties:{layout:this.layout},content:content};this.dispatchEvent(new CustomEvent("hax-insert-content",{bubbles:!0,cancelable:!0,composed:!0,detail:blox}));window.HaxStore.instance.haxBloxPicker.close()}}_exports.HaxBloxBrowserItem=HaxBloxBrowserItem;window.customElements.define(HaxBloxBrowserItem.tag,HaxBloxBrowserItem)});