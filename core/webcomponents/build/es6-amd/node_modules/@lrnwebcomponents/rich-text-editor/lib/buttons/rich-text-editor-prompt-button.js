...
static get properties(){return{/**
   ...
   * Handles button tap;
   */_buttonTap(e){e.preventDefault();this.open()}/**
   * updates prompt fields with selection data
   */updatePrompt(){this.fields.forEach(field=>{if(field.property&&""!==field.property){this.value[field.property]=this.__selectionContents.getAttribute(field.property)}else if(field.property&&""!==field.property){this.value[field.slot]=this.__selectionContents.querySelector(field.slot)}else{this.value[""]=this.__selectionContents.innerHTML}})}/**
   ...
   */updateSelection(){/**
     ...
     */let hasTag=!1;this.__selectionContents.innerHTML=``;this.fields.forEach(field=>{if(field.property&&""!==field.property){if(null!==this.value[field.property]&&""!==this.value[field.property].trim())hasTag=!0;this.__selectionContents.setAttribute(field.property,this.value[field.property].trim())}else if(field.slot&&""!==field.slot&&null!==this.value[field.slot]&&""!==this.value[field.slot].trim()){hasTag=!0;this.__selectionContents.innerHTML+=`${field.slot}${this.value[field.slot].trim()}${field.slot}`}else{this.__selectionContents.innerHTML+=`${this.value[field.property]}`}});if(!hasTag)this.__selection.unwrap()}/**
   * updates the insertion based on fields
   */confirm(){this.updateSelection();this.__selection.removeHighlight()}/**
   * updates the insertion based on fields
   */cancel(){this.__selection.innerHTML="";while(this.__revertContents.firstChild)this.__selection.appendChild(this.__revertContents.firstChild);this.__selection.normalize();this.__revertContents.remove();this.__selection.removeHighlight()}/**
   * Handles selecting text and opening prompt
   */open(){this.__revertContents=document.createElement("div");this.__revertContents.appendChild(this.__selection.getRangeContents());this.__selectionContents=this.__selection.wrapOrGetTag(this.tag);this.__selection.addHighlight();this.updatePrompt();//make sure there is a unique id so that the prompt popover appears near the selection
if(!this.__selectionContents.getAttribute("id"))this.__selectionContents.setAttribute("id","prompt"+Date.now());this.__prompt.setTarget(this);this.dispatchEvent(new CustomEvent("select",{detail:this}))}}_exports.RichTextEditorPromptButton=RichTextEditorPromptButton;window.customElements.define(RichTextEditorPromptButton.tag,RichTextEditorPromptButton)});