...
static get properties(){return{}}static get template(){return _polymerElement.html`
   ...
   */static get tag(){return"rich-text-editor-clipboard"}/**
   * component is ready
   */ready(){super.ready();let root=this;window.addEventListener("cut",root.handleCut.bind(root));window.addEventListener("copy",root.handleCopy.bind(root));window.addEventListener("paste",root.handlePaste.bind(root));window.addEventListener("cut-button",root.handleCopyButton.bind(root));window.addEventListener("copy-button",root.handleCopyButton.bind(root));window.addEventListener("paste-button",root.handlePasteButton.bind(root))}handleCut(e){e.preventDefault();this.copyToClipboard(this.getRange(),!0)}handleCopy(e){e.preventDefault();this.copyToClipboard(this.getRange())}handlePaste(e){e.preventDefault();this.pasteToClipboard(this.getRange())}handleCutButton(e){this.copyToClipboard(e.detail.range,!0)}handleCopyButton(e){this.copyToClipboard(e.detail.range)}handlePasteButton(e){this.pasteToClipboard(e.detail.range)}copyToClipboard(range,cut=!1){this.$.clipboard.innerHTML="";if(range)this.$.clipboard.appendChild(range.cloneContents());if(cut&&range.extractContents)range.extractContents()}pasteToClipboard(range){let div=document.createElement("div"),parent=range.commonAncestorContainer.parentNode,closest=parent.closest("[contenteditable=true]:not([disabled]),input:not([disabled]),textarea:not([disabled])");if(closest){div.innerHTML=this.$.clipboard.innerHTML;if(range&&range.extractContents){range.extractContents();range.insertNode(div);while(div.firstChild){div.parentNode.insertBefore(div.firstChild,div)}div.parentNode.removeChild(div)}}}/**
