define(["exports","meta","require","../../@polymer/polymer/polymer-element.js","../../@polymer/polymer/lib/utils/resolve-url.js","../es-global-bridge/es-global-bridge.js","../../@polymer/polymer/lib/elements/dom-if.js"],function(_exports,meta,_require,_polymerElement,_resolveUrl,_esGlobalBridge,_domIf){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.ImgPanZoom=void 0;meta=babelHelpers.interopRequireWildcard(meta);_require=babelHelpers.interopRequireWildcard(_require);/**
...
*/class ImgPanZoom extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
      <style>
        :host {
          display: block;
          position: relative;
...
visibilityRatio:{type:Number,value:1}}}constructor(){super();new Promise((res,rej)=>_require.default(["./lib/img-loader.js"],res,rej));const basePath=(0,_resolveUrl.pathFromUrl)(decodeURIComponent(meta.url));let location=`${basePath}lib/openseadragon/build/openseadragon/openseadragon.js`;window.addEventListener("es-bridge-openseadragon-loaded",this._openseadragonLoaded.bind(this));window.ESGlobalBridge.requestAvailability();window.ESGlobalBridge.instance.load("openseadragon",location)}_openseadragonLoaded(){this.__openseadragonLoaded=!0;if(this.dzi){this._initOpenSeadragon()}}disconnectedCallback(){super.disconnectedCallback();window.removeEventListener("es-bridge-openseadragon-loaded",this._openseadragonLoaded.bind(this))}ready(){super.ready();this.animationConfig={fade:{name:"fade-in-animation",node:this.shadowRoot.querySelector("#viewer")}};// Init openseadragon if we are using a deep zoom image
...
zoomOut(){// TODO: Replace with openseadragon native zoomOut
...
resetZoom(){this.viewer.viewport.goHome()}_srcChanged(){if(this.dzi&&this.init){// add tiled image
this._addTiledImage()}}// Add loaded images to viewer
_loadedChanged(){if(this.loaded){if(!this.init){this._initOpenSeadragon()}else{this._addImage()}}}_addImage(){this.viewer.addSimpleImage({url:this.src,index:0,replace:!0})}_addTiledImage(){this.viewer.addTiledImage({tileSource:this.src,index:0,replace:!0})}}_exports.ImgPanZoom=ImgPanZoom;window.customElements.define(ImgPanZoom.tag,ImgPanZoom)});