 ...
 */class SiteChildrenBlock extends _polymerElement.PolymerElement{/**
   ...
   */static get tag(){return"site-children-block"}constructor(){super();new Promise((res,rej)=>_require.default(["../../../../../@polymer/paper-button/paper-button.js"],res,rej))}// render function
static get template(){return _polymerElement.html`
   ...
   */_activeIdChanged(newValue){if(newValue){let el=null;//ensure that this level is included
...
if(tmpItem&&this.shadowRoot.querySelector("[data-id=\""+tmpItem.id+"\"]")){el=this.shadowRoot.querySelector("[data-id=\""+tmpItem.id+"\"]")}}}if(this._prevEl){this._prevEl.classList.remove("active")}if(el){el.classList.add("active");this._prevEl=el}}else{// shouldn't be possible but might as well list
if(this._prevEl){this._prevEl.classList.remove("active")}}}connectedCallback(){super.connectedCallback();this.__disposer=(0,_mobxModule.autorun)(()=>{this.manifest=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.manifest)});// minor timing thing to ensure store has picked active
// needed if routes set on first paint or lifecycles miss
setTimeout(()=>{this.__disposer2=(0,_mobxModule.autorun)(()=>{this.activeId=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.activeId)})},50)}disconnectedCallback(){super.disconnectedCallback();this.__disposer();if(this.__disposer2){this.__disposer2()}}}_exports.SiteChildrenBlock=SiteChildrenBlock;window.customElements.define(SiteChildrenBlock.tag,SiteChildrenBlock)});