...
let options={};if(this.baseURI){options.baseUrl=this.baseURI}this.router=new _vaadinRouter.Router(this,options);window.addEventListener("vaadin-router-location-changed",this._routerLocationChanged.bind(this))}/**
...
_async.microTask.run(()=>{setTimeout(()=>{let frag=document.createRange().createContextualFragment(html);(0,_polymerDom.dom)(this).appendChild(frag);const evt=new CustomEvent("json-outline-schema-active-body-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:{html}})},5)});// if there are, dynamically import them
...
}).catch(error=>{/* Error handling */console.log(error)})}}}}}}connectedCallback(){super.connectedCallback();(0,_renderStatus.afterNextRender)(this,function(){// tidy up the dom if this is there
...
setTimeout(()=>{if(this.routerManifest.items.find){var tmpItem=this.routerManifest.items.find(i=>i.location==e.detail.location.route.path);if(tmpItem){this.set("activeItem",tmpItem)}}},100)}}_exports.HAXCMSLegacyPlayer=HAXCMSLegacyPlayer;window.customElements.define(HAXCMSLegacyPlayer.tag,HAXCMSLegacyPlayer)});