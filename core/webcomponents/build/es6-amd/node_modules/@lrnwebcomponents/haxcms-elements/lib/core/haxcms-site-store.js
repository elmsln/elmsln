...
if(tmpItem){parent=tmpItem.parent}break;case"ancestor":tmpItem=_routerManifest.items.find(d=>parent===d.id);// walk back up to the root
while(tmpItem&&null!=tmpItem.parent){// take the parent object of this current item
tmpItem=_routerManifest.items.find(i=>i.id==tmpItem.parent)}if(tmpItem){parent=tmpItem.id}break;}_routerManifest.items.forEach((item,i)=>{store.spiderChildren(item,data,start,end,parent,!1,noDynamicLevel)});return data}}}(0,_mobxModule.decorate)(Store,{location:_mobxModule.observable.ref,// router location in url
 ...
 */_exports.store=store;(0,_mobxModule.autorun)(()=>{if(store.location&&store.location.route&&store.location.route.component){// get the id from the router
 ...
 * change the page.
 */(0,_mobxModule.autorun)(()=>{const foundItem=(0,_mobxModule.toJS)(store.findItem(store.activeId));if(foundItem){document.body.dispatchEvent(new CustomEvent("json-outline-schema-active-item-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:foundItem}))}});/**
 * When editMode changes notify HAXeditor.
 */(0,_mobxModule.autorun)(()=>{const editMode=(0,_mobxModule.toJS)(store.editMode);// trap for early setup
