 ...
 */class SimpleBlog extends(0,_HAXCMSThemeWiring.HAXCMSTheme)(_polymerElement.PolymerElement){/**
...
static get template(){return _polymerElement.html`
   ...
   */static get properties(){let props=super.properties;props.selectedPage={type:Number,reflectToAttribute:!0,value:0};return props}constructor(){super();new Promise((res,rej)=>_require.default(["./lib/simple-blog-header.js"],res,rej));new Promise((res,rej)=>_require.default(["../../@polymer/paper-icon-button/paper-icon-button.js"],res,rej));new Promise((res,rej)=>_require.default(["./lib/simple-blog-footer.js"],res,rej));this.__disposer=[];(0,_mobxModule.autorun)(reaction=>{this.activeId=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.activeId);this.__disposer.push(reaction)});(0,_mobxModule.autorun)(reaction=>{this._locationChanged(_haxcmsSiteStore.store.location);this.__disposer.push(reaction)})}/**
   * attached life cycle
   */connectedCallback(){super.connectedCallback();(0,_renderStatus.afterNextRender)(this,()=>{this.contentContainer=this.shadowRoot.querySelector("simple-blog-post").$.contentcontainer})}/**
