 ...
 */class StorybookUtilities{/**
   ...
   */cleanHTML(template,replacements=[]){// need to account for polymer goofiness when webpack rolls this up
...
template=array_matches[1];replacements.forEach(replacement=>{template=template.replace(new RegExp(replacement.find,"g"),replacement.replace)});template=template.replace(/=\\"\\"/g,"").replace(/\\"/g,"\"");return template}/**
   ...
   */addPattern(story){let template=this.cleanHTML(story.file,story.replacements);story.before=story.before||``;story.after=story.after||``;story.demo=(0,_index.storiesOf)(story.of,module);story.demo.add(story.name,()=>{return`${story.before}${template}${story.after}`});return[story.before,template,story.after]}/**
   ...
   */getBindings(props){let binding={},keys=Object.keys(props).sort();keys.forEach(key=>{// skip prototype, private properties, objects, anything in the exclusions array, or any computed property
//if (!props.hasOwnProperty(key)) continue;
let editable=!1===key.startsWith("__")&&(props[key].computed===void 0||"undefined"===props[key].computed)&&!0!==props[key].readOnly;if(editable){let keyType=props[key].type.name||props[key].type;// convert typed props
if(keyType){let method=keyType.toLowerCase(),stringifiedVal=JSON.stringify(props[key].value||""),kebab=key.replace(/[A-Z\u00C0-\u00D6\u00D8-\u00DE]/g,function(match){return"-"+match.toLowerCase()});// ensure ke-bab case
...
storybookBridge.text(key,props[key].value||``)}}}});return binding}/**
   ...
   */getSimpleColors(color="blue"){return{accentColor:{name:"accentColor",type:"Select",value:color,options:Object.keys(window.SimpleColorsUtilities.colors)},dark:{name:"dark",type:"Boolean",value:!1}}}/**
   ...
   */addLiveDemo(story,escape=!1){story.alias=story.alias!==void 0?story.alias:story.name;story.before=story.before!==void 0?story.before:``;story.after=story.after!==void 0?story.after:``;story.attr=story.attr!==void 0?story.attr:``;story.slotted=story.slotted!==void 0?story.slotted:``;story.slots=story.slots!==void 0?story.slots:{};story.props=story.props!==void 0?story.props:{};story.demo=(0,_index.storiesOf)(story.of,module);story.demo.addDecorator(storybookBridge.withKnobs);story.demo.add(story.alias,()=>{story.attrBindings=``;Object.values(this.getBindings(story.props)).forEach(prop=>{if(!1!==prop.value&&""!==prop.value){story.attrBindings+="string"!==typeof prop.value?` ${prop.id}="${prop.value}"`:` ${prop.id}="${prop.value.replace(/'/g,"&apos;").replace(/"/g,"&quot;")}"`}});story.slotBindings=``;Object.values(this.getBindings(story.slots)).forEach(slot=>{story.slotBindings+="slot"!==slot.id?`<div slot="${slot.id}">${slot.value}</div>`:`${slot.value}`});return`
        <h1>${story.alias}</h1>
        ${story.before}
   ...
   */addGlobalScript(name,location){window.ESGlobalBridge.requestAvailability();if(!window.ESGlobalBridge.webpack)window.ESGlobalBridge.webpack={};window.ESGlobalBridge.webpack[name]=!0;window.ESGlobalBridge.instance.load(name,location,!0)}}// register global bridge on window if needed
_exports.StorybookUtilities=StorybookUtilities;window.StorybookUtilities=window.StorybookUtilities||{};window.StorybookUtilities.requestAvailability=()=>{if(!window.StorybookUtilities.instance){window.StorybookUtilities.instance=new StorybookUtilities}}});