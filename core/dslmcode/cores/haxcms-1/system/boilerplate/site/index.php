...
<body no-js <?php print $site->getSitePageAttributes();?>>
  ...
  <script>document.body.removeAttribute('no-js');var cdn="<?php print $HAXCMS->getCDNForDynamic();?>";var forceUpgrade=<?php print $site->getForceUpgrade();?>;var old=false;var ancient=false;
      ...
      try { new Function('import("");'); } catch (err) {
        ...
        if (/Firefox\/6/.test(navigator.userAgent) || window.customElements) {
          defs = [
            ...
            cdn + "build/es6-amd/dist/build.js<?php print $HAXCMS->cacheBusterHash();?>",
            "./custom/build/custom.es6-amd.js<?php print $HAXCMS->cacheBusterHash();?>"
          ];
        }
        else {
          defs = [
          ...
          if (document.documentMode || /Edge/.test(navigator.userAgent)) { // stupid edge
            defs.push(cdn + "build/es5-amd/dist/build-legacy.js<?php print $HAXCMS->cacheBusterHash();?>");
          }
          else {
            defs.push(cdn + "build/es5-amd/dist/build.js<?php print $HAXCMS->cacheBusterHash();?>");
          }
        }
    ...
    link.rel = 'stylesheet';
    link.href = cdn + 'build/es6/node_modules/@lrnwebcomponents/haxcms-elements/lib/base.css<?php print $HAXCMS->cacheBusterHash();?>';
    link.type = 'text/css';
    var def = document.getElementsByTagName('link')[0];
    def.parentNode.insertBefore(link, def);
    var link2 = document.createElement('link');
    link2.rel = 'stylesheet';
    link2.href = './theme/theme.css<?php print $HAXCMS->cacheBusterHash();?>';
    link2.type = 'text/css';
