define(["exports","../../../@polymer/polymer/polymer-legacy.js","../../../@polymer/iron-resizable-behavior/iron-resizable-behavior.js","../../../@polymer/iron-scroll-target-behavior/iron-scroll-target-behavior.js","../../../@polymer/polymer/lib/utils/async.js","../../../@polymer/polymer/lib/legacy/class.js","../../../@polymer/polymer/lib/utils/flush.js","../../../@polymer/polymer/lib/legacy/polymer.dom.js","../../../@polymer/polymer/lib/utils/debounce.js"],function(_exports,_polymerLegacy,_ironResizableBehavior,_ironScrollTargetBehavior,_async,_class,_flush,_polymerDom,_debounce2){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.PolymerIronList=void 0;/**
@license
...
*/var IOS=navigator.userAgent.match(/iP(?:hone|ad;(?: U;)? CPU) OS (\d+)/),IOS_TOUCH_SCROLLING=IOS&&8<=IOS[1],DEFAULT_PHYSICAL_COUNT=3,ANIMATION_FRAME=_async.animationFrame,IDLE_TIME=_async.idlePeriod,MICRO_TASK=_async.microTask,PolymerIronList=(0,_class.Class)({behaviors:[_ironResizableBehavior.IronResizableBehavior,_ironScrollTargetBehavior.IronScrollTargetBehavior],/**
...
_flush.flush?(0,_flush.flush)():(0,_polymerDom.flush)();var newPhysicalSize=0,oldPhysicalSize=0,prevAvgCount=this._physicalAverageCount,prevPhysicalAvg=this._physicalAverage;this._iterateItems(function(pidx,vidx){oldPhysicalSize+=this._physicalSizes[pidx];this._physicalSizes[pidx]=this._physicalItems[pidx].offsetHeight;newPhysicalSize+=this._physicalSizes[pidx];this._physicalAverageCount+=this._physicalSizes[pidx]?1:0},itemSet);if(this.grid){this._updateGridMetrics();this._physicalSize=Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight}else{oldPhysicalSize=1===this._itemsPerRow?oldPhysicalSize:Math.ceil(this._physicalCount/this._itemsPerRow)*this._rowHeight;this._physicalSize=this._physicalSize+newPhysicalSize-oldPhysicalSize;this._itemsPerRow=1}// Update the average if it measured something.
...
if(forceUpdate||Math.abs(this._estScrollHeight-this._scrollHeight)>=this._viewportHeight){this.$.items.style.height=this._estScrollHeight+"px";this._scrollHeight=this._estScrollHeight}},/**
   ...
   */scrollToIndex:function scrollToIndex(idx){if("number"!==typeof idx||0>idx||idx>this.items.length-1){return}_flush.flush?(0,_flush.flush)():(0,_polymerDom.flush)();// Items should have been rendered prior scrolling to an index.
...
this._itemsPerRow=this._itemsPerRow||1;return this.grid?Math.ceil(idx/this._itemsPerRow)*this._itemsPerRow:idx},_isIndexRendered:function _isIndexRendered(idx){return idx>=this._virtualStart&&idx<=this._virtualEnd},_getPhysicalIndex:function _getPhysicalIndex(vidx){return(this._physicalStart+(vidx-this._virtualStart))%this._physicalCount},_clamp:function _clamp(v,min,max){return Math.min(max,Math.max(min,v))},_debounce:function _debounce(name,cb,asyncModule){this._debouncers=this._debouncers||{};this._debouncers[name]=_debounce2.Debouncer.debounce(this._debouncers[name],asyncModule,cb.bind(this));(0,_flush.enqueueDebouncer)(this._debouncers[name])}});_exports.PolymerIronList=PolymerIronList});