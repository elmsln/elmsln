define(["exports"], function (_exports) {
  ...
  var Resolver = /*#__PURE__*/function () {
    ...
    babelHelpers.createClass(Resolver, [{
      ...
      value: function setRoutes(routes) {
        ensureRoutes(routes);
        var newRoutes = babelHelpers.toConsumableArray(toArray(routes));
      ...
      value: function addRoutes(routes) {
        ...
        (_this$root$__children = this.root.__children).push.apply(_this$root$__children, babelHelpers.toConsumableArray(toArray(routes)));
  ...
  function generateUrls(router) {
    ...
    if (!babelHelpers.instanceof(router, Resolver)) {
  ...
  var Router = /*#__PURE__*/function (_Resolver) {
    ...
    function Router(outlet, options) {
      ...
      _this2.resolveRoute = function (context) {
        return _this2.__resolveRoute(context);
      };
      ...
      _this2.ready;
      _this2.ready = Promise.resolve(outlet);
      /**
      ...
      _this2.__lastStartedRenderId = 0;
      _this2.__navigationEventHandler = _this2.__onNavigationEvent.bind(babelHelpers.assertThisInitialized(_this2));

      _this2.setOutlet(outlet);

      _this2.subscribe(); // Using WeakMap instead of WeakSet because WeakSet is not supported by IE11


      _this2.__createdByRouter = new WeakMap();
      _this2.__addedByRouter = new WeakMap();
      return _this2;
    }
    ...
    babelHelpers.createClass(Router, [{
      ...
      value: function __resolveRoute(context) {
        ...
        var commands = {
          ...
          component: function component(_component) {
            ...
            _this3.__createdByRouter.set(element, true);
        ...
        return callbacks.then(function () {
          ...
          if (isResultNotEmpty(result)) {
            ...
            if (babelHelpers.instanceof(result, HTMLElement) || result.redirect || result === notFoundResult) {
              return result;
            }
          }
      ...
      value: function render(pathnameOrContext, shouldUpdateHistory) {
        ...
        .then(function (context) {
          return _this4.__fullyResolveChain(context);
        }).then(function (context) {
          if (_this4.__isLatestRender(context)) {
            var previousContext = _this4.__previousContext; // Check if the render was prevented and make an early return in that case

            if (context === previousContext) {
              // Replace the history with the previous context
              // to make sure the URL stays the same.
              _this4.__updateBrowserHistory(previousContext, true);

              return _this4.location;
            }

            _this4.location = createLocation(context);

            if (shouldUpdateHistory) {
              // Replace only if first render redirects, so that we don’t leave
              // the redirecting record in the history
              _this4.__updateBrowserHistory(context, renderId === 1);
            }
            ...
            if (context.__skipAttach) {
              _this4.__copyUnchangedElements(context, previousContext);

              _this4.__previousContext = context;
              return _this4.location;
            }

            _this4.__addAppearingContent(context, previousContext);

            var animationDone = _this4.__animateIfNeeded(context);

            _this4.__runOnAfterEnterCallbacks(context);

            _this4.__runOnAfterLeaveCallbacks(context, previousContext);
            ...
            return animationDone.then(function () {
              if (_this4.__isLatestRender(context)) {
                ...
                _this4.__removeDisappearingContent();
          ...
          if (renderId === _this4.__lastStartedRenderId) {
            if (shouldUpdateHistory) {
              _this4.__updateBrowserHistory(context);
            }

            removeDomNodes(_this4.__outlet && _this4.__outlet.children);
            _this4.location = createLocation(Object.assign(context, {
              resolver: _this4
            }));
      ...
      value: function __fullyResolveChain(topOfTheChainContextBeforeRedirects) {
        ...
        .then(function (contextAfterRedirects) {
          ...
          return findNextContextIfAny(contextAfterRedirects).then(function (nextContext) {
            ...
            return nextContext && nextContext !== notFoundResult && nextContext !== contextAfterRedirects ? _this5.__fullyResolveChain(topOfTheChainContextAfterRedirects, nextContext) : _this5.__amendWithOnBeforeCallbacks(contextAfterRedirects);
          });
        });
      }
    }, {
      ...
      value: function __findComponentContextAfterAllRedirects(context) {
        ...
        if (babelHelpers.instanceof(result, HTMLElement)) {
          ...
          return this.__redirect(result.redirect, context.__redirectCount, context.__renderId).then(function (context) {
            return _this6.__findComponentContextAfterAllRedirects(context);
          });
      ...
      value: function __amendWithOnBeforeCallbacks(contextWithFullChain) {
        ...
        return this.__runOnBeforeCallbacks(contextWithFullChain).then(function (amendedContext) {
          if (amendedContext === _this7.__previousContext || amendedContext === contextWithFullChain) {
            return amendedContext;
          }

          return _this7.__fullyResolveChain(amendedContext);
        });
      }
    }, {
      ...
      value: function __runOnBeforeCallbacks(newContext) {
        ...
        if (previousChain.length) {
          ...
          if (newContext.__skipAttach) {
            ...
            for (var _i2 = newChain.length - 1; _i2 >= 0; _i2--) {
            ...
            for (var _i3 = 0; _i3 < newChain.length; _i3++) {
              callbacks = this.__runOnBeforeEnterCallbacks(callbacks, newContext, {
                prevent: prevent,
                redirect: redirect
              }, newChain[_i3]);
              previousChain[_i3].element.location = createLocation(newContext, previousChain[_i3].route);
            }
          } else {
            // execute onBeforeLeave when NOT skipping attach
            for (var _i4 = previousChain.length - 1; _i4 >= newContext.__divergedChainIndex; _i4--) {
              callbacks = this.__runOnBeforeLeaveCallbacks(callbacks, newContext, {
                prevent: prevent
              }, previousChain[_i4]);
            }
        ...
        if (!newContext.__skipAttach) {
          for (var _i5 = 0; _i5 < newChain.length; _i5++) {
            if (_i5 < newContext.__divergedChainIndex) {
              if (_i5 < previousChain.length && previousChain[_i5].element) {
                previousChain[_i5].element.location = createLocation(newContext, previousChain[_i5].route);
              }
            } else {
              callbacks = this.__runOnBeforeEnterCallbacks(callbacks, newContext, {
                prevent: prevent,
              ...
              if (newChain[_i5].element) {
                newChain[_i5].element.location = createLocation(newContext, newChain[_i5].route);
              }
            }
          }
        }
        ...
        return callbacks.then(function (amendmentResult) {
          if (amendmentResult) {
            if (amendmentResult.cancel) {
              _this8.__previousContext.__renderId = newContext.__renderId;
              return _this8.__previousContext;
            }

            if (amendmentResult.redirect) {
              return _this8.__redirect(amendmentResult.redirect, newContext.__redirectCount, newContext.__renderId);
            }
          }
      ...
      value: function __runOnBeforeLeaveCallbacks(callbacks, newContext, commands, chainElement) {
        ...
        return callbacks.then(function (result) {
          if (_this9.__isLatestRender(newContext)) {
            var afterLeaveFunction = amend('onBeforeLeave', [location, commands, _this9], chainElement.element);
      ...
      value: function __runOnBeforeEnterCallbacks(callbacks, newContext, commands, chainElement) {
        ...
        return callbacks.then(function (result) {
          if (_this10.__isLatestRender(newContext)) {
            var beforeEnterFunction = amend('onBeforeEnter', [location, commands, _this10], chainElement.element);
      ...
      value: function __addAppearingContent(context, previousContext) {
        ...
        function (e) {
          return _this11.__addedByRouter.get(e) && // Do not remove the result element to avoid flickering
          e !== context.result;
