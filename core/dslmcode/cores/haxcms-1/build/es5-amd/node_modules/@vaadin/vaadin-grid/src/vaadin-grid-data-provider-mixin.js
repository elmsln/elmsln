 ...
 */_exports.ItemCache=ItemCache;var DataProviderMixin=function DataProviderMixin(superClass){return(/*#__PURE__*/function(_superClass){babelHelpers.inherits(DataProviderMixin,_superClass);function DataProviderMixin(){babelHelpers.classCallCheck(this,DataProviderMixin);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(DataProviderMixin).apply(this,arguments))}babelHelpers.createClass(DataProviderMixin,[{key:"_sizeChanged",value:function _sizeChanged(size){var delta=size-this._cache.size;this._cache.size+=delta;this._cache.effectiveSize+=delta;this._effectiveSize=this._cache.effectiveSize}},{key:"_updateRowItem",value:function _updateRowItem(item,el){el.children.forEach(function(cell){cell._instance&&(cell._instance.item=item)})}},{key:"_getItem",value:function _getItem(index,el){if(index>=this._effectiveSize){return}el.index=index;var _this$_cache$getCache=this._cache.getCacheAndIndex(index),cache=_this$_cache$getCache.cache,scaledIndex=_this$_cache$getCache.scaledIndex,item=cache.items[scaledIndex];if(item){this._toggleAttribute("loading",!1,el);this._updateItem(el,item);if(this._isExpanded(item)){cache.ensureSubCacheForScaledIndex(scaledIndex)}}else{this._toggleAttribute("loading",!0,el);this._loadPage(this._getPageForIndex(scaledIndex),cache)}}},{key:"_pagesForPhysicalItems",value:function _pagesForPhysicalItems(){var _this3=this,firstVisiblePage=this._getPageForIndex(this._firstVisibleIndex+this._vidxOffset);return[firstVisiblePage].concat(this._physicalItems.filter(function(row){return row.index}).items(function(row){return _this3._getPageForIndex(row.index)})).reduce(function(prev,curr){if(-1===prev.indexOf(curr)){prev.push(curr)}return prev},[])}},{key:"_expandedInstanceChangedCallback",value:function _expandedInstanceChangedCallback(inst,value){if(inst.item===void 0){return}if(value){this.expandItem(inst.item)}else{this.collapseItem(inst.item)}}/**
   ...
   */},{key:"collapseItem",value:function collapseItem(item){if(this._isExpanded(item)){this.splice("expandedItems",this._getItemIndexInArray(item,this.expandedItems),1)}}},{key:"_getIndexLevel",value:function _getIndexLevel(index){var _this$_cache$getCache2=this._cache.getCacheAndIndex(index),cache=_this$_cache$getCache2.cache,level=0;while(cache.parentCache){cache=cache.parentCache;level++}return level}},{key:"_canPopulate",value:function _canPopulate(){return this._hasData&&this._columnTree}},{key:"_loadPage",value:function _loadPage(page,cache){var _this4=this;// make sure same page isn't requested multiple times.
...
items.forEach(function(item,itemsIndex){var itemIndex=page*_this4.pageSize+itemsIndex;cache.items[itemIndex]=item;if(_this4._isExpanded(item)){// Force synchronous data request for expanded item sub-cache
cache.ensureSubCacheForScaledIndex(itemIndex)}});_this4._hasData=!0;delete cache.pendingRequests[page];if(!_this4._cache.isLoading()){// All active requests have finished, update the effective size and rows
_this4._setLoading(!1);_this4._cache.updateSize();_this4._effectiveSize=_this4._cache.effectiveSize;Array.from(_this4.$.items.children).filter(function(row){return!row.hidden}).forEach(function(row){var cachedItem=_this4._cache.getItemForIndex(row.index);if(cachedItem){_this4._toggleAttribute("loading",!1,row);_this4._updateItem(row,cachedItem)}});_this4._increasePoolIfNeeded(0)}_this4.__itemsReceived()})}}},{key:"_getPageForIndex",value:function _getPageForIndex(index){return Math.floor(index/this.pageSize)}/**
...
this._scrollToIndex(this._firstVisibleIndex)}this._ensureFirstPageLoaded();this._debouncerCheckSize=_debounce.Debouncer.debounce(this._debouncerCheckSize,_async.timeOut.after(2e3),this._checkSize.bind(this));this._scrollHandler()}},{key:"_ensureFirstPageLoaded",value:function _ensureFirstPageLoaded(){var _this5=this;if(!this._hasData){// load data before adding rows to make sure they have content when
// rendered for the first time.
this._loadPage(0,this._cache,function(){var hadData=_this5._hasData;_this5._hasData=!0;if(!hadData){_this5.notifyResize()}})}}},{key:"_itemsEqual",value:function _itemsEqual(item1,item2){return this.getItemId(item1)===this.getItemId(item2)}},{key:"_getItemIndexInArray",value:function _getItemIndexInArray(item,array){var _this6=this,result=-1;array.forEach(function(i,idx){if(_this6._itemsEqual(i,item)){result=idx}});return result}}],[{key:"properties",get:function get(){return{/**
       ...
       */expandedItems:{type:Object,notify:!0,value:function value(){return[]}}}}},{key:"observers",get:function get(){return["_sizeChanged(size)","_expandedItemsChanged(expandedItems.*)"]}}]);return DataProviderMixin}(superClass))};_exports.DataProviderMixin=DataProviderMixin});