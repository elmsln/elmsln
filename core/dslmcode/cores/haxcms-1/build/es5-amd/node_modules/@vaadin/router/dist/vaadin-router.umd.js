...
if(route.parent){var i=context.chain.length;while(i--&&context.chain[i].route&&context.chain[i].route!==route.parent){context.chain.pop()}}}context.chain.push(item)}}/**
   */var Resolver=/*#__PURE__*/function(){function Resolver(routes){var options=1<arguments.length&&arguments[1]!==void 0?arguments[1]:{};_classCallCheck(this,Resolver);if(Object(routes)!==routes){throw new TypeError("Invalid routes")}this.baseUrl=options.baseUrl||"";this.errorHandler=options.errorHandler;this.resolveRoute=options.resolveRoute||resolveRoute;this.context=Object.assign({resolver:this},options.context);this.root=Array.isArray(routes)?{path:"",__children:routes,parent:null,__synthetic:!0}:routes;this.root.parent=null}/**
     * Returns the current list of routes (as a shallow copy). Adding / removing
     * routes to / from the returned array does not affect the routing config,
     * but modifying the route objects does.
     *
     * @return {!Array<!Router.Route>}
     */_createClass(Resolver,[{key:"getRoutes",value:function getRoutes(){return _toConsumableArray(this.root.__children)}/**
       * Sets the routing config (replacing the existing one).
       *
       * @param {!Array<!Router.Route>|!Router.Route} routes a single route or an array of those
       *    (the array is shallow copied)
       */},{key:"setRoutes",value:function setRoutes(routes){ensureRoutes(routes);var newRoutes=_toConsumableArray(toArray(routes));this.root.__children=newRoutes}/**
       ...
       * @return {!Array<!Router.Route>}
       * @protected
       */},{key:"addRoutes",value:function addRoutes(routes){var _this$root$__children;ensureRoutes(routes);(_this$root$__children=this.root.__children).push.apply(_this$root$__children,_toConsumableArray(toArray(routes)));return this.getRoutes()}/**
...
return pathname}var base=this.__effectiveBaseUrl,normalizedUrl=this.constructor.__createUrl(pathname,base).href;if(normalizedUrl.slice(0,base.length)===base){return normalizedUrl.slice(base.length)}}},{key:"__effectiveBaseUrl",/**
       ...
       */get:function get(){return this.baseUrl?this.constructor.__createUrl(this.baseUrl,document.baseURI||document.URL).href.replace(/[^\/]*$/,""):""}}],[{key:"__createUrl",value:function __createUrl(url,base){return new URL(url,base)}}]);return Resolver}();Resolver.pathToRegexp=pathToRegexp_1;/**
   * Universal Router (https://www.kriasoft.com/universal-router/)
   *
   * Copyright (c) 2015-present Kriasoft.
   *
...
cacheRoutes(routesByName,router.root,router.root.__children);route=getRouteByName(routesByName,routeName);if(!route){throw new Error("Route \"".concat(routeName,"\" not found"))}}var regexp=cache$1.get(route.fullPath);if(!regexp){var fullPath=getRoutePath(route),rt=route.parent;while(rt){var path=getRoutePath(rt);if(path){fullPath=path.replace(/\/$/,"")+"/"+fullPath.replace(/^\//,"")}rt=rt.parent}for(var tokens=pathToRegexp$1.parse(fullPath),toPath=pathToRegexp$1.tokensToFunction(tokens),keys=Object.create(null),i=0;i<tokens.length;i++){if(!isString(tokens[i])){keys[tokens[i].name]=!0}}regexp={toPath:toPath,keys:keys};cache$1.set(fullPath,regexp);route.fullPath=fullPath}var url=regexp.toPath(params,options)||"/";if(options.stringifyQueryParams&&params){for(var queryParams={},_keys=Object.keys(params),_i=0,key;_i<_keys.length;_i++){key=_keys[_i];if(!regexp.keys[key]){queryParams[key]=params[key]}}var query=options.stringifyQueryParams(queryParams);if(query){url+="?"===query.charAt(0)?query:"?".concat(query)}}return url}}/**
   ...
   */var Router=/*#__PURE__*/function(_Resolver){_inherits(Router,_Resolver);/**
...
baseUrl:baseHref&&Resolver.__createUrl(baseHref,document.URL).pathname.replace(/[^\/]*$/,"")},options)));_this.resolveRoute=function(context){return _this.__resolveRoute(context)};var triggers=Router.NavigationTrigger;Router.setTriggers.apply(Router,Object.keys(triggers).map(function(key){return triggers[key]}));/**
       ...
       * @public
       * @type {!RouterLocation}
       */_this.location;_this.location=createLocation({resolver:_assertThisInitialized(_this)});_this.__lastStartedRenderId=0;_this.__navigationEventHandler=_this.__onNavigationEvent.bind(_assertThisInitialized(_this));_this.setOutlet(outlet);_this.subscribe();// Using WeakMap instead of WeakSet because WeakSet is not supported by IE11
_this.__createdByRouter=new WeakMap;_this.__addedByRouter=new WeakMap;return _this}_createClass(Router,[{key:"__resolveRoute",value:function __resolveRoute(context){var _this2=this,route=context.route,callbacks=Promise.resolve();if(isFunction(route.children)){callbacks=callbacks.then(function(){return route.children(copyContextWithoutNext(context))}).then(function(children){// The route.children() callback might have re-written the
...
if(babelHelpers.instanceof(result,HTMLElement)||result.redirect||result===notFoundResult){return result}}if(isString(route.redirect)){return commands.redirect(route.redirect)}if(route.bundle){return loadBundle(route.bundle).then(function(){},function(){throw new Error(log("Bundle not found: ".concat(route.bundle,". Check if the file name is correct")))})}}).then(function(result){if(isResultNotEmpty(result)){return result}if(isString(route.component)){return commands.component(route.component)}})}/**
       ...
       *
       * @return {!Promise<!Node>}
       */},{key:"setRoutes",value:function setRoutes(routes){var skipRender=1<arguments.length&&arguments[1]!==void 0?arguments[1]:!1;this.__previousContext=void 0;this.__urlForName=void 0;_get(_getPrototypeOf(Router.prototype),"setRoutes",this).call(this,routes);if(!skipRender){this.__onNavigationEvent()}return this.ready}/**
...
_this3.__updateBrowserHistory(context,1===renderId)}fireRouterEvent("location-changed",{router:_this3,location:_this3.location});// Skip detaching/re-attaching there are no render changes
...
window.Vaadin=window.Vaadin||{};/**
   ...
   */var runIfDevelopmentMode=function runIfDevelopmentMode(callback,args){if(window.Vaadin.developmentMode){return uncommentAndRun(callback,args)}};if(window.Vaadin.developmentMode===void 0){window.Vaadin.developmentMode=isDevelopmentMode()}/* This file is autogenerated from src/vaadin-usage-statistics.tpl.html */function maybeGatherAndSendStats(){/** vaadin-dev-mode:start
    ...
    // Intentionally ignored as this is not a problem in the app being developed
     ...
     vaadin-dev-mode:end **/}var usageStatistics=function usageStatistics(){if("function"===typeof runIfDevelopmentMode){return runIfDevelopmentMode(maybeGatherAndSendStats)}};window.Vaadin=window.Vaadin||{};window.Vaadin.registrations=window.Vaadin.registrations||[];window.Vaadin.registrations.push({is:"@vaadin/router",version:"1.7.0"});usageStatistics();Router.NavigationTrigger={POPSTATE:POPSTATE,CLICK:CLICK};var isUrlAvailable,urlDocument,urlBase,urlAnchor;Resolver.__ensureUrlAvailableOrPolyfilled=function(){if(isUrlAvailable===void 0){try{var url=new URL("b","http://a");url.pathname="c";isUrlAvailable="http://a/c"===url.href}catch(e){isUrlAvailable=!1}if(!isUrlAvailable){// The URL constructor is not available in IE11. Polyfill it by creating
...
return urlAnchor};exports.Resolver=Resolver;exports.Router=Router;Object.defineProperty(exports,"__esModule",{value:!0})});