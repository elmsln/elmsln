  ...
  var DataProviderMixin = function DataProviderMixin(superClass) {
    return /*#__PURE__*/function (_superClass) {
      ...
      babelHelpers.createClass(DataProviderMixin, [{
        ...
        value: function _expandedItemsChanged() {
        ...
        value: function _itemIdPathChanged() {
          this.__cacheExpandedKeys();
        }
        /** @private */
        ...
        value: function expandItem(item) {
          if (!this._isExpanded(item)) {
            this.expandedItems = [].concat(babelHelpers.toConsumableArray(this.expandedItems), [item]);
          }
        }
        /**
        ...
        value: function collapseItem(item) {
          ...
          if (this._isExpanded(item)) {
            this.expandedItems = this.expandedItems.filter(function (i) {
              return !_this4._itemsEqual(i, item);
            });
          }
        }
        /**
        ...
        value: function _loadPage(page, cache) {
          ...
          if (!cache.pendingRequests[page] && this.dataProvider) {
            ...
            this.dataProvider(params, function (items, size) {
              ...
              var currentItems = Array.from(_this5.$.items.children).map(function (row) {
                return row._item;
              }); // Populate the cache with new items

              items.forEach(function (item, itemsIndex) {
                var itemIndex = page * _this5.pageSize + itemsIndex;
                cache.items[itemIndex] = item;

                if (_this5._isExpanded(item) && currentItems.indexOf(item) > -1) {
              ...
              _this5._debouncerApplyCachedData = _debounce.Debouncer.debounce(_this5._debouncerApplyCachedData, _async.timeOut.after(0), function () {
                _this5._setLoading(false);

                _this5._cache.updateSize();

                _this5._effectiveSize = _this5._cache.effectiveSize;
                Array.from(_this5.$.items.children).filter(function (row) {
                  return !row.hidden;
                }).forEach(function (row) {
                  var cachedItem = _this5._cache.getItemForIndex(row.index);

                  if (cachedItem) {
                    _this5._getItem(row.index, row);
                  }
                });

                _this5.__scrollToPendingIndex();
              });

              if (!_this5._cache.isLoading()) {
                _this5._debouncerApplyCachedData.flush();
              }

              _this5.__itemsReceived();
            });
          }
        }
        /**
