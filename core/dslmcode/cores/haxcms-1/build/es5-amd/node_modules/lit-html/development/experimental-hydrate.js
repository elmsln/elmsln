define(["exports", "./lit-html.js", "./directive.js", "./directive-helpers.js"], function (_exports, _litHtml, _directive, _directiveHelpers) {
  ...
  _exports.digestForTemplateResult = _exports.hydrate = void 0;
  ...
  var TemplateInstance = _litHtml._Σ._TemplateInstance,
      isIterable = _litHtml._Σ._isIterable,
      resolveDirective = _litHtml._Σ._resolveDirective,
      ChildPart = _litHtml._Σ._ChildPart,
      ElementPart = _litHtml._Σ._ElementPart;
  /**
  ...
  var hydrate = function hydrate(rootValue, container) {
    ...
    var rootPart = undefined; // When we are in-between ChildPart markers, this is the current ChildPart.
    // It's needed to be able to set the ChildPart's endNode when we see a
    // close marker

    var currentChildPart = undefined; // Used to remember parent template state as we recurse into nested
    // templates

    var stack = [];
    var walker = document.createTreeWalker(container, NodeFilter.SHOW_COMMENT, null, false);
    var marker; // Walk the DOM looking for part marker comments

    while ((marker = walker.nextNode()) !== null) {
      var markerText = marker.data;
      ...
      if (markerText.startsWith('lit-part')) {
        ...
        // stack
        createAttributeParts(marker, stack, options); // Remove `defer-hydration` attribute, if any
    ...
    console.assert(rootPart !== undefined, 'there should be exactly one root part in a render container'); // eslint-disable-next-line @typescript-eslint/no-explicit-any

    container._$litPart$ = rootPart;
  };

  _exports.hydrate = hydrate;

  var openChildPart = function openChildPart(rootValue, marker, stack, options) {
    var value; // We know the startNode now. We'll know the endNode when we get to
    ...
    if (value === _litHtml.noChange) {
    ...
    } else if ((0, _directiveHelpers.isPrimitive)(value)) {
      stack.push({
        part: part,
        type: 'leaf'
      });
      part._$committedValue = value; // TODO(kschaaf): We can detect when a primitive is being hydrated on the
      ...
      // }
    } else if ((0, _directiveHelpers.isTemplateResult)(value)) {
      // Check for a template result digest
      var markerWithDigest = "lit-part ".concat(digestForTemplateResult(value));

      if (marker.data === markerWithDigest) {
        var template = ChildPart.prototype._$getTemplate(value);

        var instance = new TemplateInstance(template, part);
        stack.push({
  ...
  var closeChildPart = function closeChildPart(marker, part, stack) {
    if (part === undefined) {
  ...
  var createAttributeParts = function createAttributeParts(comment, stack, options) {
    var _a; // Get the nodeIndex from DOM. We're only using this for an integrity
    // check right now, we might not need it.


    var match = /lit-node (\d+)/.exec(comment.data);
    var nodeIndex = parseInt(match[1]); // For void elements, the node the comment was referring to will be
    ...
    var node = (_a = comment.previousSibling) !== null && _a !== void 0 ? _a : comment.parentElement;
    var state = stack[stack.length - 1];

    if (state.type === 'template-instance') {
      var instance = state.instance; // eslint-disable-next-line no-constant-condition
      ...
      while (true) {
        ...
        var templatePart = instance._$template.parts[state.templatePartIndex];

        if (templatePart === undefined || templatePart.type !== _directive.PartType.ATTRIBUTE && templatePart.type !== _directive.PartType.ELEMENT || templatePart.index !== nodeIndex) {
          break;
        }

        if (templatePart.type === _directive.PartType.ATTRIBUTE) {
          // The instance part is created based on the constructor saved in the
          // template part
          var instancePart = new templatePart.ctor(node, templatePart.name, templatePart.strings, state.instance, options);
          var value = (0, _directiveHelpers.isSingleExpression)(instancePart) ? state.result.values[state.instancePartIndex] : state.result.values; // Setting the attribute value primes committed value with the resolved
          ...
          var noCommit = !(instancePart.type === _directive.PartType.EVENT || instancePart.type === _directive.PartType.PROPERTY);
          ...
          var _instancePart = new ElementPart(node, state.instance, options);

          resolveDirective(_instancePart, state.result.values[state.instancePartIndex++]);

          instance._parts.push(_instancePart);
        }
  ...
  var digestSize = 2; // We need to specify a digest to use across rendering environments. This is a
  ...
  var digestForTemplateResult = function digestForTemplateResult(templateResult) {
    var hashes = new Uint32Array(digestSize).fill(5381);

    var _iterator = _createForOfIteratorHelper(templateResult.strings),
        _step;
    ...
    return btoa(String.fromCharCode.apply(String, babelHelpers.toConsumableArray(new Uint8Array(hashes.buffer))));
  };

  _exports.digestForTemplateResult = digestForTemplateResult;
});