...
if(this.opened){this._openedChanged(this.opened)}this._observer=(0,_polymerDom.dom)(this).observeNodes(this._onNodesChange)},/** @override */detached:function detached(){// TODO(bicknellr): Per spec, checking `this._observer` should never be
...
var activeElement=this._manager.deepActiveElement;if(activeElement===document.body||composedContains(this,activeElement)){this.__restoreFocusNode.focus()}}this.__restoreFocusNode=null;this._focusNode.blur();this._focusedChild=null}},/**
   ...
   * @param {!Event} event
   * @protected
   */_onCaptureFocus:function _onCaptureFocus(event){if(!this.withBackdrop){return}var path=(0,_polymerDom.dom)(event).path;if(-1===path.indexOf(this)){event.stopPropagation();this._applyFocus()}else{this._focusedChild=/** @type {Node} */path[0]}},/**
