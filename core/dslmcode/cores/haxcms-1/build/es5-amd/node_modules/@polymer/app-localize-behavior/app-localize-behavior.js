define(["exports","../polymer/polymer-legacy.js","../iron-ajax/iron-ajax.js","../../intl-messageformat/src/main.js"],function(_exports,_polymerLegacy,_ironAjax,_main){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.AppLocalizeBehavior=void 0;_main=babelHelpers.interopRequireDefault(_main);/**
@license
...
*/window.IntlMessageFormat=_main.default;// This isn't a complete `Object.assign` polyfill, but this element expects
// JSON and doesn't provide more than one source object.
var assign=Object.assign?Object.assign.bind(Object):function(destination,source){for(var prop in source){if(source.hasOwnProperty(prop)){destination[prop]=source[prop]}}return destination},AppLocalizeBehavior={/**
   Internal singleton cache. This is the private implementation of the
   behaviour; don't interact with it directly.
   */__localizationCache:{requests:{},/* One iron-request per unique resources path. */messages:{},/* Unique localized strings. Invalidated when the language,
   ...
   */properties:{/**
     ...
     */useKeyIfMissing:{type:Boolean,value:!1},/**
     ...
     */bubbleEvent:{type:Boolean,value:!1}},loadResources:function loadResources(path,language,merge){var proto=this.constructor.prototype;// Check if localCache exist just in case.
this.__checkLocalizationCache(proto);// If the global ajax object has not been initialized, initialize and cache
// it.
var ajax=proto.__localizationCache.ajax;if(!ajax){ajax=proto.__localizationCache.ajax=document.createElement("iron-ajax")}var request=proto.__localizationCache.requests[path];function onRequestResponse(event){this.__onRequestResponse(event,language,merge)}if(!request){ajax.url=path;var request=ajax.generateRequest();request.completes.then(onRequestResponse.bind(this),this.__onRequestError.bind(this));// Cache the instance so that it can be reused if the same path is loaded.
proto.__localizationCache.requests[path]=request}else{request.completes.then(onRequestResponse.bind(this),this.__onRequestError.bind(this))}},/**
   Returns a computed `localize` method, based on the current `language`.
   */__computeLocalize:function __computeLocalize(language,resources,formats){var proto=this.constructor.prototype;// Check if localCache exist just in case.
this.__checkLocalizationCache(proto);// Everytime any of the parameters change, invalidate the strings cache.
if(!proto.__localizationCache){proto.__localizationCache={requests:{},messages:{},ajax:null}}proto.__localizationCache.messages={};return function(){var key=arguments[0];if(!key||!resources||!language||!resources[language])return;// Cache the key/value pairs for the same language, so that we don't
// do extra work if we're just reusing strings across an application.
var translatedValue=resources[language][key];if(!translatedValue){return this.useKeyIfMissing?key:""}var messageKey=key+translatedValue,translatedMessage=proto.__localizationCache.messages[messageKey];if(!translatedMessage){translatedMessage=new _main.default(translatedValue,language,formats);proto.__localizationCache.messages[messageKey]=translatedMessage}for(var args={},i=1;i<arguments.length;i+=2){args[arguments[i]]=arguments[i+1]}return translatedMessage.format(args)}.bind(this)},__onRequestResponse:function __onRequestResponse(event,language,merge){var propertyUpdates={},newResources=event.response;if(merge){if(language){propertyUpdates.resources=assign({},this.resources||{});propertyUpdates["resources."+language]=assign(propertyUpdates.resources[language]||{},newResources)}else{propertyUpdates.resources=assign(this.resources,newResources)}}else{if(language){propertyUpdates.resources={};propertyUpdates.resources[language]=newResources;propertyUpdates["resources."+language]=newResources}else{propertyUpdates.resources=newResources}}if(this.setProperties){this.setProperties(propertyUpdates)}else{for(var key in propertyUpdates){this.set(key,propertyUpdates[key])}}this.fire("app-localize-resources-loaded",event,{bubbles:this.bubbleEvent})},__onRequestError:function __onRequestError(event){this.fire("app-localize-resources-error")},__checkLocalizationCache:function __checkLocalizationCache(proto){// do nothing if proto is undefined.
if(proto===void 0)return;// In the event proto not have __localizationCache object, create it.
if(proto.__localizationCache===void 0){proto.__localizationCache={requests:{},messages:{},ajax:null}}}};/**
 ...
 */_exports.AppLocalizeBehavior=AppLocalizeBehavior});