define(["exports"], function (_exports) {
   ...
   */
  var _a, _b; // Scopes that have had styling prepared. Note, must only be done once per
  // scope.


  var styledScopes = new Set(); // Map of css per scope. This is collected during first scope render, used when
  // styling is prepared, and then discarded.

  var scopeCssStore = new Map();
  var ENABLE_SHADYDOM_NOPATCH = true; // Note, explicitly use `var` here so that this can be re-defined when
  // bundled.
  ...
  var polyfillSupport = function polyfillSupport(Template, ChildPart) {
    var _a, _b; // polyfill-support is only needed if ShadyCSS or the ApplyShim is in use
    // We test at the point of patching, which makes it safe to load
    // webcomponentsjs and polyfill-support in either order


    if (window.ShadyCSS === undefined || window.ShadyCSS.nativeShadow && !window.ShadyCSS.ApplyShim) {
      return;
    } // console.log(
    ...
    var needsPrepareStyles = function needsPrepareStyles(name) {
      return name !== undefined && !styledScopes.has(name);
    };

    var cssForScope = function cssForScope(name) {
    ...
    var prepareStyles = function prepareStyles(name, template) {
      // Get styles
      ...
      styledScopes.add(name); // Remove stored data since it's no longer needed.

      scopeCssStore.delete(name); // ShadyCSS removes scopes and removes the style under ShadyDOM and leaves
      // it under native Shadow DOM

      window.ShadyCSS.prepareTemplateStyles(template, name); // Note, under native Shadow DOM, the style is added to the beginning of the
    ...
    Template.createElement = function (html, options) {
      ...
      if (scope !== undefined) {
        ...
        if (needsPrepareStyles(scope)) {
          var scopeCss = cssForScope(scope); // Remove styles and store textContent.

          var styles = element.content.querySelectorAll('style'); // Store the css in this template in the scope css and remove the <style>
    ...
    childPartProto._$setValue = function (value, directiveParent) {
      ...
      if (babelHelpers.instanceof(container, ShadowRoot) && needsPrepareStyles(scope)) {
        ...
        var endNode = this._$endNode; // Temporarily move this part into the renderContainer.

        renderContainer.appendChild(renderContainerMarker);
        this._$startNode = renderContainerMarker;
        this._$endNode = null; // Note, any nested template results render here and their styles will
        // be extracted and collected.

        setValue.call(this, value, directiveParent); // Get the template for this result or create a dummy one if a result
        // is not being rendered.
        // This property needs to remain unminified.

        var template = ((_b = value) === null || _b === void 0 ? void 0 : _b['_$litType$']) ? this._$committedValue._$template.el : document.createElement('template');
        prepareStyles(scope, template); // Note, this is the temporary startNode.

        renderContainer.removeChild(renderContainerMarker); // When using native Shadow DOM, include prepared style in shadowRoot.
        ...
        container.insertBefore(renderContainer, endNode); // Move part back to original container.
