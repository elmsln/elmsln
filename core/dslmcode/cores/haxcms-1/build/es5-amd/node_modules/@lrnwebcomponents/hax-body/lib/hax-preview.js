...
if(null!=element.properties[property]&&!element.properties[property].readOnly){// make sure slot is NEVER set in the preview
...
if("slot"===property){// temp prop we use
property="data-hax-slot";// move it over
element.properties[property]=element.properties.slot;// delete the slot
delete element.properties.slot;if(null!=element.properties[property]){newValue.setAttribute("data-hax-slot",element.properties[property])}}// prefix is a special attribute and must be handled this way
else if("prefix"===property){newValue.setAttribute("prefix",element.properties[property])}// set is a Polymer convention but help w/ data binding there a lot
else if("function"===typeof newValue.set){// just to be safe
try{newValue.set(property,element.properties[property])}catch(e){console.log(e)}}// vanilla / anything else we should just be able to set the prop
else{// @todo may need to bind differently for vanilla elements
newValue.setAttribute(property,element.properties[property])}}this.set("value."+property,element.properties[property]);this.notifyPath("value."+property)}}// then, let the node itself dictate defaults if things are not set
...
if(newValue&&null!=newValue&&0!=newValue.length&&newValue.tag){var frag=document.createElement(newValue.tag);frag.innerHTML=newValue.content;// clone the fragment which will force an escalation to full node
var newNode=frag.cloneNode(!0);newNode.setAttribute("hax-preview-mode","hax-preview-mode");// if there is slot we need to shift it
if(null!=newNode.getAttribute("slot")){newNode.setAttribute("data-hax-slot",newNode.getAttribute("slot"));newNode.removeAttribute("slot")}// send this into the root, which should filter it back down into the slot
...
if(""!==propData.slot){tmpel.slot=propData.slot}tmpel.innerHTML=valueChange.base[path];var cloneIt=tmpel.cloneNode(!0);// inject the slotted content but use text nodes if this is a text element
if(window.HaxStore.instance.isTextElement(node)){node.innerHTML=tmpel.innerHTML}else{// wipe just the slot in question
(0,_haxutils.wipeSlot)(node,propData.slot);(0,_polymerDom.dom)(node).appendChild(cloneIt)}this.set("element.content","<template>"+cloneIt.outerHTML+"</template>");this.notifyPath("element.content")}}}}}/**
...
if("advanced"===mode){this.advancedForm=!0}else{this.advancedForm=!1}}}}],[{key:"template",get:function get(){return(0,_polymerElement.html)(_templateObject_359975a0a8b411e9a546af19ffcfe79d())}},{key:"tag",get:function get(){return"hax-preview"}},{key:"observers",get:function get(){return["_valueChanged(value.*)"]}},{key:"properties",get:function get(){return{responsiveWidth:{type:String,value:"800px"},orientation:{type:String,value:"horizontal"},orientationDirection:{type:String,value:"width"},/**
       ...
       */element:{type:Object,notify:!0,observer:"_elementChanged"},/**
