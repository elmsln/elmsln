define(["../../@polymer/polymer/polymer-element.js", "../../@polymer/paper-tooltip/paper-tooltip.js", "../../@polymer/neon-animation/neon-animation.js"], function (_polymerElement, _paperTooltip, _neonAnimation) {
  "use strict";

  function _templateObject_7d7bfd80dedc11e9bf5a076df0f16721() {
    ...
    _templateObject_7d7bfd80dedc11e9bf5a076df0f16721 = function _templateObject_7d7bfd80dedc11e9bf5a076df0f16721() {
      return data;
    };
  ...
  function (_PolymerElement) {
    ...
    babelHelpers.createClass(ChemicalElementVisualisation, null, [{
      key: "template",
      get: function get() {
        return (0, _polymerElement.html)(_templateObject_7d7bfd80dedc11e9bf5a076df0f16721());
      }
    }, {
      key: "is",
      get: function get() {
    ...
    function ChemicalElementVisualisation() {
      ...
      return babelHelpers.possibleConstructorReturn(this, babelHelpers.getPrototypeOf(ChemicalElementVisualisation).call(this));
    }

    babelHelpers.createClass(ChemicalElementVisualisation, [{
      key: "_onSymbolChange",
      value: function _onSymbolChange() {
        this._element = this._elements[this.symbol];
      }
    }, {
      key: "_onElementChange",
      value: function _onElementChange() {
        var SIZE = 500;
        var dynamicallyAddedElements = this.shadowRoot.querySelectorAll(".added-dynamically");
        dynamicallyAddedElements.forEach(function (element) {
          return element.remove();
        });

        if (this._element) {
          var electronConfiguration = this._element["electron-configuration"];
          var electronGroup = this.$["electron-group"];
          var ringGroup = this.$["ring-group"];
          var minimumRadius = SIZE / 6;
          var maximumRadius = SIZE / 2;

          for (var i = 0; i < electronConfiguration.length; i++) {
            var ring = document.createElementNS("http://www.w3.org/2000/svg", "circle");
            var radius = minimumRadius + (i + 1) * (maximumRadius - minimumRadius) / (electronConfiguration.length + 1);
          ...
          var outerShell = electronConfiguration[electronConfiguration.length - 1];
          var secondOuterShell = electronConfiguration[electronConfiguration.length - 2];
          var electrons = [];
          var electronBackgrounds = [];

          for (var _i = 0; _i < electronConfiguration.length; _i++) {
            var configuration = electronConfiguration[_i];
            var totalValenceElectrons = configuration.s + configuration.p + configuration.d + configuration.f;

            for (var j = 0; j < totalValenceElectrons; j++) {
              var electron = document.createElementNS("http://www.w3.org/2000/svg", "circle");
              var electronBackground = document.createElementNS("http://www.w3.org/2000/svg", "circle");
              var ringRadius = minimumRadius + (_i + 1) * (maximumRadius - minimumRadius) / (electronConfiguration.length + 1);
              var phaseShift = _i * Math.PI / 8;
              var cx = Math.sin(2 * Math.PI * (j / totalValenceElectrons) + phaseShift) * ringRadius;
              var cy = Math.cos(2 * Math.PI * (j / totalValenceElectrons) + phaseShift) * ringRadius;

              var _radius = SIZE / 50;

              electron.setAttribute("cx", cx);
              electron.setAttribute("cy", cy);
              electron.setAttribute("r", _radius);
              ...
              electronBackground.setAttribute("r", _radius + SIZE / 75);
              ...
              if (j >= configuration.s) {
                if (j >= configuration.p + configuration.s) {
                  if (j >= configuration.d + configuration.p + configuration.s) {
                    ...
                    if (secondOuterShell.d <= 1 && electronConfiguration.length - _i == 3) electron.setAttribute("opacity", "1");
                  } else {
                    electron.classList.add("d-group-electron");
                    if (outerShell.p <= 0 && electronConfiguration.length - _i == 2) electron.setAttribute("opacity", "1");
                  }
                } else {
              ...
              if (_i === electronConfiguration.length - 1) {
          ...
          electronBackgrounds.forEach(function (electronBackground) {
            return electronGroup.appendChild(electronBackground);
          });
          electrons.forEach(function (electron) {
            return electronGroup.appendChild(electron);
          });
      ...
      key: "_divide",
      value: function _divide(divident, divisor) {
