 ...
 */var RichTextEditorToolbar=/*#__PURE__*/function(_PolymerElement){babelHelpers.inherits(RichTextEditorToolbar,_PolymerElement);function RichTextEditorToolbar(){babelHelpers.classCallCheck(this,RichTextEditorToolbar);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(RichTextEditorToolbar).apply(this,arguments))}babelHelpers.createClass(RichTextEditorToolbar,[{key:"connectedCallback",/**
   ...
   * @returns {void}
   */value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(RichTextEditorToolbar.prototype),"connectedCallback",this).call(this);var root=this;this.__clipboard=document.createElement("textarea");this.__clipboard.setAttribute("aria-hidden",!0);this.__clipboard.style.position="absolute";this.__clipboard.style.left="-9999px";this.__clipboard.style.top="0px";this.__clipboard.style.width="0px";this.__clipboard.style.height="0px";document.body.appendChild(this.__clipboard);window.addEventListener("paste",root._handlePaste.bind(root));if(navigator.clipboard){this.addEventListener("paste-button",root._handlePasteButton.bind(root))}window.RichTextEditorSelection.requestAvailability();window.ResponsiveUtility.requestAvailability();window.dispatchEvent(new CustomEvent("responsive-element",{detail:{element:root,attribute:"responsive-size",relativeToParent:!0}}))}/**
...
root.dispatchEvent(new CustomEvent("deselect-rich-text-editor-editor",{bubbles:!0,cancelable:!0,composed:!0,detail:{toolbar:root,editor:root.editor}}))}/**
   ...
   * @returns {void}
   */},{key:"addEditableRegion",value:function addEditableRegion(editor){var root=this;editor.addEventListener("mousedown",function(e){root.editTarget(editor)});editor.addEventListener("focus",function(e){root.editTarget(editor)});editor.addEventListener("keydown",function(e){root._handleShortcutKeys(editor,e)});editor.addEventListener("blur",function(e){if(null===e.relatedTarget||"rich-text-editor"===!e.relatedTarget.startsWith)root.editTarget(null)})}/**
   ...
   * @param {object} an HTML object that can be edited
   * @returns {void}
   */},{key:"editTarget",value:function editTarget(editor){var root=this;if(root.editor!==editor){//save changes to previous editor
...
root.dispatchEvent(new CustomEvent("select-rich-text-editor-editor",{bubbles:!0,cancelable:!0,composed:!0,detail:{toolbar:root,editor:root.editor}}));root.editor=editor;if(editor){editor.parentNode.insertBefore(root,editor);root.canceled=editor.innerHTML;root.editor.contentEditable=!0;root.controls=editor.getAttribute("id")}else{root.controls=null}root.buttons.forEach(function(button){button.target=editor;button.controls=root.controls})}}/**
   ...
   * @param {string} pasteContent the html to be pasted
   * @returns {void}
   */},{key:"pasteIntoRange",value:function pasteIntoRange(range,pasteContent){console.log("pasteIntoRange",range,pasteContent);var div=document.createElement("div"),sel=window.getSelection(),parent=range.commonAncestorContainer.parentNode,closest=parent.closest("[contenteditable=true]:not([disabled]),input:not([disabled]),textarea:not([disabled])");if(this.editor=closest){div.innerHTML=pasteContent;if(range&&range.extractContents){range.extractContents()}range.insertNode(div);while(div.firstChild){div.parentNode.insertBefore(div.firstChild,div)}div.parentNode.removeChild(div)}}/**
   * removes an editor
   *
   * @param {object} editor an HTML object that can be edited
   * @returns {void}
   */},{key:"removeEditableRegion",value:function removeEditableRegion(editor){var root=this;editor.removeEventListener("mouseout",function(e){root.getUpdatedSelection()});editor.removeEventListener("focus",function(e){root.editTarget(editor)});editor.removeEventListener("mousedown",function(e){root.editTarget(editor)});editor.removeEventListener("keydown",function(e){root._handleShortcutKeys(editor,e)});editor.removeEventListener("blur",function(e){if(null===e.relatedTarget||"rich-text-editor"===!e.relatedTarget.startsWith)root.editTarget(null);root.getUpdatedSelection()})}/**
   ...
   */},{key:"_addButton",value:function _addButton(child,parent){var root=this,button=document.createElement(child.type),keys=button.shortcutKeys?button.shortcutKeys.replace(/ctrl\+[xcv]/g,""):"";//disable clipboard keys since we're already listening for them
this.set("__shortcutKeys.".concat(keys),button);for(var key in child){button[key]=child[key]}button.setAttribute("class","button");button.addEventListener("deselect",function(e){if(root.range&&root.range.collapse)root.range.collapse(!1)});if(button.inlineWidget)root.push("__inlineWidgets",button.tag);parent.appendChild(button);return button}},{key:"_handleKeyboardShortcuts",value:function _handleKeyboardShortcuts(e){console.log("_handleKeyboardShortcuts",e)}/**
   ...
   */},{key:"_getButtons",value:function _getButtons(config){var root=this,toolbar=root.shadowRoot.querySelector("#toolbar"),more=this.shadowRoot.querySelector("#morebutton"),max=0,sizes=["xs","sm","md","lg","xl"],temp=[];toolbar.innerHTML="";this.set("__shortcutKeys",[]);config.forEach(function(item){if("button-group"===item.type){var group=document.createElement("div");group.setAttribute("class","group");if(item.collapsedUntil!==void 0&&null!==item.collapsedUntil)group.setAttribute("collapsed-until",item.collapsedUntil);max=Math.max(max,sizes.indexOf(item.collapsedUntil));item.buttons.forEach(function(button){max=Math.max(max,sizes.indexOf(button.collapsedUntil));if(navigator.clipboard||"paste"!==button.command)temp.push(root._addButton(button,group));//firefox doesn't allow for clipboard button
});toolbar.appendChild(group)}else{max=Math.max(max,sizes.indexOf(item.collapsedUntil));if(navigator.clipboard||"paste"!==item.command)temp.push(root._addButton(item,toolbar));//firefox doesn't allow for clipboard button
}toolbar.appendChild(more);more.collapseMax=sizes[max]});return temp}/**
   ...
   */},{key:"_handleShortcutKeys",value:function _handleShortcutKeys(editor,e){if(editor.contentEditable){var key=e.key;if(e.shiftKey)key="shift+"+key;if(e.altKey)key="alt+"+key;if("MacIntel"===window.navigator.platform&&e.metaKey||e.ctrlKey){key="ctrl+"+key}if(this.__shortcutKeys[key])this.__shortcutKeys[key]._keysPressed(e)}}/**
   * Handles paste.
   *
   * @param {event} e the paste event
   * @returns {void}
   */},{key:"_handlePaste",value:function _handlePaste(e){console.log("_handlePaste",e);var pasteContent="";// intercept paste event
if(e&&(e.clipboardData||e.originalEvent.clipboardData)){pasteContent=(e.originalEvent||e).clipboardData.getData("text/html")}else if(window.clipboardData){pasteContent=window.clipboardData.getData("Text")}console.log("clipboardData");this.pasteIntoRange(this.getRange(),this.getSanitizeClipboard(pasteContent));console.log("pasteIntoRange");e.preventDefault()}/**
   * Handles paste button.
   *
   * @param {event} e the paste button event
   * @returns {void}
   */},{key:"_handlePasteButton",value:function _handlePasteButton(e){var _this=this;console.log("_handlePasteButton",e);setTimeout(/*#__PURE__*/babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(){var range,sel,text;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:range=e.detail.range;sel=window.getSelection();_context.next=4;return navigator.clipboard.readText();case 4:text=_context.sent;_this.__clipboard.value=text;_this.__clipboard.focus();_this.__clipboard.select();document.execCommand("paste");sel.removeAllRanges();sel.addRange(range);_this.pasteIntoRange(range,_this.getSanitizeClipboard(_this.__clipboard.value));case 12:case"end":return _context.stop();}}},_callee)})),2e3);e.preventDefault()}/**
   * Gets the updated selected range.
   * @returns {void}
   */},{key:"_rangeChange",value:function _rangeChange(e){var root=this;root.buttons.forEach(function(button){button.range=null;button.range=root.range})}/**
...
get:function get(){return(0,_polymerElement.html)(_templateObject_9a922320729a11ea913eb9f5205e3d49())}// render function for styles
},{key:"styleTemplate",get:function get(){return(0,_polymerElement.html)(_templateObject2_9a922320729a11ea913eb9f5205e3d49())}// render function for toolbar
},{key:"toolbarTemplate",get:function get(){return(0,_polymerElement.html)(_templateObject3_9a922320729a11ea913eb9f5205e3d49())}// render function for template
},{key:"template",get:function get(){return(0,_polymerElement.html)(_templateObject4_9a922320729a11ea913eb9f5205e3d49(),this.styleTemplate,this.stickyTemplate,this.toolbarTemplate)}// properties available to the custom element for data binding
       ...
       */__inlineWidgets:{name:"__inlineWidgets",type:Array,value:[]},/**
       ...
       * Optional space-sperated list of keyboard shortcuts for the editor
       * to fire this button, see iron-a11y-keys for more info.
       */__shortcutKeys:{name:"__shortcutKeys",type:Array,value:[]}}}/**
