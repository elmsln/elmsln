...
test("can write empty child elements", function () {
  assertXmlString(xml.element("root", {}, [xml.element("album"), xml.element("single")]), {}, "<root><album/><single/></root>");
});
test("can write empty descendant elements", function () {
  var element = xml.element("root", {}, [xml.element("album", {}, [xml.element("year"), xml.element("song")])]);
  assertXmlString(element, {}, "<root><album><year/><song/></album></root>");
});
test("can write element attributes", function () {
  var element = xml.element("root", {}, [xml.element("album", {
    title: "Everything in Transit"
  })]);
  assertXmlString(element, {}, '<root><album title="Everything in Transit"/></root>');
});
test("can write text nodes", function () {
  var element = xml.element("root", {}, [xml.element("album", {}, [xml.text("Everything in Transit")])]);
  assertXmlString(element, {}, "<root><album>Everything in Transit</album></root>");
});
...
function assertXmlString(element, namespaces, expectedString) {
  assert.equal(writer.writeString(element, namespaces), '<?xml version="1.0" encoding="UTF-8" standalone="yes"?>' + expectedString);
}