...
var loader=window.WCAutoload.requestAvailability();loader.loaded=!0;// microtask timing to ensure window settings are accepted
setTimeout(/*#__PURE__*/babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(){var i,target;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:// set the basePath if it exists
...
try{target.querySelectorAll(":not(:defined)").forEach(function(el){loader.processNewElement(el)})}catch(e){// hack to convert children into array
target.querySelectorAll("*").forEach(function(el){loader.processNewElement(el)})}case 8:case"end":return _context.stop();}}},_callee)})),0)};// forces self appending which kicks all this off but AFTER dom is loaded
