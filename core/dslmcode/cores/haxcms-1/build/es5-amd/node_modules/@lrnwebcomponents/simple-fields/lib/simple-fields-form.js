...
value:function render(){return(0,_litElement.html)(_templateObject_3f49e3606d1111eaaf146103541fd890())}},{key:"updated",value:function updated(changedProperties){var _this=this;changedProperties.forEach(function(oldValue,propName){if(_this.autoload&&!_this.loadResponse&&!_this.loading){if("loadEndpoint"===propName||"autoload"===propName){_this.loadData()}}// we have response data from an end point this should create the form
if("loadResponse"===propName&&_this.loadResponse.data){_this._applyLoadedData();/**
   ...
   */},{key:"_applyLoadedData",value:function _applyLoadedData(){var sf=this.shadowRoot.querySelector("#sf");console.log(this.loadResponse.data.schema,this.loadResponse.data.fields);if(this.loadResponse.data.schema){sf.schema=this.loadResponse.data.schema}else if(this.loadResponse.data.fields){sf.fields=this.loadResponse.data.fields}if(this.loadResponse.data.value)sf.value=this.loadResponse.data.value}/**
   * Props down
