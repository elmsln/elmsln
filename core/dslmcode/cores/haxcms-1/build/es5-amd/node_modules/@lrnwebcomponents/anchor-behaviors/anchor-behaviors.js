...
window.AnchorBehaviors.getTarget = function () {
  var element = arguments.length > 0 && arguments[0] !== undefined ? arguments[0] : null;

  /** gets and sets parameters */
  var getParams = function getParams() {
    var str = window.location.hash.substring(1).replace(/^(.+)&?/, "id=$1") || window.location.search.substring(1) || "",
        uri = str ? "{\"".concat(decodeURI(str).replace(/"/g, '\\"').replace(/&/g, '","').replace(/=/g, '":"'), "\"}") : "{}",
    ...
    window.AnchorBehaviors.params = params;
      ...
      testElement = function testElement(element, params) {
    if (element && (params.id || params.resource)) {
      var eid = element.id ? element.id.replace(/#/g, "") : null,
  ...
  if (!window.AnchorBehaviors.target) {
    ...
    window.AnchorBehaviors.target = document.getElementById(window.AnchorBehaviors.params.id) || document.getElementById("#".concat(window.AnchorBehaviors.params.id)) || document.querySelector("[resource=\"#".concat(window.AnchorBehaviors.params.id || window.AnchorBehaviors.params.resource, "\"]")) || document.querySelector("[resource=\"".concat(window.AnchorBehaviors.params.id || window.AnchorBehaviors.params.resource, "\"]")) || testElement(element, window.AnchorBehaviors.params) || null;
