(function () {
  ...
  function GitGraph(options) {
    ...
    this.element = babelHelpers.typeof(options.element) === "object" ? options.element : null;
    this.author = typeof options.author === "string" ? options.author : "Sergio Flores <saxo-guy@epic.com>";
    this.reverseArrow = _booleanOptionOr(options.reverseArrow, false); // Template management
    ...
    switch (options.orientation) {
      ...
      case "horizontal":
        ...
        this.template.branch.labelRotation = _isNullOrUndefined(options, "template.branch.labelRotation") ? -90 : options.template.branch.labelRotation;
    ...
    this.offsetX = 0;
    this.offsetY = 0; // Canvas init

    this.canvas = this.element ? this.element : document.getElementById(this.elementId) || options.canvas;
    this.context = this.canvas.getContext("2d");
    this.context.textBaseline = "center"; // Tooltip layer
    ...
    tooltipContainer.appendChild(this.tooltip); // Navigation vars

    this.HEAD = null;
    this.branches = [];
    this.commits = []; // Utilities

    this.columnMax = 0; // nb of column for message position

    this.commitOffsetX = options.initCommitOffsetX || 0;
    this.commitOffsetY = options.initCommitOffsetY || 0; // Bindings
    ...
    this.canvas.addEventListener("mousedown", this.mouseDownOptions, false); // Render on window resize
  ...
  GitGraph.prototype.branch = function (options) {
    ...
    options = _isObject(options) ? options : {};
    options.parent = this;
    options.parentBranch = options.parentBranch || this.HEAD; // Add branch

    var branch = new Branch(options);
    this.branches.push(branch); // Return
  ...
  GitGraph.prototype.orphanBranch = function (options) {
    ...
    options = _isObject(options) ? options : {};
    options.parent = this; // Add branch
  ...
  GitGraph.prototype.commit = function (options) {
    this.HEAD.commit(options); // Return the main object so we can chain
  ...
  GitGraph.prototype.tag = function (options) {
    this.HEAD.tag(options); // Return the main object so we can chain
  ...
  GitGraph.prototype.render = function () {
    this.scalingFactor = _getScale(this.context); // Resize canvas

    var unscaledResolution = {
      x: Math.abs((this.columnMax + 1) * this.template.branch.spacingX) + Math.abs(this.commitOffsetX) + this.marginX * 2,
      y: Math.abs((this.columnMax + 1) * this.template.branch.spacingY) + Math.abs(this.commitOffsetY) + this.marginY * 2
    };
    ...
    this.canvas.height = unscaledResolution.y * this.scalingFactor; // Clear All
  ...
  function Branch(options) {
    // Check integrity
    if (babelHelpers.instanceof(options.parent, GitGraph) === false) {
      return;
    } // Options
    ...
    this.parent.columnMax = this.column > this.parent.columnMax ? this.column : this.parent.columnMax; // Options with auto value

    this.offsetX = this.column * this.spacingX;
    this.offsetY = this.column * this.spacingY; // Add start point
    ...
    this.color = options.color || this.template.branch.color || this.template.colors[columnIndex]; // Checkout on this new branch
  ...
  Branch.prototype.branch = function (options) {
    ...
    options.parentBranch = options.parentBranch || this; // Add branch

    var branch = new Branch(options);
    this.parent.branches.push(branch); // Return
    ...
    for (var i = 0, point; !!(point = this.path[i]); i++) {
      if (point.type === "start") {
        ...
        if (this.template.branch.mergeStyle === "bezier") {
          ...
          this.context.bezierCurveTo(path.x - this.template.commit.spacingX / 2, path.y - this.template.commit.spacingY / 2, point.x + this.template.commit.spacingX / 2, point.y + this.template.commit.spacingY / 2, point.x, point.y);
        } else {
  ...
  Branch.prototype.commit = function (options) {
    ...
    options.branch = this;
    var columnIndex = this.column % this.template.colors.length;
    options.color = options.color || this.commitDefaultOptions.color || this.template.commit.color || this.template.colors[columnIndex];
    options.parent = this.parent;
    options.parentCommit = options.parentCommit || _getParentCommitFromBranch(this); // Special compact mode

    if (this.parent.mode === "compact" && _getLast(this.parent.commits) && _getLast(this.parent.commits).branch !== options.branch && options.branch.commits.length && options.type !== "mergeCommit") {
      this.parent.commitOffsetX -= this.template.commit.spacingX;
      this.parent.commitOffsetY -= this.template.commit.spacingY;
    }

    options.messageColor = options.messageColor || this.commitDefaultOptions.messageColor || this.template.commit.message.color || options.color || null;
    options.labelColor = options.labelColor || this.commitDefaultOptions.labelColor || this.template.branch.labelColor || options.color || null;
    options.tagColor = options.tagColor || this.commitDefaultOptions.tagColor || this.template.commit.tag.color || options.color || null;
    options.dotColor = options.dotColor || this.commitDefaultOptions.dotColor || this.template.commit.dot.color || options.color || null;
    options.x = this.offsetX - this.parent.commitOffsetX;
    options.y = this.offsetY - this.parent.commitOffsetY; // Detail
    ...
    if (babelHelpers.instanceof(options.parentCommit, Commit) === false && babelHelpers.instanceof(this.parentBranch, Branch)) {
    ...
    this.commits.push(commit); // Add point(s) to path
    ...
    if (!isFirstBranch && isPathBeginning) {
      this.pushPath(this.startPoint); // Add a path joint to startpoint + template spacing
    ...
    this.pushPath(point); // Increment commitOffset for next commit position

    this.parent.commitOffsetX += this.template.commit.spacingX * (options.showLabel ? 2 : 1);
    this.parent.commitOffsetY += this.template.commit.spacingY * (options.showLabel ? 2 : 1); // Add height of detail div (vertical mode only)
  ...
  Branch.prototype.merge = function (target, commitOptions) {
    ...
    if (babelHelpers.instanceof(targetBranch, Branch) === false || targetBranch === this) {
      return this;
    }
    ...
    if (babelHelpers.typeof(commitOptions) !== "object") {
    ...
    if (commitOptions.fastForward && isFastForwardPossible) {
      ...
      this.color = targetBranch.color; // Make branch path follow target branch ones
  ...
  Commit.prototype.render = function () {
    ...
    var commitOffsetLeft = (this.parent.columnMax + 1) * this.template.branch.spacingX + commitOffsetForTags; // Label
    ...
    if (this.showLabel) {
       ...
       */
      if (_isHorizontal(this.parent) && this.template.branch.labelRotation % 180 === 0) {
        /*
    ...
    this.context.fill();
    this.context.closePath(); // Arrow

    if (this.arrowDisplay && babelHelpers.instanceof(this.parentCommit, Commit)) {
      this.arrow();
    } // Tag
  ...
  Commit.prototype.arrow = function Arrow() {
    ...
    var alpha = rotate(this.parentCommit.y - this.y, this.parentCommit.x - this.x); // Merge & Fork case
    ...
    if (this.type === "mergeCommit" || isForkCommit) {
      ...
      var isPushedInY = (isForkCommit || isReversed) && Math.abs(this.y - this.parentCommit.y) > Math.abs(this.template.commit.spacingY);
      ...
      var isPushedInX = (isForkCommit || isReversed) && Math.abs(this.x - this.parentCommit.x) > Math.abs(this.template.commit.spacingX);
    ...
    var arrowY = isReversed ? this.parentCommit.y : this.y; // Top

    var h = this.template.commit.dot.size + this.template.arrow.offset;
    var x1 = h * Math.cos(alpha) + arrowX;
    var y1 = h * Math.sin(alpha) + arrowY; // Bottom left

    var x2 = (h + size) * Math.cos(alpha - delta) + arrowX;
    var y2 = (h + size) * Math.sin(alpha - delta) + arrowY; // Bottom center

    var x3 = (h + size / 2) * Math.cos(alpha) + arrowX;
    var y3 = (h + size / 2) * Math.sin(alpha) + arrowY; // Bottom right
  ...
  function Tag(commit, options) {
    ...
    this.font = options.font || commit.template.commit.tag.font; // Set context font for calculations
    ...
    _drawTextBG(commit.context, x, y, commit.tag, this.color, this.font, 0, commit.displayTagBox); // Reset original context font
  ...
  function Template(options) {
    ...
    this.branch.labelRotation = options.branch.labelRotation !== undefined ? options.branch.labelRotation : null; // Merge style = "bezier" | "straight"
    ...
    this.branch.spacingY = options.branch.spacingY || 0; // Arrow style
    ...
    this.arrow.offset = options.arrow.offset || 2; // Commit style
    ...
    this.commit.shouldDisplayTooltipsInCompactMode = _booleanOptionOr(options.commit.shouldDisplayTooltipsInCompactMode, true); // Only one color, if null message takes branch color (full commit)

    this.commit.color = options.commit.color || null;
    this.commit.dot = {}; // Only one color, if null message takes branch color (only dot)
    ...
    this.commit.message.displayHash = _booleanOptionOr(options.commit.message.displayHash, true); // Only one color, if null message takes commit color (only message)
  ...
  function _getScale(context) {
    ...
    var scalingFactor; // Account for high-resolution displays
  ...
  function _isObject(object) {
    return babelHelpers.typeof(object) === "object";
  }
  /**
  ...
  function _isNullOrUndefined(obj, key) {
    ...
    return !key.split(".").every(function (x) {
      if (babelHelpers.typeof(obj) !== "object" || obj === null || !(x in obj)) {
        return false;
      }
