define(["exports","../../../../mobx/lib/mobx.module.js","../../../utils/utils.js"],function(_exports,_mobxModule,_utils){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.store=void 0;var Store=/*#__PURE__*/function(){function Store(){babelHelpers.classCallCheck(this,Store);this.location=null;this.jwt=null;this.editMode=!1;this.manifest=null;this.activeItemContent="";this.themeElement=null;this.activeId=null;this.userData={};this.cmsSiteEditor={instance:null};this.cmsSiteEditorBackend={instance:null};this.dashboardOpened=!1}babelHelpers.createClass(Store,[{key:"cmsSiteEditorAvailability",value:function cmsSiteEditorAvailability(){if(!this.cmsSiteEditor.instance){this.cmsSiteEditor.instance=document.createElement("haxcms-site-editor")}return this.cmsSiteEditor.instance}},{key:"_computeItems",/**
...
manifestItems.forEach(function(item,i){_this4._setChildren(item,manifestItems)});/**
       ...
       * item.
       */if(!0===(0,_utils.varGet)(manifest,"metadata.site.settings.publishPagesOn",!1)){var filterHiddenParentsRecursive=function filterHiddenParentsRecursive(item){// if the item is unpublished then remove it.
if(!1===item.metadata.published){return!1}// if the item has parents, recursively see if any parent is not published
var parent=manifestItems.find(function(i){return i.id===item.parent});if(parent){return filterHiddenParentsRecursive(parent)}// if it got this far then it should be good.
return!0};manifestItems=manifestItems.filter(function(i){return filterHiddenParentsRecursive(i)})}return Object.assign({},manifest,{items:manifestItems,accessData:accessData})}}/**
   * Return the site title
   */},{key:"siteTitle",get:function get(){var manifest=this.manifest;if(manifest&&manifest.title){return manifest.title}return""}/**
...
if(this.manifest){var firstItem=this.manifest.items.find(function(i){return"undefined"!==typeof i.id});if(firstItem){return firstItem.location.replace("pages/","").replace("/index.html","")}}return"/"}/**
...
if((0,_utils.varExists)(this.manifest,"metadata.theme")){themeData=this.manifest.metadata.theme}else{// fallback juuuuust to be safe...
themeData={"haxcms-basic-theme":{element:"haxcms-basic-theme",path:"@lrnwebcomponents/haxcms-elements/lib/core/themes/haxcms-basic-theme.js",name:"Basic theme",variables:{image:"assets/banner.jpg",icon:"icons:record-voice-over",hexCode:"#da004e",cssVariable:"pink"}}}}// ooo you sneaky devil you...
if(this.activeItem&&(0,_utils.varExists)(this.activeItem,"metadata.theme")){return this.activeItem.metadata.theme}return themeData}}/**
   * Get the active manifest index array position
   * -1 if not found
   */},{key:"activeManifestIndex",get:function get(){if(this.manifest&&this.manifest.items&&this.activeId){for(var index in this.manifest.items){if(this.manifest.items[index].id===this.activeId){return parseInt(index)}}}return-1}},{key:"activeRouterManifestIndex",get:function get(){if(this.routerManifest&&this.routerManifest.items&&this.activeId){for(var index in this.routerManifest.items){if(this.routerManifest.items[index].id===this.activeId){return parseInt(index)}}}return-1}/**
...
if(tmpItem){return tmpItem.title}}return""}},{key:"isLoggedIn",get:function get(){// account for keypair storage issue since its a string bin
if(this.jwt&&"null"!=this.jwt){return!0}return!1}/**
...
jwt:_mobxModule.observable,// json web token
dashboardOpened:_mobxModule.observable,// if haxcms backend settings are open
userData:_mobxModule.observable,// user data object for logged in users
manifest:_mobxModule.observable,// JOS / manifest
activeItemContent:_mobxModule.observable,// active site content, cleaned up
themeElement:_mobxModule.observable,// theme element
routerManifest:_mobxModule.computed,// router mixed in manifest w/ routes / paths
siteTitle:_mobxModule.computed,isLoggedIn:_mobxModule.computed,// simple boolean for state so we can style based on logged in
themeData:_mobxModule.computed,// get the active theme from manifest + activeId
...
if(window.HaxStore&&window.HaxStore.write){window.dispatchEvent(new CustomEvent("haxcms-edit-mode-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:editMode}));window.HaxStore.write("editMode",editMode,window.HaxStore.instance);// @todo hack to keep voice controls active if enabled
if(window.HaxStore.instance.globalPreferences.haxVoiceCommands){setTimeout(function(){window.HaxStore.instance.__hal.auto=!0},10)}}})});