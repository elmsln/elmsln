          ...
          for (var key in _haxcmsSiteStore.store.setupSlots) {
            switch (key) {
              ...
              case "haxcms-site-editor-ui-suffix-buttons":
                _haxcmsSiteStore.store.cmsSiteEditor.haxCmsSiteEditorUIElement.appendChild(_haxcmsSiteStore.store.setupSlots[key]);
      ...
      value: function applyContext() {
        ...
        if (!this.__appliedContext) {
          ...
          if (!["published", "11ty"].includes(context)) {
            var basePath = new URL("./", meta.url).href; // import and set the tag based on the context
