...
test("should convert docx represented by a Buffer", function () {
  var docxPath = path.join(__dirname, "test-data/single-paragraph.docx");
  return promises.nfcall(fs.readFile, docxPath).then(function (buffer) {
    return mammoth.convertToHtml({
      buffer: buffer
    });
  }).then(function (result) {
...
test("embedded style map can be retrieved", function () {
  ...
  return promises.nfcall(fs.readFile, docxPath).then(function (buffer) {
    ...
    return mammoth.readEmbeddedStyleMap({
      buffer: docx.toBuffer()
    });
...
test("options.transformDocument is used to transform document if set", function () {
  ...
  var options = {
    transformDocument: function transformDocument(document) {
...
test("word tables are converted to html tables", function () {
  ...
  return mammoth.convertToHtml({
    ...
    var expectedHtml = "<p>Above</p>" + "<table>" + "<tr><td><p>Top left</p></td><td><p>Top right</p></td></tr>" + "<tr><td><p>Bottom left</p></td><td><p>Bottom right</p></td></tr>" + "</table>" + "<p>Below</p>";
...
test("footnotes are appended to text", function () {
  ...
  return mammoth.convertToHtml({
    ...
    var expectedOutput = "<p>Ouch" + '<sup><a href="#doc-42-footnote-1" id="doc-42-footnote-ref-1">[1]</a></sup>.' + '<sup><a href="#doc-42-footnote-2" id="doc-42-footnote-ref-2">[2]</a></sup></p>' + '<ol><li id="doc-42-footnote-1"><p> A tachyon walks into a bar. <a href="#doc-42-footnote-ref-1">↑</a></p></li>' + '<li id="doc-42-footnote-2"><p> Fin. <a href="#doc-42-footnote-ref-2">↑</a></p></li></ol>';
...
test("endnotes are appended to text", function () {
  ...
  return mammoth.convertToHtml({
    ...
    var expectedOutput = "<p>Ouch" + '<sup><a href="#doc-42-endnote-2" id="doc-42-endnote-ref-2">[1]</a></sup>.' + '<sup><a href="#doc-42-endnote-3" id="doc-42-endnote-ref-3">[2]</a></sup></p>' + '<ol><li id="doc-42-endnote-2"><p> A tachyon walks into a bar. <a href="#doc-42-endnote-ref-2">↑</a></p></li>' + '<li id="doc-42-endnote-3"><p> Fin. <a href="#doc-42-endnote-ref-3">↑</a></p></li></ol>';
    assert.equal(result.value, expectedOutput);
    assert.deepEqual(result.messages, []);
  });
});
test("relationships are handled properly in footnotes", function () {
  ...
  return mammoth.convertToHtml({
    ...
    var expectedOutput = '<p><sup><a href="#doc-42-footnote-1" id="doc-42-footnote-ref-1">[1]</a></sup></p>' + '<ol><li id="doc-42-footnote-1"><p> <a href="http://www.example.com">Example</a> <a href="#doc-42-footnote-ref-1">↑</a></p></li></ol>';
    assert.equal(result.value, expectedOutput);
    assert.deepEqual(result.messages, []);
  });
});
...
test("using styleMapping throws error", function () {
  try {
    ...
    assert.equal(error.message, "Use a raw string instead of mammoth.styleMapping e.g. \"p[style-name='Title'] => h1\" instead of mammoth.styleMapping(\"p[style-name='Title'] => h1\")");
  }
});
test("can convert single paragraph to markdown", function () {
  var docxPath = path.join(__dirname, "test-data/single-paragraph.docx");
  return mammoth.convertToMarkdown({
    path: docxPath
  }).then(function (result) {
