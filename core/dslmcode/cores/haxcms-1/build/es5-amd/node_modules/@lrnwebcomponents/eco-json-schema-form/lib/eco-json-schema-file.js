...
*/var EcoJsonSchemaFile=/*#__PURE__*/function(_mixinBehaviors){babelHelpers.inherits(EcoJsonSchemaFile,_mixinBehaviors);function EcoJsonSchemaFile(){babelHelpers.classCallCheck(this,EcoJsonSchemaFile);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(EcoJsonSchemaFile).apply(this,arguments))}babelHelpers.createClass(EcoJsonSchemaFile,[{key:"clear",/**
   * Clears the list of files
   */value:function clear(){this.set("files",[]);this.shadowRoot.querySelector("#fileInput").value="";this._showDropText()}},{key:"ready",value:function ready(){babelHelpers.get(babelHelpers.getPrototypeOf(EcoJsonSchemaFile.prototype),"ready",this).call(this);if(this.raised){this.toggleAttribute("raised",!0,this.shadowRoot.querySelector("#button"))}if(this.noink){this.toggleAttribute("noink",!0,this.shadowRoot.querySelector("#button"))}if(this.droppable){this._showDropText();this.setupDrop()}}/**
   * A function to set up a drop area for drag-and-drop file uploads
   */},{key:"setupDrop",value:function setupDrop(){var uploadBorder=this.shadowRoot.querySelector("#UploadBorder");this.toggleClass("enabled",!0,uploadBorder);this.ondragover=function(e){e.stopPropagation();this.toggleClass("hover",!0,uploadBorder);// Workaround for allowgin drop from Chome's download footer on OSX
...
if(""!==this.accept&&!(-1<this.accept.indexOf(mimeType)||-1<this.accept.indexOf(fileType))){continue}file.progress=0;file.error=!1;file.complete=!1;this.push("files",file);this.uploadFile(file)}}}/**
   ...
   */},{key:"_fileClick",value:function _fileClick(){var elem=this.shadowRoot.querySelector("#fileInput");if(elem&&document.createEvent){// sanity check
var evt=document.createEvent("MouseEvents");evt.initEvent("click",!0,!1);elem.dispatchEvent(evt)}}/**
   ...
   */},{key:"_fileChange",value:function _fileChange(e){for(var length=e.target.files.length,i=0,file;i<length;i++){file=e.target.files[i];file.progress=0;file.error=!1;file.complete=!1;this.push("files",file);if(!this.multi&&0!==this.files.length){this.set("files",[]);this.set("value",{})}this.uploadFile(file)}}/**
   * Cancels the file upload for a specific file
   *
   * @param {object} file An element of the files array
   */},{key:"cancel",value:function cancel(file){if(file){if(file.xhr){file.xhr.abort()}this.splice("files",this.files.indexOf(file),1);this._showDropText()}}/**
   * Cancels the file upload
   *
   * @param {object} e An event object
   */},{key:"_cancelUpload",value:function _cancelUpload(e){this.cancel(e.model.__data__.item)}/**
   * Retries to upload the file
   *
   * @param {object} e An event object
   */},{key:"_retryUpload",value:function _retryUpload(e){e.model.set("item.error",!1);e.model.set("item.progress",0);// The async helps give visual feedback of a retry occurring, even though it's less efficient.
var self=this;_async.microTask.run(function(){self.uploadFile(e.model.__data__.item)})}/**
   * Whether or not to display the drop text
   */},{key:"_showDropText",value:function _showDropText(){this.set("_shownDropText",!this.files.length&&this.droppable)}/**
   * Uploads a file
   *
   * @param {object} file An element of the files array
   */},{key:"uploadFile",value:function uploadFile(file){if(!file){return}this.dispatchEvent(new CustomEvent("before-upload",{bubbles:!0,cancelable:!0,composed:!0,detail:!0}));this._showDropText();var prefix="files."+this.files.indexOf(file),self=this,reader=new FileReader;reader.addEventListener("load",function(){var r=reader.result;//self.push('value', {"name": unescape(encodeURIComponent( file.name )),"content":r});
...
self.set("value."+self.attributes.name.value+"."+self.files.indexOf(file),r);//self.notifyPath('value');
},!1);if(!self.value.hasOwnProperty(self.attributes.name.value)){this.set("value."+self.attributes.name.value,{})}reader.readAsDataURL(file)}},{key:"_valueChanged",value:function _valueChanged(){console.log("this.value: "+JSON.stringify(this.value))}},{key:"_schemaChanged",value:function _schemaChanged(){var schema=this.schema;/*
  ...
  */},{key:"_isSchemaValue",value:function _isSchemaValue(type){return this._isSchemaFile(type)}},{key:"_isSchemaFile",value:function _isSchemaFile(type){if(Array.isArray(type)){return-1!==type.indexOf("file")}else{return"file"===type}}},{key:"_isSchemaBoolean",value:function _isSchemaBoolean(type){if(Array.isArray(type)){return-1!==type.indexOf("boolean")}else{return"boolean"===type}}},{key:"_isSchemaNumber",value:function _isSchemaNumber(type){if(Array.isArray(type)){return-1!==type.indexOf("number")||-1!==type.indexOf("integer")}else{return"number"===type||"integer"===type}}},{key:"_isSchemaString",value:function _isSchemaString(type){if(Array.isArray(type)){return-1!==type.indexOf("string")}else{return"string"===type}}},{key:"_isSchemaObject",value:function _isSchemaObject(type){return"object"===type}},{key:"_isSchemaArray",value:function _isSchemaArray(type){return"array"===type}},{key:"stringify",value:function stringify(s){return JSON.stringify(s)}}],[{key:"tag",get:function get(){return"eco-json-schema-file"}},{key:"template",get:function get(){return(0,_polymerElement.html)(_templateObject_b46cae60696b11eabd167f7c8887b3bc())}},{key:"properties",get:function get(){return{language:{value:"en",notify:!0},resources:{value:function value(){return{}},notify:!0},schema:{type:Object,observer:"_schemaChanged"},value:{type:Object,notify:!0,value:function value(){return{}},observer:"_valueChanged"},/**
      error: {
        type: String,
      ...
      */ /**
       ...
       */paperButtonTitle:{type:String,value:""}}}}]);return EcoJsonSchemaFile}((0,_class.mixinBehaviors)([_appLocalizeBehavior.AppLocalizeBehavior],_polymerElement.PolymerElement));_exports.EcoJsonSchemaFile=EcoJsonSchemaFile;window.customElements.define(EcoJsonSchemaFile.tag,EcoJsonSchemaFile)});