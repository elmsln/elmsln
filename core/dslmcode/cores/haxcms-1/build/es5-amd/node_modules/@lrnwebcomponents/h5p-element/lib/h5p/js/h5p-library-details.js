...
(function ($) {
  ...
  H5PLibraryDetails.init = function () {
    ...
    H5PLibraryDetails.library = H5PAdminIntegration.libraryInfo; // currentContent holds the current list if data (relevant for filtering)
  ...
  H5PLibraryDetails.createPagerElement = function () {
    ...
    H5PLibraryDetails.$pagerInfo.on("click", function () {
      ...
      H5PLibraryDetails.$pagerInfo.hide(); // User has updated the pageNumber

      var pageNumerUpdated = function pageNumerUpdated() {
        var newPageNum = $gotoInput.val() - 1;
        var intRegex = /^\d+$/;
        $goto.remove();
        H5PLibraryDetails.$pagerInfo.css({
      ...
      var $goto = $("<span/>", {
        ...
        width: width
      }).append($gotoInput).insertAfter(H5PLibraryDetails.$pagerInfo);
      $gotoInput.focus();
    });
    H5PLibraryDetails.updatePager();
  };
  ...
  H5PLibraryDetails.createSearchElement = function () {
    ...
    var performSeach = function performSeach() {
      var searchString = $(".h5p-content-search > input").val(); // If search string same as previous, just do nothing
      ...
      if (searchString.trim().length === 0) {
        ...
        var listToFilter = H5PLibraryDetails.library.content; // Check if we can filter the already filtered results (for performance)
      ...
      H5PLibraryDetails.currentFilter = searchString; // Cache the current result

      H5PLibraryDetails.filterCache[searchString] = H5PLibraryDetails.currentContent;
      H5PLibraryDetails.currentPage = 0;
      H5PLibraryDetails.createContentTable(); // Display search results:
