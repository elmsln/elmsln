...
test("Files", {
  "can open files with file URI": function canOpenFilesWithFileURI() {
  ...
  "can open files with relative URI": function canOpenFilesWithRelativeURI() {
    var filePath = path.resolve(testing.testPath("tiny-picture.png"));
  ...
  "given base is not set when opening relative uri then error is raised": function givenBaseIsNotSetWhenOpeningRelativeUriThenErrorIsRaised() {
    var files = new Files(null);
  ...
  "error if relative uri cannot be opened": function errorIfRelativeUriCannotBeOpened() {
...
test("uriToPath", {
  "leading slash is retained on non-Windows file URIs": function leadingSlashIsRetainedOnNonWindowsFileURIs() {
    assert.equal(uriToPath("file:///a/b/c", "linux"), "/a/b/c");
    assert.equal(uriToPath("file:///a/b/c", "win32"), "/a/b/c");
  },
  "URI is unquoted": function URIIsUnquoted() {
    assert.equal(uriToPath("file:///a%20b"), "/a b");
  },
  "when host is set to localhost then path can be found": function whenHostIsSetToLocalhostThenPathCanBeFound() {
    assert.equal(uriToPath("file://localhost/a/b/c"), "/a/b/c");
  },
  "when host is set but not localhost then path cannot be found": function whenHostIsSetButNotLocalhostThenPathCannotBeFound() {
    assert.throws(function () {
      uriToPath("file://example/a/b/c");
    }, /Could not convert URI to path: file:\/\/example\/a\/b\/c/);
  },
  "leading slash is not dropped on Windows file URIs when platform is not Windows": function leadingSlashIsNotDroppedOnWindowsFileURIsWhenPlatformIsNotWindows() {
    assert.equal(uriToPath("file:///c:/a", "linux"), "/c:/a");
  },
  "leading slash is dropped on Windows file URIs when platform is Windows": function leadingSlashIsDroppedOnWindowsFileURIsWhenPlatformIsWindows() {
    assert.equal(uriToPath("file:///c:/a", "win32"), "c:/a");
    assert.equal(uriToPath("file:///C:/a", "win32"), "C:/a");
  },
  "relative URI is unquoted": function relativeURIIsUnquoted() {
