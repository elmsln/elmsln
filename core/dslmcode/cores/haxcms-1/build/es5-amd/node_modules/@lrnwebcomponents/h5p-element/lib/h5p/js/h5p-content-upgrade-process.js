...
H5P.ContentUpgradeProcess = function (Version) {
  ...
  function ContentUpgradeProcess(name, oldVersion, newVersion, params, id, loadLibrary, done) {
    var self = this; // Make params possible to work with

    try {
      params = JSON.parse(params);

      if (!babelHelpers.instanceof(params, Object)) {
        throw true;
      }
    } catch (event) {
      return done({
  ...
  ContentUpgradeProcess.prototype.upgrade = function (name, oldVersion, newVersion, params, metadata, done) {
    var self = this; // Load library details and upgrade routines
  ...
  var asyncSerial = function asyncSerial(obj, process, finished) {
    var id,
        isArray = babelHelpers.instanceof(obj, Array); // Keep track of each property that belongs to this object.
