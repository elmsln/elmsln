(function(root,factory){if("function"===typeof define&&define.amd){// AMD. Register as an anonymous module unless amdModuleId is set
define("Chartist",[],function(){return root.Chartist=factory()})}else if("object"===("undefined"===typeof module?"undefined":babelHelpers.typeof(module))&&module.exports){// Node. Does not work with strict CommonJS, but
// only CommonJS-like environments that support module.exports,
// like Node.
module.exports=factory()}else{root.Chartist=factory()}})(this,function(){/* Chartist.js 0.11.0
   ...
   */var Chartist={version:"0.11.0"};(function(window,document,Chartist){"use strict";/**
...
return String.fromCharCode(97+n%26)};/**
     ...
     */Chartist.extend=function(target){var i,source,sourceProp;target=target||{};for(i=1;i<arguments.length;i++){source=arguments[i];for(var prop in source){sourceProp=source[prop];if("object"===babelHelpers.typeof(sourceProp)&&null!==sourceProp&&!babelHelpers.instanceof(sourceProp,Array)){target[prop]=Chartist.extend(target[prop],sourceProp)}else{target[prop]=sourceProp}}}return target};/**
     ...
     */Chartist.ensureUnit=function(value,unit){if("number"===typeof value){value=value+unit}return value};/**
     * Converts a number or string to a quantity object.
     *
     * @memberof Chartist.Core
     * @param {String|Number} input
     * @return {Object} Returns an object containing the value as number and the unit as string.
     */Chartist.quantity=function(input){if("string"===typeof input){var match=/^(\d+)\s*(.*)$/g.exec(input);return{value:+match[1],unit:match[2]||void 0}}return{value:input}};/**
     * This is a wrapper around document.querySelector that will return the query if it's already of type Node
     *
     * @memberof Chartist.Core
     * @param {String|Node} query The query to use for selecting a Node or a DOM node that will be returned directly
     * @return {Node}
     */Chartist.querySelector=function(query){return babelHelpers.instanceof(query,Node)?query:document.querySelector(query)};/**
     * Functional style helper to produce array with given length initialized with undefined values
     *
     * @memberof Chartist.Core
     * @param length
     * @return {Array}
     */Chartist.times=function(length){return Array.apply(null,Array(length))};/**
     ...
     * @return {String}
     */Chartist.serialize=function(data){if(null===data||data===void 0){return data}else if("number"===typeof data){data=""+data}else if("object"===babelHelpers.typeof(data)){data=JSON.stringify({data:data})}return Object.keys(Chartist.escapingMap).reduce(function(result,key){return Chartist.replaceAll(result,key,Chartist.escapingMap[key])},data)};/**
     * This function de-serializes a string previously serialized with Chartist.serialize. The string will always be unescaped using Chartist.escapingMap before it's returned. Based on the input value the return type can be Number, String or Object. JSON.parse is used with try / catch to see if the unescaped string can be parsed into an Object and this Object will be returned on success.
     *
     * @memberof Chartist.Core
     * @param {String} data
     * @return {String|Number|Object}
     */Chartist.deserialize=function(data){if("string"!==typeof data){return data}data=Object.keys(Chartist.escapingMap).reduce(function(result,key){return Chartist.replaceAll(result,Chartist.escapingMap[key],key)},data);try{data=JSON.parse(data);data=data.data!==void 0?data.data:data}catch(e){}return data};/**
...
container.appendChild(svg._node);return svg};/**
     ...
     */Chartist.normalizeData=function(data,reverse,multi){var labelCount,output={raw:data,normalized:{}};// Check if we should generate some labels based on existing series data
...
if(output.normalized.series.every(function(value){return babelHelpers.instanceof(value,Array)})){// Getting the series with the the most elements
...
Array.prototype.push.apply(output.normalized.labels,Chartist.times(Math.max(0,labelCount-output.normalized.labels.length)).map(function(){return""}));if(reverse){Chartist.reverseData(output.normalized)}return output};/**
     ...
     */Chartist.safeHasProperty=function(object,property){return null!==object&&"object"===babelHelpers.typeof(object)&&object.hasOwnProperty(property)};/**
     * Checks if a value is considered a hole in the data series.
     *
     * @param {*} value
     * @returns {boolean} True if the value is considered a data hole
     */Chartist.isDataHoleValue=function(value){return null===value||value===void 0||"number"===typeof value&&isNaN(value)};/**
     * Reverses the series, labels and series data arrays.
     *
     * @memberof Chartist.Core
     * @param data
     */Chartist.reverseData=function(data){data.labels.reverse();data.series.reverse();for(var i=0;i<data.series.length;i++){if("object"===babelHelpers.typeof(data.series[i])&&data.series[i].data!==void 0){data.series[i].data.reverse()}else if(babelHelpers.instanceof(data.series[i],Array)){data.series[i].reverse()}}};/**
...
return recursiveConvert(value.data)}else if(babelHelpers.instanceof(value,Array)){// Data is of type array so we need to recurse on the series
...
return Chartist.getNumberOrUndefined(value)}}}return data.series.map(recursiveConvert)};/**
     ...
     */Chartist.normalizePadding=function(padding,fallback){fallback=fallback||0;return"number"===typeof padding?{top:padding,right:padding,bottom:padding,left:padding}:{top:"number"===typeof padding.top?padding.top:fallback,right:"number"===typeof padding.right?padding.right:fallback,bottom:"number"===typeof padding.bottom?padding.bottom:fallback,left:"number"===typeof padding.left?padding.left:fallback}};Chartist.getMetaData=function(series,index){var value=series.data?series.data[index]:series[index];return value?value.meta:void 0};/**
...
options=Chartist.extend({},options,dimension?options["axis"+dimension.toUpperCase()]:{});var highLow={high:options.high===void 0?-Number.MAX_VALUE:+options.high,low:options.low===void 0?Number.MAX_VALUE:+options.low},findHigh=options.high===void 0,findLow=options.low===void 0;// Function to recursively walk through arrays and find highest and lowest number
...
if(options.referenceValue||0===options.referenceValue){highLow.high=Math.max(options.referenceValue,highLow.high);highLow.low=Math.min(options.referenceValue,highLow.low)}// If high and low are the same because of misconfiguration or flat data (only the same value) we need
// to set the high or low to 0 depending on the polarity
if(highLow.high<=highLow.low){// If both values are 0 we set high to 1
if(0===highLow.low){highLow.high=1}else if(0>highLow.low){// If we have the same negative value for the bounds we set bounds.high to 0
highLow.high=0}else if(0<highLow.high){// If we have the same positive value for the bounds we set bounds.low to 0
...
highLow.high=1;highLow.low=0}}return highLow};/**
     ...
     * @param value
     * @returns {Boolean}
     */Chartist.isNumeric=function(value){return null===value?!1:isFinite(value)};/**
     * Returns true on all falsey values except the numeric value 0.
     *
     * @memberof Chartist.Core
     * @param value
     * @returns {boolean}
     */Chartist.isFalseyButZero=function(value){return!value&&0!==value};/**
     * Returns a number if the passed parameter is a valid number or the function will return undefined. On all other values than a valid number, this function will return undefined.
     *
     * @memberof Chartist.Core
     * @param value
     * @returns {*}
     */Chartist.getNumberOrUndefined=function(value){return Chartist.isNumeric(value)?+value:void 0};/**
     * Checks if provided value object is multi value (contains x or y properties)
     *
     * @memberof Chartist.Core
     * @param value
     */Chartist.isMultiValue=function(value){return"object"===babelHelpers.typeof(value)&&("x"in value||"y"in value)};/**
     ...
     */Chartist.rho=function(num){if(1===num){return num}function gcd(p,q){if(0===p%q){return q}else{return gcd(q,p%q)}}function f(x){return x*x+1}var x1=2,x2=2,divisor;if(0===num%2){return 2}do{x1=f(x1)%num;x2=f(f(x2))%num;divisor=gcd(Math.abs(x1-x2),num)}while(1===divisor);return divisor};/**
...
var length=Chartist.projectLength(axisLength,bounds.step,bounds),scaleUp=length<scaleMinSpace,smallestFactor=onlyInteger?Chartist.rho(bounds.range):0;// First check if we should only use integer steps and if step 1 is still larger than scaleMinSpace so we can use 1
...
while(!0){if(scaleUp&&Chartist.projectLength(axisLength,bounds.step,bounds)<=scaleMinSpace){bounds.step*=2}else if(!scaleUp&&Chartist.projectLength(axisLength,bounds.step/2,bounds)>=scaleMinSpace){bounds.step/=2;if(onlyInteger&&0!==bounds.step%1){bounds.step*=2;break}}else{break}if(1e3<optimizationCounter++){throw new Error("Exceeded maximum number of iterations while optimizing scale step!")}}}var EPSILON=2221e-19;bounds.step=Math.max(bounds.step,EPSILON);function safeIncrement(value,increment){// If increment is too small use *= (1+EPSILON) as a simple nextafter
if(value===(value+=increment)){value*=1+(0<increment?EPSILON:-EPSILON)}return value}// Narrow min and max based on new step
newMin=bounds.min;newMax=bounds.max;while(newMin+bounds.step<=bounds.low){newMin=safeIncrement(newMin,bounds.step)}while(newMax-bounds.step>=bounds.high){newMax=safeIncrement(newMax,-bounds.step)}bounds.min=newMin;bounds.max=newMax;bounds.range=bounds.max-bounds.min;var values=[];for(i=bounds.min;i<=bounds.max;i=safeIncrement(i,bounds.step)){var value=Chartist.roundWithPrecision(i);if(value!==values[values.length-1]){values.push(value)}}bounds.values=values;return bounds};/**
     ...
     */Chartist.polarToCartesian=function(centerX,centerY,radius,angleInDegrees){var angleInRadians=(angleInDegrees-90)*Math.PI/180;return{x:centerX+radius*Math.cos(angleInRadians),y:centerY+radius*Math.sin(angleInRadians)}};/**
     ...
     */Chartist.createChartRect=function(svg,options,fallbackPadding){var hasAxis=!!(options.axisX||options.axisY),yAxisOffset=hasAxis?options.axisY.offset:0,xAxisOffset=hasAxis?options.axisX.offset:0,width=svg.width()||Chartist.quantity(options.width).value||0,height=svg.height()||Chartist.quantity(options.height).value||0,normalizedPadding=Chartist.normalizePadding(options.chartPadding,fallbackPadding);// If settings were to small to cope with offset (legacy) and padding, we'll adjust
width=Math.max(width,yAxisOffset+normalizedPadding.left+normalizedPadding.right);height=Math.max(height,xAxisOffset+normalizedPadding.top+normalizedPadding.bottom);var chartRect={padding:normalizedPadding,width:function width(){return this.x2-this.x1},height:function height(){return this.y1-this.y2}};if(hasAxis){if("start"===options.axisX.position){chartRect.y2=normalizedPadding.top+xAxisOffset;chartRect.y1=Math.max(height-normalizedPadding.bottom,chartRect.y2+1)}else{chartRect.y2=normalizedPadding.top;chartRect.y1=Math.max(height-normalizedPadding.bottom-xAxisOffset,chartRect.y2+1)}if("start"===options.axisY.position){chartRect.x1=normalizedPadding.left+yAxisOffset;chartRect.x2=Math.max(width-normalizedPadding.right,chartRect.x1+1)}else{chartRect.x1=normalizedPadding.left;chartRect.x2=Math.max(width-normalizedPadding.right-yAxisOffset,chartRect.x1+1)}}else{chartRect.x1=normalizedPadding.left;chartRect.x2=Math.max(width-normalizedPadding.right,chartRect.x1+1);chartRect.y2=normalizedPadding.top;chartRect.y1=Math.max(height-normalizedPadding.bottom,chartRect.y2+1)}return chartRect};/**
...
eventEmitter.emit("draw",Chartist.extend({type:"grid",axis:axis,index:index,group:group,element:gridElement},positionalData))};/**
     ...
     * @param className
     * @param eventEmitter
     */Chartist.createGridBackground=function(gridGroup,chartRect,className,eventEmitter){var gridBackground=gridGroup.elem("rect",{x:chartRect.x1,y:chartRect.y2,width:chartRect.width(),height:chartRect.height()},className,!0);// Event for grid background draw
...
updateCurrentOptions();return{removeMediaQueryListeners:removeMediaQueryListeners,getCurrentOptions:function getCurrentOptions(){return Chartist.extend({},currentOptions)}}};/**
     ...
     */Chartist.splitIntoSegments=function(pathCoordinates,valueData,options){var defaultOptions={increasingX:!1,fillHoles:!1};options=Chartist.extend({},defaultOptions,options);for(var segments=[],hole=!0,i=0;i<pathCoordinates.length;i+=2){// If this value is a "hole" we set the hole flag
if(Chartist.getMultiValue(valueData[i/2].value)===void 0){// if(valueData[i / 2].value === undefined) {
if(!options.fillHoles){hole=!0}}else{if(options.increasingX&&2<=i&&pathCoordinates[i]<=pathCoordinates[i-2]){// X is not increasing, so we need to make sure we start a new segment
hole=!0}// If it's a valid value we need to check if we're coming out of a hole and create a new empty segment
if(hole){segments.push({pathCoordinates:[],valueData:[]});// As we have a valid value now, we are not in a "hole" anymore
hole=!1}// Add to the segment pathCoordinates and valueData
segments[segments.length-1].pathCoordinates.push(pathCoordinates[i],pathCoordinates[i+1]);segments[segments.length-1].valueData.push(valueData[i/2])}}return segments}})(window,document,Chartist);/**
   ...
   */ /* global Chartist */(function(window,document,Chartist){"use strict";Chartist.Interpolation={};/**
     ...
     * @return {Function}
     */Chartist.Interpolation.none=function(options){var defaultOptions={fillHoles:!1};options=Chartist.extend({},defaultOptions,options);return function none(pathCoordinates,valueData){for(var path=new Chartist.Svg.Path,hole=!0,i=0;i<pathCoordinates.length;i+=2){var currX=pathCoordinates[i],currY=pathCoordinates[i+1],currData=valueData[i/2];if(Chartist.getMultiValue(currData.value)!==void 0){if(hole){path.move(currX,currY,!1,currData)}else{path.line(currX,currY,!1,currData)}hole=!1}else if(!options.fillHoles){hole=!0}}return path}};/**
     ...
     * @return {Function}
     */Chartist.Interpolation.simple=function(options){var defaultOptions={divisor:2,fillHoles:!1};options=Chartist.extend({},defaultOptions,options);var d=1/Math.max(1,options.divisor);return function simple(pathCoordinates,valueData){for(var path=new Chartist.Svg.Path,prevX,prevY,prevData,i=0;i<pathCoordinates.length;i+=2){var currX=pathCoordinates[i],currY=pathCoordinates[i+1],length=(currX-prevX)*d,currData=valueData[i/2];if(currData.value!==void 0){if(prevData===void 0){path.move(currX,currY,!1,currData)}else{path.curve(prevX+length,prevY,currX-length,currY,currX,currY,!1,currData)}prevX=currX;prevY=currY;prevData=currData}else if(!options.fillHoles){prevX=currX=prevData=void 0}}return path}};/**
     ...
     * @return {Function}
     */Chartist.Interpolation.cardinal=function(options){var defaultOptions={tension:1,fillHoles:!1};options=Chartist.extend({},defaultOptions,options);var t=Math.min(1,Math.max(0,options.tension)),c=1-t;return function cardinal(pathCoordinates,valueData){// First we try to split the coordinates into segments
...
return Chartist.Interpolation.none()([])}else if(1<segments.length){// If the split resulted in more that one segment we need to interpolate each segment individually and join them
...
if(4>=pathCoordinates.length){return Chartist.Interpolation.none()(pathCoordinates,valueData)}for(var path=new Chartist.Svg.Path().move(pathCoordinates[0],pathCoordinates[1],!1,valueData[0]),z,i=0,iLen=pathCoordinates.length,p;iLen-2*!z>i;i+=2){p=[{x:+pathCoordinates[i-2],y:+pathCoordinates[i-1]},{x:+pathCoordinates[i],y:+pathCoordinates[i+1]},{x:+pathCoordinates[i+2],y:+pathCoordinates[i+3]},{x:+pathCoordinates[i+4],y:+pathCoordinates[i+5]}];if(z){if(!i){p[0]={x:+pathCoordinates[iLen-2],y:+pathCoordinates[iLen-1]}}else if(iLen-4===i){p[3]={x:+pathCoordinates[0],y:+pathCoordinates[1]}}else if(iLen-2===i){p[2]={x:+pathCoordinates[0],y:+pathCoordinates[1]};p[3]={x:+pathCoordinates[2],y:+pathCoordinates[3]}}}else{if(iLen-4===i){p[3]=p[2]}else if(!i){p[0]={x:+pathCoordinates[i],y:+pathCoordinates[i+1]}}}path.curve(t*(-p[0].x+6*p[1].x+p[2].x)/6+c*p[2].x,t*(-p[0].y+6*p[1].y+p[2].y)/6+c*p[2].y,t*(p[1].x+6*p[2].x-p[3].x)/6+c*p[2].x,t*(p[1].y+6*p[2].y-p[3].y)/6+c*p[2].y,p[2].x,p[2].y,!1,valueData[(i+2)/2])}return path}}};/**
     ...
     * @return {Function}
     */Chartist.Interpolation.monotoneCubic=function(options){var defaultOptions={fillHoles:!1};options=Chartist.extend({},defaultOptions,options);return function monotoneCubic(pathCoordinates,valueData){// First we try to split the coordinates into segments
// This is necessary to treat "holes" in line charts
var segments=Chartist.splitIntoSegments(pathCoordinates,valueData,{fillHoles:options.fillHoles,increasingX:!0});if(!segments.length){// If there were no segments return 'Chartist.Interpolation.none'
return Chartist.Interpolation.none()([])}else if(1<segments.length){// If the split resulted in more that one segment we need to interpolate each segment individually and join them
// afterwards together into a single path.
...
if(4>=pathCoordinates.length){return Chartist.Interpolation.none()(pathCoordinates,valueData)}var xs=[],ys=[],i,n=pathCoordinates.length/2,ms=[],ds=[],dys=[],dxs=[],path;// Populate x and y coordinates into separate arrays, for readability
for(i=0;i<n;i++){xs[i]=pathCoordinates[2*i];ys[i]=pathCoordinates[2*i+1]}// Calculate deltas and derivative
...
ms[0]=ds[0];ms[n-1]=ds[n-2];for(i=1;i<n-1;i++){if(0===ds[i]||0===ds[i-1]||0<ds[i-1]!==0<ds[i]){ms[i]=0}else{ms[i]=3*(dxs[i-1]+dxs[i])/((2*dxs[i]+dxs[i-1])/ds[i-1]+(dxs[i]+2*dxs[i-1])/ds[i]);if(!isFinite(ms[i])){ms[i]=0}}}// Now build a path from the slopes
path=new Chartist.Svg.Path().move(xs[0],ys[0],!1,valueData[0]);for(i=0;i<n-1;i++){path.curve(// First control point
xs[i]+dxs[i]/3,ys[i]+ms[i]*dxs[i]/3,// Second control point
xs[i+1]-dxs[i]/3,ys[i+1]-ms[i+1]*dxs[i]/3,// End point
xs[i+1],ys[i+1],!1,valueData[i+1])}return path}}};/**
     ...
     * @param options
     * @returns {Function}
     */Chartist.Interpolation.step=function(options){var defaultOptions={postpone:!0,fillHoles:!1};options=Chartist.extend({},defaultOptions,options);return function step(pathCoordinates,valueData){for(var path=new Chartist.Svg.Path,prevX,prevY,prevData,i=0;i<pathCoordinates.length;i+=2){var currX=pathCoordinates[i],currY=pathCoordinates[i+1],currData=valueData[i/2];// If the current point is also not a hole we can draw the step lines
if(currData.value!==void 0){if(prevData===void 0){path.move(currX,currY,!1,currData)}else{if(options.postpone){// If postponed we should draw the step line with the value of the previous value
path.line(currX,prevY,!1,prevData)}else{// If not postponed we should draw the step line with the value of the current value
path.line(prevX,currY,!1,currData)}// Line to the actual point (this should only be a Y-Axis movement
path.line(currX,currY,!1,currData)}prevX=currX;prevY=currY;prevData=currData}else if(!options.fillHoles){prevX=prevY=prevData=void 0}}return path}}})(window,document,Chartist);/**
   * A very basic event module that helps to generate and catch events.
   *
   * @module Chartist.Event
   */ /* global Chartist */(function(window,document,Chartist){"use strict";Chartist.EventEmitter=function(){var handlers=[];/**
...
if(handler){handlers[event].splice(handlers[event].indexOf(handler),1);if(0===handlers[event].length){delete handlers[event]}}else{// If no handler is specified we remove all handlers for this event
...
if(handlers["*"]){handlers["*"].forEach(function(starHandler){starHandler(event,data)})}}return{addEventHandler:addEventHandler,removeEventHandler:removeEventHandler,emit:emit}}})(window,document,Chartist);/**
   * This module provides some basic prototype inheritance utilities.
   *
   * @module Chartist.Class
   */ /* global Chartist */(function(window,document,Chartist){"use strict";function listToArray(list){var arr=[];if(list.length){for(var i=0;i<list.length;i++){arr.push(list[i])}}return arr}/**
...
function cloneDefinitions(){var args=listToArray(arguments),target=args[0];args.splice(1,args.length-1).forEach(function(source){Object.getOwnPropertyNames(source).forEach(function(propName){// If this property already exist in target we delete it first
delete target[propName];// Define the property with the descriptor from source
Object.defineProperty(target,propName,Object.getOwnPropertyDescriptor(source,propName))})});return target}Chartist.Class={extend:extend,cloneDefinitions:cloneDefinitions}})(window,document,Chartist);/**
   * Base for all chart types. The methods in Chartist.Base are inherited to all chart types.
   *
   * @module Chartist.Base
   */ /* global Chartist */(function(window,document,Chartist){"use strict";// TODO: Currently we need to re-draw the chart on window resize. This is usually very bad and will affect performance.
...
if(this.options.plugins){this.options.plugins.forEach(function(plugin){if(babelHelpers.instanceof(plugin,Array)){plugin[0](this,plugin[1])}else{plugin(this)}}.bind(this))}// Event for data transformation that allows to manipulate the data before it gets rendered in the charts
...
Chartist.Base=Chartist.Class.extend({constructor:Base,optionsProvider:void 0,container:void 0,svg:void 0,eventEmitter:void 0,createChart:function createChart(){throw new Error("Base chart type can't be instantiated!")},update:update,detach:detach,on:on,off:off,version:Chartist.version,supportsForeignObject:!1})})(window,document,Chartist);/**
   * Chartist SVG module for simple SVG DOM abstraction
   *
   * @module Chartist.Svg
   */ /* global Chartist */(function(window,document,Chartist){"use strict";/**
...
if(babelHelpers.instanceof(name,Element)){this._node=name}else{this._node=document.createElementNS(Chartist.namespaces.svg,name);// If this is an SVG element created then custom namespace
if("svg"===name){this.attr({"xmlns:ct":Chartist.namespaces.ct})}}if(attributes){this.attr(attributes)}if(className){this.addClass(className)}if(parent){if(insertFirst&&parent._node.firstChild){parent._node.insertBefore(this._node,parent._node.firstChild)}else{parent._node.appendChild(this._node)}}}/**
     ...
     */function attr(attributes,ns){if("string"===typeof attributes){if(ns){return this._node.getAttributeNS(ns,attributes)}else{return this._node.getAttribute(attributes)}}Object.keys(attributes).forEach(function(key){// If the attribute value is undefined we can skip this one
if(attributes[key]===void 0){return}if(-1!==key.indexOf(":")){var namespacedAttribute=key.split(":");this._node.setAttributeNS(Chartist.namespaces[namespacedAttribute[0]],key,attributes[key])}else{this._node.setAttribute(key,attributes[key])}}.bind(this));return this}/**
     ...
     */function parent(){return babelHelpers.instanceof(this._node.parentNode,SVGElement)?new Chartist.Svg(this._node.parentNode):null}/**
     * This method returns a Chartist.Svg wrapper around the root SVG element of the current tree.
     *
     * @memberof Chartist.Svg
     * @return {Chartist.Svg} The root SVG element wrapped in a Chartist.Svg element
     */function root(){var node=this._node;while("svg"!==node.nodeName){node=node.parentNode}return new Chartist.Svg(node)}/**
...
if("string"===typeof content){var container=document.createElement("div");container.innerHTML=content;content=container.firstChild}// Adding namespace to content element
...
fnObj._node.appendChild(content);return fnObj}/**
     ...
     * @param {String} names A white space separated list of class names
     * @return {Chartist.Svg} The wrapper of the current element
     */function removeClass(names){var removedClasses=names.trim().split(/\s+/);this._node.setAttribute("class",this.classes(this._node).filter(function(name){return-1===removedClasses.indexOf(name)}).join(" "));return this}/**
...
easing=babelHelpers.instanceof(animationDefinition.easing,Array)?animationDefinition.easing:Chartist.Svg.Easing[animationDefinition.easing];delete animationDefinition.easing}// If numeric dur or begin was provided we assume milli seconds
...
Object.keys(Chartist.Svg.prototype).filter(function(prototypeProperty){return-1===["constructor","parent","querySelector","querySelectorAll","replace","append","classes","height","width"].indexOf(prototypeProperty)}).forEach(function(prototypeProperty){list[prototypeProperty]=function(){var args=Array.prototype.slice.call(arguments,0);list.svgElements.forEach(function(element){Chartist.Svg.prototype[prototypeProperty].apply(element,args)});return list}})}Chartist.Svg.List=Chartist.Class.extend({constructor:SvgList})})(window,document,Chartist);/**
   ...
   */ /* global Chartist */(function(window,document,Chartist){"use strict";/**
     ...
     * @type {Object}
     */var elementDescriptions={m:["x","y"],l:["x","y"],c:["x1","y1","x2","y2","x","y"],a:["rx","ry","xAr","lAf","sf","x","y"]},defaultOptions={// The accuracy in digit count after the decimal point. This will be used to round numbers in the SVG path. If this option is set to false then no rounding will be performed.
accuracy:3};/**
     ...
     */function position(pos){if(pos!==void 0){this.pos=Math.max(0,Math.min(this.pathElements.length,pos));return this}else{return this.pos}}/**
...
if("Z"===chunks[chunks.length-1][0].toUpperCase()){chunks.pop()}// Using svgPathElementDescriptions to map raw path arrays into objects that contain the command and the parameters
// For example {command: 'M', x: '10', y: '10'}
var elements=chunks.map(function(chunk){var command=chunk.shift(),description=elementDescriptions[command.toLowerCase()];return Chartist.extend({command:command},description.reduce(function(result,paramName,index){result[paramName]=+chunk[index];return result},{}))}),spliceArgs=[this.pos,0];// Preparing a splice call with the elements array as var arg params and insert the parsed elements at the current position
Array.prototype.push.apply(spliceArgs,elements);Array.prototype.splice.apply(this.pathElements,spliceArgs);// Increase the internal position by the element count
this.pos+=elements.length;return this}/**
     ...
     * @return {Chartist.Svg.Path} The current path object for easy call chaining.
     */function scale(x,y){forEachParam(this.pathElements,function(pathElement,paramName){pathElement[paramName]*="x"===paramName[0]?x:y});return this}/**
     ...
     * @return {Chartist.Svg.Path} The current path object for easy call chaining.
     */function translate(x,y){forEachParam(this.pathElements,function(pathElement,paramName){pathElement[paramName]+="x"===paramName[0]?x:y});return this}/**
     ...
     * @return {Chartist.Svg.Path} The current path object for easy call chaining.
     */function transform(transformFnc){forEachParam(this.pathElements,function(pathElement,paramName,pathElementIndex,paramIndex,pathElements){var transformed=transformFnc(pathElement,paramName,pathElementIndex,paramIndex,pathElements);if(transformed||0===transformed){pathElement[paramName]=transformed}});return this}/**
     ...
     */function splitByCommand(command){var split=[new Chartist.Svg.Path];this.pathElements.forEach(function(pathElement){if(pathElement.command===command.toUpperCase()&&0!==split[split.length-1].pathElements.length){split.push(new Chartist.Svg.Path)}split[split.length-1].pathElements.push(pathElement)});return split}/**
...
if("start"===chartOptions.axisY.position){labelOffset.x=useForeignObject?this.chartRect.padding.left+chartOptions.axisY.labelOffset.x:this.chartRect.x1-10}else{labelOffset.x=this.chartRect.x2+chartOptions.axisY.labelOffset.x+10}}if(axisOptions.showGrid){Chartist.createGrid(projectedValue,index,this,this.gridOffset,this.chartRect[this.counterUnits.len](),gridGroup,[chartOptions.classNames.grid,chartOptions.classNames[this.units.dir]],eventEmitter)}if(axisOptions.showLabel){Chartist.createLabel(projectedValue,labelLength,index,labelValues,this,axisOptions.offset,labelOffset,labelGroup,[chartOptions.classNames.label,chartOptions.classNames[this.units.dir],"start"===axisOptions.position?chartOptions.classNames[axisOptions.position]:chartOptions.classNames.end],useForeignObject,eventEmitter)}}.bind(this))}Chartist.Axis=Chartist.Class.extend({constructor:Axis,createGridAndLabels:createGridAndLabels,projectValue:function projectValue(value,index,data){throw new Error("Base axis can't be instantiated!")}});Chartist.Axis.units=axisUnits})(window,document,Chartist);/**
   ...
   */ /* global Chartist */(function(window,document,Chartist){"use strict";function AutoScaleAxis(axisUnit,data,chartRect,options){// Usually we calculate highLow based on the data but this can be overriden by a highLow object in the options
...
showLabel:!0,// If the axis grid should be drawn or not
showGrid:!0,// Interpolation function that allows you to intercept the value from the axis label
...
type:void 0},// Options for Y-Axis
...
type:void 0,// This value specifies the minimum height in pixel of the scale steps
scaleMinSpace:20,// Use only integer values (whole numbers) for the scale steps
onlyInteger:!1},// Specify a fixed width for the chart as a string (i.e. '100px' or '50%')
width:void 0,// Specify a fixed height for the chart as a string (i.e. '100px' or '50%')
height:void 0,// If the line should be drawn or not
showLine:!0,// If dots should be drawn or not
showPoint:!0,// If the line chart should draw an area
showArea:!1,// The base for the area chart that will be used to close the area shape (is normally 0)
areaBase:0,// Specify if the lines should be smoothed. This value can be true or false where true will result in smoothing using the default smoothing interpolation function Chartist.Interpolation.cardinal and false results in Chartist.Interpolation.none. You can also choose other smoothing / interpolation functions available in the Chartist.Interpolation module, or write your own interpolation function. Check the examples for a brief description.
lineSmooth:!0,// If the line chart should add a background fill to the .ct-grids group.
showGridBackground:!1,// Overriding the natural low of the chart allows you to zoom in or limit the charts lowest displayed value
low:void 0,// Overriding the natural high of the chart allows you to zoom in or limit the charts highest displayed value
high:void 0,// Padding of the chart drawing area to the container element and labels as a number or padding object {top: 5, right: 5, bottom: 5, left: 5}
...
fullWidth:!1,// If true the whole data is reversed including labels, the series order as well as the whole series data arrays.
reverseData:!1,// Override the class names that get used to generate the SVG structure of the chart
classNames:{chart:"ct-chart-line",label:"ct-label",labelGroup:"ct-labels",series:"ct-series",line:"ct-line",point:"ct-point",area:"ct-area",grid:"ct-grid",gridGroup:"ct-grids",gridBackground:"ct-grid-background",vertical:"ct-vertical",horizontal:"ct-horizontal",start:"ct-start",end:"ct-end"}};/**
     * Creates a new chart
     *
     */function createChart(options){var data=Chartist.normalizeData(this.data,options.reverseData,!0);// Create new svg object
...
Chartist.Line=Chartist.Base.extend({constructor:Line,createChart:createChart})})(window,document,Chartist);/**
...
onlyInteger:!1},// Options for Y-Axis
...
onlyInteger:!1},// Specify a fixed width for the chart as a string (i.e. '100px' or '50%')
width:void 0,// Specify a fixed height for the chart as a string (i.e. '100px' or '50%')
height:void 0,// Overriding the natural high of the chart allows you to zoom in or limit the charts highest displayed value
high:void 0,// Overriding the natural low of the chart allows you to zoom in or limit the charts lowest displayed value
low:void 0,// Unless low/high are explicitly set, bar chart will be centered at zero by default. Set referenceValue to null to auto scale.
...
stackBars:!1,// If set to 'overlap' this property will force the stacked bars to draw from the zero line.
...
horizontalBars:!1,// If set to true then each bar will represent a series and the data array is expected to be a one dimensional array of data values rather than a series array of series. This is useful if the bar chart should represent a profile rather than some data over time.
distributeSeries:!1,// If true the whole data is reversed including labels, the series order as well as the whole series data arrays.
reverseData:!1,// If the bar chart should add a background fill to the .ct-grids group.
showGridBackground:!1,// Override the class names that get used to generate the SVG structure of the chart
classNames:{chart:"ct-chart-bar",horizontalBars:"ct-horizontal-bars",label:"ct-label",labelGroup:"ct-labels",series:"ct-series",bar:"ct-bar",grid:"ct-grid",gridGroup:"ct-grids",gridBackground:"ct-grid-background",vertical:"ct-vertical",horizontal:"ct-horizontal",start:"ct-start",end:"ct-end"}};/**
     * Creates a new chart
     *
     */function createChart(options){var data,highLow;if(options.distributeSeries){data=Chartist.normalizeData(this.data,options.reverseData,options.horizontalBars?"x":"y");data.normalized.series=data.normalized.series.map(function(value){return[value]})}else{data=Chartist.normalizeData(this.data,options.reverseData,options.horizontalBars?"x":"y")}// Create new svg element
...
var gridGroup=this.svg.elem("g").addClass(options.classNames.gridGroup),seriesGroup=this.svg.elem("g"),labelGroup=this.svg.elem("g").addClass(options.classNames.labelGroup);if(options.stackBars&&0!==data.normalized.series.length){// If stacked bars we need to calculate the high low from stacked values from each series
...
highLow.high=+options.high||(0===options.high?0:highLow.high);highLow.low=+options.low||(0===options.low?0:highLow.low);var chartRect=Chartist.createChartRect(this.svg,options,defaultOptions.padding),valueAxis,labelAxisTicks,labelAxis,axisX,axisY;// We need to set step count based on some options combinations
...
if(options.horizontalBars){if(options.axisX.type===void 0){valueAxis=axisX=new Chartist.AutoScaleAxis(Chartist.Axis.units.x,data.normalized.series,chartRect,Chartist.extend({},options.axisX,{highLow:highLow,referenceValue:0}))}else{valueAxis=axisX=options.axisX.type.call(Chartist,Chartist.Axis.units.x,data.normalized.series,chartRect,Chartist.extend({},options.axisX,{highLow:highLow,referenceValue:0}))}if(options.axisY.type===void 0){labelAxis=axisY=new Chartist.StepAxis(Chartist.Axis.units.y,data.normalized.series,chartRect,{ticks:labelAxisTicks})}else{labelAxis=axisY=options.axisY.type.call(Chartist,Chartist.Axis.units.y,data.normalized.series,chartRect,options.axisY)}}else{if(options.axisX.type===void 0){labelAxis=axisX=new Chartist.StepAxis(Chartist.Axis.units.x,data.normalized.series,chartRect,{ticks:labelAxisTicks})}else{labelAxis=axisX=options.axisX.type.call(Chartist,Chartist.Axis.units.x,data.normalized.series,chartRect,options.axisX)}if(options.axisY.type===void 0){valueAxis=axisY=new Chartist.AutoScaleAxis(Chartist.Axis.units.y,data.normalized.series,chartRect,Chartist.extend({},options.axisY,{highLow:highLow,referenceValue:0}))}else{valueAxis=axisY=options.axisY.type.call(Chartist,Chartist.Axis.units.y,data.normalized.series,chartRect,Chartist.extend({},options.axisY,{highLow:highLow,referenceValue:0}))}}// Projected 0 point
var zeroPoint=options.horizontalBars?chartRect.x1+valueAxis.projectValue(0):chartRect.y1-valueAxis.projectValue(0),stackedBarValues=[];// Used to track the screen coordinates of stacked bars
...
var biPol=seriesIndex-(data.raw.series.length-1)/2,periodHalfLength,seriesElement;// Half of the period width between vertical grid lines used to position bars
...
if(babelHelpers.instanceof(labelAxis,Chartist.StepAxis)){// Offset to center bar between grid lines, but only if the step axis is not stretched
...
if(value===void 0){return}var positions={};positions[labelAxis.units.pos+"1"]=projected[labelAxis.units.pos];positions[labelAxis.units.pos+"2"]=projected[labelAxis.units.pos];if(options.stackBars&&("accumulate"===options.stackMode||!options.stackMode)){// Stack mode: accumulate (default)
...
Chartist.Bar=Chartist.Base.extend({constructor:Bar,createChart:createChart})})(window,document,Chartist);/**
...
width:void 0,// Specify a fixed height for the chart as a string (i.e. '100px' or '50%')
height:void 0,// Padding of the chart drawing area to the container element and labels as a number or padding object {top: 5, right: 5, bottom: 5, left: 5}
...
total:void 0,// If specified the donut CSS classes will be used and strokes will be drawn instead of pie slices.
donut:!1,// If specified the donut segments will be drawn as shapes instead of strokes.
donutSolid:!1,// Specify the donut stroke width, currently done in javascript for convenience. May move to CSS styles in the future.
// This option can be set as number or string to specify a relative width (i.e. 100 or '30%').
donutWidth:60,// If a label should be shown or not
showLabel:!0,// Label position offset from the standard position which is half distance of the radius. This value can be either positive or negative. Positive values will position the label away from the center.
...
reverseData:!1,// If true empty values will be ignored to avoid drawing unncessary slices and labels
ignoreEmptyValues:!1};/**
     ...
     * @param direction
     * @return {string}
     */function determineAnchorPosition(center,label,direction){var toTheRight=label.x>center.x;if(toTheRight&&"explode"===direction||!toTheRight&&"implode"===direction){return"start"}else if(toTheRight&&"implode"===direction||!toTheRight&&"explode"===direction){return"end"}else{return"middle"}}/**
     * Creates the pie chart
     *
     * @param options
     */function createChart(options){var data=Chartist.normalizeData(this.data),seriesGroups=[],labelsGroup,chartRect,radius,labelRadius,totalDataSum,startAngle=options.startAngle;// Create SVG.js draw
...
if("outside"===options.labelPosition||options.donut&&!options.donutSolid){labelRadius=radius}else if("center"===options.labelPosition){// If labelPosition is center we start with 0 and will later wait for the labelOffset
...
var center={x:chartRect.x1+chartRect.width()/2,y:chartRect.y2+chartRect.height()/2},hasSingleValInSeries=1===data.raw.series.filter(function(val){return val.hasOwnProperty("value")?0!==val.value:0!==val}).length;// Check if there is only one non-zero value in the series array.
...
if(0===data.normalized.series[index]&&options.ignoreEmptyValues)return;// If the series is an object and contains a name or meta data we add a custom attribute
...
var endAngle=0<totalDataSum?startAngle+360*(data.normalized.series[index]/totalDataSum):0,overlappigStartAngle=Math.max(0,startAngle-(0===index||hasSingleValInSeries?0:.2));// Use slight offset so there are no transparent hairline issues
// If we need to draw the arc for all 360 degrees we need to add a hack where we close the circle
// with Z and use 359.99 degrees
if(359.99<=endAngle-overlappigStartAngle){endAngle=overlappigStartAngle+359.99}var start=Chartist.polarToCartesian(center.x,center.y,radius,overlappigStartAngle),end=Chartist.polarToCartesian(center.x,center.y,radius,endAngle),innerStart,innerEnd,donutSolidRadius,path=new Chartist.Svg.Path(!options.donut||options.donutSolid).move(end.x,end.y).arc(radius,radius,0,180<endAngle-startAngle,0,start.x,start.y);// If regular pie chart (no donut) we add a line to the center of the circle for completing the pie
if(!options.donut){path.line(center.x,center.y)}else if(options.donutSolid){donutSolidRadius=radius-donutWidth.value;innerStart=Chartist.polarToCartesian(center.x,center.y,donutSolidRadius,startAngle-(0===index||hasSingleValInSeries?0:.2));innerEnd=Chartist.polarToCartesian(center.x,center.y,donutSolidRadius,endAngle);path.line(innerStart.x,innerStart.y);path.arc(donutSolidRadius,donutSolidRadius,0,180<endAngle-startAngle,1,innerEnd.x,innerEnd.y)}// Create the SVG path
...
if(options.showLabel){var labelPosition;if(1===data.raw.series.length){// If we have only 1 series, we can position the label in the center of the pie
...
labelPosition=Chartist.polarToCartesian(center.x,center.y,labelRadius,startAngle+(endAngle-startAngle)/2)}var rawValue;if(data.normalized.labels&&!Chartist.isFalseyButZero(data.normalized.labels[index])){rawValue=data.normalized.labels[index]}else{rawValue=data.normalized.series[index]}var interpolatedValue=options.labelInterpolationFnc(rawValue,index);if(interpolatedValue||0===interpolatedValue){var labelElement=labelsGroup.elem("text",{dx:labelPosition.x,dy:labelPosition.y,"text-anchor":determineAnchorPosition(center,labelPosition,options.labelDirection)},options.classNames.label).text(""+interpolatedValue);// Fire off draw event
