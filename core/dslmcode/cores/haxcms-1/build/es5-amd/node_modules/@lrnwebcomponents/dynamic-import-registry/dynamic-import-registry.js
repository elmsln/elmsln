...
if(e.detail.tag&&e.detail.path){this.register(e.detail)}}/**
   ...
   */},{key:"loadDefinition",value:function(){var _loadDefinition=babelHelpers.asyncToGenerator(/*#__PURE__*/regeneratorRuntime.mark(function _callee(tag){var _this2=this;return regeneratorRuntime.wrap(function _callee$(_context){while(1){switch(_context.prev=_context.next){case 0:// must be lowercase
tag=tag.toLowerCase();// only import if we already had it
if(!(!window.customElements.get(tag)&&this.list[tag]&&!this.__loaded[tag])){_context.next=12;break}// let's assume it's there cause we got here
// this can help things on polyfill environments
this.__loaded[tag]=!0;_context.prev=3;_context.next=6;return new Promise(function(res,rej){return _require.default(["".concat(_this2.basePath).concat(_this2.list[tag])],res,rej)}).then(function(module){// dispatch custom event in case anyone cares
_this2.dispatchEvent(new CustomEvent("dynamic-import-registry-loaded",{detail:{tag:tag,path:_this2.list[tag],module:module}}))});case 6:_context.next=12;break;case 8:_context.prev=8;_context.t0=_context["catch"](3);console.warn(_context.t0);// fire on error too
this.dispatchEvent(new CustomEvent("dynamic-import-registry-failure",{detail:{tag:tag,path:this.list[tag],module:null}}));case 12:case"end":return _context.stop();}}},_callee,this,[[3,8]])}));function loadDefinition(_x){return _loadDefinition.apply(this,arguments)}return loadDefinition}()// simple path from a url modifier
