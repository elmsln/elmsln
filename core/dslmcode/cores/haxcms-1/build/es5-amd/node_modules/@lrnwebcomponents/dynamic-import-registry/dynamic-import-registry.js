...
if(!window.customElements.get(name)&&this.list[tag]&&!this.__loaded[tag]){// let's assume it's there cause we got here
...
_this2.dispatchEvent(new CustomEvent("dynamic-import-registry-loaded",{detail:{tag:name,path:_this2.list[tag],module:module}}))})}catch(e){console.warn(e);// fire on error too
this.dispatchEvent(new CustomEvent("dynamic-import-registry-failure",{detail:{tag:name,path:this.list[tag],module:null}}))}}}// simple path from a url modifier
