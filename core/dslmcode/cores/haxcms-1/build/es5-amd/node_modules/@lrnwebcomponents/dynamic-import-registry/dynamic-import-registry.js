...
if(item.tag&&item.path){if(!this.list[item.tag]){this.list[item.tag]=item.path;this.dispatchEvent(new CustomEvent("dynamic-import-registry--new-registration",{detail:{tag:item.tag,path:item.path}}))}}else{console.warn("DynamicImportRegistry: registration requires tag and path be set")}}/**
...
if(!window.customElements.get(name)&&this.list[tag]){new Promise(function(res,rej){return _require.default(["".concat(_this2.basePath).concat(_this2.list[tag])],res,rej)}).then(function(module){// dispatch custom event in case anyone cares
_this2.dispatchEvent(new CustomEvent("dynamic-import-registry--loaded",{detail:{tag:name,path:_this2.list[tag],module:module}}))}).catch(function(e){console.error(e);// fire on error too
_this2.dispatchEvent(new CustomEvent("dynamic-import-registry--failure",{detail:{tag:name,path:_this2.list[tag],module:null}}))})}}// simple path from a url modifier
