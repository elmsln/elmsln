 ...
 * @customElement
 * @polymer
 */var A11yMediaBehaviors=/*#__PURE__*/function(_SimpleColors){babelHelpers.inherits(A11yMediaBehaviors,_SimpleColors);function A11yMediaBehaviors(){babelHelpers.classCallCheck(this,A11yMediaBehaviors);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(A11yMediaBehaviors).apply(this,arguments))}babelHelpers.createClass(A11yMediaBehaviors,[{key:"connectedCallback",/**
   * life cycle, element is afixed to the DOM
   */value:function connectedCallback(){babelHelpers.get(babelHelpers.getPrototypeOf(A11yMediaBehaviors.prototype),"connectedCallback",this).call(this)}/**
   * returns true if an attribute is not null
   *
   * @param {object} the attribute to check
   * @returns {boolean} attr !== undefined && attr !== null
   */},{key:"_hasAttribute",value:function _hasAttribute(attr){return attr!==void 0&&null!==attr}/**
   * returns true if an attribute is set to a value
   *
   * @param {object} the attribute to check
   * @param {object} the value to check
   * @returns {boolean} attr === val
   */},{key:"_testAttribute",value:function _testAttribute(attr,val){return attr===val}/**
   ...
   */},{key:"_addResponsiveUtility",value:function _addResponsiveUtility(options){var root=this;window.ResponsiveUtility.requestAvailability();window.dispatchEvent(new CustomEvent("responsive-element",{detail:options!==void 0?options:{element:root,attribute:"responsive-size",relativeToParent:!0}}))}/**
   ...
   *
   */},{key:"_getHHMMSS",value:function _getHHMMSS(val,max){max=max===void 0?val:max;var a=function a(val){return 10>val?"0"+val:val},b=function b(val,i,none){return max>=i?a(Math.floor(val/i))+":":none},c=function c(val){return 100>val?val+"0":val};return b(val,3600,"")+b(val%3600,60,"00:")+a(Math.round(val%60))}/**
   ...
   */},{key:"_handlePrintClick",value:function _handlePrintClick(e){this.dispatchEvent(new CustomEvent("print-transcript",{detail:this}))}/**
   * handles the print transcript button
   */},{key:"_handleDownloadClick",value:function _handleDownloadClick(e){this.dispatchEvent(new CustomEvent("download-transcript",{detail:this}))}/**
   * handles transcript printing
   */},{key:"_handleDownload",value:function _handleDownload(e){var root=this;root.dispatchEvent(new CustomEvent("downloading-transcript",{detail:root}));root.$.transcript.download(root.mediaTitle)}/**
   * handles transcript printing
   */},{key:"_handlePrinting",value:function _handlePrinting(e){var root=this;root.dispatchEvent(new CustomEvent("printing-transcript",{detail:root}));root.$.transcript.print(root.mediaTitle)}}],[{key:"tag",/**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */get:function get(){return"a11y-media-behaviors"}// properties available to the custom element for data binding
   ...
   * gets behaviors
   */},{key:"behaviors",get:function get(){return[_simpleColors.SimpleColors,ResponsiveUtility]}}]);return A11yMediaBehaviors}(_simpleColors.SimpleColors);_exports.A11yMediaBehaviors=A11yMediaBehaviors;window.customElements.define(A11yMediaBehaviors.tag,A11yMediaBehaviors)});