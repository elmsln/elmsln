 ...
 */var EditableTable=/*#__PURE__*/function(_displayBehaviors){babelHelpers.inherits(EditableTable,_displayBehaviors);function EditableTable(){babelHelpers.classCallCheck(this,EditableTable);return babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(EditableTable).apply(this,arguments))}babelHelpers.createClass(EditableTable,[{key:"deleteColumn",/**
   * Delete a column at the given index
   * @param {number} index the index of the column
   */value:function deleteColumn(index){for(var i=0;i<this.data.length;i++){this.splice("data."+i,index,1)}var temp=this.data.slice();this.set("data",temp)}/**
   * Delete a row at the given index
   * @param {number} index the index of the row
   */},{key:"deleteRow",value:function deleteRow(index){this.splice("data",index,1);var temp=this.data.slice();this.set("data",temp)}/**
   * Insert a column at the given index
   * @param {number} index the index of the column
   */},{key:"insertColumn",value:function insertColumn(index){for(var temp=this.data.slice(),i=0;i<temp.length;i++){temp[i].splice(index,0,"")}this.set("data",temp)}/**
   * Insert a row at the given index
   * @param {number} index the index of the row
   */},{key:"insertRow",value:function insertRow(index){for(var temp=this.data.slice(),temp2=[],i=0;i<temp[0].length;i++){temp2.push("")}temp.splice(index+1,0,temp2);this.set("data",temp)}/**
   ...
   */},{key:"toggleEditMode",value:function toggleEditMode(edit){edit=edit!==void 0?edit:!this.editMode;this.dispatchEvent(new CustomEvent("toggle-edit-mode",{bubbles:!0,cancelable:!0,composed:!0,detail:this}));if(edit){this.shadowRoot.querySelector("editable-table-display").toggleFilter();this.shadowRoot.querySelector("editable-table-display").sortData("none",-1);this.shadowRoot.querySelector("#inner").focus()}this.editMode=edit}/**
   * Handles when the caption paper-input changed
   */},{key:"_captionChanged",value:function _captionChanged(){this.caption=this.shadowRoot.querySelector("#caption").value}/**
   * Fires when data changed
   * @event change
   * @param {event} the event
   */},{key:"_dataChanged",value:function _dataChanged(newValue,oldValue){if(!newValue||1>newValue.length||1>newValue[0].length){var table=this.children.item(0);if(babelHelpers.typeof(table)!==("undefined"===typeof void 0?"undefined":babelHelpers.typeof(void 0))&&null!==table&&"TABLE"===table.tagName){this.importHTML(table)}else{this.set("data",[["","",""],["","",""],["","",""]])}}}/**
   ...
   */},{key:"_handleRowColumnMenu",value:function _handleRowColumnMenu(e){if(e.detail.insert&&e.detail.row){this.insertRow(e.detail.index)}else if(e.detail.insert&&!e.detail.row){this.insertColumn(e.detail.index)}else if(!e.detail.insert&&e.detail.row){this.deleteRow(e.detail.index)}else{this.deleteColumn(e.detail.index)}}/**
   ...
   */},{key:"_isSortDisabled",value:function _isSortDisabled(hideSort,columnHeader){return hideSort||!columnHeader}/**
   * Sets focus on the cell's textarea if the cell is clicked
   * @param {event} e the event
   */},{key:"_onCellClick",value:function _onCellClick(e){if(e.model&&e.model.root&&e.model.root.nodeList[0]){e.model.root.nodeList[0].focus()}}/**
   * Updates data when cell value changes
   * @param {event} e the event
   */},{key:"_onCellValueChange",value:function _onCellValueChange(e){var temp=this.data.slice();temp[e.detail.row][e.detail.column]=e.detail.value;this.set("data",[]);this.set("data",temp)}/**
   * Updates table properties when setting changes
   * @param {event} e the event
   */},{key:"_onTableSettingChange",value:function _onTableSettingChange(e){this[e.detail.id]=e.detail.toggled}/**
   ...
   */},{key:"_tableDisplayHidden",value:function _tableDisplayHidden(hideBordered,hideCondensed,hideStriped,hideResponsive){return hideBordered&&hideCondensed&&hideStriped&&hideResponsive}/**
   ...
   */},{key:"_tableSortHidden",value:function _tableSortHidden(hideSort,hideFilter){return hideSort&&hideFilter}}],[{key:"template",get:function get(){return(0,_polymerElement.html)(_templateObject_5de516307ae711ea9f6d454984daed83())}},{key:"tag",get:function get(){return"editable-table"}},{key:"properties",get:function get(){return{/**
       ...
       */hideDisplay:{type:Boolean,computed:"_tableDisplayHidden(hideBordered,hideCondensed,hideStriped,hideResponsive)"},/**
       * Hide the filtering option.
       */hideFilter:{type:Boolean,value:!1},/**
       * Hide the table sorting & filtering menu group
       */hideSortFilter:{type:Boolean,computed:"_tableSortHidden(hideSort,hideFilter)"},/**
       * Hide the sorting option.
       */hideSort:{type:Boolean,value:!1},/**
       * Hide the responsive table styles menu option
       */hideResponsive:{type:Boolean,value:!1},/**
       * Hide the striped table styles menu option
       */hideStriped:{type:Boolean,value:!1}}}}]);return EditableTable}((0,_editableTableBehaviors.displayBehaviors)(_polymerElement.PolymerElement));_exports.EditableTable=EditableTable;window.customElements.define(EditableTable.tag,EditableTable)});