...
(function ($, Version) {
  var info,
      ...
      $container,
      librariesCache = {},
      scriptsCache = {}; // Initialize

  $(document).ready(function () {
    // Get library info
    info = H5PAdminIntegration.libraryInfo; // Get and reset container

    var $wrapper = $("#h5p-admin-container").html("");
    $log = $('<ul class="content-upgrade-log"></ul>').appendTo($wrapper);
    $container = $("<div><p>" + info.message + "</p></div>").appendTo($wrapper); // Make it possible to select version
  ...
  var getVersionSelect = function getVersionSelect(versions) {
    var html = "";
  ...
  function ContentUpgrade(libraryId) {
    var self = this; // Get selected version

    self.version = new Version(info.versions[libraryId]);
    self.version.libraryId = libraryId; // Create throbber with loading text and progress

    self.throbber = new Throbber(info.inProgress.replace("%ver", self.version));
    self.started = new Date().getTime();
    self.io = 0; // Track number of working
  ...
  ContentUpgrade.prototype.initWorkers = function () {
    ...
    var messageHandlers = {
      ...
      error: function error(_error) {
        self.printError(_error.err);
        self.workDone(_error.id, null, this);
      },
      loadLibrary: function loadLibrary(details) {
        var worker = this;
  ...
  ContentUpgrade.prototype.nextBatch = function (outData) {
    var self = this; // Track time spent on IO

    var start = new Date().getTime();
    $.post(info.infoUrl, outData, function (inData) {
      self.io += new Date().getTime() - start;

      if (!babelHelpers.instanceof(inData, Object)) {
      ...
      self.token = inData.token; // Start processing
  ...
  ContentUpgrade.prototype.processBatch = function (parameters, skipped) {
    var self = this; // Track upgraded params

    self.upgraded = {};
    self.skipped = skipped; // Track current batch
