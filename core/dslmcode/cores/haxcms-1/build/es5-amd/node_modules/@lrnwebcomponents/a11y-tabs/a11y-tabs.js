...
value:function render(){var _this2=this;return(0,_litElement.html)(_templateObject_3aa8c9b06d2111ea950863318e7634ec(),this.iconClass,this.tabs.map(function(tab,i){return(0,_litElement.html)(_templateObject2_3aa8c9b06d2111ea950863318e7634ec(),_this2._tabButton(tab))}))}// haxProperty definition
}],[{key:"styles",//styles function
get:function get(){return[(0,_litElement.css)(_templateObject3_3aa8c9b06d2111ea950863318e7634ec())]}},{key:"haxProperties",get:function get(){return{}}// properties available to the custom element for data binding
       ...
       */vertical:{type:Boolean,reflect:!0,attribute:"vertical"},/**
       * an array of tab data based on slotted `a11y-tab` elements
       */__tabs:{type:Array}})}/**
   * Store the tag name to make it easier to obtain directly.
   * @notice function name must be here for tooling to operate correctly
   */},{key:"tag",get:function get(){return"a11y-tabs"}}]);function A11yTabs(){var _this;babelHelpers.classCallCheck(this,A11yTabs);_this=babelHelpers.possibleConstructorReturn(this,babelHelpers.getPrototypeOf(A11yTabs).call(this));_this.disabled=!1;_this.hidden=!1;_this.iconBreakpoint=400;_this.layoutBreakpoint=600;_this.responsiveSize="xs";_this.vertical=!1;_this.__tabs=[];window.ResponsiveUtility.requestAvailability();return _this}/**
   * mutation observer for tabs
   * @readonly
   * @returns {object}
   */babelHelpers.createClass(A11yTabs,[{key:"connectedCallback",/**
   * life cycle, element is afixed to the DOM
   */value:function connectedCallback(){var _this3=this;babelHelpers.get(babelHelpers.getPrototypeOf(A11yTabs.prototype),"connectedCallback",this).call(this);this.updateTabs();this._breakpointChanged();this.observer.observe(this,{attributes:!1,childList:!0,subtree:!1});this.addEventListener("a11y-tab-changed",function(e){return _this3.updateTabs()})}/**
   * life cycle, element is removed from the DOM
   */},{key:"disconnectedCallback",value:function disconnectedCallback(){var _this4=this;if(this.observer&&this.observer.disconnect)this.observer.disconnect();this.removeEventListener("a11y-tab-changed",function(e){return _this4.updateTabs()});this._unsetBreakpoints();babelHelpers.get(babelHelpers.getPrototypeOf(A11yTabs.prototype),"disconnectedCallback",this).call(this)}/**
   * handle updates
   */},{key:"updated",value:function updated(changedProperties){var _this5=this;changedProperties.forEach(function(oldValue,propName){if("id"===propName)_this5._idChanged(_this5.id,oldValue);if("activeTab"===propName)_this5._activeTabChanged(_this5.activeTab,oldValue);if("iconBreakpoint"===propName)_this5._breakpointChanged();if("layoutBreakpoint"===propName)_this5._breakpointChanged();if("responsiveSize"===propName)_this5._setVertical()})}/**
   * selects a tab
   * @param {string} id the active tab's id
   */},{key:"selectTab",value:function selectTab(id){var tabs=this.querySelectorAll(this.tabQuery),filtered=Object.keys(tabs||[]).filter(function(tab){return tabs[tab].id===id}),selected=filtered[0]&&tabs[filtered[0]]?tabs[filtered[0]]:tabs[0];if(selected&&selected.id!==id){this.activeTab=selected.id;return}else if(tabs&&0<tabs.length){tabs.forEach(function(tab){tab.hidden=tab.id!==id})}}/**
   ...
   */},{key:"_activeTabChanged",value:function _activeTabChanged(newValue,oldValue){if(newValue!==oldValue)this.selectTab(newValue)}/**
   ...
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabButton",value:function _tabButton(tab){var _this6=this;return(0,_litElement.html)(_templateObject4_3aa8c9b06d2111ea950863318e7634ec(),tab.id,tab.id,tab.id===this.activeTab?"active":"",function(e){return _this6._handleTab(tab)},tab.id===this.activeTab||tab.disabled,tab.flag,this._tabIcon(tab,"flagIcon"),this._tabLabel(tab),this._tabFlag(tab),this._tabIcon(tab,"icon"),this._tabTooltip(tab))}/**
   * makes tab flag
   *
   * @param {string} flag tab's flag
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabFlag",value:function _tabFlag(tab){return(0,_litElement.html)(_templateObject5_3aa8c9b06d2111ea950863318e7634ec(),!tab.flag,tab.flag)}/**
   * makes tab icon
   *
   * @param {string} icon tab's icon
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabIcon",value:function _tabIcon(tab,icon){return(0,_litElement.html)(_templateObject6_3aa8c9b06d2111ea950863318e7634ec(),!tab[icon],tab[icon])}/**
   * makes tab label
   *
   * @param {string} flag tab's flag
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabLabel",value:function _tabLabel(tab){return(0,_litElement.html)(_templateObject7_3aa8c9b06d2111ea950863318e7634ec(),tab.label)}/**
   * makes tab tooltip
   *
   ...
   * @returns object
   * @memberof A11yTabs
   */},{key:"_tabTooltip",value:function _tabTooltip(tab){return(0,_litElement.html)(_templateObject8_3aa8c9b06d2111ea950863318e7634ec(),tab.id,tab.label)}/**
   * Fires when element is rno longer needs specific breakpoints tracked.
   *
   * @event responsive-element-deleted
   */},{key:"_unsetBreakpoints",value:function _unsetBreakpoints(){window.dispatchEvent(new CustomEvent("responsive-element-deleted",{bubbles:!0,cancelable:!0,composed:!0,detail:this}))}},{key:"observer",get:function get(){var _this7=this,callback=function callback(){return _this7.updateTabs()};return new MutationObserver(callback)}/**
   ...
   * @readonly
   * @returns {object}
   */},{key:"tabs",get:function get(){var _this8=this;return this.__tabs?Object.keys(this.__tabs).map(function(i){return _this8.__tabs[i]}):[]}/**
   * determines if tabs should show icons only
   * @readonly
   * @returns {boolean}
   */},{key:"iconClass",get:function get(){var horizontal=!this.vertical&&-1<this.responsiveSize.indexOf("s"),vertical=this.vertical&&"xs"===this.responsiveSize,breakpoints=this.iconBreakpoint>this.layoutBreakpoint&&"sm"===this.responsiveSize;return this.hasIcons&&(horizontal||vertical||breakpoints)?"icons-only":"label-and-icons"}/**
   * determines if all tabs have icons
   * @readonly
   * @returns {boolean}
   */},{key:"hasIcons",get:function get(){var _this9=this,hasIcons=!0;if(!this.id)this.id=this._generateUUID();if(this.__tabs&&0<this.__tabs.length)this.__tabs.forEach(function(tab,index){if(!tab.icon)hasIcons=!1;tab.order=index+1;tab.total=_this9.__tabs.length});return hasIcons}}]);return A11yTabs}(_litElement.LitElement);_exports.A11yTabs=A11yTabs;window.customElements.define(A11yTabs.tag,A11yTabs)});