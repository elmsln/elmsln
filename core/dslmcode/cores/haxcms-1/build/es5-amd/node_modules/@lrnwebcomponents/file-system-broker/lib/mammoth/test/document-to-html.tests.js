...
test("should convert document containing multiple paragraphs to multiple p elements", function () {
  var document = new documents.Document([paragraphOfText("Hello."), paragraphOfText("Goodbye.")]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(document).then(function (result) {
...
test("uses style mappings to pick HTML element for docx paragraph", function () {
  var document = new documents.Document([paragraphOfText("Hello.", "Heading1", "Heading 1")]);
  var converter = new DocumentConverter({
    styleMap: [{
...
test("bold runs can exist inside other tags", function () {
  var run = new documents.Paragraph([runOfText("Hello.", {
    isBold: true
  })]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(run).then(function (result) {
...
test("docx hyperlink can be collapsed", function () {
  var hyperlink = new documents.Document([new documents.Hyperlink([runOfText("Hello ")], {
    href: "http://www.example.com"
...
test("bookmarks are converted to anchors", function () {
  var bookmarkStart = new documents.BookmarkStart({
    name: "_Peter"
  });
  var converter = new DocumentConverter({
    idPrefix: "doc-42-"
...
test("docx table is converted to table in HTML", function () {
  var table = new documents.Table([new documents.TableRow([new documents.TableCell([paragraphOfText("Top left")]), new documents.TableCell([paragraphOfText("Top right")])]), new documents.TableRow([new documents.TableCell([paragraphOfText("Bottom left")]), new documents.TableCell([paragraphOfText("Bottom right")])])]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml = "<table>" + "<tr><td><p>Top left</p></td><td><p>Top right</p></td></tr>" + "<tr><td><p>Bottom left</p></td><td><p>Bottom right</p></td></tr>" + "</table>";
...
test("table style mappings can be used to map tables", function () {
  ...
  var converter = new DocumentConverter({
    styleMap: [{
      ...
      to: htmlPaths.topLevelElement("table", {
        class: "normal-table"
      })
    }]
  });
...
test("header rows are wrapped in thead", function () {
  var table = new documents.Table([new documents.TableRow([new documents.TableCell([])], {
    isHeader: true
  }), new documents.TableRow([new documents.TableCell([])], {
    isHeader: true
  }), new documents.TableRow([new documents.TableCell([])], {
    isHeader: false
  })]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml = "<table>" + "<thead><tr><th></th></tr><tr><th></th></tr></thead>" + "<tbody><tr><td></td></tr></tbody>" + "</table>";
    assert.equal(result.value, expectedHtml);
  });
});
...
test("empty cells are preserved in table", function () {
  var table = new documents.Table([new documents.TableRow([new documents.TableCell([paragraphOfText("")]), new documents.TableCell([paragraphOfText("Top right")])])]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml = "<table>" + "<tr><td></td><td><p>Top right</p></td></tr>" + "</table>";
    assert.equal(result.value, expectedHtml);
  });
});
test("empty rows are preserved in table", function () {
  var table = new documents.Table([new documents.TableRow([new documents.TableCell([paragraphOfText("Row 1")])]), new documents.TableRow([])]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
...
test("table cells are written with colSpan if not equal to one", function () {
  var table = new documents.Table([new documents.TableRow([new documents.TableCell([paragraphOfText("Top left")], {
    colSpan: 2
  }), new documents.TableCell([paragraphOfText("Top right")])])]);
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml = "<table>" + '<tr><td colspan="2"><p>Top left</p></td><td><p>Top right</p></td></tr>' + "</table>";
    assert.equal(result.value, expectedHtml);
  });
});
test("table cells are written with rowSpan if not equal to one", function () {
  ...
  var converter = new DocumentConverter();
  return converter.convertToHtml(table).then(function (result) {
    var expectedHtml = "<table>" + '<tr><td rowspan="2"></td></tr>' + "</table>";
    assert.equal(result.value, expectedHtml);
  });
});
...
test("footnotes are included after the main body", function () {
  ...
  var document = new documents.Document([new documents.Paragraph([runOfText("Knock knock"), new documents.Run([footnoteReference])])], {
  ...
  return converter.convertToHtml(document).then(function (result) {
    var expectedOutput = '<p>Knock knock<sup><a href="#doc-42-footnote-4" id="doc-42-footnote-ref-4">[1]</a></sup></p>' + '<ol><li id="doc-42-footnote-4"><p>Who\'s there? <a href="#doc-42-footnote-ref-4">↑</a></p></li></ol>';
    assert.equal(result.value, expectedOutput);
  });
});
test("comments are ignored by default", function () {
  var reference = documents.commentReference({
    commentId: "4"
  });
  ...
  var document = documents.document([documents.paragraph([runOfText("Knock knock"), documents.run([reference])])], {
    comments: [comment]
  });
  var converter = new DocumentConverter({});
  return converter.convertToHtml(document).then(function (result) {
...
test("comment references are linked to comment after main body", function () {
  ...
  var converter = new DocumentConverter({
    idPrefix: "doc-42-",
    styleMap: [{
      from: documentMatchers.commentReference,
      to: htmlPaths.element("sup")
    }]
  });
  return converter.convertToHtml(document).then(function (result) {
    var expectedHtml = '<p>Knock knock<sup><a href="#doc-42-comment-4" id="doc-42-comment-ref-4">[TP1]</a></sup></p>' + '<dl><dt id="doc-42-comment-4">Comment [TP1]</dt><dd><p>Who\'s there? <a href="#doc-42-comment-ref-4">↑</a></p></dd></dl>';
    assert.equal(result.value, expectedHtml);
    assert.deepEqual(result.messages, []);
  });
});
test("images are written with data URIs", function () {
  ...
  var image = new documents.Image({
    readImage: function readImage(encoding) {
...
test("can add custom handler for images", function () {
  ...
  var converter = new DocumentConverter({
    convertImage: function convertImage(element, messages) {
...
test("when initials are not blank then comment author label is initials", function () {
  assert.equal(commentAuthorLabel({
    authorInitials: "TP"
  }), "TP");
});
