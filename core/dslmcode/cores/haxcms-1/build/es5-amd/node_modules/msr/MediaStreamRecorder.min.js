...
function MultiStreamRecorder(mediaStream) {
  if (!mediaStream) throw "MediaStream is mandatory.";
  var self = this,
      isMediaRecorder = isMediaRecorderCompatible();
  this.stream = mediaStream, this.start = function (timeSlice) {
    ...
    for (var prop in this) {
      "function" != typeof this[prop] && (audioRecorder[prop] = videoRecorder[prop] = this[prop]);
    }
...
function StereoAudioRecorderHelper(mediaStream, root) {
  function interleave(leftChannel, rightChannel) {
    for (var length = leftChannel.length + rightChannel.length, result = new Float32Array(length), inputIndex = 0, index = 0; length > index;) {
  ...
  function writeUTFBytes(view, offset, string) {
    for (var lng = string.length, i = 0; lng > i; i++) {
  ...
  this.record = function () {
    ...
    if (!isPaused) {
      ...
      for (var lng = interleaved.length, index = 44, volume = 1, i = 0; lng > i; i++) {
...
function WhammyRecorderHelper(mediaStream, root) {
  ...
  function isBlankFrame(frame, _pixTolerance, _frameTolerance) {
    ...
    var matchPixCount,
      ...
      b: 0
        ...
        pixTolerance = _pixTolerance && _pixTolerance >= 0 && 1 >= _pixTolerance ? _pixTolerance : 0,
        frameTolerance = _frameTolerance && _frameTolerance >= 0 && 1 >= _frameTolerance ? _frameTolerance : 0,
        image = new Image();
  ...
  this.record = function (timeSlice) {
    this.width || (this.width = 320), this.height || (this.height = 240), this.video && babelHelpers.instanceof(this.video, HTMLVideoElement) && (this.width || (this.width = video.videoWidth || video.clientWidth || 320), this.height || (this.height = video.videoHeight || video.clientHeight || 240)), this.video || (this.video = {
      width: this.width,
      height: this.height
    }), this.canvas && this.canvas.width && this.canvas.height || (this.canvas = {
      width: this.width,
      height: this.height
    }), canvas.width = this.canvas.width, canvas.height = this.canvas.height, this.video && babelHelpers.instanceof(this.video, HTMLVideoElement) ? (this.isHTMLObject = !0, video = this.video.cloneNode()) : (video = document.createElement("video"), video.src = URL.createObjectURL(mediaStream), video.width = this.video.width, video.height = this.video.height), video.muted = !0, video.play(), lastTime = new Date().getTime(), whammy = new Whammy.Video(root.speed, root.quality), console.log("canvas resolutions", canvas.width, "*", canvas.height), console.log("video width/height", video.width || canvas.width, "*", video.height || canvas.height), drawFrames();
  }, this.clearOldRecordedFrames = function () {
    whammy.frames = [];
  };
  var requestDataInvoked = !1;
...
function GifRecorder(mediaStream) {
  ...
  this.start = function (timeSlice) {
    function drawVideoFrame(time) {
      return isPaused ? void setTimeout(drawVideoFrame, 500, time) : (lastAnimationFrame = requestAnimationFrame(drawVideoFrame), void 0 === babelHelpers.typeof(lastFrameTime) && (lastFrameTime = time), void (90 > time - lastFrameTime || (video.paused && video.play(), context.drawImage(video, 0, 0, imageWidth, imageHeight), gifEncoder.addFrame(context), lastFrameTime = time)));
    }
  ...
  function whammyInWebWorker(frames) {
    function ArrayToWebM(frames) {
      ...
      for (var clusterMaxDuration = 3e4, EBML = [{
        ...
        } while (frameNumber < frames.length && clusterMaxDuration > clusterDuration);
    ...
    function generateEBML(json) {
      for (var ebml = [], i = 0; i < json.length; i++) {
        ...
        "object" == babelHelpers.typeof(data) && (data = generateEBML(data)), "number" == typeof data && (data = bitsToBuffer(data.toString(2))), "string" == typeof data && (data = strToBuffer(data));
    ...
    function parseWebP(riff) {
      for (var VP8 = riff.RIFF[0].WEBP[0], frameStart = VP8.indexOf("*"), i = 0, c = []; 4 > i; i++) {
