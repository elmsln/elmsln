import{html as e,PolymerElement as t}from"../../../@polymer/polymer/polymer-element.js";import"../../../@polymer/iron-ajax/iron-ajax.js";import"../../../@polymer/iron-form-element-behavior/iron-form-element-behavior.js";import"../../../@polymer/app-layout/app-layout.js";import"../../../@polymer/app-layout/app-toolbar/app-toolbar.js";import"../../simple-toast/simple-toast.js";import"../../simple-modal/simple-modal.js";import"../../../@polymer/paper-input/paper-input.js";import"../../../@polymer/paper-listbox/paper-listbox.js";import"../../lrnsys-button/lrnsys-button.js";import"../../grafitto-filter/grafitto-filter.js";import"../../simple-fields/lib/simple-fields-container.js";import"../lrnsys-comment.js";class LrnsysCommentList extends t{static get template(){return e`
    ...
    `}static get tag(){return"lrnsys-comment-list"}static get properties(){return{csrfToken:{type:String},opsRequestMethod:{type:Object,value:{list:"GET",create:"POST",update:"PUT",delete:"DELETE",like:"PATCH"}},activeComment:{type:Object,notify:!0},newComment:{type:Object,notify:!0},comments:{type:Object,notify:!0},sourcePath:{type:String,notify:!0},commentOpsBase:{type:String,notify:!0},createStubUrl:{type:String,notify:!0},reqUrl:{type:String,notify:!0,computed:"_computeCommentOpsUrl(activeComment, commentOpsBase, csrfToken)"}}}connectedCallback(){super.connectedCallback(),this.shadowRoot.querySelector("#filtercomments").addEventListener("value-changed",(e=>{this.shadowRoot.querySelector("#filteredcomments").like=e.target.value}))}disconnectedCallback(){this.shadowRoot.querySelector("#filtercomments").removeEventListener("value-changed",(e=>{this.shadowRoot.querySelector("#filteredcomments").like=e.target.value})),super.disconnectedCallback()}_computeCommentOpsUrl(e,t,o){if(void 0!==e)return t+"/"+e.id+"?token="+o}handleLike(e){this.activeComment=e.detail.comment,this.shadowRoot.querySelector("#ajaxlikerequest").generateRequest()}constructor(){super(),window.SimpleModal.requestAvailability(),setTimeout((()=>{this.addEventListener("comment-save",this.handleSave.bind(this)),this.addEventListener("comment-editing",this.handleEditing.bind(this)),this.addEventListener("comment-reply",this.handleReply.bind(this)),this.addEventListener("comment-like",this.handleLike.bind(this)),this.addEventListener("comment-delete-dialog",this.handleDeleteDialog.bind(this))}),0)}_handleLikeResponse(e){}handleDeleteDialog(e){this.activeComment=e.detail.comment;let t=document.createElement("p"),o=document.createTextNode("Are you sure you want to delete your comment? This cannot be undone.");t.appendChild(o);let n=document.createElement("div");n.classList.add("buttons");let s=document.createElement("button");s.setAttribute("dialog-dismiss","dialog-dismiss"),s.style.padding="16px",s.style.margin="16px",o=document.createTextNode("Keep comment"),s.appendChild(o);let a=document.createElement("button");a.setAttribute("dialog-confirm","dialog-confirm"),a.setAttribute("autofocus","autofocus"),a.addEventListener("click",this._handleDeleteConfirm.bind(this)),a.style.color="white",a.style.backgroundColor="red",a.style.padding="16px",a.style.margin="16px",o=document.createTextNode("Delete the comment"),a.appendChild(o),n.appendChild(a),n.appendChild(s);const i=new CustomEvent("simple-modal-show",{bubbles:!0,composed:!0,cancelable:!0,detail:{title:"Delete comment",elements:{content:t,buttons:n},styles:{"--simple-modal-width":"15vw","--simple-modal-max-width":"15vw","--simple-modal-z-index":"100000000","--simple-modal-min-height":"10vh"},invokedBy:e.detail.target,clone:!1}});this.dispatchEvent(i)}handleEditing(e){const t=new CustomEvent("simple-toast-show",{bubbles:!0,composed:!0,cancelable:!0,detail:{text:"Be awesome to each other",duration:4e3}});this.dispatchEvent(t)}handleTopReply(e){this.set("newComment",[]),this.set("activeComment",[]),this.shadowRoot.querySelector("#ajaxcreatestub").generateRequest()}handleReply(e){this.set("newComment",[]),this.activeComment=e.detail.comment,this.shadowRoot.querySelector("#ajaxcreatestub").generateRequest()}_updateReply(e){var t=this.activeComment,o=this.comments.data;if(this.newComment=this.newComment.data,0==o.length)o.push(this.newComment);else if(void 0===t.id)o.push(this.newComment);else for(var n=0;n<o.length;n++)o[n].id==t.id&&o.splice(n+1,0,this.newComment);this.activeComment=this.newComment,this.set("comments.data",[]),this.set("comments.data",o),this.notifyPath("comments.data")}_handleDeleteConfirm(e){this.shadowRoot.querySelector("#ajaxdeleterequest").generateRequest()}_handleDeleteResponse(e){for(var t=this.activeComment,o=this.comments.data,n=0;n<o.length;n++)if(o[n].id==t.id){o.splice(n,1),this.set("activeComment",[]),this.set("comments.data",[]),this.set("comments.data",o),this.notifyPath("comments.data");const e=new CustomEvent("simple-toast-show",{bubbles:!0,composed:!0,cancelable:!0,detail:{text:"Comment deleted",duration:4e3}});return this.dispatchEvent(e),!0}}handleSave(e){this.activeComment=e.detail.comment,this.shadowRoot.querySelector("#ajaxupdaterequest").generateRequest()}_handleUpdateResponse(e){const t=new CustomEvent("simple-toast-show",{bubbles:!0,composed:!0,cancelable:!0,detail:{text:"Comment saved!",duration:4e3}});this.dispatchEvent(t)}_toArray(e){return null==e?[]:Object.keys(e).map((function(t){return e[t]}))}}window.customElements.define(LrnsysCommentList.tag,LrnsysCommentList);export{LrnsysCommentList};