import{LitElement,html,css}from"../../lit-element/lit-element.js";class WordCount extends LitElement{static get styles(){return[css`
        :host {
          display: block;
        }
        :host:after {
    ...
    `}static get tag(){return"word-count"}constructor(){super(),this.wordsPrefix="Word count:"}connectedCallback(){super.connectedCallback(),this.__observer=new MutationObserver(this._updateWords.bind(this)),this.__observer.observe(this,{attributes:!0,characterData:!0,childList:!0,subtree:!0})}disconnectedCallback(){this.__observer.disconnect(),super.disconnectedCallback()}update(changedProperties){super.update(changedProperties),(changedProperties.has("wordsPrefix")||changedProperties.has("words"))&&this.setAttribute("words-text",`${this.wordsPrefix} ${this.words}`)}static get properties(){return{words:{type:Number},wordsPrefix:{type:String}}}_updateWords(mutations){""!==this.textContent?this.words=parseInt(this.textContent.split(/\s+/g).length-1):this.words=0}}window.customElements.define(WordCount.tag,WordCount);export{WordCount};