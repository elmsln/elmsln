import{LitElement as t,html as e,css as a}from"../../lit/index.js";import"../../@github/time-elements/dist/index.js";import"./lib/threaded-discussion-form.js";class ThreadedDiscussion extends t{static get styles(){return[a`
        :host {
          display: block;
        ...
        <div class="comment-header">
          ...
          <div>
            ...
            <p class="comment-date">
              ${this.relativeDates?e`
                    <relative-time .datetime="${t.date}">
                      ${this._getDate(t.date)}
                    </relative-time>
                  `:e`
                    <local-time
                      month="${this.dateFormat&&this.dateFormat.month?this.dateFormat.month:"long"}"
                      day="${this.dateFormat&&this.dateFormat.day?this.dateFormat.day:"numeric"}"
                      year="${this.dateFormat&&this.dateFormat.year?this.dateFormat.year:"numeric"}"
                      hour="${this.dateFormat&&this.dateFormat.hour?this.dateFormat.hour:"2-digit"}"
                      minute="${this.dateFormat&&this.dateFormat.minute?this.dateFormat.minute:"2-digit"}"
                      second="${this.dateFormat&&this.dateFormat.second?this.dateFormat.second:"2-digit"}"
    ...
    `}static get tag(){return"threaded-discussion"}static get properties(){return{latest:{type:Boolean,attribute:"latest",reflect:!0},commentButtonLabel:{type:String,attribute:"comment-button-label"},commentTextareaLabel:{type:String,attribute:"comment-textarea-label"},commentIcon:{type:String,attribute:"comment-icon"},data:{type:Object},dateLocale:{type:String},dateFormat:{type:Object},demo:{type:Boolean},loadingText:{type:String,attribute:"loading-text"},map:{type:Object},params:{type:Object},relativeDates:{attribute:"relative-dates",type:Boolean,reflect:!0},replyButtonLabel:{type:String,attribute:"reply-button-label",reflect:!0},replyTextareaLabel:{type:String,attribute:"reply-textarea-label",reflect:!0},replyIcon:{type:String,attribute:"reply-icon",reflect:!0},source:{type:String},submit:{type:String},__data:{type:Array},__loading:{type:Boolean}}}constructor(){super(),this.latest=!1,this.data=[],this.dateLocale="en-US",this.dateFormat={year:"numeric",month:"long",day:"numeric",hour:"numeric",minute:"2-digit"},this.demo=!1,this.map={},this.params={},this.submit=window.location.href,this.__data=[],this.__loading=!1,setTimeout((()=>{import("../lrndesign-avatar/lrndesign-avatar.js")}),0)}updated(t){super.updated&&super.updated(t),t.forEach(((t,e)=>{["params","source"].includes(e)&&this.source&&(this.__loading=!0,this.fetchDiscussion())}))}sortData(t){return t.sort(((t,e)=>{let a="string"==typeof t.date?Date.parse(t.date):t.date,i="string"==typeof e.date?Date.parse(e.date):e.date;return this.latest?i-a:a-i}))}_handleRawData(t){return this.__loading=!1,console.log(t),this._getArray(t||[]).filter((t=>!this._getMap(t,"thread","replyThread"))).map((e=>{let a=this._getMap(e,"id"),i=this._getMap(e,"replies")||(t||[]).filter((t=>this._getMap(t,"thread","replyThread")===a));return{id:a,firstName:this._getMap(e,"firstName"),lastName:this._getMap(e,"lastName"),avatar:this._getMap(e,"avatar"),body:this._getMap(e,"body"),color:this._getMap(e,"color"),date:this._getMap(e,"date"),replies:this._getArray(i||[]).map((t=>({id:this._getMap(t,"id","replyId"),thread:this._getMap(t,"thread","replyThread")||a,firstName:this._getMap(t,"firstName","replyFirstName"),lastName:this._getMap(t,"lastName","replyLastName"),avatar:this._getMap(t,"avatar","replyAvatar"),body:this._getMap(t,"body","replyBody"),color:this._getMap(t,"color","replyColor"),date:this._getMap(t,"date","replyDate")})))}}))}_getMap(t,e,a,i){return t[this._mapProp(e,a,i)]}_mapProp(t,e,a){let i=this.map||{};return e=e||t,i[a=a||e]||i[e]||t}_handleSubmit(t){this.fetchDiscussion()}_handleDemo(t){if(t.detail&&t.detail.textarea){let e={},a=this.__data;if(e.id=`comment-${Date.now()}`,e.firstName="DEMO",e.lastName="USER",e.date=this._getDate(new Date),e.body=t.detail.textarea.value,t.detail.thread){let i=a.filter((e=>e.id===t.detail.thread)),s=i?i[0]:void 0;e.thread=t.detail.thread,s?s.replies=[...s.replies,e]:a.push(e)}else e.replies=[],a.push(e);this.__data=[],this.__data=a,t.detail.textarea.value=""}}async fetchDiscussion(){await fetch(this._getPath(this.source,this.params)).then((t=>t.json())).then((t=>this.__data=this._handleRawData(t)))}_getArray(t){return"array"==typeof t?t:Object.keys(t||{}).map((e=>{let a=t[e];return a.id=e,a}))}_getDate(t){let e="string"==typeof this.dateFormat?JSON.parse(this.dateFormat):this.dateFormat;return t&&new Date(t)?new Date(t).toLocaleString(this.dateLocale,e):""}_getThreadParams(t){let e=this.params;return e[this._mapProp("id")]=t,e}_getPath(t,e){let a=Object.keys(e||{}).map((t=>`${encodeURI(t)}=${encodeURI(e[t])}`)).join("&");return a?`${t}?${a}`:t}}window.customElements.define(ThreadedDiscussion.tag,ThreadedDiscussion);export{ThreadedDiscussion};