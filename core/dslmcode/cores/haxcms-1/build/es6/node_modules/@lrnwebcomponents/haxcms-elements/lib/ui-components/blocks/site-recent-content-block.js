  ...
  _activeIdChanged(newValue) {
    if (newValue) {
      ...
      if (this.shadowRoot.querySelector('[data-id="' + newValue + '"]')) {
        ...
        let tmpItem = store.manifest.items.find(i => i.id == newValue); // fallback, maybe there's a child of this currently active

        while (el === null && tmpItem && tmpItem.parent != null) {
          // take the parent object of this current item
          tmpItem = store.manifest.items.find(i => i.id == tmpItem.parent); // see if IT lives in the dom, if not, keep going until we run out
