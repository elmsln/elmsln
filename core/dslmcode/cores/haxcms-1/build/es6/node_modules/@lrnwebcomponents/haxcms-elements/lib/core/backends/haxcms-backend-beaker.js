...
class HAXCMSBackendBeaker extends LitElement {
  ...
  _appPicked(e) {
    if (e.detail.connection.protocol === "dat") {
      ...
      reader.onload = event => {
        let fileLocation = "files/" + window.HaxStore.instance.haxManager.shadowRoot.querySelector("#fileupload").files[0].name;
        this.shadowRoot.querySelector("#beaker").write(fileLocation, event.target.result);
        window.HaxStore.instance.haxManager.shadowRoot.querySelector("#url").value = fileLocation;
        window.HaxStore.instance.haxManager.newAssetConfigure();
      };

      reader.readAsArrayBuffer(window.HaxStore.instance.haxManager.shadowRoot.querySelector("#fileupload").files[0]);
    }
  }
  /**
   * Save page data
  ...
  async saveNode(e) {
    ...
    store.cmsSiteEditor.instance.shadowRoot.querySelector("#toast").show("Page updated!");
  ...
  async saveOutline(e) {
    ...
    this.manifest = store.cmsSiteEditor.instance.manifest; // set items specifically since it's just an outline update
    ...
    store.cmsSiteEditor.instance.shadowRoot.querySelector("#toast").show("Outline saved!");
    ...
    store.cmsSiteEditor.instance.dispatchEvent(new CustomEvent("json-outline-schema-changed", {
      bubbles: true,
      composed: true,
      cancelable: false,
  ...
  async deleteNode(e) {
    ...
    store.cmsSiteEditor.instance.shadowRoot.querySelector("#toast").show(`${page.title} deleted`);
    store.cmsSiteEditor.instance.dispatchEvent(new CustomEvent("haxcms-trigger-update", {
      bubbles: true,
      composed: true,
      cancelable: false,
  ...
  async createNode(e) {
    ...
    this.manifest.items.forEach((element, index) => {
      ...
      if (typeof element.location === typeof undefined) {
        ...
        this.shadowRoot.querySelector("#beaker").write(page.location, "<p>My great new content!</p>");
        this.manifest.items[index] = element;
      }
    });
    this.shadowRoot.querySelector("#beaker").write("site.json", JSON.stringify(this.manifest, null, 2)); // simulate save events since they wont fire

    store.cmsSiteEditor.instance.shadowRoot.querySelector("#toast").show(`${page.title} created!`);
    store.cmsSiteEditor.instance.dispatchEvent(new CustomEvent("haxcms-trigger-update", {
      bubbles: true,
      composed: true,
      cancelable: false,
  ...
  async saveManifest(e) {
    ...
    store.cmsSiteEditor.instance.shadowRoot.querySelector("#toast").show("Site details saved!");
    store.cmsSiteEditor.instance.dispatchEvent(new CustomEvent("haxcms-trigger-update", {
      bubbles: true,
      composed: true,
      cancelable: false,
