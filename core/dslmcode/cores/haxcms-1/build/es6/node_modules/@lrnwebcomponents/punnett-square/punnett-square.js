...
import{LitElement as t,html as e,css as a}from"../../lit-element/lit-element.js";import{unsafeHTML as r}from"../../lit-html/directives/unsafe-html.js";import"../../@polymer/paper-input/paper-input.js";class PunnettSquare extends t{static get tag(){return"punnett-square"}static get properties(){return{variant1:{type:String},variant2:{type:String},editable:{type:Boolean},__results:{type:Array}}}constructor(){super(),this.variant1="",this.variant2="",this.editable=!1;const t=this.querySelectorAll("[data-variant]");let e=[];for(let a of t){let t=a.getAttribute("data-variant");t=this.__sortVariant(t);let r={variant:t,ref:a};e=[...e,r]}this.__results=e}__sortVariant(t){return t.split("").sort().join("")}__ensureArray(t){return("string"==typeof t?t.split(",").map(t=>t.trim()):[]).map(t=>"string"==typeof t?t.split("").map(t=>t.trim()):[])}render(){let t=this.__ensureArray(this.variant1),a=this.__ensureArray(this.variant2);const r=t[0].length<1&&a[0].length<1;return console.log("asdf:",r),e`
    ...
    `}renderVariant(t){const a=this.__sortVariant(t),n=this.__results.find(t=>t.variant===a);return n?e`
        ${r(n.ref.outerHTML)}
      `:e`
        ${a}
