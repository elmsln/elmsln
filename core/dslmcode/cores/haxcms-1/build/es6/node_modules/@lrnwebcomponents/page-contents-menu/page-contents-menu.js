...
import{LitElement as e,html as t,css as i}from"../../lit-element/lit-element.js";class PageContentsMenu extends e{static get styles(){return[i`
    ...
    `}keyToggle(e){["Enter","Space"].includes(e.key)&&this.toggleSettings(e)}keyScroll(e){"Enter"==e.key&&this.scrollToObject(e)}toggleSettings(e){this.mobile&&(this.hideSettings=!this.hideSettings)}scrollToObject(e){e.path&&e.path[0]&&e.path[0].getAttribute("data-index")&&(e.preventDefault(),e.stopPropagation(),e.stopImmediatePropagation(),this.items[parseInt(e.path[0].getAttribute("data-index"))].object.scrollIntoView({behavior:"smooth",block:"start",inline:"nearest"}),this.hideSettings=!0)}renderItem(e,i){return null==e.link&&e.object?t`
        <li class="item">
          <a
