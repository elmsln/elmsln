 ...
 */
import{LitElement as e,html as s,css as t}from"../../../lit-element/lit-element.js";import{LrndesignGalleryBehaviors as i}from"./lrndesign-gallery-behaviors.js";import"./lrndesign-gallery-zoom.js";import"./lrndesign-gallery-details.js";class LrndesignGalleryMasonry extends i{static get tag(){return"lrndesign-gallery-masonry"}static get styles(){return[...super.styles,t`
        :host {
          margin: 15px 0;
          padding: 0;
          max-width: 100%;
    ...
    `}updated(e){super.updated(e),e.forEach((e,s)=>{"sources"===s&&this._updateItems(),"responsiveSize"===s&&this._updateItems()})}_getStyle(e,s){return[`--lrndesign-gallery-image-aspect:${e||100/75};`,`--lrndesign-gallery-image-height:${100/e||75}%;`,`--lrndesign-gallery-image-width:${100*e/s||.75}%`].join("")}_handleImgLoad(e){(e.path&&e.path[0]?e.path[0]:e.originalTarget?e.originalTarget:e.target).naturalWidth>0&&this._updateItems()}_updateItems(){let e=[],s={xs:2,sm:3,md:4,lg:5,xl:6}[this.responsiveSize||"xs"],t=Math.ceil(this.sources.length/s);for(let i=0;i<t;i++){let t={aspect:0,cols:[]};console.log("row",i);for(let e=i*s;e<Math.min(this.sources.length,(i+1)*s);e++){let s=JSON.parse(JSON.stringify(this.sources[e])),i=this.shadowRoot.querySelector(`img[src="${s.src}"]`);s.aspect=i&&i.naturalWidth>0?i.naturalWidth/i.naturalHeight:100/75,t.aspect+=s.aspect,t.cols.push(s)}e.push(t)}this.__items=e,console.log("this.__items",this.__items)}}window.customElements.define(LrndesignGalleryMasonry.tag,LrndesignGalleryMasonry);export{LrndesignGalleryMasonry};