 ...
 */
import{html,PolymerElement}from"../../@polymer/polymer/polymer-element.js";import"../../@polymer/paper-tooltip/paper-tooltip.js";import"../../@polymer/paper-input/paper-input.js";import"../../@polymer/iron-ajax/iron-ajax.js";import{displayBehaviors}from"./lib/editable-table-behaviors.js";import"./lib/editable-table-editor-rowcol.js";import"./lib/editable-table-editor-toggle.js";import"./lib/editable-table-editor-cell.js";import"./lib/editable-table-styles.js";import"./lib/editable-table-display.js";class EditableTable extends(displayBehaviors(PolymerElement)){static get template(){return html`
    ...
    `}static get tag(){return"editable-table"}static get properties(){return{editMode:{type:Boolean,value:!1},hideBordered:{type:Boolean,value:!1},hideCondensed:{type:Boolean,value:!1},hideDisplay:{type:Boolean,computed:"_tableDisplayHidden(hideBordered,hideCondensed,hideStriped,hideResponsive)"},hideFilter:{type:Boolean,value:!1},hideSortFilter:{type:Boolean,computed:"_tableSortHidden(hideSort,hideFilter)"},hideSort:{type:Boolean,value:!1},hideResponsive:{type:Boolean,value:!1},hideStriped:{type:Boolean,value:!1}}}deleteColumn(index){for(let i=0;i<this.data.length;i++)this.splice("data."+i,index,1);let temp=this.data.slice();this.set("data",temp)}deleteRow(index){this.splice("data",index,1);let temp=this.data.slice();this.set("data",temp)}insertColumn(index){let temp=this.data.slice();for(let i=0;i<temp.length;i++)temp[i].splice(index,0,"");this.set("data",temp)}insertRow(index){let temp=this.data.slice(),temp2=new Array;for(let i=0;i<temp[0].length;i++)temp2.push("");temp.splice(index+1,0,temp2),this.set("data",temp)}toggleEditMode(edit){edit=void 0!==edit?edit:!this.editMode,this.dispatchEvent(new CustomEvent("toggle-edit-mode",{bubbles:!0,cancelable:!0,composed:!0,detail:this})),edit&&(this.shadowRoot.querySelector("editable-table-display").toggleFilter(),this.shadowRoot.querySelector("editable-table-display").sortData("none",-1),this.shadowRoot.querySelector("#inner").focus()),this.editMode=edit}_captionChanged(){this.caption=this.shadowRoot.querySelector("#caption").value}_dataChanged(newValue,oldValue){if(!newValue||newValue.length<1||newValue[0].length<1){let table=this.children.item(0);null!=table&&"TABLE"===table.tagName?this.importHTML(table):this.set("data",[["","",""],["","",""],["","",""]])}}_getCurrentRow(index,data){let row=null;return null!=data&&void 0!==data[index]&&null!==data[index]&&(row=data[index]),row}_handleRowColumnMenu(e){e.detail.insert&&e.detail.row?this.insertRow(e.detail.index):e.detail.insert&&!e.detail.row?this.insertColumn(e.detail.index):!e.detail.insert&&e.detail.row?this.deleteRow(e.detail.index):this.deleteColumn(e.detail.index)}_isFirstRow(index){return 0===index}_isSortDisabled(hideSort,columnHeader){return hideSort||!columnHeader}_onCellClick(e){e.model&&e.model.root&&e.model.root.nodeList[0]&&e.model.root.nodeList[0].focus()}_onCellValueChange(e){let temp=this.data.slice();temp[e.detail.row][e.detail.column]=e.detail.value,this.set("data",[]),this.set("data",temp)}_onTableSettingChange(e){this[e.detail.id]=e.detail.toggled}_setMinimumData(data){(data.length<1||data[0].length<1)&&this.set("data",[["","",""],["","",""],["","",""]])}_tableDisplayHidden(hideBordered,hideCondensed,hideStriped,hideResponsive){return hideBordered&&hideCondensed&&hideStriped&&hideResponsive}_tableSortHidden(hideSort,hideFilter){return hideSort&&hideFilter}}window.customElements.define(EditableTable.tag,EditableTable);export{EditableTable};