import{store as t}from"../../core/haxcms-site-store.js";import{autorun as e,toJS as i}from"../../../../../mobx/dist/mobx.esm.js";import"../query/site-query.js";import{LitElement as s,css as r,html as a}from"../../../../../lit/index.js";class SiteRecentContentBlock extends s{constructor(){super(),this.limit=10,this.startIndex=0,this.sort={"metadata.created":"ASC"},this.conditions={},this.title="Recent content",this.__disposer=e(()=>{const e=i(t.activeId);Promise.resolve().then(()=>{this.activeId=e})})}updated(t){super.updated&&super.updated(t),t.has("activeId")&&this._activeIdChanged(this.activeId)}static get tag(){return"site-recent-content-block"}static get styles(){return[r`
        :host {
          display: block;
      ...
      `]}__resultChanged(t){t.detail&&t.detail.value&&(this.__items=[...t.detail.value])}render(){return a`
      <aside>
        <div class="header">
          <h3>${title}</h3>
        </div>
