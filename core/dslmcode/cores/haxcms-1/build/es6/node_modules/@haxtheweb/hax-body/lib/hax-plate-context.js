import{LitElement as e,css as t,html as i}from"../../../lit/index.js";import"./hax-toolbar.js";import"../../simple-toolbar/lib/simple-toolbar-menu-item.js";import{HAXStore as o}from"./hax-store.js";import"./hax-toolbar-menu.js";import"./hax-toolbar.js";import"./hax-context-item.js";import{wipeSlot as a}from"../../utils/lib/slot.js";import{autorun as l,toJS as s}from"../../../mobx/dist/mobx.esm.js";import{HaxContextBehaviors as n}from"./hax-context-behaviors.js";import{normalizeEventPath as r}from"../../utils/lib/events.js";import{I18NMixin as c}from"../../i18n-manager/lib/I18NMixin.js";class HaxPlateContext extends(c(n(e))){constructor(){super(),this.disableOps=!1,this.disableItemOps=!1,this.insertAbove=!0,this.disableDuplicate=!1,this.hasActiveEditingElement=!1,this.haxUIElement=!0,this.t={edit:"Edit",dragHandle:"Drag handle",moveUp:"Move up",moveDown:"Move down",addColumn:"Add column",removeColumn:"Remove column",remove:"Remove",duplicate:"Duplicate",confirmDelete:"Confirm delete",changeTo:"Change to",editElement:"Edit Mode",modifyHTMLSource:"Modify HTML source",regions:"Available regions",insertItemAbove:"Insert item above",insertItemAboveOrBelow:"Insert item above or below",insertItemBelow:"Insert item below",selectLayout:"Select Layout Element",lockContent:"Lock content",unlockContent:"Unlock content"},this.registerLocalization({context:this,namespace:"hax"}),this.ceButtons=[],this.activeTagName="",this.activeTagIcon="hax:paragraph",this.addEventListener("hax-context-item-selected",this.handleCECustomEvent.bind(this))}static get tag(){return"hax-plate-context"}static get styles(){return[super.styles,t`
      ...
      `]}render(){return i`
      <hax-toolbar always-expanded ?hidden=${o.isPlatformAudience("novice")&&o.isTextElement(this.activeNode)}>
        <div class="group">
          <hax-toolbar-menu
          ...
          >
            ...
            ${this.activeNode&&this.activeNode.parentNode&&o.isLayoutElement(this.activeNode.parentNode)?(this.slottedItems||[]).map((e,t)=>i`
                      <simple-toolbar-menu-item
                        slot="menuitem"
          ...
          <hax-toolbar-menu
            ...
            ?hidden="${o.isPlatformAudience("novice")}"
          ...
          >
            ...
            ${(this.slottedItems||[]).map((e,t)=>(e.items||[]).map((o,a)=>i`
                  <simple-toolbar-menu-item
                    slot="menuitem"
                  ...
                  >
                    <hax-context-item
                      ...
                      .eventData="${{target:o,grid:e.grid,slot:e.slot,index:a,editMode:e.editMode}}"
                      role="menuitem"
                      show-text-label
                    >
                    </hax-context-item>
          ...
          >
            ...
            ${o.isLayoutElement(this.activeNode)?(this.slottedItems||[]).map((e,t)=>i`
                      <simple-toolbar-menu-item
                        slot="menuitem"
      ...
      >
          ...
          <hax-context-item
            ...
            ?hidden="${!this.activeNode||o.isPlatformAudience("novice")}"
            id="rightremove"
          >
          </hax-context-item>
          ${this.ceButtons.map(e=>i` <hax-context-item
              action
              icon="${e.icon}"
              label="${e.label}"
        ...
        <div class="group" ?hidden="${o.isPlatformAudience("novice")}">
            <hax-context-item
              action
    ...
    `}get filteredBlocks(){return this.getFilteredBlocks(this.formatBlocks)}__updatePlatePosition(e){let t=this.shadowRoot.querySelector("#right"),i=this.shadowRoot.querySelector("#rightremove");t.disabled=!1,i.disabled=!1,e&&"GRID-PLATE"==e.tagName?"1-1-1-1"==e.layout&&(t.disabled=!0):i.disabled=!0}__dblClickFireInsert(e){this.dispatchEvent(new CustomEvent("hax-context-item-selected",{bubbles:!0,cancelable:!0,composed:!0,detail:{target:e.target,eventName:"insert-below-active"}}))}handleCECustomEvent(e){let t=e.detail;if("hax-ce-custom-button"===t.eventName)this.activeNode&&"function"==typeof this.activeNode[t.value]&&this.activeNode[t.value](e)&&o.refreshActiveNodeForm()}updated(e){super.updated&&super.updated(e),e.forEach((e,t)=>{"onScreen"===t&&this.onScreen&&this._resetCEMenu(),"activeNode"===t&&(this.isLocked=this._isNodeLocked(this.activeNode)),"activeNode"!==t&&"activeEditingElement"!==t||(this.activeSchema=o.activeSchema(),this.editElementProperty=!(!this.activeSchema||!this.activeSchema.editElement)&&this.activeSchema.editElement,this.editElementToggled=this.activeNode&&this.editElementProperty&&!!this.activeNode[this.editElementProperty])})}firstUpdated(e){super.firstUpdated(e),l(()=>{const e=s(o.activeNode),t=s(o.editMode);Promise.resolve().then(()=>{this.activeNode=e,this.activeNode&&this.activeNode.classList&&this._resetCEMenu(),this.activeNode&&t&&setTimeout(()=>{this.__updatePlatePosition(this.activeNode)},0)})}),l(()=>{const e=s(o.activeEditingElement);Promise.resolve().then(()=>{this.hasActiveEditingElement=!!e})}),this.shadowRoot.querySelector("#drag").addEventListener("dragstart",this._dragStart),this.shadowRoot.querySelector("#drag").addEventListener("dragend",this._dragEnd)}_dragEnd(e){let t=r(e)?r(e)[0]:void 0;t&&t.close(!0),o._lockContextPosition=!1}_dragStart(e){let t=s(o.activeNode),i=r(e)?r(e)[0]:void 0;i&&i.close(!0),o.__dragTarget=t,o._lockContextPosition=!0,o.activeHaxBody.__activeHover=null,o.activeHaxBody._hideContextMenu(o.activeHaxBody.contextMenus.add),e.dataTransfer&&(e.dataTransfer.effectAllowed="move",e.dataTransfer.dropEffect="move",e.dataTransfer.setDragImage(t,-20,-20)),e.stopPropagation(),e.stopImmediatePropagation()}async _resetCEMenu(){if(this.shadowRoot&&a(this,"*"),this.ceButtons=[],o.activeHaxBody&&null!=this.activeNode){let e=o.haxSchemaFromTag(this.activeNode.tagName);this.sourceView=e.canEditSource,o.activeGizmo&&(this.activeTagName=o.activeGizmo.title,this.activeTagIcon=o.activeGizmo.icon)}else o.activeGizmo?(this.activeTagName=o.activeGizmo.title,this.activeTagIcon=o.activeGizmo.icon):(this.activeTagName="",this.activeTagIcon="");await o.runHook(this.activeNode,"inlineContextMenu",[this])}_isNodeLocked(e){return!(!e||!e.getAttribute)&&(null!=e.getAttribute("data-hax-lock")||!(!e.parentNode||!e.parentNode.getAttribute||null==e.parentNode.getAttribute("data-hax-lock")))}_toggleActiveNodeLock(e){if(!this.activeNode||!this.activeNode.getAttribute)return;e.preventDefault(),e.stopPropagation();const t=this.activeNode;let i=null!=t.getAttribute("data-hax-lock");i?(t.removeAttribute("data-hax-lock"),i=!1):(t.setAttribute("data-hax-lock","data-hax-lock"),i=!0),this.isLocked=i,this.dispatchEvent(new CustomEvent("hax-toggle-active-node-lock",{bubbles:!0,composed:!0,cancelable:!0,detail:{lock:i,node:t}}))}static get properties(){return{...super.properties,activeTagIcon:{type:String},activeSchema:{type:Object},editElementProperty:{type:String},editElementToggled:{type:Boolean},activeTagName:{type:String},disableOps:{type:Boolean},disableItemOps:{type:Boolean},insertAbove:{type:Boolean},disableDuplicate:{type:Boolean},canMoveElement:{type:Boolean},ceButtons:{type:Array},hasActiveEditingElement:{type:Boolean},onScreen:{type:Boolean,attribute:"on-screen",reflect:!0},sourceView:{type:Boolean},isLocked:{type:Boolean}}}}globalThis.customElements.define(HaxPlateContext.tag,HaxPlateContext);export{HaxPlateContext};