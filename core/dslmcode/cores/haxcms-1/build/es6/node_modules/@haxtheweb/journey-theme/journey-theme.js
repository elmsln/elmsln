 ...
 */
import{css as t,html as e,nothing as i}from"../../lit/index.js";import{HAXCMSLitElementTheme as a}from"../haxcms-elements/lib/core/HAXCMSLitElementTheme.js";import{store as o}from"../haxcms-elements/lib/core/haxcms-site-store.js";import{autorun as s,toJS as r}from"../../mobx/dist/mobx.esm.js";import"../haxcms-elements/lib/ui-components/active-item/site-active-title.js";import"../haxcms-elements/lib/ui-components/magic/site-collection-list.js";import"../haxcms-elements/lib/ui-components/navigation/site-breadcrumb.js";import"../simple-cta/simple-cta.js";import{DDDVariables as n,DDDPaletteStyles as l}from"../d-d-d/lib/DDDStyles.js";import"../simple-tooltip/simple-tooltip.js";import"../simple-icon/lib/simple-icon-button-lite.js";import"../scroll-button/scroll-button.js";import{licenseList as d}from"../license-element/license-element.js";import{UserScaffoldInstance as c}from"../user-scaffold/user-scaffold.js";class JourneyTheme extends a{static get tag(){return"journey-theme"}constructor(){super();const t=c.readMemory("HAXCMSSitePalette");this.dataPalette=""===t||null===t?11:t,this._items=[],this.location=null,this.activeItem={},this.ancestorItem={},this.basePath=null,this.manifest={},this.__disposer=this.__disposer||[],this.t={readMore:"Read more",home:"Home"};try{this.basePath=globalThis.document.querySelector("base").href}catch(t){this.basePath=globalThis.location.origin}this.HAXSiteCustomRenderRoutes={"x/displays/tags":{items:this.HAXSiteRenderXTagsItems}},this.__disposer.push(s(t=>{const e=r(o.manifest);Promise.resolve().then(()=>{this.manifest=e,this.manifest&&this.manifest.metadata&&this.manifest.metadata.site&&(this.lastUpdated=new Date(1e3*this.manifest.metadata.site.updated).toDateString(),this.copyrightYear=new Date(1e3*this.manifest.metadata.site.created).getFullYear());let t=new d;this.manifest&&this.manifest.license&&t[this.manifest.license]&&(this.licenseName=t[this.manifest.license].name,this.licenseLink=t[this.manifest.license].link,this.licenseImage=t[this.manifest.license].image),this._items=this.getItemChildren(null)})})),this.__disposer.push(s(t=>{const e=r(o.activeItem);Promise.resolve().then(()=>{this.activeItem=e})})),this.__disposer.push(s(t=>{const e=r(o.ancestorItem);Promise.resolve().then(()=>{this.ancestorItem=e})})),this.__disposer.push(s(t=>{const e=r(o.location);Promise.resolve().then(()=>{let t=e;this.location=t})})),this.__disposer.push(s(t=>{const e=r(o.pageCounter);Promise.resolve().then(()=>{const t=e;this.pageCurrent=t.current,this.pageTotal=t.total})}))}getItemChildren(t){if(this.manifest&&this.manifest.items)return this.manifest.items.filter(e=>e.parent===t)}updated(t){super.updated(t),t.has("dataPalette")&&c.writeMemory("HAXCMSSitePalette",this.dataPalette,"long")}static get properties(){return{...super.properties,_items:{type:Array},activeItem:{type:Object},ancestorItem:{type:Object},location:{type:Object},basePath:{type:String},dataPalette:{type:Number,reflect:!0,attribute:"data-palette"},licenseName:{type:String},licenseLink:{type:String},licenseImage:{type:String},lastUpdated:{type:String},copyrightYear:{type:Number},pageCurrent:{type:Number},pageTotal:{type:Number}}}HAXSiteRenderXTagsItems(t){return e`
      <div>
        ${t.map(t=>e` <a
              href="${t.slug}"
              part="child-page-link"
              class="child-page-link"
              >${t.metadata.image?e`<img
                  ...
                  />`:e`<img
                    ...
                    decoding="async"
                    fetchpriority="low"
                    src="${o.manifest&&o.manifest.metadata&&o.manifest.metadata.author?o.manifest.metadata.author.image:""}"
                    alt="${o.manifest&&o.manifest.metadata&&o.manifest.metadata.author?o.manifest.metadata.author.name:""}"
                  />`}
              ${t.title}
            </a>`)}
      </div>
    `}HAXCMSGlobalStyleSheetContent(){return[...super.HAXCMSGlobalStyleSheetContent(),t`
      ...
      `]}static get styles(){return[n,l,super.styles,t`
        :host {
    ...
    <header>
      ...
      <div class="author">
        <a href="${this.basePath}">${this.manifest.metadata.author.image?e` <img
                class="author-image"
                loading="lazy"
                decoding="async"
                fetchpriority="low"
              ...
              />`:""}
          ${this.location&&"home"===this.location.route.name?e`<h1>${this.manifest.title}</h1>
                <h2>${this.manifest.description}</h2>`:e`<div class="site-title-heading">${this.manifest.title}</div>
                <div class="site-description">${this.manifest.description}</div>`}
        </a>
      </div>
    </header>
    <nav class="lower-header-box ${this.location&&"home"===this.location.route.name?"home":"not-home"}" aria-label="Site sections">
      ...
      ${this.location&&"home"!==this.location.route.name?e` ${this._items.map((t,i)=>e`
                <simple-tooltip for="top-${t.id}" position="bottom"
                  >${t.title}</simple-tooltip
                >
                <a
                  tabindex="-1"
                  href="${t.slug}"
                  class="article-link-icon top ${this.activeItem&&(t.id===this.activeItem.id||this.ancestorItem&&t.id===this.ancestorItem.id)?"active":""}"
                  ><simple-icon-button-lite
                    id="top-${t.id}"
                    title="${t.title}"
                    label="${t.title}"
                    class="article"
                    icon="${t.metadata.icon?t.metadata.icon:"av:album"}"
                  ></simple-icon-button-lite
                ></a>
              `)}`:""}
    </nav>
    <main class="main ${this.location&&"home"===this.location.route.name?"home":"not-home"}"> 
        ...
        ${this.location&&"home"===this.location.route.name?e` ${this._items.map((t,i)=>e`
                  <article class="post ${i%2==0?"even":"odd"}">
                    ...
                    <div class="article-wrap">
                      ...
                      ${this.getItemChildren(t.id).length>0?e` <div class="child-pages-container">
      ...
      <article part="transitioncontent" class="${this.location&&"home"===this.location.route.name?"home":"not-home"}">
        ...
        <div id="contentcontainer"><div id="slot">${this.location&&"home"!==this.location.route.name?e`<slot></slot>
                ...
                ></site-collection-list>`:""}</div></div>
      </article>
    </main>
    <footer>
      <div class="author">
        <div class="spacing"><a href="${this.basePath}" title="${this.t.home}">${this.manifest.metadata.author.image?e` <img
                class="author-image"
                loading="lazy"
                decoding="async"
                fetchpriority="low"
