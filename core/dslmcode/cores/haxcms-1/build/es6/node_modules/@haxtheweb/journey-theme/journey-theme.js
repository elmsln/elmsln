 ...
 */
import{css as t,html as e,nothing as i}from"../../lit/index.js";import{HAXCMSLitElementTheme as a}from"../haxcms-elements/lib/core/HAXCMSLitElementTheme.js";import{store as o}from"../haxcms-elements/lib/core/haxcms-site-store.js";import{autorun as s,toJS as r}from"../../mobx/dist/mobx.esm.js";import"../haxcms-elements/lib/ui-components/active-item/site-active-title.js";import"../haxcms-elements/lib/ui-components/magic/site-collection-list.js";import"../haxcms-elements/lib/ui-components/navigation/site-breadcrumb.js";import"../simple-cta/simple-cta.js";import{DDDAllStyles as n}from"../d-d-d/lib/DDDStyles.js";import"../simple-tooltip/simple-tooltip.js";import"../simple-icon/lib/simple-icon-button-lite.js";import"../scroll-button/scroll-button.js";import{licenseList as l}from"../license-element/license-element.js";import{UserScaffoldInstance as c}from"../user-scaffold/user-scaffold.js";class JourneyTheme extends a{static get tag(){return"journey-theme"}constructor(){super(),this.siteTheme=c.readMemory("HAXCMSSiteTheme")||"",this.dataPrimary=2,this._items=[],this.location=null,this.activeItem={},this.ancestorItem={},this.basePath=null,this.manifest={},this.__disposer=this.__disposer||[],this.t={readMore:"Read more",home:"Home"};try{this.basePath=globalThis.document.querySelector("base").href}catch(t){this.basePath=globalThis.location.origin}this.HAXSiteCustomRenderRoutes={"x/tags":{items:this.HAXSiteRenderXTagsItems}},s((t=>{this.manifest=r(o.manifest),this.lastUpdated=new Date(1e3*o.manifest.metadata.site.updated).toDateString(),this.copyrightYear=new Date(1e3*o.manifest.metadata.site.created).getFullYear();let e=new l;this.manifest.license&&e[this.manifest.license]&&(this.licenseName=e[this.manifest.license].name,this.licenseLink=e[this.manifest.license].link,this.licenseImage=e[this.manifest.license].image),this._items=this.getItemChildren(null),this.__disposer.push(t)})),s((t=>{this.activeItem=r(o.activeItem),this.__disposer.push(t)})),s((t=>{this.ancestorItem=r(o.ancestorItem),this.__disposer.push(t)})),s((t=>{let e=r(o.location);this.location=e,this.__disposer.push(t)})),s((t=>{const e=r(o.pageCounter);this.pageCurrent=e.current,this.pageTotal=e.total,this.__disposer.push(t)}))}getItemChildren(t){if(this.manifest&&this.manifest.items)return this.manifest.items.filter((e=>e.parent===t))}updated(t){if(super.updated(t),t.has("siteTheme")){switch(this.siteTheme){case"earth":default:this.dataPrimary=1;break;case"water":this.dataPrimary=11;break;case"fire":this.dataPrimary=23;break;case"sand":case"violet":this.dataPrimary=2;break;case"rose":this.dataPrimary=47}c.writeMemory("HAXCMSSiteTheme",this.siteTheme,"long")}}static get properties(){return{...super.properties,_items:{type:Array},activeItem:{type:Object},ancestorItem:{type:Object},location:{type:Object},basePath:{type:String},dataPrimary:{type:String,reflect:!0,attribute:"data-primary"},siteTheme:{type:String,reflect:!0,attribute:"site-theme"},licenseName:{type:String},licenseLink:{type:String},licenseImage:{type:String},lastUpdated:{type:String},copyrightYear:{type:Number},pageCurrent:{type:Number},pageTotal:{type:Number}}}HAXSiteRenderXTagsItems(t){return e`
    <div>
    ${t.map((t=>e`
      <a href="${t.slug}" part="child-page-link" class="child-page-link">${t.metadata.image?e`<img src="${t.metadata.image}" loading="lazy"
        decoding="async"
        ...
        decoding="async"
        fetchpriority="low"
        src="${o.manifest.metadata.author.image}"
        alt="${o.manifest.metadata.author.name}"
        />`}
        ${t.title}
      </a>`))}
    </div>
    `}HAXCMSGlobalStyleSheetContent(){return[...super.HAXCMSGlobalStyleSheetContent(),t`
      :root {
      ...
      `]}static get styles(){return[n,super.styles,t`
        :host {
      ...
      `]}firstUpdated(t){super.firstUpdated(t),this.HAXCMSThemeSettings.autoScroll=!0,this.HAXCMSThemeSettings.scrollTarget=this.shadowRoot.querySelector(".lower-header-box"),this.HAXCMSThemeSettings.scrollSettings={behavior:"instant",block:"start",inline:"start"},globalThis.AbsolutePositionStateManager.requestAvailability().scrollTarget=this.HAXCMSThemeSettings.scrollTarget}disconnectedCallback(){if(this.__disposer)for(var t in this.__disposer)this.__disposer[t].dispose();super.disconnectedCallback()}toggleSiteTheme(t){switch(this.siteTheme){case"earth":this.siteTheme="water";break;case"water":this.siteTheme="fire";break;case"fire":this.siteTheme="sand";break;case"sand":this.siteTheme="rose";break;case"rose":this.siteTheme="violet";break;case"violet":this.siteTheme="";break;default:this.siteTheme="earth"}}render(){return e`
    <header>
    ...
    <div class="lower-header-box ${this.location&&"home"===this.location.route.name?"home":"not-home"}">
      ...
      ${this.location&&"home"!==this.location.route.name?e`
          ${this._items.map(((t,i)=>e`
            ...
            <a tabindex="-1" href="${t.slug}" class="article-link-icon top ${this.activeItem&&(t.id===this.activeItem.id||this.ancestorItem&&t.id===this.ancestorItem.id)?"active":""}"><simple-icon-button-lite id="${t.id}" title="${t.title}" label="${t.title}" class="article" icon="${t.metadata.icon?t.metadata.icon:"av:album"}"></simple-icon-button-lite></a>
          `))}`:""}
    </div>
      ...
      <div class="articles">
        ${this.location&&"home"===this.location.route.name?e`
          ${this._items.map(((t,i)=>e`
            ...
            <a tabindex="-1" href="${t.slug}" class="article-link-icon"><simple-icon-button-lite id="v-${t.id}" label="${t.title}" title="${t.title}" class="article" icon="${t.metadata.icon?t.metadata.icon:"av:album"}"></simple-icon-button-lite></a>
                ...
                ${this.getItemChildren(t.id).length>0?e`
                  <div class="child-pages-container">
                    ${this.getItemChildren(t.id).map((t=>e`
                      ...
                      <a id="v-${t.id}" href="${t.slug}" title="${t.title}" class="child-page-link">${t.metadata.image?e`<img src="${t.metadata.image}" loading="lazy"
                        decoding="async"
                        fetchpriority="low" alt="${t.title}"/>`:e`<img 
                          loading="lazy"
                          decoding="async"
    ...
    <footer>
      <div class="author">
        <div class="spacing"><a href="${this.basePath}" title="${this.t.home}">${this.manifest.metadata.author.image?e`
          <img 
            class="author-image"
            loading="lazy"
            decoding="async"
          ...
          <div>
              ${this.pageCurrent>0?e`Page ${this.pageCurrent} of ${this.pageTotal}`:i} <br /><br />
