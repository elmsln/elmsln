...
const patchBrowser = async () => {
  ...
  const scriptElm = Array.from(doc.querySelectorAll('script')).find(s => regex.test(s.src) || s.getAttribute('data-namespace') === NAMESPACE);
...
const attachStyles = (elm, cmpMeta, mode) => {
  const styleId = addStyle(supportsShadowDom && elm.shadowRoot ? elm.shadowRoot : elm.getRootNode(), cmpMeta, mode, elm);
...
const renderVdom = (hostElm, hostRef, cmpMeta, renderFnResults) => {
  ...
  patch(oldVNode, rootVnode);
  {
    if (checkSlotRelocate) {
      ...
      for (let i = 0; i < relocateNodes.length; i++) {
        const relocateNode = relocateNodes[i]; // by default we're just going to insert it directly
...
const scheduleUpdate = (elm, hostRef, cmpMeta, isInitialLoad) => {
  ...
  let promise; // there is no ancestorc omponent or the ancestor component
...
const initializeComponent = async (elm, hostRef, cmpMeta, hmrVersionId, Cstr) => {
  ...
  /* hasInitializedComponent */
    ...
    if (!Cstr.$isStyleRegistered$ && Cstr.style) {
      ...
      let scopeId = getScopeId(cmpMeta.$tagName$);
      const shadowCssPolyfill = cmpMeta.$flags$ & 8
      /* needsShadowDomShim */
      ;
      const runtimeScopedCss = BUILD.runtimeScopeCss
      /* scopedCssEncapsulation */
      ;

      if (shadowCssPolyfill || runtimeScopedCss) {
        style = await import('./shadow-css-bbdf056f-05cd1ccb.js').then(m => m.scopeCss(style, scopeId, false));
      }
...
const disconnectedCallback = elm => {
  ...
  /* isTmpDisconnected */
    ...
    const hostRef = getHostRef(elm); // clear CSS var-shim tracking
