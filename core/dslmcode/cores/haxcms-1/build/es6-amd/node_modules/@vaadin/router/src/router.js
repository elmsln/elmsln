 ...
 */class Router extends _resolver.default{/**
...
baseUrl:baseElement&&baseElement.getAttribute("href")},options));this.resolveRoute=context=>this.__resolveRoute(context);const triggers=Router.NavigationTrigger;Router.setTriggers.apply(Router,Object.keys(triggers).map(key=>triggers[key]));/**
     ...
     */this.location;this.location=createLocation({resolver:this});this.__lastStartedRenderId=0;this.__navigationEventHandler=this.__onNavigationEvent.bind(this);this.setOutlet(outlet);this.subscribe()}__resolveRoute(context){const route=context.route;let callbacks=Promise.resolve();if((0,_utils.isFunction)(route.children)){callbacks=callbacks.then(()=>route.children(copyContextWithoutNext(context))).then(children=>{// The route.children() callback might have re-written the
...
if(result instanceof HTMLElement||result.redirect||result===_utils.notFoundResult){return result}}if((0,_utils.isString)(route.redirect)){return commands.redirect(route.redirect)}if(route.bundle){return(0,_utils.loadBundle)(route.bundle).then(()=>{},()=>{throw new Error((0,_utils.log)(`Bundle not found: ${route.bundle}. Check if the file name is correct`))})}}).then(result=>{if(isResultNotEmpty(result)){return result}if((0,_utils.isString)(route.component)){return commands.component(route.component)}})}/**
...
for(let i=currentContext.__divergedChainIndex;i<currentContext.chain.length;i++){const currentComponent=currentContext.chain[i].element||{},location=createLocation(currentContext,currentContext.chain[i].route);runCallbackIfPossible(currentComponent.onAfterEnter,[location,{},currentContext.resolver],currentComponent)}}__animateIfNeeded(context){const from=(this.__disappearingContent||[])[0],to=(this.__appearingContent||[])[0],promises=[],chain=context.chain;let config;for(let i=chain.length;0<i;i--){if(chain[i-1].route.animate){config=chain[i-1].route.animate;break}}if(from&&to&&config){const leave=(0,_utils.isObject)(config)&&config.leave||"leaving",enter=(0,_utils.isObject)(config)&&config.enter||"entering";promises.push((0,_animate.default)(from,leave));promises.push((0,_animate.default)(to,enter))}return Promise.all(promises).then(()=>context)}/**
   ...
   * method.
   */unsubscribe(){window.removeEventListener("vaadin-router-go",this.__navigationEventHandler)}__onNavigationEvent(event){const pathname=event?event.detail.pathname:window.location.pathname;if((0,_utils.isString)(this.__normalizePathname(pathname))){if(event&&event.preventDefault){event.preventDefault()}this.render(pathname,!0)}}/**
   ...
   */static setTriggers(...triggers){(0,_setNavigationTriggers.default)(triggers)}/**
   ...
   *
   * @return {string}
   */urlForName(name,params){if(!this.__urlForName){this.__urlForName=(0,_generateUrls.default)(this)}return getPathnameForRouter(this.__urlForName(name,params),this)}/**
   ...
   * @return {boolean}
   */static go(pathname){return(0,_utils.fireRouterEvent)("go",{pathname})}}_exports.Router=Router});