  ...
  const ElmslnStudioUtilities = function (SuperClass) {
    return class extends SuperClass {
      ...
      getStatus(submission, assignment, ifOverdue, ifLate, ifSubmitted, ifElse) {
        let late = !!submission && !!submission.date && !!assignment.date && submission.date > assignment.date,
      ...
      getActivityLink(activity, nocomment = false, sort = false) {
        return activity.activity === "submission" ? `/project/${activity.portfolioId || activity.id}${activity.portfolioId ? `?submission=${activity.id}` : ""}${nocomment ? "" : activity.portfolioId ? `&comment=true&sort-latest=${sort}` : `?comment=true&sort-latest=${sort}`}` : activity.activity === "discussion" ? `/project/${activity.projectId || activity.submissionId}${activity.portfolioId ? `?submission=${activity.submissionId}&` : "?"}comment=${activity.id}` : `/project/${activity.projectId || activity.submissionId}${activity.portfolioId ? `?submission=${activity.submissionId}&` : "?"}comment=${activity.feedbackId}`;
      }

      getActivityTitle(activity) {
        return (0, _litElement.html)`
        ${activity.activity === "submission" ? `${[activity.firstName, activity.lastName].join(" ")} submitted ${activity.assignment}` : activity.activity === "discussion" ? `${[activity.firstName, activity.lastName].join(" ")}'s feedback on ${[activity.creatorFirstName, activity.creatorLastName].join(" ")}'s ${activity.assignment}` : `${[activity.firstName, activity.lastName].join(" ")} replied to ${[activity.reviewerFirstName, activity.reviewerLastName].join(" ")}`}
      `;
      }
      /**
      ...
      rubricTable(rubric) {
        return !rubric ? "" : (0, _litElement.html)`
            <table class="rubric-table">
              <caption>
                Rubric
              </caption>
              ${!rubric.key || !rubric.key ? `` : (0, _litElement.html)`
                    <thead>
                      <th scope="col">Criteria</th>
                      ${rubric.key.map(col => (0, _litElement.html)`<th scope="col">
                          ${col.description && (col.points || col.points > -1) ? `${col.description} (${col.points})` : col.description || col.points}
                        </th>`)}
                    </thead>
                  `}
              <tbody>
                ${Object.keys(rubric.values || {}).map(key => (0, _litElement.html)`
                      <tr>
                        <th scope="row">${key}</th>
                        ${(rubric.values[key] || []).map(col => (0, _litElement.html)`<td>${col}</td>`)}
                      </tr>
                    `)}
              </tbody>
            </table>
      ...
      message404(type, backLabel, backHref) {
        return (0, _litElement.html)` <div id="primary" class="primary-404">
        <p id="message-404">
      ...
      loading(color, slot, size = "small") {
        return !slot ? (0, _litElement.html)`<hexagon-loader
            loading
            item-count="3"
            size="${size}"
            color="${color}"
          >
          </hexagon-loader>` : (0, _litElement.html)`<hexagon-loader
            loading
            item-count="3"
            size="${size}"
            color="${color}"
