 ...
 */class SiteTopMenu extends _polymerElement.PolymerElement{/**
   ...
   */static get tag(){return"site-top-menu"}constructor(){super();this.__disposer=[];new Promise((res,rej)=>_require.default(["../../../../../@polymer/paper-tooltip/paper-tooltip.js"],res,rej));new Promise((res,rej)=>_require.default(["../../../../../@polymer/paper-button/paper-button.js"],res,rej))}// render function
static get template(){return _polymerElement.html`
   ...
   */static get properties(){return{/**
       ...
       */conditions:{type:Object,value:{parent:null}},position:{type:String,value:"bottom"},editMode:{type:Boolean,reflectToAttribute:!0}}}humanIndex(index){return index+1}/**
...
this.$.indicator.classList.remove("activated")}}}connectedCallback(){super.connectedCallback();(0,_mobxModule.autorun)(reaction=>{this.manifest=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.manifest);this.__disposer.push(reaction)});(0,_mobxModule.autorun)(reaction=>{this.editMode=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.editMode);this.__disposer.push(reaction)});// minor timing thing to ensure store has picked active
// needed if routes set on first paint or lifecycles miss
setTimeout(()=>{(0,_mobxModule.autorun)(reaction=>{this.activeId=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.activeId);this.__disposer.push(reaction)})},50);window.addEventListener("resize",()=>{this._activeIdChanged(this.activeId)},!0)}disconnectedCallback(){// clean up state
for(var i in this.__disposer){this.__disposer[i].dispose()}window.removeEventListener("resize",()=>{this._activeIdChanged(this.activeId)},!0);super.disconnectedCallback()}}_exports.SiteTopMenu=SiteTopMenu;window.customElements.define(SiteTopMenu.tag,SiteTopMenu)});