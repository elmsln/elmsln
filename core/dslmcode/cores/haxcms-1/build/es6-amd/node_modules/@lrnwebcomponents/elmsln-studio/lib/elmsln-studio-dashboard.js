      ...
      <div id="primary">
        <div id="profile">
          ...
          <accent-card accent-color="purple" class="card primary">
            ...
            ${!this.profile || Object.keys(this.profile).length < 1 ? this.loading("purple", "footer") : (0, _litElement.html)`
                  <lrndesign-avatar
                  ...
                  <table slot="content">
                    <tbody>
                      <tr>
                        ...
                        <td>
                          ${!this.profile || !this.profile.completed || !this.profile.due ? "unknown" : `${this.profile.completed.length} / ${this.profile.completed.length + this.profile.due.length}`}
                        </td>
                      </tr>
                      ${!this.profile || (this.profile.features || []).length < 1 ? `` : (0, _litElement.html)`
                            <tr>
                              <th scope="row">
                                <simple-icon-lite
                                  icon="star"
                              ...
                              <td>
                                ${!this.profile || !this.profile.features ? "unknown" : this.profile.features.length}
                              </td>
                            </tr>
                          `}
                      <tr>
                        ...
                        <td>
                          ${!this.profile || !this.profile.given ? "0" : (0, _litElement.html)` ${this.profile.given.length} `}
                        </td>
                      </tr>
                    </tbody>
                  </table>
          ...
          <nav-card accent-color="green" class="card primary due">
            ...
            >
            ${!this.profile || Object.keys(this.profile).length < 1 ? this.loading("green", "footer") : (0, _litElement.html)`
                  <div slot="linklist">
                    ${(this.profile.due || []).slice(0, 5).map(a => (0, _litElement.html)`
                        <nav-card-item
                          id="due-${a.id}-item"
                          accent-color="${this.isLate(a.date) ? "red" : "grey"}"
                          allow-grey
                          avatar="${this.isLate(a.date) ? "report-problem" : "assignment"}"
                          invert
                        >
                          <elmsln-studio-link
                            id="due-${a.id}"
        ...
        <div id="work">
          ...
          >
            ...
            >
            ${!this.profile || Object.keys(this.profile).length < 1 ? this.loading("orange", "footer") : (0, _litElement.html)` <div slot="linklist">
                  ${(this.profile.submissions || []).slice(0, 5).map(s => (0, _litElement.html)`
                      <nav-card-item
                        id="due-${s.id}-item"
                        accent-color="${this.isLate(s.assignmentDate, s.date) ? "amber" : "green"}"
                        allow-grey
                        avatar="${this.isLate(s.assignmentDate, s.date) ? "assignment-late" : "assignment-turned-in"}"
                        invert
                      >
                        <elmsln-studio-link
                          id="sub-${s.id}"
                      ...
                      <simple-tooltip for="due-${s.id}-item" position="left">
                        ${this.isLate(s.assignmentDate, s.date) ? "Submitted Late" : "Submitted"}
                      </simple-tooltip>
                    `)}
                </div>`}
          </nav-card>
          ...
          >
            ...
            ${!this.profile || Object.keys(this.profile).length < 1 ? this.loading("cyan", "footer") : (0, _litElement.html)` <div slot="linklist">
                  ${(this.profile.feedback || []).slice(0, 5).map(f => (0, _litElement.html)`
                      <nav-card-item
      ...
      <div id="secondary">
        ...
        <accent-card accent-color="pink" class="card feed secondary">
          <nav-card flat no-border slot="content">
            ...
            >
            ${!this.submissions ? this.loading("pink", "body") : (0, _litElement.html)` <div slot="linklist">
                  ${(this.submissions || []).map(s => (0, _litElement.html)`
                      <nav-card-item
                        accent-color="${this.accentColor([s.firstName, s.lastName].join(" "))}"
                        .avatar="${s.avatar}"
          ...
          <nav-card flat no-border slot="content">
            ...
            ${!this.discussion ? this.loading("pink", "body") : (0, _litElement.html)`
                  <div slot="linklist">
                    ${(this.discussion || []).map(d => (0, _litElement.html)`
                        <nav-card-item
                          accent-color="${this.accentColor([d.firstName, d.lastName].join(" "))}"
                          .avatar="${d.avatar}"
