define(["exports","require","../../../@polymer/polymer/polymer-element.js","../../../@polymer/polymer/lib/utils/render-status.js","../../../@polymer/polymer/lib/elements/dom-repeat.js","../../../@polymer/polymer/lib/elements/dom-if.js","../../../@polymer/iron-ajax/iron-ajax.js","../../simple-picker/simple-picker.js","../../responsive-utility/lib/responsive-utility-behaviors.js","./editable-table-behaviors.js","./editable-table-styles.js"],function(_exports,_require,_polymerElement,_renderStatus,_domRepeat,_domIf,_ironAjax,_simplePicker,_responsiveUtilityBehaviors,_editableTableBehaviors,_editableTableStyles){Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.EditableTableDisplay=void 0,_require=babelHelpers.interopRequireWildcard(_require);class EditableTableDisplay extends((0,_editableTableBehaviors.displayBehaviors)((0,_responsiveUtilityBehaviors.ResponsiveUtilityBehaviors)(_polymerElement.PolymerElement))){static get template(){return _polymerElement.html`
    ...
    `}static get tag(){return"editable-table-display"}static get properties(){return{filterColumn:{type:Number,value:null},filtered:{type:Boolean,value:!1},filterText:{type:String,value:null},options:{type:Array,computed:"_getTheadOptions(thead)"},selected:{type:Number,value:1},sortMode:{type:String,value:"none"},sortColumn:{type:Number,value:-1},hidden:{type:Boolean,computed:"_hasNoData(data)"}}}_dataChanged(newValue,oldValue){if(!newValue||newValue.length<1||newValue[0].length<1){let table=this.children.item(0);null!=table&&"TABLE"===table.tagName&&this.importHTML(table)}this.dispatchEvent(new CustomEvent("change",{bubbles:!0,cancelable:!0,composed:!0,detail:newValue}))}_hasNoData(data){return!data||data.length<1||data[0].length<1}_getTheadOptions(thead){let temp=[];if(null!=thead&&thead.length>0)for(let i=1;i<thead[0].length;i++)temp.push([{alt:thead[0][i],value:i}]);return temp}_isColHidden(index,selected=1){return selected=selected||1,0!==parseInt(index)&&parseInt(index)!==parseInt(selected)}_isFiltered(column,filterColumn,filtered){return null!==filterColumn&&filterColumn===column&&filtered}_isNegative(cell){return this._isNumeric(cell)&&0===cell.trim().indexOf("-")}_isNumericColumn(index){let numeric=!0;for(let i=0;i<this.tbody.length;i++)this._isNumeric(this.tbody[i][index])||(numeric=!1);return numeric}_isRowHeader(rowHeader,index){return 0===index&&rowHeader}_tableChanged(){this._updateCols()}_selectedChanged(){this._updateCols()}_changeSortMode(e){this.sortColumn===e.detail.columnIndex&&"asc"===this.sortMode?this.sortMode="desc":this.sortColumn===e.detail.columnIndex&&"desc"===this.sortMode?this.sortMode="none":(this.sortMode="asc",this.sortColumn=e.detail.columnIndex),e.detail.setSortMode(this.sortMode),console.log("_changeSortMode",e.detail,this.sortMode),this.sortData(this.sortMode,e.detail.columnIndex)}_updateCols(){let selected=this.shadowRoot.querySelector("#column").value,cols=this.shadowRoot.querySelector("#table").querySelectorAll("th,td");if(cols.length>0)for(let i=0;i<cols.length;i++){let col=cols[i];this._isColHidden(col.cellIndex,selected)?col.setAttribute("xs-hidden",!0):col.removeAttribute("xs-hidden")}}filterRows(filterColumn,filterText){return null!=filterText?function(tr){return tr[filterColumn].toLowerCase().trim()===filterText.toLowerCase().trim()}:null}sortData(type,column){if("none"!==type&&!1!==type){let temp=this.tbody.slice();for(let i=0;i<temp.length;i++)temp[i].unshift(temp[i][column]);"asc"===type?temp.sort():temp.reverse();for(let i=0;i<temp.length;i++)this.set("tbody."+i,[]),this.set("tbody."+i,temp[i].slice(1));console.log("sortData",type,column,temp,this.data)}else console.log("sortData",type,column,this.data)}toggleFilter(e){void 0===e||this.filterColumn==e.detail.columnIndex&&this.filtered?(this.filtered=!1,this.filterText=null,this.filterColumn=null):(this.filterText=e.detail.text,this.filterColumn=e.detail.columnIndex,this.filtered=!0)}constructor(){super(),new Promise((res,rej)=>_require.default(["./editable-table-sort.js"],res,rej)),new Promise((res,rej)=>_require.default(["./editable-table-filter.js"],res,rej))}connectedCallback(){super.connectedCallback(),(0,_renderStatus.afterNextRender)(this,function(){this.addEventListener("change-sort-mode",this._changeSortMode.bind(this)),this.addEventListener("toggle-filter",this.toggleFilter.bind(this))})}disconnectedCallback(){this.removeEventListener("change-sort-mode",this._changeSortMode.bind(this)),this.removeEventListener("toggle-filter",this.toggleFilter.bind(this)),super.disconnectedCallback()}}_exports.EditableTableDisplay=EditableTableDisplay,window.customElements.define(EditableTableDisplay.tag,EditableTableDisplay)});