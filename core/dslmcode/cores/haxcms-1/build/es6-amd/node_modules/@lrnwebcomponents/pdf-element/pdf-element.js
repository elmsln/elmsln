...
@hero hero.svg
*/class PdfElement extends(0,_schemaBehaviors.SchemaBehaviors)(_polymerElement.PolymerElement){constructor(){super();new Promise((res,rej)=>_require.default(["../../@polymer/paper-card/paper-card.js"],res,rej));new Promise((res,rej)=>_require.default(["../../@polymer/app-layout/app-toolbar/app-toolbar.js"],res,rej));new Promise((res,rej)=>_require.default(["../../@polymer/paper-spinner/paper-spinner.js"],res,rej));(0,_renderStatus.afterNextRender)(this,function(){this.HAXWiring=new _HAXWiring.HAXWiring;this.HAXWiring.setup(PdfElement.haxProperties,PdfElement.tag,this)})}static get template(){return _polymerElement.html`
      <style>
        :host {
          display: block;
          width: 100%;
...
var parsedFileName=currentThis.src.split("/").pop(),self=currentInstance;currentThis.sidebarOpen=!0;if(1<=page&&page<=currentInstance.totalPagesNum){self.currentPage=page;self.queueRenderPage(self.currentPage);currentInstance.currentPage=page;currentThis.currentPage=page;this.$.input.blur()}else{this.$.input.value=self.currentPage;this.$.input.blur()}}/*
   ...
   */download(){if(this.instance.pdfExists){this.instance.download()}}}_exports.PdfElement=PdfElement;window.customElements.define(PdfElement.tag,PdfElement)});