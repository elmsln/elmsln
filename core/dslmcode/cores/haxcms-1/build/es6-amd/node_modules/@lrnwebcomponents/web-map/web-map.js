...
if(this.poster){if(L.Browser.gecko){this.poster.removeAttribute("usemap")}(0,_polymerDom.dom)(this.$.map).appendChild(this.poster)}}}}disconnectedCallback(){this._removeEvents();super.disconnectedCallback()}connectedCallback(){super.connectedCallback();async.microTask.run(()=>{// the dimension attributes win, if they're there. A map does not
// have an intrinsic size, unlike an image or video, and so must
// have a defined width and height.
var s=window.getComputedStyle(this),wpx=s.width,hpx=s.height,w=parseInt(wpx.replace("px","")),h=parseInt(hpx.replace("px",""));if(""===wpx||""===hpx){return}if(!this.width||this.width!==w){this.$.map.style.width=wpx;this.width=w}else{this.$.map.style.width=this.width+"px"}if(!this.height||this.height!==h){this.$.map.style.height=h;this.height=h}else{this.$.map.style.height=this.height+"px"}// create the Leaflet map if this is the first time attached is called
if(!this._map){this._map=L.map(this.$.map,{center:new L.LatLng(this.lat,this.lon),projection:this.projection,crs:M[this.projection],zoom:this.zoom,zoomControl:!1,// because the M.MapMLLayer invokes _tileLayer._onMoveEnd when
...
if(this.controls){this._layerControl=M.mapMlLayerControl(null,{collapsed:!0}).addTo(this._map);this._zoomControl=L.control.zoom().addTo(this._map)}// the attribution control is not optional
