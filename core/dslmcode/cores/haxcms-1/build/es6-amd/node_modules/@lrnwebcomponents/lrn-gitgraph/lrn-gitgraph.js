define(["exports","../../@polymer/polymer/polymer-element.js","./lib/gitgraph.js/src/gitgraph.js"],function(_exports,_polymerElement,_gitgraph){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.LrnGitgraph=void 0;/**
 * `lrn-gitgraph`
 ...
 */class LrnGitgraph extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
      <style>
        :host {
          display: block;
          overflow-x: scroll;
...
branches[item.branch].commit({sha1:item.commit,message:item.subject,author:item.author,tag:item.refs})})}}}_treeRemoveDuplicates(tree){var htTree=[],htCommits=[];tree.forEach(function(t){if(!htCommits.includes(t.commit)){htTree.push(t);htCommits.push(t.commit)}});return htTree}ready(){super.ready();var root=this,config={template:root.template,// could be: "blackarrow" or "metro" or `myTemplate` (custom Template object)
reverseArrow:!1,// to make arrows point to ancestors, if displayed
orientation:root.orientation,element:root.shadowRoot.querySelector("#gitGraph")};if(""!==root.mode){config.mode=root.mode}root.config=config}}_exports.LrnGitgraph=LrnGitgraph;window.customElements.define(LrnGitgraph.tag,LrnGitgraph)});