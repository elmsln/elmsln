...
static get template(){return _polymerElement.html`
    ...
    `}static get properties(){return{/**
       ...
       */lastError:{type:Object,observer:"_lastErrorChanged"},_timeStamp:{type:String},/**
       * queryParams
       ...
       * Injected by HAXcms
       */baseURI:{type:String}}}_lastErrorChanged(newValue){if(newValue){console.error(newValue);const evt=new CustomEvent("simple-toast-show",{bubbles:!0,composed:!0,cancelable:!0,detail:{text:newValue.statusText}});window.dispatchEvent(evt)}}/**
   * ready life cycle
   */constructor(){super();window.addEventListener("hax-store-ready",this.storeReady.bind(this));new Promise((res,rej)=>_require.default(["../../../../@polymer/paper-progress/paper-progress.js"],res,rej));// attempt to set polymer passive gestures globally
...
if(document.getElementById("haxcmsoutdatedfallback")){document.getElementById("haxcmsoutdatedfallback").style.display="none"}this.__disposer=[];(0,_mobxModule.autorun)(reaction=>{this.themeData=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.themeData);this.__disposer.push(reaction)});(0,_mobxModule.autorun)(reaction=>{this.activeItem=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.activeItem);this.__disposer.push(reaction)});this._timeStamp=""}connectedCallback(){super.connectedCallback();(0,_renderStatus.afterNextRender)(this,function(){this.dispatchEvent(new CustomEvent("haxcms-ready",{bubbles:!0,composed:!0,cancelable:!1,detail:this}));if(document.getElementById("haxcmsoutdatedfallback")){document.body.removeChild(document.getElementById("haxcmsoutdatedfallback"))}window.addEventListener("haxcms-trigger-update",this._triggerUpdatedData.bind(this));window.addEventListener("haxcms-trigger-update-node",this._triggerUpdatedNode.bind(this));// dyanmcially import the editor builder which figures out if we should have one
new Promise((res,rej)=>_require.default(["./haxcms-editor-builder.js"],res,rej)).then(response=>{this.editorBuilder=document.createElement("haxcms-editor-builder");// attach editor builder after we've appended to the screen
document.body.appendChild(this.editorBuilder);// get fresh data if not published
if("published"!==this.editorBuilder.getContext()){this._timeStamp="?"+Math.floor(Date.now()/1e3)}}).catch(error=>{/* Error handling */console.log(error)});var evt=document.createEvent("UIEvents");evt.initUIEvent("resize",!0,!1,window,0);window.dispatchEvent(evt)})}/**
   * Detached life cycle
...
//console.log(tagName + ' dynamic import');
   ...
   */_activeItemChanged(newValue,oldValue){if(newValue&&typeof newValue.id!==typeof void 0){this.set("queryParams.nodeId",newValue.id);this.notifyPath("queryParams.nodeId");// if published, keep it static on request
// @todo might revisit this in the future
if(this.editorBuilder&&"published"===this.editorBuilder.getContext()){this._timeStamp=""}else{this._timeStamp="?"+Math.floor(Date.now()/1e3)}this.$.activecontent.generateRequest()}// we had something, now we don't. wipe out the content area of the theme
...
if(this.editorBuilder&&"published"!==this.editorBuilder.getContext()){this._timeStamp="?"+Math.floor(Date.now()/1e3)}this.$.manifest.generateRequest()}/**
...
if(!newValue.metadata.dynamicElementLoader){newValue.metadata.dynamicElementLoader={"a11y-gif-player":"@lrnwebcomponents/a11y-gif-player/a11y-gif-player.js","citation-element":"@lrnwebcomponents/citation-element/citation-element.js","hero-banner":"@lrnwebcomponents/hero-banner/hero-banner.js","image-compare-slider":"@lrnwebcomponents/image-compare-slider/image-compare-slider.js","license-element":"@lrnwebcomponents/license-element/license-element.js","lrn-aside":"@lrnwebcomponents/lrn-aside/lrn-aside.js","lrn-calendar":"@lrnwebcomponents/lrn-calendar/lrn-calendar.js","lrn-math":"@lrnwebcomponents/lrn-math/lrn-math.js","lrn-table":"@lrnwebcomponents/lrn-table/lrn-table.js","lrn-vocab":"@lrnwebcomponents/lrn-vocab/lrn-vocab.js","lrndesign-blockquote":"@lrnwebcomponents/lrndesign-blockquote/lrndesign-blockquote.js","magazine-cover":"@lrnwebcomponents/magazine-cover/magazine-cover.js","media-behaviors":"@lrnwebcomponents/media-behaviors/media-behaviors.js","media-image":"@lrnwebcomponents/media-image/media-image.js","meme-maker":"@lrnwebcomponents/meme-maker/meme-maker.js","multiple-choice":"@lrnwebcomponents/multiple-choice/multiple-choice.js","paper-audio-player":"@lrnwebcomponents/paper-audio-player/paper-audio-player.js","person-testimonial":"@lrnwebcomponents/person-testimonial/person-testimonial.js","place-holder":"@lrnwebcomponents/place-holder/place-holder.js","q-r":"@lrnwebcomponents/q-r/q-r.js","full-width-image":"@lrnwebcomponents/full-width-image/full-width-image.js","self-check":"@lrnwebcomponents/self-check/self-check.js","simple-concept-network":"@lrnwebcomponents/simple-concept-network/simple-concept-network.js","stop-note":"@lrnwebcomponents/stop-note/stop-note.js","tab-list":"@lrnwebcomponents/tab-list/tab-list.js","task-list":"@lrnwebcomponents/task-list/task-list.js","video-player":"@lrnwebcomponents/video-player/video-player.js","wave-player":"@lrnwebcomponents/wave-player/wave-player.js","wikipedia-query":"@lrnwebcomponents/wikipedia-query/wikipedia-query.js"}}_haxcmsSiteStore.store.manifest=newValue;this.dispatchEvent(new CustomEvent("json-outline-schema-changed",{bubbles:!0,composed:!0,cancelable:!1,detail:newValue}))}}/**
   ...
   */_themeChanged(newValue,oldValue){if(newValue&&oldValue){if(_haxcmsSiteStore.store.cmsSiteEditor&&_haxcmsSiteStore.store.cmsSiteEditor.instance&&typeof _haxcmsSiteStore.store.cmsSiteEditor.instance.haxCmsSiteEditorElement!==typeof void 0){_haxcmsSiteStore.store.cmsSiteEditor.instance.appendChild(_haxcmsSiteStore.store.cmsSiteEditor.instance.haxCmsSiteEditorElement)}}if(newValue){this.themeLoaded=!1;let theme=newValue;// wipe out what we got
...
if(typeof this.__imported[theme.element]!==typeof void 0){(0,_polymerDom.dom)(this).appendChild(this.themeElement);this.themeLoaded=!0}else{// import the reference to the item dynamically, if we can
try{new Promise((res,rej)=>_require.default([(0,_resolveUrl.pathFromUrl)(decodeURIComponent(meta.url))+"../../../../"+newValue.path],res,rej)).then(e=>{// add it into ourselves so it unpacks and we kick this off!
(0,_polymerDom.dom)(this).appendChild(this.themeElement);this.__imported[theme.element]=theme.element;this.themeLoaded=!0})}catch(err){// error in the event this is a double registration
...
setTimeout(()=>{(0,_elementMixin.updateStyles)()},500)}}}_exports.HAXCMSSiteBuilder=HAXCMSSiteBuilder;window.customElements.define(HAXCMSSiteBuilder.tag,HAXCMSSiteBuilder)});