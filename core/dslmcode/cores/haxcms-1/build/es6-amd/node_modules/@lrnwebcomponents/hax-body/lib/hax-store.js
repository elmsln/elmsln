define(["exports", "meta", "require", "../../../lit-element/lit-element.js", "../../simple-popover/lib/simple-tour.js", "../../utils/utils.js", "../../../mobx/dist/mobx.esm.js", "../../hax-body-behaviors/hax-body-behaviors.js"], function (_exports, meta, _require, _litElement, _simpleTour, _utils, _mobxEsm, _haxBodyBehaviors) {
  ...
  class HaxStore extends (0, _utils.winEventsElement)((0, _haxBodyBehaviors.HAXElement)(_litElement.LitElement)) {
    ...
    insertLogicFromValues(values, context, failOnAnything = false) {
      ...
      let typeName = type; // we want to simplify insert but if we get wildcard... do whatever
      ...
      let haxElements = this.guessGizmo(type, values, false, preferExclusive); // see if we got anything
    ...
    toast(message, duration = 2000, classStyle = "capsule", closeText = null, eventCallback = null) {
    ...
    _globalPreferencesChanged(newValue) {
      ...
      if (this.__storageDataProcessed && newValue && typeof newValue.haxVoiceCommands !== typeof undefined && this.ready) {
        ...
        if (newValue.haxVoiceCommands && !this.__hal) {
          ...
          new Promise((res, rej) => _require.default(["../../hal-9000/hal-9000.js"], res, rej)).then(esModule => {
            ...
            this._initVoiceCommands(); // inject tag into shadowRoot after we import the definition
            ...
            this.__hal.auto = true;
            this.shadowRoot.appendChild(this.__hal); // establish the initial commands, even if they were captured
    ...
    updated(changedProperties) {
      ...
      changedProperties.forEach((oldValue, propName) => {
        ...
        if (["ready", "__appStoreData", "haxAutoloader"].includes(propName) && this.ready && this.__appStoreData && this.haxAutoloader) {
          clearTimeout(this.__readyToProcessAppStoreData);
    ...
    `;
      ...
      setTimeout(() => {
        ...
        if (this.storageData.globalPreferences) {
          this.write("globalPreferences", this.storageData.globalPreferences, this);
        }
      }, 0);
    }
    ...
    _onPaste(e) {
      if (this.editMode && document.activeElement.tagName !== "HAX-TRAY" && document.activeElement.tagName !== "BODY" && document.activeElement.tagName !== "SIMPLE-MODAL") {
        ...
        let pasteContent = "";
        let originalContent = ""; // intercept paste event
        ...
        if (this.isBase64(originalContent)) {
          ...
          return this.retrieveImageFromClipboardAsBlob(e, imageBlob => {
            ...
            if (imageBlob) {
              ...
              let img = document.createElement("img"); // turn blob into a url to visualize locally, this is just temporary

              img.src = URLObj.createObjectURL(imageBlob);
              this.activeNode.parentNode.insertBefore(img, this.activeNode.nextElementSibling);
            ...
            let p = this.activeHaxBody.haxInsert("p", "", {}); // cannot believe this actually works
        ...
        let inlinePaste = false; // the string to import as sanitized by hax

        let newContent = ""; // clear empty span tags that can pop up
        ...
        let haxElements = this.htmlToHaxElements(pasteContent); // if interpretation as HTML fails then let's ignore this whole thing
        ...
        try {
          ...
          let sel = this.getSelection(); // tee up a wrapper so we can walk and put every element in

          let newNodes = document.createElement("div"); // defined so that we can
            ...
            if (inlinePaste) {
              ...
              setTimeout(() => {
                if (activeEl && activeEl.childNodes && activeEl.childNodes[0]) {
                  this._positionCursorInNode(activeEl.childNodes[0], activeEl.childNodes[0].length);
    ...
    constructor() {
      ...
      this.voiceDebug = false; // keyboard shortcuts, implementing haxHook: gizmoRegistration can ovewrite these as needed
      ...
      this._isSandboxed = typeof test.reload === "function"; // imports app, stax definitions
      ...
      (0, _mobxEsm.autorun)(() => {
        this._globalPreferencesChanged((0, _mobxEsm.toJS)(this.globalPreferences));
      });
      (0, _mobxEsm.autorun)(() => {
        this._editModeChanged((0, _mobxEsm.toJS)(this.editMode));
      });
    }
    /**
    ...
    _haxStoreInsertContent(e) {
      if (e.detail) {
        ...
        if (window.customElements.get(details.tag)) {
          let prototypeNode = document.createElement(details.tag); // @see haxHooks: preProcessInsertContent
        ...
        if (typeof details.__type !== typeof undefined && details.__type === "inline") {
          let node = (0, _utils.haxElementToNode)({
            tag: details.tag,
    ...
    _haxStoreRegisterProperties(e) {
      if (e.detail && e.detail.properties && e.detail.tag) {
        ...
        if (!this.elementList[e.detail.tag]) {
          ...
          if (e.detail.properties.type == "grid" && !this.validGridTagList.find(element => {
            return element === e.detail.tag;
          })) {
    ...
    get activeGizmo() {
      let gizmo = (0, _mobxEsm.toJS)(this._calculateActiveGizmo(this.activeNode));
  ...
  const HAXStore = window.HaxStore.requestAvailability(); // debugging / developer console shortcuts
