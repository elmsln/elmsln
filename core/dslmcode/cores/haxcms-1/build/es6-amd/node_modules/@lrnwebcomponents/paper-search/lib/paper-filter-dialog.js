define(["exports","../../../@polymer/polymer/polymer-element.js","../../../@polymer/polymer/lib/legacy/polymer.dom.js"],function(_exports,_polymerElement,_polymerDom){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.PaperFilterDialog=void 0;class PaperFilterDialog extends _polymerElement.PolymerElement{static get tag(){return"paper-filter-dialog"}/**
...
/**
   ...
   */open(){// Attach dialog to the body to ensure it's on top of all existing overlays
...
this._selectedFilters=Object.assign({},this.selectedFilters)}.bind(this),1)}close(){this.$.dialog.close()}/**
   * Handles if the user taps on a filter
   */_tapSelectFilter(e){this.$.selector.select(e.model.filter);this._preselectFilterValues()}/**
   * Separate function for unit testing
   */_preselectFilterValues(){// Check all values that are selected
var selectedValueIds=this._selectedFilters[this._selectedFilter.id],isSelected=function(value){return!!selectedValueIds&&0<=selectedValueIds.indexOf(value.id)};this._selectedFilterValues=this._selectedFilter.values.map(function(value){return Object.assign({},value,{selected:isSelected(value)})})}_tapReset(e){this._selectedFilters={}}_tapApply(e){this.selectedFilters=this._selectedFilters;this.dispatchEvent(new CustomEvent("save",{bubbles:!0,cancelable:!0,composed:!0,detail:!0}))}_tapSelectValues(e){// Captured IDs of the selected items
const selectedValues=this._selectedFilterValues.filter(function(value){return value.selected}).map(function(value){return value.id});this._selectedFilters=Object.assign({},this._selectedFilters,{[this._selectedFilter.id]:selectedValues});this.$.selector.deselect(this._selectedFilter)}/**
   ...
   */_hasSelectedFilters(selectedFilters){// Iterate until we find a filter that is selected
...
_getSelectedValuesNames(filter,_selectedFilters){var selectedValueIds=_selectedFilters[filter.id];if(!filter.values||!selectedValueIds){return this.noValuesLabel}// Capture names of all selected values
...
return value.name});return 0<names.length?names.join(", "):this.noValuesLabel}}_exports.PaperFilterDialog=PaperFilterDialog;window.customElements.define(PaperFilterDialog.tag,PaperFilterDialog)});