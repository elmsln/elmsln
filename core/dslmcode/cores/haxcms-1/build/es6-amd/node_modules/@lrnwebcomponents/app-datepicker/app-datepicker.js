...
*/let AppDatepicker=(0,_polymerLegacy.Polymer)({_template:_polymerLegacy.html`
    <style>
      :host {
        display: block;
        position: relative;
  ...
  `,is:"app-datepicker",behaviors:[_neonAnimationRunnerBehavior.NeonAnimationRunnerBehavior],properties:{/**
     * Set locale for the datepicker.
     */locale:{type:String,value:function(){if(window.Intl){return window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US"}return"en-US"}},/**
...
let effectiveChildren=_flattenedNodesObserver.FlattenedNodesObserver.getFlattenedNodes(this).filter(n=>n.nodeType===Node.ELEMENT_NODE);if(effectiveChildren&&0<effectiveChildren.length){for(var i=0;i<effectiveChildren.length;i++){// addEventListener to paper-button with dialog-confirm.
...
_computeDaysOfMonth:function(_activeYear,_activeMonth,_firstDayOfWeek,_locale){// No op for Intl is undefined (for Safari below v10.x.x).
...
_computeShiftedDisableDays:function(_firstDayOfWeek,_isDisableDays){_firstDayOfWeek=0<_firstDayOfWeek&&7>_firstDayOfWeek?_firstDayOfWeek:0;var _sdd=this.disableDays.map(function(_day){_day=_day-_firstDayOfWeek;return 0>_day?7+_day:_day});this.set("_shiftedDisableDays",_sdd)},// Increment the month via user interaction.
_incrementMonth:function(ev){this.debounce("_incrementMonth",function(){this.set("_isIncrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0===++_month%12){this._activeYear++}this.set("_activeMonth",_month%12);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("incrementEntry")}this.set("_isIncrementMonth",!1)}.bind(this))},100)},// Decrement the month via user interaction.
_decrementMonth:function(ev){this.debounce("_decrementMonth",function(){this.set("_isDecrementMonth",!0);window.requestAnimationFrame(function(){var _month=this._activeMonth;if(0>--_month){this._activeYear--;_month=11}this.set("_activeMonth",_month);if(!this.noAnimation){this.cancelAnimation();this.playAnimation("decrementEntry")}this.set("_isDecrementMonth",!1)}.bind(this))},100)},// Re-compute active month year if new date is selected.
_computeActiveMonthYear:function(_activeYear,_activeMonth,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _amy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",month:"short",year:"numeric"}).format(Date.UTC(_activeYear,_activeMonth,1));this.set("_activeMonthYear",_amy)}},// Re-compute short version of full date to show on the picker.
_computeShortSelectedDate:function(_selectedYear,_selectedMonth,_selectedDate,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssd=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",weekday:"short",month:"short",day:"numeric"}).format(Date.UTC(_selectedYear,_selectedMonth,_selectedDate));this.set("_shortSelectedDate",_ssd);// When datepicker has no button for date confirmation,
// by asserting `autoUpdateDate` will trigger auto update on
// date change.
if(this.autoUpdateDate){this.enforceDateChange()}}},_computeShowSelectedYear:function(_selectedYear,_locale){if(window.Intl){_locale=_locale||window.Intl&&window.Intl.DateTimeFormat&&window.Intl.DateTimeFormat().resolvedOptions&&window.Intl.DateTimeFormat().resolvedOptions().locale||"en-US";var _ssy=new window.Intl.DateTimeFormat(_locale,{timeZone:"UTC",year:"numeric"}).format(Date.UTC(_selectedYear,0,1));this.set("_showSelectedYear",_ssy)}},// When a day of month is selected, a lot of tasks need to be carried out.
_chooseDaysOfMonth:function(ev){var _target=ev.target;// daysOfMonth is chooseable when:
...
_isToday:function(_item,_activeYear,_activeMonth){var _now=new Date,_isToday=_item===_now.getDate()&&_activeYear===_now.getFullYear()&&_activeMonth===_now.getMonth();return _isToday?" is-today":""},// Re-compute class when the item (day of month) is empty date & non-selectable.
_isEmptyDate:function(_item){return this._isNumber(_item)?"":" is-non-selectable"},// Re-compute class if the item (day of month) is the selected date.
_isChosenDaysOfMonth:function(_item,_selectedYear,_selectedMonth,_selectedDate){// able to retain selected daysOfMonth even after navigating to other months.
...
_isDisableDays:function(_index,_firstDayOfWeek,_minDate,_maxDate,_item){var _isLessThanMinDate=!1,_isMoreThanMaxDate=!1,_isDisableDay=!1,_isDisableDates=!1,_isDisableDays=this._shiftedDisableDays.some(function(_n){return _n===_index%7});// ------ < _minDate ---------------- _maxDate > ------
...
_isListOfYearsSelected:function(selectedYear,year){return+selectedYear===+year?" is-selected":""},_computeDaysOfWeek:function(_firstDayOfWeek,_locale){// _daysOfWeek needs to be changed as well with firstDayOfWeek and locale.
...
_computeChosenDaysOfMonth:function(_daysOfMonth,_selectedDate){// depends on _daysOfMonth and recalculate the index of _chosenDaysOfMonth.
...
_convertDateStringToDateObject:function(_date){var _checkDate=_date instanceof Date||"string"!==typeof _date?_date:new Date(_date),_isValidDate="Invalid Date"!==_checkDate.toDateString();return _isValidDate?_checkDate:null},// Dynamically load items into the iron-list when switching page to year view.
_updateList:function(_activeView){for(var _newList=[],y=1900,i=y;2100>=i;i++){_newList.push({year:i})}this.set("_listOfYears",_newList)},// On neon-animaition-finish, udpate scroller position of the iron-list.
// It is to fix the iron-list so that the selected year will always be centered by
// hardcoded with fixed length of #items.
_onAnimationFinish:function(ev){var _target=ev.detail;// TO fix the scrolling of iron-list, hard coded with fixed length of #items.
...
async.microTask.run(()=>{_target.toPage._focusPhysicalItem(_focusableItem)})}else{// When user navigate to calendar view from list view,
// re-focus showSelectedYear.
async.microTask.run(()=>{this.$.showSelectedYear.focus()})}},_updateListScroller:function(_list){var _sl=(0,_polymerDom.dom)(_list.root).querySelector("#items"),_slh=_sl.getBoundingClientRect().height||12863.994140625,_sli=Math.floor(_slh/(2100-1900+1)*(this._activeYear-1900-2))+1;// Adjust center position when datepickers has no buttons and in landscape mode.
...
async.microTask.run(()=>{_list.scroll(0,_sli);// Select initial item for the list.
_list.selectItem(this._activeYear-1900)});// wait for at least 17ms to update the scroller position.
...
_onIronSelectorSelectedChanged:function(ev){if("year"===ev.detail.value){if(!this._isListRendered){this._updateList();this.set("_isListRendered",!0)}else{// When noAnimation is truthy, this will update the list scroller.
if(this.noAnimation){this._updateListScroller(this.shadowRoot.querySelector("#listOfYears"))}}}},_onListRendered:function(ev){// When noAnimation is truthy, this will update the list scroller.
if(ev.target.if&&this.noAnimation){async.microTask.run(()=>{this._updateListScroller(this.shadowRoot.querySelector("#listOfYears"))})}},_goCalendar:function(ev){// Listen for enter key from keyboard.
...
_computeSeparateFormat:function(_format){var re=/^(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)\W+(yyyy|yy|m{1,4}|d{1,2}|do)$/g,m=re.exec(_format),_temp={},_tempArr=[];if(null!==m){_tempArr.push(m[1]);_tempArr.push(m[2]);_tempArr.push(m[3]);for(var i=0,matched;3>i;i++){matched=_tempArr[i];if(0<=matched.indexOf("y")){_temp.y=matched}else if(0<=matched.indexOf("m")){_temp.m=matched}else if(0<=matched.indexOf("d")){_temp.d=matched}}}// Only set _format if the new format is valid.
...
_bindSelectedFulldate:function(_selectedYear,_selectedMonth,_selectedDate,_format){// show long date (eg. Fri, May 12 2017 instead.
...
_updateToReflectExternalChange:function(_inputDate){// Due to limitation of input date received by platform's Date,
...
_updateThemeColor:function(_theme){var _themes=["dark-theme","light-theme","goog-theme"],_themeIdx=_themes.indexOf(_theme),_distributedButtons=(0,_polymerDom.dom)(this).querySelectorAll("paper-button"),_distributedButtonsLen=_distributedButtons.length,_colorCode=["#bcbcbc","#737373","#616161"][_themeIdx];if(0<=_themeIdx){_themes.splice(_themeIdx,1);this.toggleClass(_themes[0],!1,this);this.toggleClass(_themes[1],!1,this);this.toggleClass(_theme,!0,this)}else{this.toggleClass("dark-theme",!1,this);this.toggleClass("light-theme",!1,this);this.toggleClass("goog-theme",!1,this)}// workaround to update custom property of distributed buttons.
...
_updateDatepickerView:function(_view){this.toggleClass("horizontal-view","horizontal"===_view,this);this.toggleClass("vertical-view","vertical"===_view,this)},// workaround to update custom property of distributed children until Polymer supports Native custom properties.
_updateDistributedButtonInkColorCustomProp:function(_node,_colorCode){this.updateStyles({"--paper-button-ink-color":_colorCode})}// TODO: To add GIF to README.md.
