...
static get properties(){return{/**
   ...
   */ready(){super.ready();const iconSets=new _ironMeta.IronMeta({type:"iconset"});let temp=[],root=this;// need to access iconset imperatively now
if(typeof iconSets!==typeof void 0&&iconSets.list&&iconSets.list.length){var index=0;iconSets.list.forEach(function(item){let name=item.name;if(!root._hideIconset(name)){temp.push({name:name,icons:[]});item.getIconNames().forEach(icon=>{temp[index].icons.push(icon)});index++}})}this.__iconList=temp}/**
   ...
   */_hideIconset(name){console.log("_hideIconset",name,this.includeSets,this.excludeSets);let isets=null!==this.includeSets?this.includeSets.split(/ /):[],included=0===isets.length||isets.includes(name),esets=null!==this.excludeSets?this.excludeSets.split(/ /):[],excluded=0<esets.length.length&&esets.includes(name);return!included||excluded}/**
