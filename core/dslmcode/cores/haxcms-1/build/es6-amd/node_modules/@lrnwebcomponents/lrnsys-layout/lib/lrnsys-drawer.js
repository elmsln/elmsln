...
let nodes=(0,_polymerDom.dom)(this).getEffectiveChildNodes(),h=document.createElement("span"),c=document.createElement("span");for(var i in nodes){if(typeof nodes[i].tagName!==typeof void 0){switch(nodes[i].getAttribute("slot")){case"header":let tmp=nodes[i].cloneNode(!0);tmp.removeAttribute("slot");h.appendChild(tmp);break;case"button":// do nothing
...
classes.forEach((item,index)=>{if(""!=item){if(this.focusState){this.$.flyouttrigger.classList.add(item)}else{this.$.flyouttrigger.classList.remove(item)}}})}this.focusState=!this.focusState}/**
   ...
   */_getTextLabelClass(){if(!this.avatar&&!this.icon){return"text-label-only"}return"text-label"}}_exports.LrnsysDrawer=LrnsysDrawer;window.customElements.define(LrnsysDrawer.tag,LrnsysDrawer)});