define(["exports","require","../../../@polymer/polymer/polymer-element.js","../../../@polymer/polymer/lib/legacy/polymer.dom.js","../../../@polymer/polymer/lib/utils/render-status.js","../../../@polymer/app-localize-behavior/app-localize-behavior.js","../../../@polymer/polymer/lib/legacy/class.js","../../../@polymer/iron-flex-layout/iron-flex-layout-classes.js"],function(_exports,_require,_polymerElement,_polymerDom,_renderStatus,_appLocalizeBehavior,_class,_ironFlexLayoutClasses){Object.defineProperty(_exports,"__esModule",{value:!0}),_exports.EcoJsonSchemaArray=void 0,_require=babelHelpers.interopRequireWildcard(_require);class EcoJsonSchemaArray extends((0,_class.mixinBehaviors)([_appLocalizeBehavior.AppLocalizeBehavior],_polymerElement.PolymerElement)){static get tag(){return"eco-json-schema-array"}constructor(){super(),new Promise((res,rej)=>_require.default(["../../../@polymer/iron-icons/iron-icons.js"],res,rej)),new Promise((res,rej)=>_require.default(["../../../@polymer/iron-icons/editor-icons.js"],res,rej)),new Promise((res,rej)=>_require.default(["../../../@polymer/paper-icon-button/paper-icon-button.js"],res,rej)),new Promise((res,rej)=>_require.default(["../../a11y-collapse/a11y-collapse.js"],res,rej)),new Promise((res,rej)=>_require.default(["../../a11y-collapse/lib/a11y-collapse-group.js"],res,rej))}static get template(){return _polymerElement.html`
    ...
    `}static get properties(){return{globalOptions:{type:Object,value:{icon:"settings",tooltip:"configure item"},notify:!0},propertyName:{type:String,value:null},schema:{type:Object,value:{},notify:!0,observer:"_schemaChanged"},__headings:{type:Array,value:[],notify:!0}}}ready(){super.ready(),this.__headings=[],this._schemaChanged(),this.addEventListener("form-field-changed",e=>{this._updateHeadings(e)})}disconnectedCallback(){this.removeEventListener("form-field-changed",e=>{this._updateHeadings(e)}),super.disconnectedCallback()}_schemaChanged(){(0,_renderStatus.afterNextRender)(this,()=>{let itemLabel=this.schema.items.itemLabel;this.schema.value&&this.schema.value.forEach(val=>{this.push("__headings",val[itemLabel])}),this.shadowRoot.querySelectorAll(".item-fields").forEach(item=>{let index=item.getAttribute("data-index"),propertyName=`${this.propertyPrefix}${this.propertyName}`,prefix=`${propertyName}.${index}`,val=this.schema.value[index];this.dispatchEvent(new CustomEvent("build-fieldset",{bubbles:!1,cancelable:!0,composed:!0,detail:{container:item,path:propertyName,prefix:prefix,properties:this.schema.properties.map(prop=>{let newprop=JSON.parse(JSON.stringify(prop));return newprop.value=val[prop.name],newprop}),type:EcoJsonSchemaArray.tag,value:this.schema.value||[]}}))})})}_onAddItem(e){let val={};this.schema.properties.forEach(prop=>{val[prop.name]=prop.value}),this.push("schema.value",val),this.notifyPath("schema.value.*"),this._schemaChanged()}_onRemoveItem(e){let index=e.target.controls.replace(/item-/,"");this.splice("schema.value",index,1),this.notifyPath("schema.value.*"),this._schemaChanged()}_updateHeadings(e){let propname=e.detail.getAttribute("name"),val=e.detail.value,pathArr=propname?propname.split("."):[],index=pathArr.length>2?pathArr[pathArr.length-2]:null,update=e.detail.propertyName===this.schema.items.itemLabel?val:this.__headings[index];index&&this.set(`__headings.${index}`,update)}_getHeading(headings,label,index){return this.__headings&&this.__headings[index]&&"string"==typeof this.__headings[index]&&""!==this.__headings[index].trim("")?this.__headings[index].trim(""):label&&"string"==typeof label&&""!==label.trim("")?`${label.trim("")} ${index+1}`:`Item ${index+1}`}}_exports.EcoJsonSchemaArray=EcoJsonSchemaArray,window.customElements.define(EcoJsonSchemaArray.tag,EcoJsonSchemaArray)});