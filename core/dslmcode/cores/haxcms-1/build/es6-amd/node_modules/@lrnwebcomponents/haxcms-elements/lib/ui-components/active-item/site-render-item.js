 ...
 */class SiteRenderItem extends _polymerElement.PolymerElement{/**
   ...
   */static get tag(){return"site-render-item"}// render function
static get template(){return _polymerElement.html`
    ...
    `}/**
   * Props
   */static get properties(){return{render:{type:Boolean,value:!1},itemId:{type:String},location:{type:String,computed:"_computeLocation(itemId, render)",observer:"locationChanged"},/**
       * item content
       */itemContent:{type:String,notify:!0,observer:"_itemContentChanged"}}}_computeLocation(itemId,render){if(itemId&&render){let item=_haxcmsSiteStore.store.findItem(itemId);// ensure it loads fresh
return item.location+"?"+Math.floor(Date.now()/1e3)}}locationChanged(newValue){if(newValue){this.$.content.generateRequest()}}/**
   * React to content being loaded from a page.
   */_itemContentChanged(newValue,oldValue){if(newValue){var html=newValue;// only append if not empty
...
}).catch(error=>{/* Error handling */console.log(error)})}}}}}}connectedCallback(){super.connectedCallback();this.__disposer=[];(0,_mobxModule.autorun)(reaction=>{this.manifest=(0,_mobxModule.toJS)(_haxcmsSiteStore.store.manifest);this.__disposer.push(reaction)})}disconnectedCallback(){for(var i in this.__disposer){this.__disposer[i]}super.disconnectedCallback()}}_exports.SiteRenderItem=SiteRenderItem;window.customElements.define(SiteRenderItem.tag,SiteRenderItem)});