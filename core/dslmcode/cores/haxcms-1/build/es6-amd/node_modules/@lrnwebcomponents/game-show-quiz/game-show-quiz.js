    ...
    submitAnswer(e) {
      ...
      if (!this.__activeQuestionDetails.isBonus) {
        ...
        if (unlockCheck === unlockThreashold) {
          this.shadowRoot.querySelectorAll('responsive-grid-col button[data-type="bonus"][data-display-points="' + this.__activeQuestionDetails.points + '"]').forEach(item => {
      ...
      if (this.points[this.__activeType].attempted == 11 && !this.__activeQuestionDetails.isBonus) {
        // get last row
        this.shadowRoot.querySelectorAll('responsive-grid-col button[data-is-bonus][data-type="' + this.__activeType + '"]').forEach(item => {
          item.removeAttribute("disabled");
          let uuid = item.getAttribute("data-question-uuid"); // bonus always last row, make data match the operation
      ...
      if (this.remainingAttempts === 2) {
        this.shadowRoot.querySelectorAll("responsive-grid-col button[data-value='3']:not([disabled]):not([data-is-bonus])").forEach(item => {
          item.setAttribute("disabled", "disabled");
        });
      }

      if (this.remainingAttempts === 1) {
        this.shadowRoot.querySelectorAll("responsive-grid-col button[data-value='2']:not([disabled]):not([data-is-bonus])").forEach(item => {
          item.setAttribute("disabled", "disabled");
        });
        this.shadowRoot.querySelectorAll('responsive-grid-col button[data-value="3"]:not([disabled]):not([data-is-bonus])').forEach(item => {
          item.setAttribute("disabled", "disabled");
      ...
      if (this.remainingAttempts <= 0) {
        this.shadowRoot.querySelectorAll("responsive-grid-col button:not([disabled]):not([data-is-bonus])").forEach(item => {
          item.setAttribute("disabled", "disabled");
        });
