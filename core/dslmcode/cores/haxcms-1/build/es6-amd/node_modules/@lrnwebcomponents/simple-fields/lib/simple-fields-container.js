define(["exports","../../../lit-element/lit-element.js"],(function(e,t){"use strict";Object.defineProperty(e,"__esModule",{value:!0}),e.SimpleFieldsContainer=void 0;class SimpleFieldsContainer extends t.LitElement{static get tag(){return"simple-fields-container"}static get styles(){return[t.css`
        :host {
          display: block;
          width: 100%;
    ...
    `}get fieldId(){return`${this.id||"field"}.input`}get fieldMainTemplate(){return t.html`
      <div
    ...
    `}get fieldMeta(){return t.html`
      <div id="fieldmeta" aria-live="polite">
        <slot name="field-meta"></slot>
      </div>
    `}get labelTemplate(){return t.html`
      <label
        for="${this.fieldId}"
        class="label-main"
    ...
    `}get numberError(){let e="fieldset"===this.type&&this.field.querySelector("input[type=checkbox]"),t=!!this._getFieldValue()&&this._getFieldValue().length,i="select"===this.type?this.min:!!e&&this.minchecked,s="select"===this.type?this.max:!!e&&this.maxchecked;return s&&t&&s<t?s-t:!!(i&&t&&i>t)&&i-t}get numeric(){return["date","month","week","time","datetime-local","number","range"].includes(this.type)}get patternError(){return this.pattern&&""!==this.pattern&&this._getFieldValue()&&(this.field.multiple?this._getFieldValue().filter(e=>!e.match(this.pattern)):!this._getFieldValue().match(this.pattern))}get prefixTemplate(){return t.html`
      <slot name="prefix"></slot>
      ${this.prefix}
    `}get requiredError(){return!this._getFieldValue()&&this.required}get slottedFieldObserver(){return new MutationObserver(this._updateField)}get suffixTemplate(){return t.html`
      <slot name="suffix"></slot>
      ${this.suffix}
    `}focus(){this.field&&this.field.focus()}select(){!this.field||"text"!==this.type&&"textarea"!==this.type||this.field.select()}setRangeText(e,t,i,s){!this.field||"text"!==this.type&&"textarea"!==this.type||this.field.setRangeText(e,t,i,s)}setSelectionRange(e,t,i){!this.field||"text"!==this.type&&"textarea"!==this.type||this.field.setSelectionRange(e,t,i)}stepDown(e=1){this.field&&this.numeric&&this.field.stepDown()}stepUp(e=1){this.field&&this.numeric&&this.field.stepUp()}validate(){let e=this.field.querySelector("legend");this.requiredError?(this.error=!0,this.errorMessage=this.requiredMessage||"required"):this.numberError?(this.error=!0,this.errorMessage=this.numberMessage||(this.numberError>0?`select ${this.numberError} more`:`select ${0-this.numberError} fewer`)):this.patternError&&(this.error=!0,this.errorMessage=this.patternMessage||"invalid format"),"fieldset"===this.type&&e&&(e.innerHTML=e.innerHTML.replace(/\**\s*$/,this.error?"*":""),e.style.color=this.error?"var(--simple-fields-error-color, #dd2c00)":"")}_fireErrorChanged(){this.dispatchEvent(new CustomEvent("error-changed",{bubbles:!0,cancelable:!0,composed:!0,detail:this}))}_generateUUID(){return"ss-s-s-s-sss".replace(/s/g,Math.floor(65536*(1+Math.random())).toString(16).substring(1))}_getFieldValue(){let e,t;return this.field&&("fieldset"===this.type&&this.field.querySelector("input[type=radio]")?(e=this.field.querySelector("input:checked"),t=e?e.value:void 0):"fieldset"===this.type&&this.field.querySelector("input[type=checkbox]")?(t=[],e=this.field.querySelectorAll("input:checked"),e.forEach(e=>t.push(e.value))):t="checkbox"===this.type?this.field.checked?this.field.value||!0:void 0:"radio"===this.type?this.field.checked?this.field.value||!0:void 0:"select"===this.type?this.multiple?Object.keys(this.field.selectedOptions).map(e=>this.field.selectedOptions[e].value):this.field.selectedOptions[0].value:this.field.value),t}_getValidType(e){return"datetime"===e&&this.validTypes.includes(e)?"datetime-local":this.validTypes.includes(e)?e:"text"}_handleFieldChange(){"text"!==this.type&&"textarea"!==this.type||this._updateCount(),this.autovalidate&&this.validate(),"textarea"===this.type&&this.autoGrow()}_observeAndListen(e=!0){e?(this.slottedFieldObserver.observe(this,{attributeFilter:["disabled","readonly","required","slot"],childlist:!0}),this._updateField(),this.addEventListener("click",this.focus),this.addEventListener("focusout",this._onFocusout),this.addEventListener("focusin",this._onFocusin)):(this.slottedFieldObserver.disconnect(),this.removeEventListener("click",this.focus),this.removeEventListener("focusout",this._onFocusout),this.removeEventListener("focusin",this._onFocusin))}_onFocusin(){this.error=!1}_onFocusout(){this.autovalidate&&this.validate()}_updateField(){let e=this.field;if(this.field=this.querySelector&&this.querySelector("[slot=field]")?this.querySelector("[slot=field]"):void 0,this.id=`${this.fieldId||""}-wrapper`,this.field){let e=this.field.tagName.toLowerCase(),t=this.field.getAttribute("type")||"text";if(this.type=this._getValidType("input"===e?t:e),this.required=this.field.required,this.disabled=this.field.disabled,this.readonly=this.field.readonly,this.field.setAttribute("aria-describedby","field-bottom"),this.field.addEventListener("change",e=>this._handleFieldChange()),this.field.addEventListener("input",e=>this._handleFieldChange()),"select"===this.type&&(this.multiple=this.field.multiple),"textarea"===this.type&&(this.field.getAttribute("rows")||this.field.setAttribute("rows",1),this.field.addEventListener("keydown",e=>e.stopPropagation())),"fieldset"===this.type){let e=this.querySelector("legend");e&&(e.style.fontSize="var(--simple-fields-detail-font-size, 12px)",e.style.fontFamily="var(--simple-fields-detail-font-family, sans-serif)",e.style.lineHeight="var(--simple-fields-detail-line-height, 22px)",e.style.paddingInlineStart=0,e.style.paddingInlineEnd=0),this.querySelectorAll("label, input").forEach(e=>e.style.marginRight="var(--simple-fields-margin, 16px)"),this.querySelectorAll("label input").forEach(e=>e.style.marginLeft="calc(0 - var(--simple-fields-margin, 16px))")}}else this.disabled=!1,this.readonly=!1,this.required=!1,this.type=void 0,e&&("textarea"===e.tagName.toLowerCase()&&e.addEventListener("keydown",e=>e.stopPropagation()),e.removeEventListener("change",e=>this._handleFieldChange()),e.removeEventListener("input",e=>this._handleFieldChange()))}_updateCount(){let e="",t=new RegExp("[\\w\\-\\']+","gim"),i=this.field.getAttribute("maxlength")||this.maxlength||!1,s=this.maxwords||!1,l="word"===this.counter?s:i,r=new RegExp(`.{0,${i||1}}`,"g"),o=new RegExp(`([\\w\\-\\']+\\W*){0,${s||1}}`,"g"),length=()=>this.field.value?this.field.value.length:0,wordlength=()=>this.field.value&&this.field.value.match(t)?this.field.value.match(t).length:0,correctLength=(e,t,i)=>{e&&t&&t<e&&this.field.value.match(i)&&(this.field.value=this.field.value.match(i)[0].trim())};correctLength(length(),i,r),correctLength(wordlength(),s,o),e="word"===this.counter?wordlength():length(),"none"!==this.counter&&this.shadowRoot&&this.shadowRoot.querySelector("#fieldmeta")&&(this.shadowRoot.querySelector("#fieldmeta").innerHTML=l?`${e}/${l}`:e)}}e.SimpleFieldsContainer=SimpleFieldsContainer,window.customElements.define(SimpleFieldsContainer.tag,SimpleFieldsContainer)}));