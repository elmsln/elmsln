define(["exports","../../lit-element/lit-element.js","../../@polymer/paper-input/paper-input.js"],(function(t,e,a){"use strict";Object.defineProperty(t,"__esModule",{value:!0}),t.PunnettSquare=void 0;
/**
   * Copyright 2019 PSU
   * @license Apache-2.0, see License.md for full text.
   */
class PunnettSquare extends e.LitElement{static get tag(){return"punnett-square"}static get properties(){return{variant1:{type:String},variant2:{type:String},editable:{type:Boolean}}}constructor(){super(),this.variant1="",this.variant2="",this.editable=!1}connectedCallback(){super.connectedCallback()}__ensureArray(t){return("string"==typeof t?t.split(",").map(t=>t.trim()):[]).map(t=>"string"==typeof t?t.split("").map(t=>t.trim()):[])}render(){let t=this.__ensureArray(this.variant1),a=this.__ensureArray(this.variant2);return e.html`
      <style>
        :host {
          display: block;
          --punnett-square-table-border: 1px solid black;
          --punnett-square-table-padding: 1em;
          --punnett-square-input-font-size: 2em;
          --punnett-square-input-text-align: center;
        }

        table {
          border-collapse: var(--punnett-square-table-border-collapse, collapse);
          width: var(--punnett-square-table-width, 100%);
          border: var(--punnett-square-table-border);
        }

        th {
          border: var(--punnett-square-th-border, var(--punnett-square-table-border));
          padding: var(--punnett-square-th-padding, var(--punnett-square-table-padding));
        }

        td {
          border: var(--punnett-square-td-border, var(--punnett-square-table-border));
          text-align: var(--punnett-square-table-text-align, center);
          padding: var(--punnett-square-td-padding, var(--punnett-square-table-padding));
        }

        #inputs {
          display: var(--punnett-square-inputs-display, flex);
          align-items: var(--punnett-square-inputs-align-items, center);
        }

        .input {
          font-size: var(--punnett-square-input-font-size);
          text-align: var(--punnett-square-input-text-align);
        }

        paper-input {
          --paper-input-container-input: {
            font-size: var(--punnett-square-paper-input-font-size, --punnett-square-input-font-size);
            text-align: var(--punnett-square-paper-input-text-align, --punnett-square-input-text-align);
          }
        }
      </style>
      <div id="inputs">
        Variant1: <div class="input">${this.editable?e.html`
                <paper-input
                  value="${this.variant1}"
                  @value-changed=${t=>this.variant1=t.detail.value}
                ></paper-input>
              `:e.html`
                ${this.variant1}
              `}</div>
        Variant2: <div class="input">${this.editable?e.html`
                <paper-input
                  value="${this.variant2}"
                  @value-changed=${t=>this.variant2=t.detail.value}
                ></paper-input>
              `:e.html`
                ${this.variant2}
              `}</div>
      </div>
      <table>
        <thead>
          <tr>
            <th></th>
            ${t.map(t=>e.html`
                  ${t.map(t=>e.html`
                        <th>${t}</th>
                      `)}
                `)}
          </tr>
        <tbody>
          ${a.map((a,n)=>a.map(a=>e.html`
                  <tr>
                    <th>${a}</th>
                    ${t.map(t=>t.map(t=>e.html`
                            <td>${t}${a}</td>
                          `))}
                  </tr>
                `))}
        </tbody>
        </thead>
      </table>
    `}}t.PunnettSquare=PunnettSquare,customElements.define(PunnettSquare.tag,PunnettSquare)}));