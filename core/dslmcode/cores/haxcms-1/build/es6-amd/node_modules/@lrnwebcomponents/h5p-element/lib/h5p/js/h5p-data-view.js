...
var H5PDataView = function ($) {
  ...
  H5PDataView.prototype.loadData = function () {
    var self = this; // Throbb

    self.setMessage(H5PUtils.throbber(self.l10n.loading)); // Create URL

    var url = self.source;
    url += (url.indexOf("?") === -1 ? "?" : "&") + "offset=" + self.offset + "&limit=" + self.limit; // Add sorting
    ...
    $.ajax({
      ...
      // Error handling
      self.setMessage($("<p/>", {
        text: self.l10n.ajaxFailed
      }));
    }).done(function (data) {
      if (!data.rows.length) {
        self.setMessage($("<p/>", {
          text: filtering ? self.l10n.noData : self.l10n.empty
        }));
      } else {
        // Update table data
  ...
  H5PDataView.prototype.updatePagination = function (num) {
    ...
    if (self.pagination === undefined) {
      ...
      var $pagerContainer = $("<div/>", {
        class: "h5p-pagination"
      });
