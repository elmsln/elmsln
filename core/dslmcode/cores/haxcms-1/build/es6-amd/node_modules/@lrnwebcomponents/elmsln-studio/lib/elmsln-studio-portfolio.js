define(["exports", "../../../lit-element/lit-element.js", "./elmsln-studio-styles.js", "./elmsln-studio-utilities.js", "../../img-view-modal/img-view-modal.js", "../../hax-iconset/hax-iconset.js", "../../threaded-discussion/threaded-discussion.js"], function (_exports, _litElement, _elmslnStudioStyles, _elmslnStudioUtilities, _imgViewModal, _haxIconset, _threadedDiscussion) {
  "use strict";
  ...
  class ElmslnStudioPortfolio extends (0, _elmslnStudioUtilities.ElmslnStudioUtilities)((0, _elmslnStudioStyles.ElmslnStudioStyles)(_litElement.LitElement)) {
    ...
    render() {
      ...
      <div id="breadcrumb">
        ...
        <span> > </span>
        ${!this.portfolio ? `` : (0, _litElement.html)`
                ${!this.portfolio.projectId || !this.portfolio.project ? `` : (0, _litElement.html)`
                      <elmsln-studio-link
                    ...
                    `}
              `}
        ${!this.portfolio || !this.assignment ? (0, _litElement.html)`<em>Assignment</em>` : (0, _litElement.html)`
                <elmsln-studio-link
        ...
        <span> > </span>
        <span
          >${!this.portfolio ? (0, _litElement.html)`<em>Student</em>` : this.fullName(this.portfolio)}</span
        >
      </div>
      <div
        id="primary"
      ...
      >
        <article id="portfolio-project">
          <h1>
            <lrndesign-avatar
              accent-color="${!this.portfolio ? "grey" : this.accentColor(this.fullName(this.portfolio))}"
              aria-hidden="true"
              label="${!this.portfolio ? "undefined" : this.fullName(this.portfolio)}"
              .src="${this.portfolio && this.portfolio.avatar ? this.portfolio.avatar : undefined}"
              two-chars
            >
            </lrndesign-avatar>
            <span class="student-name"
              >${!this.portfolio ? "Loading..." : this.fullName(this.portfolio)}</span
            >
            ${!this.portfolio || !this.portfolio.project ? `` : (0, _litElement.html)`
                    <span class="project-name">${this.portfolio.project}</span>
                  `}
          </h1>
          ${!this.portfolio ? this.loading("grey") : (0, _litElement.html)`
            ...
            <article id="portfolio-project">
              ...
              <div id="project-buttons">
                ...
                >
                  ...
                  <span class="sr-only"
                    >Sort Submissions
                    ${!this.sortLatest ? "Oldest First" : "Newest First"}</simple-tooltip
                  >
                </div>
                ${this.sortedSubmissions.map(s => (0, _litElement.html)`
                    <section
                    ...
                    >
                      ...
                      <div class="submission-body">
                        ${s.links && s.links.length > 0 ? (0, _litElement.html)`
                            ...
                            ` : !s.sources || s.sources.length === 0 ? (0, _litElement.html)` ${s.body} ` : this.getThumnailGrid(s)}
                      </div>
                    </section>
                  `)}
            </article>
    ...
    get nav() {
      let prevHref,
          ...
          getLink = s => {
        let assignmentId = this.submissionFilter ? this.submissionFilter.replace(/-\w+$/, "") : portfolio.assignmentId,
            submissionId = `${assignmentId}-${s.userId}`,
            submissions = !assignmentId || !s.submissions ? [] : s.submissions.filter(sub => sub.id === submissionId),
      ...
      prevHref = portfolio.prev ? getLink(portfolio.prev) : undefined, nextHref = portfolio.next ? getLink(portfolio.next) : undefined;
    ...
    get assignment() {
      let filter = !this.portfolio || !this.portfolio.submissions ? [] : this.portfolio.submissions.filter(s => !this.submissionFilter || s.id === this.submissionFilter);
