...
(function ($) {
  ...
  H5PLibraryList.createLibraryList = function (libraries) {
    ...
    $table.addClass("libraries"); // Add libraries

    $.each(libraries.listData, function (index, library) {
      var $libraryRow = H5PUtils.createTableRow([library.title, '<input class="h5p-admin-restricted" type="checkbox"/>', {
        ...
        class: "h5p-admin-center"
      }, '<div class="h5p-admin-buttons-wrapper">' + '<button class="h5p-admin-upgrade-library"></button>' + (library.detailsUrl ? '<button class="h5p-admin-view-library" title="' + t.viewLibrary + '"></button>' : "") + (library.deleteUrl ? '<button class="h5p-admin-delete-library"></button>' : "") + "</div>"]);
    ...
    if (selected === null) {
      ...
      $checkbox.change(function () {
        ...
        $.ajax({
          ...
          $checkbox.attr("disabled", false); // Reset
