define(["exports","require","../../@polymer/polymer/polymer-element.js","../../@polymer/polymer/lib/legacy/polymer.dom.js","../../@polymer/polymer/lib/utils/flattened-nodes-observer.js","../../@polymer/polymer/lib/utils/flush.js","../../@polymer/polymer/lib/utils/render-status.js","../../@polymer/polymer/lib/utils/async.js","./lib/haxutils.js","../simple-colors/simple-colors.js","./lib/hax-text-context.js","./lib/hax-ce-context.js","./lib/hax-plate-context.js","./lib/hax-input-mixer.js","./lib/hax-shared-styles.js"],function(_exports,_require,_polymerElement,_polymerDom,_flattenedNodesObserver,_flush,_renderStatus,async,_haxutils,_simpleColors,_haxTextContext,_haxCeContext,_haxPlateContext,_haxInputMixer,_haxSharedStyles){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.HaxBody=void 0;_require=babelHelpers.interopRequireWildcard(_require);async=babelHelpers.interopRequireWildcard(async);/**
 * `hax-body`
...
(0,_renderStatus.afterNextRender)(this,function(){this._observer=new _flattenedNodesObserver.FlattenedNodesObserver(this,info=>{// MAKE SURE WE KNOW WHAT JUST GOT ADDED HERE
...
if(e.clipboardData||e.originalEvent.clipboardData){text=(e.originalEvent||e).clipboardData.getData("text/plain")}else if(window.clipboardData){text=window.clipboardData.getData("Text")}try{let range=window.HaxStore.getRange(),sel=window.HaxStore.getSelection(),newNode=document.createTextNode(text),newRange=document.createRange();if(range&&sel){range.deleteContents();range.insertNode(newNode);newRange.setStart(newNode,text.length);newRange.collapse(!0);sel.removeAllRanges();sel.addRange(newRange)}}catch(e){console.log(e)}}});document.body.addEventListener("hax-store-property-updated",this._haxStorePropertyUpdated.bind(this));window.addEventListener("scroll",this._keepContextVisible.bind(this));this.addEventListener("focusin",this._focusIn.bind(this));this.addEventListener("mousedown",this._focusIn.bind(this))})}/**
   ...
   */connectedCallback(){super.connectedCallback();(0,_renderStatus.afterNextRender)(this,function(){// in case we miss this on the initial setup. possible in auto opening environments.
...
let p=document.createElement("p");p.innerHTML="<br/>";this.haxReplaceNode(this.activeNode,p,(0,_polymerDom.dom)(this.activeNode).parentNode);// allow swap out to happen
...
if(status){(0,_haxutils.wipeSlot)(this)}}/**
   ...
   */haxInsert(tag,content,properties={},waitForLock=!0){this.__activeHover=null;// verify this tag is a valid one
...
let haxElement=window.HaxStore.nodeToHaxElement(node,null);// support for deep API call to clean up special elements
if(typeof node.preProcessHaxInsertContent!==typeof void 0){haxElement=node.preProcessHaxInsertContent(haxElement)}// convert it back to a clone, seems odd I'm sure but this ensures that all props are copied
...
if(null!==node){(0,_polymerDom.dom)(parent).insertBefore(nodeClone,(0,_polymerDom.dom)(node).nextSibling)}else{(0,_polymerDom.dom)(parent).appendChild(nodeClone)}setTimeout(()=>{// test for a grid plate clone
...
setTimeout(()=>{let children=(0,_polymerDom.dom)(replacement).getEffectiveChildNodes();// see if there's a child element and focus that instead if there is
...
setTimeout(()=>{html=(0,_haxutils.encapScript)(html);let fragment=document.createElement("div");fragment.insertAdjacentHTML("beforeend",html);while(null!==fragment.firstChild){if(typeof fragment.firstChild.tagName!==typeof void 0){// ensure import doesn't import non-sandbox safe things!
...
this._hideContextMenu(this.$.haxinputmixer)}/**
   ...
   */_focusIn(e){var normalizedEvent=(0,_polymerDom.dom)(e);if(this.__focusLogic(normalizedEvent.localTarget)){e.stopPropagation()}}/**
...
if(this.editMode&&!this.__tabTrap){let containerNode=target,activeNode=null;// ensure this is valid
...
if(this.activeContainerNode!==containerNode&&!containerNode.classList.contains("ignore-activation")){this.hideContextMenus();this.activeContainerNode=containerNode;window.HaxStore.write("activeContainerNode",containerNode,this);stopProp=!0}else if(containerNode.classList.contains("ignore-activation")){stopProp=!0}// test for active node changing
if(this.activeNode!==activeNode&&!activeNode.classList.contains("ignore-activation")){this.activeNode=activeNode;window.HaxStore.write("activeNode",activeNode,this);this.positionContextMenus(activeNode,containerNode);stopProp=!0}}}else{this.__tabTrap=!1}return stopProp}/**
...
if(!1===newValue){this.removeAttribute("contenteditable");this.hideContextMenus()}}/**
   ...
   */_haxResolvePreviousElement(node){node=(0,_polymerDom.dom)(node).previousElementSibling;while(null!=node&&typeof node.tagName!==typeof void 0&&"HAX-"===node.tagName.substring(0,4)){node=(0,_polymerDom.dom)(node).previousElementSibling}return node}/**
   * Test if this is a HAX element or not
   ...
   */_applyContentEditable(status,target=this.$.body){let children=(0,_polymerDom.dom)(target).getDistributedNodes();// fallback for content nodes if not polymer managed nodes above
...
while(null!=node&&!this._haxElementTest(node)){node=(0,_polymerDom.dom)(node).previousSibling}}if(null!=node){setTimeout(()=>{node.focus()},50)}}}catch(e){console.log(e)}}}_exports.HaxBody=HaxBody;window.customElements.define(HaxBody.tag,HaxBody)});