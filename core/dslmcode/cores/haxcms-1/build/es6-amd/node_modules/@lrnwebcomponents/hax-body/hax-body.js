define(["exports", "meta", "require", "../../lit-element/lit-element.js", "../simple-colors/simple-colors.js", "../utils/utils.js", "../undo-manager/undo-manager.js", "./lib/hax-store.js", "../../mobx/dist/mobx.esm.js"], function (_exports, meta, _require, _litElement, _simpleColors, _utils, _undoManager, _haxStore, _mobxEsm) {
  "use strict";
  ...
  class HaxBody extends (0, _undoManager.UndoManagerBehaviors)(_simpleColors.SimpleColors) {
    ...
    _mouseMove(e) {
      if (this.editMode && _haxStore.HAXStore.ready) {
        ...
        this.__mouseQuickTimer = setTimeout(() => {
          if (this.__activeHover && this.__activeHover != e.path[0].closest("[data-hax-ray]:not(li)") && !e.path[0].closest("#addincontext")) {
            let keep;
        ...
        this.__mouseTimer = setTimeout(() => {
          ...
          if (target) {
            ...
            if (!e.path[0].closest(".column:not(.has-nodes")) {
              ...
              this.__activeHover = e.path[0].closest(".column").parentNode.parentNode.host;
              ...
              this.__slot = e.path[0].closest(".column").getAttribute("id").replace("col", "col-"); // based on what we learned we don't have nodes in the path column
              ...
              this.__activeHover = e.path[0].closest(".column").parentNode.parentNode.host.children[0]; // we focus on the column unlike anything else

              activeRect = e.path[0].closest(".column").getBoundingClientRect(); // right in the middle of the column
            ...
            if (mutation.addedNodes.length > 0) {
              mutation.addedNodes.forEach(node => {
                if (this._validElementTest(node)) {
                  ...
                  this.__applyNodeEditableState(node, this.editMode); // special support for Header tags showing up w.o. identifiers
    ...
    keyboardShortCutProcess(guess) {
      ...
      if (map[guess.replace(" ", "")]) {
        let el = document.createElement(map[guess.replace(" ", "")]); // silly thing for contenteditable to show it as full space height
      ...
      switch (detail.eventName) {
        case "hax-source-view-toggle":
          if (!this.activeNode.__haxSourceView) {
            ...
            this.__activeEditingElement.wordWrap = true; // could be 1st time this shows up so ensure we import
            ...
            if (this.activeNode.getAttribute && this.activeNode.getAttribute("slot") != null) {
              this.__activeEditingElement.setAttribute("slot", this.activeNode.getAttribute("slot"));
            }

            this.__ignoreActive = true;
            (0, _utils.wrap)(this.activeNode, this.__activeEditingElement);
          } else {
            this.activeNode.__haxSourceView = false; // run internal state hook if it exist and if we get a response

            let replacement = _haxStore.HAXStore.runHook(this.__activeEditingElement, "activeElementChanged", [this.activeNode, false]);
            ...
            (0, _utils.unwrap)(this.__activeEditingElement);
          }
    ...
    _editModeChanged(newValue, oldValue) {
      ...
      children.forEach(el => {
        _haxStore.HAXStore.runHook(el, "editModeChanged", [newValue]);
      });
    }
    /**
    ...
    async _activeNodeChanged(newValue, oldValue) {
      ...
      if (this.editMode && oldValue) {
        ...
        if (oldSchema.editingElement != "core" || oldValue.parentNode && oldValue.parentNode.haxUIElement && oldValue.parentNode === this.__activeEditingElement) {
          this.__ignoreActive = true; // run internal state hook if it exist and if we get a response

          let replacement = _haxStore.HAXStore.runHook(this.__activeEditingElement, "activeElementChanged", [oldValue, false]);
      ...
      if (this.editMode && newValue) {
        ...
        if (newSchema && newSchema.editingElement && newSchema.editingElement != "core") {
          ...
          this.__activeEditingElement = document.createElement(newSchema.editingElement.tag); // test for slots to match to ensure this is maintained

          if (newValue.getAttribute && newValue.getAttribute("slot") != null) {
            this.__activeEditingElement.setAttribute("slot", newValue.getAttribute("slot"));
          ...
          this.__ignoreActive = true;
          (0, _utils.wrap)(newValue, this.__activeEditingElement); // @see haxHooks activeElementChanged, this is run on the editing element too

          _haxStore.HAXStore.runHook(this.__activeEditingElement, "activeElementChanged", [newValue, true]);
        }
      }
    }
    /**
