...
if (document.documentElement.requestFullScreen) {
  ...
  H5P.safariBrowser = H5P.safariBrowser === null ? 0 : parseInt(H5P.safariBrowser[1]); // Do not allow fullscreen for safari < 7.
...
H5P.init = function (target) {
  ...
  if (H5P.fullscreenSupported === undefined) {
     ...
     * @type {boolean}
     */
    H5P.fullscreenSupported = !H5PIntegration.fullscreenDisabled && !H5P.fullscreenDisabled && (!(H5P.isFramed && H5P.externalEmbed !== false) || !!(document.fullscreenEnabled || document.webkitFullscreenEnabled || document.mozFullScreenEnabled)); // -We should consider document.msFullscreenEnabled when they get their
  ...
  H5P.jQuery(".h5p-content:not(.h5p-initialized)", target).each(function () {
    ...
    if (H5P.isFramed) {
      ...
      if (H5P.externalEmbed === false) {
        ...
        var resizeIframe = function () {
          ...
          iframe.parentElement.style.height = iframe.parentElement.clientHeight + "px"; // Note:  Force layout reflow
        ...
        // External embed
        var parentIsFriendly = false; // Handle that the resizer is loaded after the iframe
        ...
        H5P.communicator.on("hello", function () {
          ...
          parentIsFriendly = true; // Make iframe responsive
    ...
    H5P.instances.push(instance); // Resize content.
...
H5P.communicator = function () {
  ...
  function Communicator() {
    var self = this; // Maps actions to functions

    var actionHandlers = {}; // Register message listener
    ...
    self.send = function (action, data) {
      ...
      data.context = "h5p";
      data.action = action; // Parent origin can be anything
...
H5P.fullScreen = function ($element, instance, exitCallback, body, forceSemiFullScreen) {
  ...
  var done = function (classes) {
    H5P.isFullscreen = false;
    $classes.removeClass(classes); // Do not rely on window resize events.
...
H5P.newRunnable = function (library, contentId, $attachTo, skipResize, extras) {
  ...
  var standalone = extras.standalone || false; // This order makes it possible for an H5P library to override H5P.ContentType functions!

  constructor.prototype = H5P.jQuery.extend({}, H5P.ContentType(standalone).prototype, constructor.prototype);
  var instance; // Some old library versions have their own custom third parameter.
...
H5P.findCopyrights = function (info, parameters, contentId, extras) {
  ...
  var lastContentTypeName; // Cycle through parameters
  ...
  for (var field in parameters) {
    ...
    if (value instanceof Array) {
      ...
      buildFromMetadata(value, lastContentTypeName, contentId); // Check if object is a file with copyrights (old core)
...
H5P.openReuseDialog = function ($element, contentData, library, instance, contentId) {
  ...
  const dialog = new H5P.Dialog("reuse", H5P.t("reuseContent"), html, $element); // Selecting embed code when dialog is opened
...
H5P.openEmbedDialog = function ($element, embedCode, resizeCode, size, instance) {
  ...
  H5P.jQuery(dialog).on("dialog-opened", function (event, $dialog) {
    ...
    updateEmbed(); // Select text and expand textareas
    ...
    positionInner(); // Expand advanced embed
...
H5P.attachToastTo = function (element, message, config) {
  ...
  const getToastCoordinates = function (element, toast, position) {
    ...
    let left = 0;
    let top = 0; // Compute horizontal position
  ...
  config.duration = config.duration || 3000; // Build toast
  ...
  document.body.appendChild(toast); // The message has to be set before getting the coordinates

  const coordinates = getToastCoordinates(element, toast, config.position);
  toast.style.left = Math.round(coordinates.left) + "px";
  toast.style.top = Math.round(coordinates.top) + "px";
  toast.classList.remove("h5p-toast-disabled");
  const timer = setTimeout(removeToast, config.duration); // The toast can also be removed by clicking somewhere
...
H5P.MediaCopyright = function (copyright, labels, order, extraFields) {
  ...
  var humanizeLicense = function (license, version) {
    var copyrightLicense = H5P.copyrightLicenses[license]; // Build license string
...
H5P.trim = function (value) {
  return value.replace(/^\s+|\s+$/g, ""); // TODO: Only include this or String.trim(). What is best?
...
H5P.createTitle = function (rawTitle, maxLength) {
  ...
  var title = H5P.jQuery("<div></div>").text( // Strip tags
...
(function ($) {
  ...
  H5P.getUserData = function (contentId, dataId, done, subContentId) {
    ...
    if (preloadedData && preloadedData[subContentId] && preloadedData[subContentId][dataId] !== undefined) {
      ...
      contentUserDataAjax(contentId, dataId, subContentId, function (err, data) {
        ...
        preloadedData[subContentId][dataId] = data; // Done. Try to decode JSON
  ...
  H5P.setClipboard = function (clipboardItem) {
    localStorage.setItem("h5pClipboard", JSON.stringify(clipboardItem)); // Trigger an event so all 'Paste' buttons may be enabled.
  ...
  $(document).ready(function () {
    ...
    if (H5PIntegration.saveFreq !== false) {
      ...
      var lastStoredOn = 0; // Store the current state of the H5P when leaving the page.
