...
*/class CMSToken extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
      <style>
        :host {
          display: inline;
          min-width: 112px;
    ...
    `}static get tag(){return"cms-token"}static get properties(){return{/**
       * Loading state
   ...
   */_displayModeChanged(newValue,oldValue){if(typeof newValue!==typeof void 0&&""!=newValue&&typeof this.token!==typeof void 0){// @todo need more sanity checks then this to get default and replace better
this.token=this.token.replace(oldValue,newValue)}}/**
   * Generate body data.
   */_generateBodyData(token,$editingState){if(null!==token&&""!==token){let tokenPrefix=this.tokenPrefix,tokenSuffix=this.tokenSuffix;return{token:`${tokenPrefix}${token}${tokenSuffix}`,cachedResponse:$editingState}}}/**
   * Handle the response from the token processing endpoint
   */_handleTokenResponse(newValue,oldValue){if(null!==newValue&&typeof newValue.content!==typeof void 0){// store the text and url callbacks in the event we're in an editing mode
...
_async.microTask.run(()=>{let template=document.createElement("template");template.innerHTML=newValue.content;(0,_polymerDom.dom)(this).appendChild(document.importNode(template.content,!0));this.loading=!1})}}/**
   * Token end point changed
   */_tokenChanged(newValue,oldValue){// ensure we have something and are not loading currently
if(typeof newValue!==typeof void 0&&""!==newValue&&!this.loading){// support going from a null element to a real one
if(typeof this.tokenEndPoint===typeof void 0&&typeof window.cmstokenEndPoint!==typeof void 0){this.tokenEndPoint=window.cmstokenEndPoint}if(this.tokenEndPoint){this.loading=!0;_async.microTask.run(()=>{this.$.tokenrequest.generateRequest()})}}}/**
   * Window visibility callback to monitor when we are being seen
   */_windowVisibilityChanged(e){// ensure we aren't already loading
...
this._clickInvoked=!1}}/**
   ...
   */__tokenClicked(e){// set flag so we know to generate a new request when we come back into focus
this._clickInvoked=!0}/**
   * Attached to the DOM, now fire.
   */connectedCallback(){super.connectedCallback();if(typeof this.token!==typeof void 0&&null!==this.token&&""!==this.token){let slot=(0,_polymerDom.dom)(this).getEffectiveChildNodes();// only kick off request if there's nothing in it
