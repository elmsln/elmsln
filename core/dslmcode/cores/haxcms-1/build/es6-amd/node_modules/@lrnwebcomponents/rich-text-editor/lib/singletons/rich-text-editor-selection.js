...
static get properties(){return{editor:{type:Object,value:null},hidden:{type:Boolean,value:!0,reflectToAttribute:!0},observer:{type:Object,value:null},range:{type:Object,value:null,observer:"_updateToolbar"},toolbar:{type:Object,value:null}}}/**
   ...
   */expandSelection(searchTag=null,wrapTag=null){return this.selectAncestor(searchTag)||this.wrap(wrapTag?document.createElement(wrapTag):null)}/**
   ...
   * @returns {void}
   */addHighlight(){let root=this;root.range.surroundContents(root);root.range.selectNode(root.firstChild);root.dispatchEvent(new CustomEvent("highlight",{detail:root}));root.hidden=!1}/**
   * gets the contents of the selected range
   * @returns {object} the range contents
   */getRangeContents(){return this.range?this.range.cloneContents():null}/**
   ...
   * @returns {object} the selected node
   */selectAncestor(tagName=null){let wrapper=null,getMatchingTag=ancestor=>{if(ancestor&&ancestor.tagName&&(!tagName||ancestor.tagName.toLowerCase()===tagName.toLowerCase())){return ancestor}else if(ancestor.parentNode&&1===ancestor.parentNode.childNodes.length){return getMatchingTag(ancestor.parentNode)}else{return null}};//try to find an ancestor that matches the tag
if(this.range){let ancestor=this.range.commonAncestorContainer;wrapper=getMatchingTag(ancestor);if(wrapper)this.range.selectNode(wrapper)}return wrapper}/**
   * sets the selection range to the specified node
   * @param {object} node the node to select
   * @returns {void}
   */selectNode(node=null){if(node){if(!this.range){let sel=window.getSelection();this.range=document.createRange();sel.removeAllRanges();sel.addRange(this.range)}this.range.selectNode(node)}}/**
   * sets the selection range to the specified node's contents
   * @param {object} node the node to select
   * @returns {void}
   */selectNodeContents(node=null){if(node){if(!this.range){let sel=window.getSelection();this.range=document.createRange();sel.removeAllRanges();sel.addRange(this.range)}this.range.selectNodeContents(node)}}/**
   ...
   */wrap(wrapper){wrapper=wrapper||document.createElement("span");this.range.surroundContents(wrapper);return wrapper}/**
   ...
   * Updates the toolbar
   */_updateToolbar(){if(this.toolbar)this.toolbar.range=this.range}}_exports.RichTextEditorSelection=RichTextEditorSelection;window.customElements.define(RichTextEditorSelection.tag,RichTextEditorSelection);window.RichTextEditorSelection={};window.RichTextEditorSelection.instance=null;/**
