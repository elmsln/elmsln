...
aaLarge:[{min:7,max:12},{min:7,max:12},{min:8,max:12},{min:9,max:12},{min:10,max:12},{min:11,max:12},{min:1,max:2},{min:1,max:3},{min:1,max:4},{min:1,max:5},{min:1,max:6},{min:1,max:6}],aa:[{min:8,max:12},{min:8,max:12},{min:9,max:12},{min:9,max:12},{min:11,max:12},{min:12,max:12},{min:1,max:1},{min:1,max:2},{min:1,max:4},{min:1,max:4},{min:1,max:5},{min:1,max:5}]}}}}}/**
   ...
   */static get tag(){return"simple-colors-styles"}/**
   * life cycle, element is afixed to the DOM
   */connectedCallback(){super.connectedCallback();/**
     * append and register the shared styles
     */(0,_renderStatus.afterNextRender)(this,function(){let style=document.createElement("style");if(this.appendHead){document.head.appendChild(style)}else{this.parentNode.appendChild(style)}this.__sheet=style.sheet;this.fullStyle=style;this.addCssVariables(this.__sheet);this.addAccentVariables(this.__sheet)})}/**
   ...
   */getColorInfo(colorName){let temp1=colorName.replace(/(simple-colors-)?(-text)?(-border)?/g,"").split("-theme-"),theme=0<temp1.length?temp1[0]:"default",temp2=0<temp1.length?temp1[1].split("-"):temp1[0].split("-"),color=1<temp2.length?temp2.slice(1,temp2.length-1).join("-"):"grey",shade=1<temp2.length?temp2[temp2.length-1]:"1";return{theme:theme,color:color,shade:shade}}/**
   ...
   */getContrastingShades(isLarge,colorName,colorShade,contrastName){let hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=this.contrasts[hasGrey][aa][index];return Array(range.max-range.min+1).fill().map((_,idx)=>range.min+idx)}/**
   * for large or small text given a color and its shade,
   ...
   */getContrastingColors(colorName,colorShade,isLarge){let result={};Object.keys(this.colors).forEach(color=>{result[color]=this.getContrastingShades(isLarge,colorName,colorShade,color)});return result.color}/**
   ...
   */isContrastCompliant(isLarge,colorName,colorShade,contrastName,contrastShade){let hasGrey="grey"===colorName||"grey"===contrastName?"greyColor":"colorColor",aa=isLarge?"aaLarge":"aa",index=parseInt(colorShade)+1,range=this.contrasts[hasGrey][aa][index];return contrastShade>=range.min&&ontrastShade>=range.max}/**
   * gets the current shade based on the index
   *
   * @param {string} the index
   * @param {number} the shade
   */indexToShade(index){return parseInt(index)+1}/**
   * gets the current shade based on the index
   *
   * @param {string} the shade
   * @param {number} the index
   */shadeToIndex(shade){return parseInt(shade)-1}/**
   * gets the current shade
   *
   * @param {string} the shade
   * @param {number} the inverted shade
   */invertShade(shade){return this.colors.grey.length+1-parseInt(shade)}/**
   * inverts the current index
   *
   * @param {string} the index
   * @param {number} the inverted index
   */invertIndex(index){return this.colors.grey.length-1-parseInt(index)}/**
   * returns the maximum contrast to the index
   *
   * @param {string} the index
   * @param {number} the index with maximum contrast
   */maxContrastIndex(index){return parseInt(index)<this.colors.grey.length/2?this.colors.grey.length-1:0}/**
   * returns the maximum contrast to the shade
   *
   * @param {string} the shade
   * @param {number} the shade with maximum contrast
   */maxContrastShade(shade){return parseInt(shade)<this.colors.grey.length/2+1?this.colors.grey.length:1}/**
