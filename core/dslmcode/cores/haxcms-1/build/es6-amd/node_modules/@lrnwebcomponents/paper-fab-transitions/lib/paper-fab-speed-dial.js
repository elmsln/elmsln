...
*/class PaperFabSpeedDial extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
      <custom-style>
        <style>
    ...
    `}static get tag(){return"paper-fab-speed-dial"}static get properties(){return{/**
       ...
       */openAnimationConfig:{type:Array,value(){return[{name:"fade-in-animation",timing:{delay:150,duration:50}},{name:"paper-square-grow-animation",startSize:56,timing:{delay:150,duration:200}}]}},/**
       * An animation config. If provided, this will be used to animate the
       * closing of the options dropdown.
       */closeAnimationConfig:{type:Array,value(){return[{name:"fade-out-animation",timing:{duration:200}}]}}}}static get observers(){return["_updateDropdown(direction, offset)"]}ready(){super.ready();var fab=(0,_polymerDom.dom)(this.$.fabContainer).getDistributedNodes()[0];fab.addEventListener("click",function(){this.$.dropdown.open()}.bind(this));var content=(0,_polymerDom.dom)(this.$.contentContainer).getDistributedNodes()[0];content.addEventListener("click",function(){this.$.dropdown.close()}.bind(this))}/**
   * Show the speed dial options.
   */open(){this.$.dropdown.open()}/**
   * Hide the speed dial options.
   */close(){this.$.dropdown.close()}_updateDropdown(direction,offset){var d=this.$.dropdown;switch(direction){case"bottom":d.verticalAlign="top";d.horizontalOffset=0;d.verticalOffset=offset;d.classList.add("vertical");d.classList.remove("horizontal");break;case"top":d.verticalAlign="bottom";d.horizontalOffset=0;d.verticalOffset=offset;d.classList.add("vertical");d.classList.remove("horizontal");break;case"right":d.horizontalAlign="left";d.horizontalOffset=offset;d.verticalOffset=0;d.classList.add("horizontal");d.classList.remove("vertical");break;case"left":d.horizontalAlign="right";d.horizontalOffset=offset;d.verticalOffset=0;d.classList.add("horizontal");d.classList.remove("vertical");break;default:throw"Invalid direction. Must be one of [top, bottom, left, right].";}}}_exports.PaperFabSpeedDial=PaperFabSpeedDial;window.customElements.define(PaperFabSpeedDial.tag,PaperFabSpeedDial)});