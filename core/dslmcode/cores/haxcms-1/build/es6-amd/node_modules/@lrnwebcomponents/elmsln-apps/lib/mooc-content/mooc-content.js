 ...
 */class MoocContent extends _polymerElement.PolymerElement{static get template(){return _polymerElement.html`
    ...
    <main id="etb-tool-nav" data-offcanvas="">
    ...
    </main>`}static get tag(){return"mooc-content"}static get observers(){return["_routeChanged(data, route, endPoint)"]}static get properties(){return{elmslnCourse:{type:String},elmslnSection:{type:String},basePath:{type:String},csrfToken:{type:String},endPoint:{type:String},/**
       * Source of data
       ...
       */_loading:{type:Boolean,observer:"_contentLoading"},/**
       * loading pegged to the ajax call running
   ...
   */_modalTap(e){var normalizedEvent=(0,_polymerDom.dom)(e),local=normalizedEvent.localTarget;// verify that it is a buttonß
if("LRNSYS-BUTTON"===local.tagName){if(null!=this.activeNodeItem){this.activeNodeItem.classList.remove("book-menu-item-active")}this.activeNodeItem=local;this.activeNodeItem.classList.add("book-menu-item-active");this.$.outlinepopover.toggleDialog()}}/**
   ...
   */_contentLoading(newValue,oldValue){var _this=this;if(("undefined"===typeof newValue?"undefined":typeof newValue)!==("undefined"===typeof void 0?"undefined":typeof void 0)&&!newValue){setTimeout(function(){_this.loading=!1;_this._resetScroll("anchor");_this.$["main-content"].focus()},500)}else{this.loading=!0}}/**
   * Callback to push the data into the page.
   */handleResponse(e){// handle the HTML we just got
if(typeof this.pageData.data!==("undefined"===typeof void 0?"undefined":typeof void 0)){var data=this.pageData.data;// test for advanced ops
if(typeof data.ops.redirect!==("undefined"===typeof void 0?"undefined":typeof void 0)){this.set("route.path",data.ops.redirect);this._routeChanged(this.data,this.route,this.endPoint)}else{this.outlineTitle=data.bookOutline.subject;this.$.content.innerHTML=data.content;this.$.navigation.innerHTML=data.topNavigation;this.$.outline.innerHTML=data.bookOutline.content;this.$.options.innerHTML=data.options;// inject styles, destroying previous ones
...
if(typeof this.outlineData.data===("undefined"===typeof void 0?"undefined":typeof void 0)){this.$.fulloutlinepath.generateRequest()}_async.microTask.run(()=>{window.dispatchEvent(new Event("resize"))})}}}/**
   * Callback to push the data into the page.
   */handleOutlineResponse(e){// handle the HTML we just got
var data=this.outlineData.data;if(("undefined"===typeof data?"undefined":typeof data)!==("undefined"===typeof void 0?"undefined":typeof void 0)){this.$.outlinemodal.innerHTML=data.outline;this.aliases=data.aliases}}connectedCallback(){super.connectedCallback();(0,_renderStatus.afterNextRender)(this,function(){if(window.Drupal){window.Drupal.attachBehaviors(document,window.Drupal.settings)}})}/**
   ...
   */_routeChanged(data,route,endPoint){if("string"===typeof route.path){// target for url alias that might be delivered into content
...
else if(typeof this.aliases[urlAlias]!==("undefined"===typeof void 0?"undefined":typeof void 0)){this.nid=this.aliases[urlAlias].replace("node/","");// trigger change if data location changed
...
window.location.reload()}}ready(){super.ready();this.__modal=window.SimpleModal.requestAvailability()}/**
   * Reset scroll position visually and internally data wise.
   */_resetScroll(){var item=0<arguments.length&&arguments[0]!==void 0?arguments[0]:"anchor";this.resetScroll=!0;this.$[item].scrollIntoView({block:"nearest",behavior:"smooth"})}/**
   * Simple way to convert from object to array.
   */_toArray(obj){if(null==obj){return[]}return Object.keys(obj).map(function(key){return obj[key]})}/**
   ...
   * what this enables.
   */__injectStyle(style){// target and wipe our id area by force
...
(0,_polymerDom.dom)(this.$.hackycontainer).appendChild(customStyle)}/**
   * highjack shadowDom
   */_attachDom(dom){this.appendChild(dom)}}_exports.MoocContent=MoocContent;window.customElements.define(MoocContent.tag,MoocContent)});