define(["exports", "../../lit-element/lit-element.js", "../simple-colors/simple-colors.js", "../simple-icon/lib/simple-iconset-demo.js", "../simple-icon/simple-icon.js", "../simple-icon/lib/simple-icon-lite.js", "../simple-icon/lib/simple-iconset-manifest.js", "../hax-iconset/lib/hax-iconset-manifest.js"], function (_exports, _litElement, _simpleColors, _simpleIconsetDemo, _simpleIcon, _simpleIconLite, _simpleIconsetManifest, _haxIconsetManifest) {
   ...
   */
  const LoremDataBehaviors = function (SuperClass) {
    return class extends SuperClass {
      ...
      get icons() {
        return [...(_simpleIconsetManifest.SimpleIconIconsetsManifest || []), ...(_haxIconsetManifest.HaxIconsetManifest || [])].map(iconset => ((iconset || {}).icons || []).map(icon => iconset.name == "icons" ? icon : `${iconset.name}:${icon}`)).flat();
      }
      ...
      randomAspect(minwidth = 200, maxwidth = 1000, minheight = 200, maxheight = 1000) {
      ...
      randomHex() {
        return `#${[0, 0, 0].map(i => this.randomNumber(i, 255).toString(16)).map(i => i.length < 2 ? `0${i}` : `${i}`).join("")}`;
      }
      /**
      ...
      randomFlickr(aspect = this.randomAspect(), searchTerms = [], searchAll = false, multiple = -1) {
        return `https://loremflickr.com/${aspect}${searchTerms.length === 0 ? `` : `/${searchTerms.join(",")}${searchAll ? `/all` : ""}`}${multiple > -1 ? `?random=${multiple}` : ""}`;
      }

      randomImage(aspect = this.randomAspect(), greyscale, topic, multiple) {
        return topic && ["man", "woman", "person"].includes(topic) ? this.randomProfileImage(aspect, topic) : typeof multiple === typeof undefined ? this.randomPlaceImg(aspect, greyscale, topic) : topic === "any" ? this.randomPicsum(aspect, greyscale, undefined, multiple) : this.randomFlickr(aspect, [topic], false, multiple);
      }
      ...
      randomKitten(aspect = this.randomAspect(), greyscale) {
        if (typeof greyscale === typeof undefined) greyscale = this.randomWeightedOption([{
          value: false,
          weight: 3
        }, {
          value: true,
      ...
      randomObject(schema = {}) {
        let obj = {};
        Object.keys(schema).forEach(key => obj[key] = this.randomType(schema[key]));
        return obj;
      }
      /**
      ...
      randomPicsum(aspect = this.randomAspect(), greyscale, blur = 0, id) {
        ...
        return `https://picsum.photos/${id > -1 ? `id/${id}` : ``}/${aspect}/${params.length == 0 ? "" : `?${params.join("&")}`}`;
      }
      ...
      randomPlaceImg(aspect = this.randomAspect(), filter, topic = "any") {
        ...
        return `//placeimg.com/${aspect}/${topic}${filter === "greyscale" ? "/greyscale" : filter === "sepia" ? "/sepia" : ""}`;
      }
      ...
      randomPassage(min = 2, max = 5, sentMinPerPara = 2, sentMaxPerPara = 5, wordMinPerSent = 3, wordMaxPerSent = 10) {
        let passage = [];

        for (let i = this.randomNumber(min, max); i > 0; i--) {
          passage.push(`<p>${this.randomParagraph(sentMinPerPara, sentMaxPerPara, wordMinPerSent, wordMaxPerSent)}</p>`);
        }
      ...
      randomWeightedOption(arr = []) {
        return this.randomOption(arr.map(item => {
          let arr2 = [];
  ...
  class LoremData extends LoremDataBehaviors(_litElement.LitElement) {
    static get styles() {
    ...
    render() {
      ...
      ${Object.keys(this.schemas || []).map(key => (0, _litElement.html)`
          <p>
            <label>
