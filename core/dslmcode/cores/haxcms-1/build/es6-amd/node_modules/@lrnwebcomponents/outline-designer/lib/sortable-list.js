define(["exports","../../../@polymer/polymer/polymer-element.js","../../../@polymer/polymer/lib/mixins/gesture-event-listeners.js","../../../@polymer/polymer/lib/utils/async.js","../../../@polymer/polymer/lib/utils/gestures.js"],function(_exports,_polymerElement,_gestureEventListeners,async,Gestures){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.SortableList=void 0;async=babelHelpers.interopRequireWildcard(async);Gestures=babelHelpers.interopRequireWildcard(Gestures);/**
 ...
 */class SortableList extends(0,_gestureEventListeners.GestureEventListeners)(_polymerElement.PolymerElement){// render function
static get template(){return _polymerElement.html`
    ...
    `}static get is(){return"sortable-list"}static get properties(){return{/**
...
_trackEnd(event){if(!this.dragging){return}const rect=this._rects[this.items.indexOf(this._target)];this._target.classList.remove("item--pressed");this._setDragging(!1);this._translate3d(rect.left,rect.top,1,this._target)}_onTransitionEnd(){if(this.dragging||!this._target){return}const fragment=document.createDocumentFragment();this.items.forEach(item=>{item.style.transform="";item.style.width=item.__originalWidth;item.style.height=item.__originalHeight;item.classList.remove("item--transform");fragment.appendChild(item)});if(this.children[0]){this.insertBefore(fragment,this.children[0])}else{this.appendChild(fragment)}this.style.height="";this._target.classList.remove("item--dragged");this._rects=null;this._targetRect=null;this._updateItems();this.dispatchEvent(new CustomEvent("sort-finish",{composed:!0,detail:{target:this._target}}));this._target=null}_onDragStart(event){event.preventDefault()}_onContextMenu(event){if(this.dragging){event.preventDefault();this._trackEnd()}}_onTouchMove(event){event.preventDefault()}_updateItems(){if(this.dragging){return}const items=this.$.slot.assignedNodes().filter(node=>{if(node.nodeType===Node.ELEMENT_NODE&&(!this.sortable||node.matches(this.sortable))){return!0}});this._setItems(items)}_itemFromCoords({x,y}){if(!this._rects){return}let match=null;this._rects.forEach((rect,i)=>{if(x>=rect.left&&x<=rect.left+rect.width&&y>=rect.top&&y<=rect.top+rect.height){match=this.items[i]}});return match}_itemFromEvent(event){const path=event.composedPath();for(var i=0;i<path.length;i++){if(-1<this.items.indexOf(path[i])){return path[i]}}}_getItemsRects(){return this.items.map(item=>{return item.getBoundingClientRect()})}_observeItems(){if(!this._observer){this._observer=new MutationObserver(_=>{this._updateItems()});this._observer.observe(this,{childList:!0})}}_unobserveItems(){if(this._observer){this._observer.disconnect();this._observer=null}}/**
   ...
   */_moveItemArray(array,oldIndex,newIndex){if(newIndex>=array.length){var k=newIndex-array.length;while(k--+1){array.push(void 0)}}array.splice(newIndex,0,array.splice(oldIndex,1)[0]);return array}_translate3d(x,y,z,el){el.style.transform=`translate3d(${x}px, ${y}px, ${z}px)`}}_exports.SortableList=SortableList;window.customElements.define("sortable-list",SortableList)});