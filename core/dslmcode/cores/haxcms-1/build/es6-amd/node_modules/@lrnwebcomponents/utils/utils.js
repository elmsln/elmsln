define(["exports"], function (_exports) {
  ...
  function stripMSWord(input) {
    ...
    output = output.replace(/ start='.*?'/g, ""); // ID's wont apply meaningfully on a paste
    ...
    output = output.replace(/ class="(\s|.)*?"/gim, ""); // 7. clean out empty paragraphs and endlines that cause weird spacing
    ...
    output = output.replace(/<\/p><br \/><\/b>/gm, "</p></b>"); // some other things we know not to allow to wrap

    output = output.replace(/<b><p>/gm, "<p>");
    output = output.replace(/<\/p><\/b>/gm, "</p>"); // drop list wrappers
  ...
  function nodeToHaxElement(node, eventName = "insert-element") {
    ...
    if (typeof tmpProps !== typeof undefined) {
      ...
      for (var attribute in node.attributes) {
        ...
        if (typeof node.attributes[attribute].name !== typeof undefined && node.attributes[attribute].name != "class" && node.attributes[attribute].name != "style" && node.attributes[attribute].name != "id" && node.attributes.hasOwnProperty(attribute) && typeof node.attributes[attribute].value !== undefined && node.attributes[attribute].value != null && node.attributes[attribute].value != "" && !tmpProps.hasOwnProperty(dashToCamel(node.attributes[attribute].name))) {
          props[dashToCamel(node.attributes[attribute].name)] = node.attributes[attribute].value;
    ...
    let slotContent = window.HaxStore.instance.getHAXSlot(node); // support fallback on inner text if there were no nodes
