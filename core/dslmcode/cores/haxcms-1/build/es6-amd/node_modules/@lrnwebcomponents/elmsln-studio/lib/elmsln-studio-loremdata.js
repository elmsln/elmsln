...
${JSON.stringify(this.sortDates(this.toArray(this.submissions)))}</textarea
    ...
    makeAssignment(projectId, a) {
      ...
      this.assignments[id].submissions = studentsList.map((studentId, i) => {
        let sid = this.makeSubmission(id, studentId, this.nextDate(created, i, studentsList.length, 7));
    ...
    makeSubmission(assignmentId, studentId, created) {
      let id = `submission-${Object.keys(this.submissions).length}`,
          reviewers = [this.randomItem(Object.keys(this.instructors)), ...this.draw(Object.keys(this.students), 1, 3)],
      ...
      this.submissions[id].feedback = reviewers.map((userId, i) => {
        return this.makeFeedback(id, userId, this.nextDate(created, i, reviewers.length, 3));
      });
    ...
    randomImage(topic = "any") {
      let filter = this.randomItem(["", "", "", "", "/greyscale", "/greyscale", "/sepia"]),
    ...
    refreshData() {
      ...
      this.sentences = this.paragraphs.map(p => p.split(/[\.\?\!]+\s+/)).flat().map(s => `${s}${this.randomItem(".", ".", ".", ".", "?", "?", "?", "!")}`);
      /* lorem ipsum word list */

      this.words = [...new Set(this.paragraphs.map(p => p.split(/\W+/)).flat().map(w => w.toLowerCase()))];
      /* total weeks to complete all assgnments */

      this.weeks = Object.keys(this.projects || {}).map(i => this.projects[i].assignments).flat().length;
      ...
      Object.keys(this.projects || {}).forEach(p => {
        ...
        this.projects[p].assignments = this.projects[p].assignments.map(a => this.makeAssignment(p, a));
      });
      /** view all activity */

      this.activity = this.sortDates([...Object.keys(this.submissions).map(i => {
        return {
          id: i,
