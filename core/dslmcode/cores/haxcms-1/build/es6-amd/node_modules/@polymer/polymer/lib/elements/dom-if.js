define(["exports","../../polymer-element.js","../utils/templatize.js","../utils/debounce.js","../utils/flush.js","../utils/async.js","../utils/path.js","../utils/wrap.js"],function(_exports,_polymerElement,_templatize,_debounce,_flush,_async,_path,_wrap){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.DomIf=void 0;/**
@license
 ...
 */class DomIf extends _polymerElement.PolymerElement{// Not needed to find template; can be removed once the analyzer
...
this.__renderDebouncer=_debounce.Debouncer.debounce(this.__renderDebouncer,_async.microTask,()=>this.__render());(0,_flush.enqueueDebouncer)(this.__renderDebouncer)}/**
   ...
   * @return {void}
   */render(){(0,_flush.flush)()}__render(){if(this.if){if(!this.__ensureInstance()){// No template found yet
...
if(parentNode){if(!this.__ctor){let template=/** @type {HTMLTemplateElement} */(0,_wrap.wrap)(this).querySelector("template");if(!template){// Wait until childList changes and template should be there by then
let observer=new MutationObserver(()=>{if((0,_wrap.wrap)(this).querySelector("template")){observer.disconnect();this.__render()}else{throw new Error("dom-if requires a <template> child")}});observer.observe(this,{childList:!0});return!1}this.__ctor=(0,_templatize.templatize)(template,this,{// dom-if templatizer instances require `mutable: true`, as
...
this.__invalidProps=this.__invalidProps||Object.create(null);this.__invalidProps[(0,_path.root)(prop)]=!0}}}})}if(!this.__instance){this.__instance=new this.__ctor;(0,_wrap.wrap)(parentNode).insertBefore(this.__instance.root,this)}else{this.__syncHostProperties();let c$=this.__instance.children;if(c$&&c$.length){// Detect case where dom-if was re-attached in new position
let lastChild=(0,_wrap.wrap)(this).previousSibling;if(lastChild!==c$[c$.length-1]){for(let i=0,n;i<c$.length&&(n=c$[i]);i++){(0,_wrap.wrap)(parentNode).insertBefore(n,this)}}}}}return!0}__syncHostProperties(){let props=this.__invalidProps;if(props){for(let prop in props){this.__instance._setPendingProperty(prop,this.__dataHost[prop])}this.__invalidProps=null;this.__instance._flushProperties()}}__teardownInstance(){if(this.__instance){let c$=this.__instance.children;if(c$&&c$.length){// use first child parent, for case when dom-if may have been detached
let parent=(0,_wrap.wrap)(c$[0]).parentNode;// Instance children may be disconnected from parents when dom-if
// detaches if a tree was innerHTML'ed
if(parent){parent=(0,_wrap.wrap)(parent);for(let i=0,n;i<c$.length&&(n=c$[i]);i++){parent.removeChild(n)}}}this.__instance=null;this.__invalidProps=null}}/**
   ...
   * @protected
   * @suppress {visibility}
   */_showHideChildren(){let hidden=this.__hideTemplateChildren__||!this.if;if(this.__instance){this.__instance._showHideChildren(hidden)}}}_exports.DomIf=DomIf;customElements.define(DomIf.is,DomIf)});