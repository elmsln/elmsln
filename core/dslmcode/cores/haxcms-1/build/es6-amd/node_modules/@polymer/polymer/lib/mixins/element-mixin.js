define(["exports","../utils/boot.js","../utils/settings.js","../utils/mixin.js","../utils/style-gather.js","../utils/resolve-url.js","../elements/dom-module.js","./property-effects.js","./properties-mixin.js","../utils/wrap.js"],function(_exports,_boot,_settings,_mixin,_styleGather,_resolveUrl,_domModule,_propertyEffects,_propertiesMixin,_wrap){"use strict";Object.defineProperty(_exports,"__esModule",{value:!0});_exports.updateStyles=_exports.ElementMixin=_exports.version=void 0;/**
 * @fileoverview
 ...
 */const version="3.2.0";_exports.version=version;const builtCSS=window.ShadyCSS&&window.ShadyCSS.cssBuild,ElementMixin=(0,_mixin.dedupingMixin)(base=>{/**
   * @constructor
...
let template=/** @type {PolymerElementConstructor} */this.template;if(template){if("string"===typeof template){console.error("template getter must return HTMLTemplateElement");template=null}else if(!_settings.legacyOptimizations){template=template.cloneNode(!0)}}this.prototype._template=template}/**
...
super._readyClients()}/**
     ...
     */_attachDom(dom){const n=(0,_wrap.wrap)(this);if(n.attachShadow){if(dom){if(!n.shadowRoot){n.attachShadow({mode:"open"})}n.shadowRoot.appendChild(dom);if(_settings.syncInitialRender&&window.ShadyDOM){ShadyDOM.flushInitial(n.shadowRoot)}return n.shadowRoot}return null}else{throw new Error("ShadowDOM not available. "+// TODO(sorvell): move to compile-time conditional when supported
...
if(_settings.legacyOptimizations&&!(prop in this._properties)){console.warn(`Property '${prop}' used in template but not declared in 'properties'; `+`attribute will not be observed.`)}return super._addTemplatePropertyEffect(templateInfo,prop,effect)}}return PolymerElement});/**
 ...
 * @return {void}
 */const updateStyles=function(props){if(window.ShadyCSS){window.ShadyCSS.styleDocument(props)}};_exports.updateStyles=updateStyles});