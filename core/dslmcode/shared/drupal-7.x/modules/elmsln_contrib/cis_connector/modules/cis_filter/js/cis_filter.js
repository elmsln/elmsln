...
(function ($) {
  Drupal.theme.prototype.cis_filter_modal = function () {
    ...
    html += '    <span class="popups-close"><a class="close ctools-modal-cis-filter-close" href="#">' + Drupal.CTools.Modal.currentSettings.closeImage + '</a></span>';
  ...
  attach: function (context, settings) {
    $('.ctools-modal-cis-filter-close').on("click", function () {
  ...
  Drupal.behaviors.gradebookAssessmentSubmit = {
    'attach': function(context) {
      ...
      $('#modal-content .form-submit', context).bind('click', function() {
        ...
        $.ajax({
          url: Drupal.settings.basePath + 'cis/service/ajax/' + Drupal.settings.cisFilterToken + '/' + $('#modal-content input[name=aid]').attr('value') + '/' + $('#modal-content input[name=submission]').attr('value'),
          type: 'POST',
          data: data,
        })
        .done(function(data) {
          ...
          // close window
          $('.ctools-modal-cis-filter-close').click();
        })
        .fail(function(data) {
