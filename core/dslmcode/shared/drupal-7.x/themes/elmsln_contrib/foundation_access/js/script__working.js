(function ($, Drupal) {
	...
	Drupal.behaviors.foundation_access = {
		attach: function(context, settings) {
			...
			////////////
			/// rebuild with following sudo code
			/// 
			///		Detect sidenav top
			///			add value to sidenav top variable
			///		Detect sidenav bottom
			///			add value to sidenav bottom variable
			///		Detect stickybar class is active
			///			add all heights to total sticky elements variable
			///		Detect window is smaller than height threshold
			///			if threshhold 1
			///				remove sticky class from element 1
			///				Detect stickybar classes
			///			if threshhold 2
			///				remove sticky class from element 2
			///			if threshhold 3
			///				remove sticky class from element 3
			///		detect sidenav height
			///			if equal to or smaller than the view port and total skicky elements
			///				Make fixed to scroll
			///			if larger than the view port and total skicky elements
			///				Make nav static to scroll
			///		Scrolled nav passed top viewport
			///			if by a small amount
			///				Push the nav's top into view (use animation) with slow animation
			///			if by a large amount
			///				Push the nav's top into view (use animation) with fast animation
			///		Scrolled nav passed bottom viewport and/or sticky elements
			///			if by a small amount
			///				Push the nav's bottom into view (use animation) with slow animation
			///			if by a large amount
			///				Push the nav's bottom into view (use animation) with fast animation
			///					
			///				
			///		
			///		
			///		
			///		Is there a lot of dist from top of nav to ideal position
			///		Is there a lot of dist from bottom of nav to ideal position
			///	
			///		
			///		
			////////////
			
			// Make elements visible based on window height
			// function assignStickyElement() {
				
			// 	var windowHeight = $(window).height();
			// 	var element_1 = $(".element-1");
			// 	var element_2 = $(".element-2");
			// 	var element_3 = $(".element-3");
			// 	var threshold_1 = 400;
			// 	var threshold_2 = 640;
			// 	var threshold_3 = 1200;

			// 	if (windowHeight <= threshold_1 && windowHeight <= threshold_2 && windowHeight <= threshold_3) {
			// 		($element_1).removeClass();
			// 		($element_2).removeClass();
			// 		($element_3).removeClass();
			// 	}
			// 	else if (windowHeight <= threshold_1 && windowHeight <= threshold_2 && windowHeight > threshold_3) {
			// 		($element_1).removeClass();
			// 		($element_2).removeClass();
			// 	}
			// 	else if (windowHeight <= threshold_1 && windowHeight > threshold_2 && windowHeight > threshold_3) {
			...
			// 	var element_total = $elementsTotal;
			// }
			// // Make elements visible based on window height
			// function stickyElementVisibility() {
			...
			function compactStickyOutline() {
				$('#activeoutline').css({display:'none'});
